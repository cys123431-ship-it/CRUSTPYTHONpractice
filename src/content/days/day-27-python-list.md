...
exercises:
  - id: ex-day-27-python-list-predict
    ...
    starter: "minutes = [10, 20]
      ...
      print(sum(minutes))"
    answer: "60"
    hint: list는 원소를 순서대로 담고 append는 맨 끝에 새 원소를 넣습니다. 설명을 떠올리고 `[10,20]` 단계부터 순서대로 적어 보세요.
    explanation:
      "[10,20]  →  [10,20,30]  →  60 순서로 실행됩니다. `append(30)` 부분이 `60` 단계를 확정해 최종 출력 '60'가 됩니다. 이 흐름을
      떠올리면 `가변 시퀀스의 끝에 추가함` 동작이 왜 필요한지 알 수 있습니다."
    commonMistakes:
  ...
  - id: ex-day-27-python-list-fill
    ...
    answer: "minutes = [10, 20]
      ...
      print(sum(minutes))"
    hint: 힌트 문장을 완전하게 읽으면 `[10,20]으로 시작합니다` 단계에 필요한 표현이 `append(30)`입니다.
    explanation:
      빈칸에 들어갈 표현은 'append(30)'입니다. `minutes.append(30)` 줄을 완성해야 `가변 시퀀스의 끝에 추가함` 동작이 이어져 실행 결과 '60'가 됩니다.
      힌트의 첫 단계 `[10,20]으로 시작합니다`이 바로 이 줄입니다. 이어서 끝에 30을 추가해 길이 3이 됩니다 sum이 60을 반환합니다 순서로 진행됩니다.
    commonMistakes:
      - append의 반환값을 새 리스트라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-27-python-list-guided
    ...
    explanation:
      바꾼 뒤 출력은 '61'입니다. 원본 출력 '60'에서 달라졌습니다. 바뀐 줄은 `minutes = [10, 20]`에서 `minutes = [11, 20]`로 바뀌었습니다.
      바뀐 프로그램은 `[10,20]` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `61`가 됩니다. 원본 추적 `[10,20]  →  [10,20,30]  →  60`와
  ...
  - id: ex-day-27-python-list-debug
    ...
    answer: "minutes = [10, 20]
      ...
      print(sum(minutes))"
    hint: list는 원소를 순서대로 담고 append는 맨 끝에 새 원소를 넣습니다. 설명과 어긋나는 줄을 찾으세요. `append의 반환값을 새 리스트라고 생각함` 상황이 단서가 됩니다.
    explanation:
      틀린 줄은 `minutes.pop(30)`입니다. 여기서는 `pop(30)`을 써서 `append(30)` 동작이 깨집니다. 이대로 실행하면 `append의 반환값을 새 리스트라고
      생각함` 문제가 생겨 원본 추적 `[10,20]  →  [10,20,30]  →  60`대로 '60'가 나오지 않습니다. 고친 줄 `minutes.append(30)`에서는 `append(30)`가
      `가변 시퀀스의 끝에 추가함` 동작을 지켜 '60'까지 도달합니다.
    commonMistakes:
      - append의 반환값을 새 리스트라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-27-python-list-independent
    ...
    answer: "minutes = [10, 20]
      ...
      print(sum(minutes))"
    hint: list는 원소를 순서대로 담고 append는 맨 끝에 새 원소를 넣습니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `append(30)` 부분이 `가변 시퀀스의 끝에 추가함` 동작을 지켜 실행 결과 '60'가 됩니다. 같은 개념을 다른 입력으로 바꿔도
      `append(30)`부터 `60`까지 추적할 수 있으면 정답입니다. `append의 반환값을 새 리스트라고 생각함` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - append의 반환값을 새 리스트라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-27-python-list-output
    ...
    answerIndex: 0
    explanation:
      "[10,20]  →  [10,20,30]  →  60 순서로 실행되어 출력은 '60'입니다. `append(30)` 부분이 마지막 단계를 확정하므로 다른 선택지는 이 추적과
      맞지 않습니다."
  - id: quiz-day-27-python-list-model
    ...
    answerIndex: 1
    explanation:
      list는 원소를 순서대로 담고 append는 맨 끝에 새 원소를 넣습니다. 이 설명이 맞는 이유는 `가변 시퀀스의 끝에 추가함` 동작을 지키는 조건과 같기 때문입니다. `append의
      반환값을 새 리스트라고 생각함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-27-python-list-pitfall
    ...
    choices:
      ...
      - append의 반환값을 새 리스트라고 생각함
    answerIndex: 2
    explanation: append의 반환값을 새 리스트라고 생각함. 이 실수가 나오면 원본 추적 `[10,20] → [10,20,30] → 60`대로 '60'가 나오지 않으므로 먼저 확인해야 합니다.
  - id: quiz-day-27-python-list-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 0
    explanation: 문법은 달라도 `가변 시퀀스의 끝에 추가함` 목적과 입력·출력은 유지합니다. 실행 결과 '60'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
...
원본 출력은 `60`이고, 바뀐 코드의 실행 결과는 `61`입니다. 바뀐 줄은 `minutes = [10, 20]`에서 `minutes = [11, 20]`로 바뀌었습니다. 바뀐 프로그램은 `[10,20]` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `61`가 됩니다. 원본 추적 `[10,20]  →  [10,20,30]  →  60`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
