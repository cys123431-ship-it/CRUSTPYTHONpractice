...
exercises:
  ...
  - id: ex-day-27-python-list-fill
    ...
    answer: "minutes = [10, 20]
      ...
      print(sum(minutes))"
    hint: "[10,20]으로 시작합니다"
    explanation: 빈칸에는 'append(30)'이 들어갑니다. [10,20]으로 시작합니다 끝에 30을 추가해 길이 3이 됩니다 sum이 60을 반환합니다
    commonMistakes:
      - append의 반환값을 새 리스트라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  ...
  - id: quiz-day-27-python-list-model
    question: Python list 추가와 순회을 이해하는 데 맞는 설명은?
    choices:
...
기록 개수가 늘어날 수 있으면 크기를 바꿀 수 있는 순서 자료구조가 편리합니다. Day 27 "`Python list 추가와 순회`"에서는 Python 코드가 `60`을(를) 만드는 과정을 따라가며, 가변 시퀀스의 끝에 추가함 동작이 왜 필요한지 확인합니다. 가변 시퀀스의 끝에 추가함을(를) 빠뜨리면 `append의 반환값을 새 리스트라고 생각함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 26](/learn/day-26-c-array)에서는 "C 배열과 인덱스"을(를) 배웠습니다. "C 배열과 인덱스"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`Python list 추가와 순회`"에서 새로 달라지는 조건을 찾아보세요. Day 27의 답은 `60`이며, 핵심 표현은 `append(30)`입니다.

## 머릿속 그림

list는 원소를 순서대로 담고 append는 맨 끝에 새 원소를 넣습니다. Day 27에서는 아래 흐름 순서대로 상태가 바뀌며, `append(30)`이(가) 결과를 가릅니다.

```text
[10,20]  →  [10,20,30]  →  60
```

위 흐름에서 `append(30)`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `60`입니다.
...
아래 Python 코드는 Day 27 "`Python list 추가와 순회`"의 독립 예제입니다. 전체 3줄에서 `append(30)`이(가) 핵심이며, 실행 결과는 `60`입니다.
...
위 단계에서 `append(30)`이(가) 빠지면 `60`이(가) 나오지 않습니다. `처음 등장하는 숫자를 10에서 11로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
원본 출력은 `60`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `append(30)`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 27의 `Python list 추가와 순회`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: append의 반환값을 새 리스트라고 생각함.** 정상 코드에서는 `60`이(가) 출력됩니다.
...
핵심 동작 **가변 시퀀스의 끝에 추가함**은(는) 세 언어에서 같은 입력과 출력(`60`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `용량과 길이를 직접 관리하는 배열` 규칙으로 "`Python list 추가와 순회`"의 `60`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `list.append와 sum` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `Vec::push와 iter().sum()` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`60` 맞히기) → 빈칸(`append(30)` 채우기) → 변경(`처음 등장하는 숫자를 10에서 11로`) → 오류 수정(`append의 반환값을 새 리스트라고 생각함` 찾기) → 독립 구현(`Python list 추가와 순회을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `60` 및 `append(30)` 설명과 대조하세요.

## 스스로 설명하기

- "`Python list 추가와 순회`"이(가) 필요한 상황을 `가변 시퀀스의 끝에 추가함` 동작으로 설명해 보세요.
- 예제에서 `append(30)`이(가) 실행되기 직전의 상태와 직후의 출력 `60`을(를) 말해 보세요.
- "`append의 반환값을 새 리스트라고 생각함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `가변 시퀀스의 끝에 추가함` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

list는 원소를 순서대로 담고 append는 맨 끝에 새 원소를 넣습니다. Day 27 "`Python list 추가와 순회`"의 예제는 `append(30)`을(를) 실행해 `60`을(를) 출력합니다. "`append의 반환값을 새 리스트라고 생각함`" 여부를 확인하고 Day 27을(를) 완료하세요.
