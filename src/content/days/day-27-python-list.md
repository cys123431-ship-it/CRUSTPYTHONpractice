...
exercises:
  - id: ex-day-27-python-list-predict
    ...
    starter: "minutes = [10, 20]
      ...
      print(sum(minutes))"
    answer: "60"
    hint: "[10,20]에서 시작해 60까지 순서대로 적어 보세요. list는 원소를 순서대로 담고 append는 맨 끝에 새 원소를 넣습니다."
    explanation:
      1. [10,20]으로 시작합니다. 2. 끝에 30을 추가해 길이 3이 됩니다. 3. sum이 60을 반환합니다. `[10,20] → [10,20,30] → 60` 흐름으로
      실제 출력 `60`이 됩니다. 핵심 `append(30)`은 같은 리스트의 맨 뒤를 바꾸는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-27-python-list-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: Python list 추가와 순회의 핵심 표현을 스스로 적는다.
    ...
    answer: "minutes = [10, 20]
      ...
      print(sum(minutes))"
    hint: 필요한 표현은 가변 시퀀스의 끝에 추가함 동작을 잇는 append(30)입니다.
    explanation:
      빈칸에 들어갈 표현은 'append(30)'입니다. 이 표현이 없으면 실행 중 오류로 멈춥니다. 1. [10,20]으로 시작합니다. 2. 끝에 30을 추가해 길이 3이 됩니다.
      3. sum이 60을 반환합니다. `[10,20] → [10,20,30] → 60` 흐름으로 실제 출력 `60`이 됩니다. 핵심 `append(30)`은 같은 리스트의 맨 뒤를 바꾸는 자리에 쓰입니다.
    commonMistakes:
      - append의 반환값을 새 리스트라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-27-python-list-guided
    ...
    explanation:
      바꾼 뒤 출력은 '61'입니다. 원본은 `60`, 수정본은 `61`이다. 첫 변경 줄 `minutes = [10, 20]`에서 `minutes = [11, 20]`으로 첫 값을
      바꾸면, 뒤에 30이 붙고 합이 61이 된다.
    commonMistakes:
      - append의 반환값을 새 리스트라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-27-python-list-debug
    ...
    answer: "minutes = [10, 20]
      ...
      print(sum(minutes))"
    hint: append의 반환값을 새 리스트라고 생각함 상황에서 어긋나는 줄을 list는 원소를 순서대로 담고 append는 맨 끝에 새 원소를 넣습니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `minutes.pop(30)`입니다. `pop`은 꺼낼 위치 번호를 받는데 길이 2인 리스트에 30번을 달라고 하므로, 첫 실행에서 IndexError라는 런타임
      오류가 나고 프로그램이 멈춥니다. 오류 분류는 런타임 오류입니다. 흐름이 깨지는 첫 순간은 존재하지 않는 위치를 꺼내는 호출입니다. 고친 줄 `minutes.append(30)`에서는 `가변 시퀀스의
      끝에 추가함` 동작이 지켜집니다. `append`는 새 리스트를 돌려주지 않고 같은 리스트를 바꾸므로, 반환값을 새 리스트라고 받아 쓰면 `None`이 들어갑니다. 정상 코드는 시작→추가→합산의
      순서로 `60`을 출력합니다.
    commonMistakes:
      - append의 반환값을 새 리스트라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-27-python-list-independent
    ...
    answer: "minutes = [10, 20]
      ...
      print(sum(minutes))"
    hint:
      빈 리스트에 append한 뒤 길이와 첫 원소가 어떻게 달라지는지 시험하세요. `items = items.append(30)`이면 `None`이 들어갑니다. 예상 출력과 경계 조건도 함께
      적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 빈 리스트에 append한 뒤 길이와 첫 원소가 어떻게 달라지는지 시험하세요. `items = items.append(30)`이면
      `None`이 들어갑니다. 같은 가변 시퀀스의 끝에 추가함 동작을 구현하고 실행 결과 '60'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - append의 반환값을 새 리스트라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-27-python-list-output
    ...
    answerIndex: 0
    explanation:
      먼저 [10,20] 리스트가 만들어집니다. 이어서 append로 맨 뒤에 30이 생겨 [10,20,30]이 되고, sum이 합 60을 반환해 출력하므로 `60`이 됩니다. `실행
      전에 반드시 오류가 난다`는 틀린 선택지인데 리스트 연산이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기 때문입니다. 다른 선택지는 이 추적과
      맞지 않습니다.
  - id: quiz-day-27-python-list-model
    ...
    answerIndex: 1
    explanation:
      list는 원소를 순서대로 담는다는 뜻이고, append는 맨 끝에 새 원소를 넣는다는 뜻은 같은 리스트가 바뀐다는 뜻입니다. `append의 반환값을 새 리스트라고 생각함`은
      반대 사례인데, append는 `None`을 돌려주므로 변수에 받으면 리스트가 사라지기 때문입니다.
  - id: quiz-day-27-python-list-pitfall
    ...
    choices:
      ...
      - append의 반환값을 새 리스트라고 생각함
    answerIndex: 2
    explanation: 먼저 확인할 실수는 `append의 반환값을 새 리스트라고 생각함`입니다. 잘못된 코드는 존재하지 않는 위치를 꺼내려 해 IndexError로 멈춥니다. 위치 번호와 값을 구별하세요.
  - id: quiz-day-27-python-list-transfer
    ...
    answerIndex: 0
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `가변 시퀀스의 끝에 추가함`이라는 의미와 입력·출력 계약입니다. C의 크기 고정 배열과 Rust의 Vec push로 같은 동작을 구현하고,
...
예제의 핵심 표현은 `append(30)`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본은 `60`, 수정본은 `61`이다. 첫 변경 줄 `minutes = [10, 20]`에서 `minutes = [11, 20]`으로 첫 값을 바꾸면, 뒤에 30이 붙고 합이 61이 된다.
...
두 줄을 나란히 놓으면 정상 줄 `minutes.append(30)`이 `list는 원소를 순서대로 담고 append는 맨 끝에 새 원소를 넣습니다.` 설명과 맞고, 잘못된 줄은 `append의 반환값을 새 리스트라고 생각함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `[10,20]`부터 `60`까지 처음 어긋난 곳을 찾습니다.
