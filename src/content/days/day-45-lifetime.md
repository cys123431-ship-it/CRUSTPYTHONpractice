...
exercises:
  ...
  - id: ex-day-45-lifetime-fill
    ...
    explanation: 빈칸에 들어갈 표현은 "'a"입니다. study 길이는 5, log 길이는 3입니다 첫 참조를 선택합니다 유효한 문자열 study를 출력합니다
    commonMistakes:
      - 지역에서 만든 String의 참조를 함수 밖으로 반환함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-45-lifetime-guided
    ...
    explanation:
      바꾼 뒤 출력은 'study!'입니다. 원본 출력 'study'에서 달라졌습니다. 다른 줄(fn main() { println!("{}", longer("study", "log"));
      } → fn main() { println!("{}!", longer("study", "log")); })에서 시작한 차이가 최종 출력에 반영되었습니다.
    commonMistakes:
      - 지역에서 만든 String의 참조를 함수 밖으로 반환함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
...
quiz:
  - id: quiz-day-45-lifetime-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 0
    explanation: x 길이 5  →  y 길이 3  →  x 반환  →  study 순서로 실행되어 출력은 'study'입니다.
...
원본과 달라졌습니다. 원본 출력은 `study`입니다. 다른 줄(`fn main() { println!("{}", longer("study", "log")); }` → `fn main() { println!("{}!", longer("study", "log")); }`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`x 길이 5` → …)과 바뀐 줄부터 대조해 보세요.
