...
exercises:
  - id: ex-day-45-lifetime-predict
    ...
    answer: study
    hint: "'a는 반환 참조가 입력 참조들이 함께 유효한 범위를 넘지 않겠다는 관계를 표시합니다. 설명을 떠올리고 `x 길이 5` 단계부터 순서대로 적어 보세요."
    explanation:
      x 길이 5  →  y 길이 3  →  x 반환  →  study 순서로 실행됩니다. `'a` 부분이 `study` 단계를 확정해 최종 출력 'study'가 됩니다. 이 흐름을
      떠올리면 `입력 참조와 반환 참조 수명 관계 명시` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-45-lifetime-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: "'Rust 참조의 유효 수명' 개념의 핵심 표현을 스스로 적는다."
    ...
    hint: 힌트 문장을 완전하게 읽으면 `study 길이는 5, log 길이는 3입니다` 단계에 필요한 표현이 `'a`입니다.
    explanation:
      '빈칸에 들어갈 표현은 "''a"입니다. `fn longer<''a>(x: &''a str, y: &''a str) -> &''a str {` 줄을 완성해야 `입력 참조와 반환
      참조 수명 관계 명시` 동작이 이어져 실행 결과 ''study''가 됩니다. 힌트의 첫 단계 `study 길이는 5, log 길이는 3입니다`이 바로 이 줄입니다. 이어서 첫 참조를 선택합니다
      유효한 문자열 study를 출력합니다 순서로 진행됩니다.'
    commonMistakes:
      - 지역에서 만든 String의 참조를 함수 밖으로 반환함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-45-lifetime-guided
    ...
    explanation:
      바꾼 뒤 출력은 'study!'입니다. 원본 출력 'study'에서 달라졌습니다. 바뀐 줄은 `fn main() { println!("{}", longer("study", "log"));
      }`에서 `fn main() { println!("{}!", longer("study", "log")); }`로 바뀌었습니다. 바뀐 프로그램은 `x 길이 5` 단계에서 시작해 바뀐 줄에서 다른
      중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `study!`가 됩니다. 원본 추적 `x 길이 5  →  y 길이 3  →  x 반환  →  study`와 바뀐 줄 이후를 순서대로 비교하면
      처음 달라지는 곳이 보입니다.
    commonMistakes:
      - 지역에서 만든 String의 참조를 함수 밖으로 반환함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-45-lifetime-debug
    ...
    answer:
      ...
      \ main() { println!(\"{}\", longer(\"study\", \"log\")); }"
    hint:
      "'a는 반환 참조가 입력 참조들이 함께 유효한 범위를 넘지 않겠다는 관계를 표시합니다. 설명과 어긋나는 줄을 찾으세요. `지역에서 만든 String의 참조를 함수 밖으로 반환함` 상황이
      단서가 됩니다."
    explanation:
      "틀린 줄은 `fn longer<'static>(x: &'a str, y: &'a str) -> &'a str {`입니다. 여기서는 `'static`을 써서 `'a`
      동작이 깨집니다. 이대로 실행하면 `지역에서 만든 String의 참조를 함수 밖으로 반환함` 문제가 생겨 원본 추적 `x 길이 5  →  y 길이 3  →  x 반환  →  study`대로 'study'가
      나오지 않습니다. 고친 줄 `fn longer<'a>(x: &'a str, y: &'a str) -> &'a str {`에서는 `'a`가 `입력 참조와 반환 참조 수명 관계 명시` 동작을
      지켜 'study'까지 도달합니다."
    commonMistakes:
      - 지역에서 만든 String의 참조를 함수 밖으로 반환함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-45-lifetime-independent
    ...
    answer:
      "fn longer<'a>(x: &'a str, y: &'a str) -> &'a str {\n    if x.len() >= y.len() { x } else { y }\n}\nfn\
      \ main() { println!(\"{}\", longer(\"study\", \"log\")); }"
    hint: "'a는 반환 참조가 입력 참조들이 함께 유효한 범위를 넘지 않겠다는 관계를 표시합니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요."
    explanation:
      한 가지 예시 해법은 위 코드입니다. `'a` 부분이 `입력 참조와 반환 참조 수명 관계 명시` 동작을 지켜 실행 결과 'study'가 됩니다. 같은 개념을 다른 입력으로 바꿔도
      `'a`부터 `study`까지 추적할 수 있으면 정답입니다. `지역에서 만든 String의 참조를 함수 밖으로 반환함` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - 지역에서 만든 String의 참조를 함수 밖으로 반환함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
...
quiz:
  - id: quiz-day-45-lifetime-output
    ...
    answerIndex: 0
    explanation:
      x 길이 5  →  y 길이 3  →  x 반환  →  study 순서로 실행되어 출력은 'study'입니다. `'a` 부분이 마지막 단계를 확정하므로 다른 선택지는 이 추적과
      맞지 않습니다.
  - id: quiz-day-45-lifetime-model
    question: "'Rust 참조의 유효 수명' 개념을 이해하는 데 맞는 설명은?"
    choices:
    ...
    answerIndex: 1
    explanation:
      "'a는 반환 참조가 입력 참조들이 함께 유효한 범위를 넘지 않겠다는 관계를 표시합니다. 이 설명이 맞는 이유는 `입력 참조와 반환 참조 수명 관계 명시` 동작을 지키는 조건과
      같기 때문입니다. `지역에서 만든 String의 참조를 함수 밖으로 반환함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다."
  - id: quiz-day-45-lifetime-pitfall
    ...
    answerIndex: 2
    explanation:
      지역에서 만든 String의 참조를 함수 밖으로 반환함. 이 실수가 나오면 원본 추적 `x 길이 5 → y 길이 3 → x 반환 → study`대로 'study'가 나오지 않으므로
      먼저 확인해야 합니다.
  - id: quiz-day-45-lifetime-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 0
    explanation: 문법은 달라도 `입력 참조와 반환 참조 수명 관계 명시` 목적과 입력·출력은 유지합니다. 실행 결과 'study'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
원본 출력은 `study`이고, 바뀐 코드의 실행 결과는 `study!`입니다. 바뀐 줄은 `fn main() { println!("{}", longer("study", "log")); }`에서 `fn main() { println!("{}!", longer("study", "log")); }`로 바뀌었습니다. 바뀐 프로그램은 `x 길이 5` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `study!`가 됩니다. 원본 추적 `x 길이 5  →  y 길이 3  →  x 반환  →  study`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
...
두 줄을 나란히 놓으면 정상 줄 `fn longer<'a>(x: &'a str, y: &'a str) -> &'a str {`이 `'a는 반환 참조가 입력 참조들이 함께 유효한 범위를 넘지 않겠다는 관계를 표시합니다.` 설명과 맞고, 잘못된 줄은 `지역에서 만든 String의 참조를 함수 밖으로 반환함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `x 길이 5`부터 `study`까지 처음 어긋난 곳을 찾습니다.
...
예측(`study` 맞히기) → 빈칸(`'a` 채우기) → 변경(`첫 문자열을 '{}'에서 '{}!'로`) → 오류 수정(`지역에서 만든 String의 참조를 함수 밖으로 반환함` 찾기) → 독립 구현(`'Rust 참조의 유효 수명' 개념을 보여주는 작은 프로그램` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `study` 및 `'a` 설명과 대조하세요.
