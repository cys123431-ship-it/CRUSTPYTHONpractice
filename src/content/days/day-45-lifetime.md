...
둘 중 하나의 참조를 반환하는 함수는 결과가 얼마나 오래 유효한지 호출자에게 알려야 합니다. Day 45 "`Rust 참조의 유효 수명`"에서는 Rust 코드가 `study`을(를) 만드는 과정을 따라가며, 입력 참조와 반환 참조 수명 관계 명시 동작이 왜 필요한지 확인합니다. 입력 참조와 반환 참조 수명 관계 명시을(를) 빠뜨리면 `지역에서 만든 String의 참조를 함수 밖으로 반환함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 44](/learn/day-44-mutable-arguments)에서는 "Python 함수의 가변 인자"을(를) 배웠습니다. "Python 함수의 가변 인자"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`Rust 참조의 유효 수명`"에서 새로 달라지는 조건을 찾아보세요. Day 45의 답은 `study`이며, 핵심 표현은 `'a`입니다.

## 머릿속 그림

'a는 반환 참조가 입력 참조들이 함께 유효한 범위를 넘지 않겠다는 관계를 표시합니다. Day 45에서는 아래 흐름 순서대로 상태가 바뀌며, `'a`이(가) 결과를 가릅니다.

```text
x 길이 5  →  y 길이 3  →  x 반환  →  study
```

위 흐름에서 `'a`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `study`입니다.
...
아래 Rust 코드는 Day 45 "`Rust 참조의 유효 수명`"의 독립 예제입니다. 전체 4줄에서 `'a`이(가) 핵심이며, 실행 결과는 `study`입니다.
...
Rust는 `rustc --edition=2024` 또는 Cargo로 실행하면 `study`이(가) 나옵니다. 브라우저에서는 임의 컴파일 대신 하단 실습 칸의 답안 비교를 사용하세요.
...
위 단계에서 `'a`이(가) 빠지면 `study`이(가) 나오지 않습니다. `첫 문자열을 '{}'에서 '{}!'로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `study`이(가) 됩니다. `x 길이 5` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `study`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `'a`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 45의 `Rust 참조의 유효 수명`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 지역에서 만든 String의 참조를 함수 밖으로 반환함.** 정상 코드에서는 `study`이(가) 출력됩니다.
...
핵심 동작 **입력 참조와 반환 참조 수명 관계 명시**은(는) 세 언어에서 같은 입력과 출력(`study`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `호출자가 반환 포인터의 수명을 직접 확인` 규칙으로 "`Rust 참조의 유효 수명`"의 `study`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `참조 수명을 런타임이 관리` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `반환 참조와 입력 참조의 lifetime 연결` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`Rust 참조의 유효 수명`"이(가) 필요한 상황을 `입력 참조와 반환 참조 수명 관계 명시` 동작으로 설명해 보세요.
- 예제에서 `'a`이(가) 실행되기 직전의 상태와 직후의 출력 `study`을(를) 말해 보세요.
- "`지역에서 만든 String의 참조를 함수 밖으로 반환함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `입력 참조와 반환 참조 수명 관계 명시` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

'a는 반환 참조가 입력 참조들이 함께 유효한 범위를 넘지 않겠다는 관계를 표시합니다. Day 45 "`Rust 참조의 유효 수명`"의 예제는 `'a`을(를) 실행해 `study`을(를) 출력합니다. "`지역에서 만든 String의 참조를 함수 밖으로 반환함`" 여부를 확인하고 Day 45을(를) 완료하세요.
