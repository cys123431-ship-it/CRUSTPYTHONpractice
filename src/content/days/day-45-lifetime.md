...
exercises:
  - id: ex-day-45-lifetime-predict
    ...
    answer: study
    hint: x 길이 5에서 시작해 study까지 순서대로 적어 보세요. 'a는 반환 참조가 입력 참조들이 함께 유효한 범위를 넘지 않겠다는 관계를 표시합니다.
    explanation:
      1. study 길이는 5, log 길이는 3입니다. 2. 첫 참조를 선택합니다. 3. 유효한 문자열 study를 출력합니다. `x 길이 5 → y 길이 3 → x 반환 →
      study` 흐름으로 실제 출력 `study`가 됩니다. 핵심 수명 표기는 입력과 반환 참조의 유효 범위를 묶는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-45-lifetime-fill
    ...
    hint: 필요한 표현은 입력 참조와 반환 참조 수명 관계 명시 동작을 잇는 'a입니다.
    explanation:
      빈칸에 들어갈 표현은 "'a"입니다. 이 표현이 없으면 컴파일 단계에서 막혀 실행 자체가 되지 않습니다. 1. study 길이는 5, log 길이는 3입니다. 2. 첫 참조를
      선택합니다. 3. 유효한 문자열 study를 출력합니다. `x 길이 5 → y 길이 3 → x 반환 → study` 흐름으로 실제 출력 `study`가 됩니다. 핵심 수명 표기는 입력과 반환 참조의
      유효 범위를 묶는 자리에 쓰입니다.
    commonMistakes:
      - 지역에서 만든 String의 참조를 함수 밖으로 반환함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-45-lifetime-guided
    ...
    answer:
      "fn longer<'a>(x: &'a str, y: &'a str) -> &'a str {\n    if x.len() >= y.len() { x } else { y }\n}\nfn\
      \ main() { println!(\"{}!\", longer(\"study\", \"log\")); }"
    hint: 첫 문자열을 '{}'에서 '{}!'로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 'study'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 'study!'입니다. 원본은 `study`, 수정본은 `study!`이다. 첫 변경 줄의 형식 문자열 `"{}"`에서 `"{}!"`로 표시 모양을 바꾸면,
      계산된 반환 참조는 그대로이고 화면에만 느낌표가 붙는다.
    commonMistakes:
      - 지역에서 만든 String의 참조를 함수 밖으로 반환함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-45-lifetime-debug
    ...
    answer:
      "fn longer<'a>(x: &'a str, y: &'a str) -> &'a str {\n    if x.len() >= y.len() { x } else { y }\n}\nfn\
      \ main() { println!(\"{}\", longer(\"study\", \"log\")); }"
    hint: 지역에서 만든 String의 참조를 함수 밖으로 반환함 상황에서 어긋나는 줄을 'a는 반환 참조가 입력 참조들이 함께 유효한 범위를 넘지 않겠다는 관계를 표시합니다. 설명과 대조해 보세요.
    explanation:
      "틀린 줄은 `fn longer<수명/static>(x: 수명/a str, y: 수명/a str) -> 수명/a str {`처럼 선언과 사용의 수명 이름이 어긋난 선언입니다.
      선언되지 않은 수명 이름을 쓰므로 컴파일 단계에서 오류가 나고 실행 파일 자체가 만들어지지 않아 아무것도 출력되지 않습니다. 오류 분류는 컴파일 오류입니다. 흐름이 깨지는 첫 순간은 함수 선언
      줄입니다. 고친 줄의 일치된 수명 표기에서는 `입력 참조와 반환 참조 수명 관계 명시` 동작이 지켜집니다. 지역 문자열의 참조를 밖으로 돌려주는 코드는 원본이 사라지므로 같은 단계에서 막힙니다.
      정상 코드는 길이 비교→첫 참조 선택→유효 문자열 출력의 순서로 `study`를 출력합니다."
    commonMistakes:
      - 지역에서 만든 String의 참조를 함수 밖으로 반환함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-45-lifetime-independent
    ...
    answer:
      "fn longer<'a>(x: &'a str, y: &'a str) -> &'a str {\n    if x.len() >= y.len() { x } else { y }\n}\nfn\
      \ main() { println!(\"{}\", longer(\"study\", \"log\")); }"
    hint: 수명 표기는 원본을 오래 살게 만드는 마법이 아닙니다. 소유 반환과 호출자 데이터 참조 반환으로 고치세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 수명 표기는 원본을 오래 살게 만드는 마법이 아닙니다. 소유 반환과 호출자 데이터 참조 반환으로 고치세요. 같은 입력 참조와
      반환 참조 수명 관계 명시 동작을 구현하고 실행 결과 'study'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 지역에서 만든 String의 참조를 함수 밖으로 반환함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
...
quiz:
  - id: quiz-day-45-lifetime-output
    ...
    answerIndex: 0
    explanation:
      먼저 study 길이 5와 log 길이 3을 비교합니다. 이어서 첫 참조가 선택되어 유효한 문자열 study가 반환되고, println!이 표시하므로 출력 `study`가 됩니다.
      `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 수명 관계와 호출이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 println!이 실제로 호출되기 때문입니다. 다른
      선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-45-lifetime-model
    ...
    answerIndex: 1
    explanation:
      수명 표기는 입력 참조와 반환 참조가 함께 유효해야 하는 범위를 관계로 표시한다는 뜻이고, 원본이 살아 있는 동안만 쓴다는 뜻은 함수 안 지역 값의 참조를 밖으로 내보낼 수
      없다는 뜻입니다. `지역에서 만든 String의 참조를 함수 밖으로 반환함`은 반대 사례인데, 함수가 끝나면 원본이 사라지기 때문입니다.
  - id: quiz-day-45-lifetime-pitfall
    ...
    answerIndex: 2
    explanation:
      먼저 확인할 실수는 `지역에서 만든 String의 참조를 함수 밖으로 반환함`입니다. 잘못된 코드는 선언이 어긋나 컴파일 오류가 나므로 화면 출력 비교 이전에 실행 파일 생성
      자체가 막힙니다.
  - id: quiz-day-45-lifetime-transfer
    ...
    answerIndex: 0
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `입력 참조와 반환 참조 수명 관계 명시`라는 의미와 입력·출력 계약입니다. C의 유효 수명 수동 관리와 Python의 참조 유효성으로
      같은 동작을 구현하고, 실행 결과 `study`로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
원본 실행 결과는 `study`입니다. 아래 코드에서 원본과 달라지는 첫 줄을 표시하고, 그 줄의 변경 전후 값을 적어 보세요.
...
원본은 `study`, 수정본은 `study!`이다. 첫 변경 줄의 형식 문자열 `"{}"`에서 `"{}!"`로 표시 모양을 바꾸면, 계산된 반환 참조는 그대로이고 화면에만 느낌표가 붙는다.
