...
exercises:
  ...
  - id: ex-day-08-boolean-fill
    ...
    hint: ready는 true입니다
    explanation: 빈칸에는 '&& !focused'이 들어갑니다. ready는 true입니다 !focused는 !false이므로 true입니다 true && true가 true입니다
    commonMistakes:
      - "&&와 ||를 혼동해 하나만 참이어도 통과시킴"
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-08-boolean-guided
    ...
    explanation:
      예시 답안에서는 첫 문자열을 '{}'에서 '{}!'로 바꿨습니다. 원래 출력은 'true'입니다. 바꾼 줄에서 시작해 중간 값과 마지막 출력을 다시 추적하세요. 출력이 같더라도
...
quiz:
  - id: quiz-day-08-boolean-output
    ...
    choices:
      ...
      - "true"
    answerIndex: 2
    explanation: ready=true  →  !focused=true  →  true 출력 순서로 실행되어 'true'을 출력합니다.
  - id: quiz-day-08-boolean-model
    question: 불리언과 논리 연산을 이해하는 데 맞는 설명은?
    choices:
...
여러 조건을 합쳐 행동을 결정할 때 참·거짓을 명확하게 계산해야 합니다. Day 08 "`불리언과 논리 연산`"에서는 Rust 코드가 `true`을(를) 만드는 과정을 따라가며, 두 조건의 논리곱을 구함 동작이 왜 필요한지 확인합니다. 두 조건의 논리곱을 구함을(를) 빠뜨리면 `&&와 ||를 혼동해 하나만 참이어도 통과시킴` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 07](/learn/day-07-foundation-review)에서는 "기초 1주차 회상과 오류 찾기"을(를) 배웠습니다. "기초 1주차 회상과 오류 찾기"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`불리언과 논리 연산`"에서 새로 달라지는 조건을 찾아보세요. Day 08의 답은 `true`이며, 핵심 표현은 `&& !focused`입니다.

## 머릿속 그림

&&는 두 조건이 모두 참이어야 하고 !는 조건의 참·거짓을 뒤집습니다. Day 08에서는 아래 흐름 순서대로 상태가 바뀌며, `&& !focused`이(가) 결과를 가릅니다.

```text
ready=true  →  !focused=true  →  true 출력
```

위 흐름에서 `&& !focused`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `true`입니다.
...
아래 Rust 코드는 Day 08 "`불리언과 논리 연산`"의 독립 예제입니다. 전체 5줄에서 `&& !focused`이(가) 핵심이며, 실행 결과는 `true`입니다.
...
Rust는 `rustc --edition=2024` 또는 Cargo로 실행하면 `true`이(가) 나옵니다. 브라우저에서는 임의 컴파일 대신 하단 실습 칸의 답안 비교를 사용하세요.
...
위 단계에서 `&& !focused`이(가) 빠지면 `true`이(가) 나오지 않습니다. `첫 문자열을 '{}'에서 '{}!'로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `true`이(가) 됩니다. `ready=true` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 코드에서 `첫 문자열을 '{}'에서 '{}!'로` 바꾸면 아래와 같이 됩니다.
...
원본 출력은 `true`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `&& !focused`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 08의 `불리언과 논리 연산`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: &&와 ||를 혼동해 하나만 참이어도 통과시킴.** 정상 코드에서는 `true`이(가) 출력됩니다.
...
핵심 동작 **두 조건의 논리곱을 구함**은(는) 세 언어에서 같은 입력과 출력(`true`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `stdbool.h의 bool과 &&, !` 규칙으로 "`불리언과 논리 연산`"의 `true`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `True and not focused` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `true && !focused` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`true` 맞히기) → 빈칸(`&& !focused` 채우기) → 변경(`첫 문자열을 '{}'에서 '{}!'로`) → 오류 수정(`&&와 ||를 혼동해 하나만 참이어도 통과시킴` 찾기) → 독립 구현(`불리언과 논리 연산을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `true` 및 `&& !focused` 설명과 대조하세요.

## 스스로 설명하기

- "`불리언과 논리 연산`"이(가) 필요한 상황을 `두 조건의 논리곱을 구함` 동작으로 설명해 보세요.
- 예제에서 `&& !focused`이(가) 실행되기 직전의 상태와 직후의 출력 `true`을(를) 말해 보세요.
- "`&&와 ||를 혼동해 하나만 참이어도 통과시킴`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `두 조건의 논리곱을 구함` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

&&는 두 조건이 모두 참이어야 하고 !는 조건의 참·거짓을 뒤집습니다. Day 08 "`불리언과 논리 연산`"의 예제는 `&& !focused`을(를) 실행해 `true`을(를) 출력합니다. "`&&와 ||를 혼동해 하나만 참이어도 통과시킴`" 여부를 확인하고 Day 08을(를) 완료하세요.
