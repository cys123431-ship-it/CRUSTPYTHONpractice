...
exercises:
  - id: ex-day-53-rust-enum-predict
    ...
    answer: 통과
    hint: enum은 정해진 경우만 값으로 만들게 하고 match는 모든 경우를 처리하게 합니다. 설명을 떠올리고 `state=Pass` 단계부터 순서대로 적어 보세요.
    explanation:
      state=Pass  →  Pass arm  →  통과 순서로 실행됩니다. `ResultLabel::Pass` 부분이 `통과` 단계를 확정해 최종 출력 '통과'가 됩니다. 이
      흐름을 떠올리면 `한정된 상태를 타입으로 표현` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-53-rust-enum-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: "'Rust enum으로 상태 표현' 개념의 핵심 표현을 스스로 적는다."
    ...
    answer:
      "enum ResultLabel { Pass, Retry }\nfn main() {\n    let state = ResultLabel::Pass;\n    let label = match\
      \ state { ResultLabel::Pass => \"통과\", ResultLabel::Retry => \"재시도\" };\n    println!(\"{label}\");\n}"
    hint: 힌트 문장을 완전하게 읽으면 `Pass 상태를 선택합니다` 단계에 필요한 표현이 `ResultLabel::Pass`입니다.
    explanation:
      빈칸에 들어갈 표현은 'ResultLabel::Pass'입니다. `let state = ResultLabel::Pass;` 줄을 완성해야 `한정된 상태를 타입으로 표현` 동작이
      이어져 실행 결과 '통과'가 됩니다. 힌트의 첫 단계 `Pass 상태를 선택합니다`이 바로 이 줄입니다. 이어서 match가 Pass arm을 실행합니다 통과를 출력합니다 순서로 진행됩니다.
    commonMistakes:
      - 가능한 enum 경우를 match에서 하나 빠뜨림
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-53-rust-enum-guided
    ...
    prompt: "예시를 직접 타이핑한 뒤 다음을 바꾸세요: 첫 문자열을 '통과'에서 '통과!'로. 출력이 바뀌는지 먼저 예측하고, 그대로라면 그 이유도 설명하세요."
    starter:
      "enum ResultLabel { Pass, Retry }\nfn main() {\n    let state = ResultLabel::Pass;\n    let label = match\
      \ state { ResultLabel::Pass => \"통과\", ResultLabel::Retry => \"재시도\" };\n    println!(\"{label}\");\n}"
    answer:
      "enum ResultLabel { Pass, Retry }\nfn main() {\n    let state = ResultLabel::Pass;\n    let label = match\
      \ state { ResultLabel::Pass => \"통과!\", ResultLabel::Retry => \"재시도\" };\n    println!(\"{label}\");\n}"
    hint: "다음을 바꾼 뒤 원본 실행 추적과 처음 달라지는 지점을 찾아보세요: 첫 문자열을 '통과'에서 '통과!'로."
    explanation:
      바꾼 뒤 출력은 '통과!'입니다. 원본 출력 '통과'에서 달라졌습니다. 바뀐 줄은 `let label = match state { ResultLabel::Pass => "통과",
      ResultLabel::Retry => "재시도" };`에서 `let label = match state { ResultLabel::Pass => "통과!", ResultLabel::Retry
      => "재시도" };`로 바뀌었습니다. 바뀐 프로그램은 `state=Pass` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `통과!`가 됩니다. 원본
      추적 `state=Pass  →  Pass arm  →  통과`와 바뀐 줄 이후를 순서대로 비교하면 처음 달라지는 곳이 보입니다.
    commonMistakes:
      - 가능한 enum 경우를 match에서 하나 빠뜨림
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-53-rust-enum-debug
    ...
    answer:
      ...
      \ state { ResultLabel::Pass => \"통과\", ResultLabel::Retry => \"재시도\" };\n    println!(\"{label}\");\n}"
    hint:
      enum은 정해진 경우만 값으로 만들게 하고 match는 모든 경우를 처리하게 합니다. 설명과 어긋나는 줄을 찾으세요. `가능한 enum 경우를 match에서 하나 빠뜨림` 상황이 단서가
      됩니다.
    explanation:
      틀린 줄은 `let state = ResultLabel::Retry;`입니다. 여기서는 `ResultLabel::Retry`을 써서 `ResultLabel::Pass` 동작이
      깨집니다. 이대로 실행하면 `가능한 enum 경우를 match에서 하나 빠뜨림` 문제가 생겨 원본 추적 `state=Pass  →  Pass arm  →  통과`대로 '통과'가 나오지 않습니다.
      고친 줄 `let state = ResultLabel::Pass;`에서는 `ResultLabel::Pass`가 `한정된 상태를 타입으로 표현` 동작을 지켜 '통과'까지 도달합니다.
    commonMistakes:
      - 가능한 enum 경우를 match에서 하나 빠뜨림
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-53-rust-enum-independent
    ...
    answer:
      "enum ResultLabel { Pass, Retry }\nfn main() {\n    let state = ResultLabel::Pass;\n    let label = match\
      \ state { ResultLabel::Pass => \"통과\", ResultLabel::Retry => \"재시도\" };\n    println!(\"{label}\");\n}"
    hint: enum은 정해진 경우만 값으로 만들게 하고 match는 모든 경우를 처리하게 합니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `ResultLabel::Pass` 부분이 `한정된 상태를 타입으로 표현` 동작을 지켜 실행 결과 '통과'가 됩니다. 같은 개념을 다른
      입력으로 바꿔도 `ResultLabel::Pass`부터 `통과`까지 추적할 수 있으면 정답입니다. `가능한 enum 경우를 match에서 하나 빠뜨림` 상황과 빈 입력 같은 경계도 함께 설명해
      보세요.
    commonMistakes:
      - 가능한 enum 경우를 match에서 하나 빠뜨림
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
...
quiz:
  - id: quiz-day-53-rust-enum-output
    ...
    answerIndex: 2
    explanation:
      state=Pass  →  Pass arm  →  통과 순서로 실행되어 출력은 '통과'입니다. `ResultLabel::Pass` 부분이 마지막 단계를 확정하므로 다른 선택지는
      이 추적과 맞지 않습니다.
  - id: quiz-day-53-rust-enum-model
    ...
    answerIndex: 0
    explanation:
      enum은 정해진 경우만 값으로 만들게 하고 match는 모든 경우를 처리하게 합니다. 이 설명이 맞는 이유는 `한정된 상태를 타입으로 표현` 동작을 지키는 조건과 같기 때문입니다.
      `가능한 enum 경우를 match에서 하나 빠뜨림` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-53-rust-enum-pitfall
    ...
    answerIndex: 1
    explanation:
      가능한 enum 경우를 match에서 하나 빠뜨림. 이 실수가 나오면 원본 추적 `state=Pass → Pass arm → 통과`대로 '통과'가 나오지 않으므로 먼저 확인해야
      합니다.
  - id: quiz-day-53-rust-enum-transfer
    ...
    answerIndex: 2
    explanation: 문법은 달라도 `한정된 상태를 타입으로 표현` 목적과 입력·출력은 유지합니다. 실행 결과 '통과'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
원본 출력은 `통과`이고, 바뀐 코드의 실행 결과는 `통과!`입니다. 바뀐 줄은 `let label = match state { ResultLabel::Pass => "통과", ResultLabel::Retry => "재시도" };`에서 `let label = match state { ResultLabel::Pass => "통과!", ResultLabel::Retry => "재시도" };`로 바뀌었습니다. 바뀐 프로그램은 `state=Pass` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `통과!`가 됩니다. 원본 추적 `state=Pass  →  Pass arm  →  통과`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
