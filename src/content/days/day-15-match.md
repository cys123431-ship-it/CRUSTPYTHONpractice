...
exercises:
  ...
  - id: ex-day-15-match-fill
    ...
    answer:
      ...
      \ };\n    println!(\"{label}\");\n}"
    hint: level은 2입니다
    explanation: 빈칸에 들어갈 표현은 '2 =>'입니다. level은 2입니다 1 패턴을 건너뛰고 2 패턴을 선택합니다 label에 기본을 저장해 출력합니다
    commonMistakes:
      - 모든 패턴을 덮지 않아 컴파일이 실패함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-15-match-guided
    ...
    explanation: 바꾼 뒤 출력은 '심화'입니다. 원본 출력 '기본'에서 달라졌습니다. 다른 줄(let level = 2; → let level = 3;)에서 시작한 차이가 최종 출력에 반영되었습니다.
    commonMistakes:
      - 모든 패턴을 덮지 않아 컴파일이 실패함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
...
quiz:
  - id: quiz-day-15-match-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 0
    explanation: level=2  →  2 arm 선택  →  기본 순서로 실행되어 출력은 '기본'입니다.
...
원본과 달라졌습니다. 원본 출력은 `기본`입니다. 다른 줄(`let level = 2;` → `let level = 3;`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`level=2` → …)과 바뀐 줄부터 대조해 보세요.
