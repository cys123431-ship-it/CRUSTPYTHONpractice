...
조건 분기가 여러 개일 때 빠진 경우가 없도록 모든 패턴을 명시합니다. Day 15 "`Rust match로 경우 나누기`"에서는 Rust 코드가 `기본`을(를) 만드는 과정을 따라가며, 완전한 패턴 분기 동작이 왜 필요한지 확인합니다. 완전한 패턴 분기을(를) 빠뜨리면 `모든 패턴을 덮지 않아 컴파일이 실패함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 14](/learn/day-14-branch-review)에서는 "분기와 경계조건 회상"을(를) 배웠습니다. "분기와 경계조건 회상"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`Rust match로 경우 나누기`"에서 새로 달라지는 조건을 찾아보세요. Day 15의 답은 `기본`이며, 핵심 표현은 `2 =>`입니다.

## 머릿속 그림

match는 위에서 아래로 비교해 처음 맞는 arm을 선택하며 _는 나머지 모든 값을 받습니다. Day 15에서는 아래 흐름 순서대로 상태가 바뀌며, `2 =>`이(가) 결과를 가릅니다.

```text
level=2  →  2 arm 선택  →  기본
```

위 흐름에서 `2 =>`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `기본`입니다.
...
아래 Rust 코드는 Day 15 "`Rust match로 경우 나누기`"의 독립 예제입니다. 전체 5줄에서 `2 =>`이(가) 핵심이며, 실행 결과는 `기본`입니다.
...
Rust는 `rustc --edition=2024` 또는 Cargo로 실행하면 `기본`이(가) 나옵니다. 브라우저에서는 임의 컴파일 대신 하단 실습 칸의 답안 비교를 사용하세요.
...
위 단계에서 `2 =>`이(가) 빠지면 `기본`이(가) 나오지 않습니다. `처음 등장하는 숫자를 2에서 3로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `기본`이(가) 됩니다. `level=2` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `기본`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `2 =>`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 15의 `Rust match로 경우 나누기`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 모든 패턴을 덮지 않아 컴파일이 실패함.** 정상 코드에서는 `기본`이(가) 출력됩니다.
...
핵심 동작 **완전한 패턴 분기**은(는) 세 언어에서 같은 입력과 출력(`기본`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `switch와 default, 또는 if/else` 규칙으로 "`Rust match로 경우 나누기`"의 `기본`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `match/case와 case _` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `match와 모든 arm을 덮는 _` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`Rust match로 경우 나누기`"이(가) 필요한 상황을 `완전한 패턴 분기` 동작으로 설명해 보세요.
- 예제에서 `2 =>`이(가) 실행되기 직전의 상태와 직후의 출력 `기본`을(를) 말해 보세요.
- "`모든 패턴을 덮지 않아 컴파일이 실패함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `완전한 패턴 분기` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

match는 위에서 아래로 비교해 처음 맞는 arm을 선택하며 _는 나머지 모든 값을 받습니다. Day 15 "`Rust match로 경우 나누기`"의 예제는 `2 =>`을(를) 실행해 `기본`을(를) 출력합니다. "`모든 패턴을 덮지 않아 컴파일이 실패함`" 여부를 확인하고 Day 15을(를) 완료하세요.
