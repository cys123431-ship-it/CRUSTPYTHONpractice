...
exercises:
  - id: ex-day-37-c-strings-predict
    ...
    starter:
      ...
      %zu\\n\", strlen(word));\n    return 0;\n}"
    answer: "3"
    hint: c a t \0에서 시작해 출력까지 순서대로 적어 보세요. word에는 c,a,t,\0 네 바이트가 들어 있고 strlen은 널 이전의 글자 세 개만 셉니다.
    explanation:
      1. 배열 크기는 널 포함 4입니다. 2. strlen이 널까지 전진합니다. 3. 널 이전 길이 3을 출력합니다. `c a t 종료문자 → strlen=3 → 출력` 흐름으로
      실제 출력 `3`이 됩니다. 핵심 `strlen(word)`는 종료 문자를 만나기 전까지 세는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-37-c-strings-fill
    ...
    answer:
      "#include <stdio.h>\n#include <string.h>\nint main(void) {\n    char word[] = \"cat\";\n    printf(\"\
      %zu\\n\", strlen(word));\n    return 0;\n}"
    hint: 필요한 표현은 널 종료된 문자열 길이 계산 동작을 잇는 strlen(word)입니다.
    explanation:
      빈칸에 들어갈 표현은 'strlen(word)'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. 배열 크기는 널 포함 4입니다. 2. strlen이 널까지 전진합니다.
      3. 널 이전 길이 3을 출력합니다. `c a t 종료문자 → strlen=3 → 출력` 흐름으로 실제 출력 `3`이 됩니다. 핵심 `strlen(word)`는 종료 문자를 만나기 전까지 세는
      자리에 쓰입니다.
    commonMistakes:
      - 널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-37-c-strings-guided
    ...
    prompt: 예시를 직접 타이핑한 뒤 첫 문자열을 'cat'에서 'cat!'로 바꿔 보세요. 출력이 바뀌는지 먼저 예측하고, 그대로라면 그 이유도 설명하세요.
    starter:
      "#include <stdio.h>\n#include <string.h>\nint main(void) {\n    char word[] = \"cat\";\n    printf(\"\
      %zu\\n\", strlen(word));\n    return 0;\n}"
    answer:
      "#include <stdio.h>\n#include <string.h>\nint main(void) {\n    char word[] = \"cat!\";\n    printf(\"\
      %zu\\n\", strlen(word));\n    return 0;\n}"
    hint: 첫 문자열을 'cat'에서 'cat!'로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '3'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '4'입니다. 원본은 `3`, 수정본은 `4`이다. 첫 변경 줄 `char word[] = "cat";`에서 `char word[] = "cat!";`로 글자를
      하나 더하면, 종료 문자 앞 글자가 네 개가 되어 길이가 4가 된다.
    commonMistakes:
      - 널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-37-c-strings-debug
    ...
    answer:
      ...
      %zu\\n\", strlen(word));\n    return 0;\n}"
    hint:
      널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음 상황에서 어긋나는 줄을 word에는 c,a,t,\0 네 바이트가 들어 있고 strlen은 널 이전의 글자 세 개만 셉니다. 설명과
      대조해 보세요.
    explanation:
      틀린 줄은 `printf("%zu\n", sizeof(word));`입니다. `sizeof`는 종료 칸을 포함한 저장 공간 4를 재므로 실행은 되지만 결과는 `4`가 됩니다.
      오류 분류는 잘못된 출력입니다. 글자 수와 저장 크기가 바뀌는 첫 순간은 측정 연산자 선택입니다. 고친 줄 `printf("%zu\n", strlen(word));`에서는 `널 종료된 문자열 길이
      계산` 동작이 지켜집니다. 정상 코드는 4바이트 저장→널 이전 3글자 세기→출력의 순서로 `3`을 출력합니다.
    commonMistakes:
      - 널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-37-c-strings-independent
    ...
    answer:
      "#include <stdio.h>\n#include <string.h>\nint main(void) {\n    char word[] = \"cat\";\n    printf(\"\
      %zu\\n\", strlen(word));\n    return 0;\n}"
    hint: 길이 3의 배열에 `"cat"`을 담으면 종료 칸이 부족합니다. 한글의 UTF-8 바이트 수는 영문 글자 수와 다를 수 있습니다. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 길이 3의 배열에 `"cat"`을 담으면 종료 칸이 부족합니다. 한글의 UTF-8 바이트 수는 영문 글자 수와 다를 수 있습니다.
      같은 널 종료된 문자열 길이 계산 동작을 구현하고 실행 결과 '3'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-37-c-strings-output
    ...
    answerIndex: 1
    explanation:
      먼저 word에 c, a, t와 종료 문자의 네 바이트가 저장됩니다. 이어서 strlen이 종료 문자 전까지 세어 3을 얻고, printf가 표시하므로 출력 `3`이 됩니다.
      `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 문자열이 널로 끝나 실행이 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 printf가 실제로 호출되기 때문입니다. 다른
      선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-37-c-strings-model
    ...
    answerIndex: 2
    explanation:
      word에는 c,a,t와 종료 문자의 네 바이트가 들어 있다는 뜻이고, strlen은 널 이전의 글자 세 개만 셈다는 뜻은 저장 공간과 글자 수를 구별한다는 뜻입니다. `널
      종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음`은 반대 사례인데, 종료 문자가 없으면 strlen이 배열 밖까지 읽기 때문입니다.
  - id: quiz-day-37-c-strings-pitfall
    ...
    answerIndex: 0
    explanation:
      먼저 확인할 실수는 `널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음`입니다. 잘못된 코드는 실행은 되지만 `4`라는 잘못된 출력을 냅니다. 글자 수와 저장 크기를
      구별하세요.
  - id: quiz-day-37-c-strings-transfer
    ...
    answerIndex: 1
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `널 종료된 문자열 길이 계산`이라는 의미와 입력·출력 계약입니다. Python의 len과 Rust의 len으로 같은 동작을 구현하고,
...
예제의 핵심 표현은 `strlen(word)`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본은 `3`, 수정본은 `4`이다. 첫 변경 줄 `char word[] = "cat";`에서 `char word[] = "cat!";`로 글자를 하나 더하면, 종료 문자 앞 글자가 네 개가 되어 길이가 4가 된다.
...
두 줄을 나란히 놓으면 정상 줄 `printf("%zu\n", strlen(word));`이 `word에는 c,a,t,\0 네 바이트가 들어 있고 strlen은 널 이전의 글자 세 개만 셉니다.` 설명과 맞고, 잘못된 줄은 `널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음` 쪽으로 어긋납니다. 결과가 예상과 다르면 `c a t \0`부터 `3`까지 처음 어긋난 곳을 찾습니다.
