...
exercises:
  - id: ex-day-37-c-strings-predict
    ...
    starter:
      ...
      %zu\\n\", strlen(word));\n    return 0;\n}"
    answer: "3"
    hint: word에는 c,a,t,\0 네 바이트가 들어 있고 strlen은 널 이전의 글자 세 개만 셉니다. 설명을 떠올리고 `c a t \0` 단계부터 순서대로 적어 보세요.
    explanation:
      c a t \0  →  strlen=3  →  출력 순서로 실행됩니다. `strlen(word)` 부분이 `출력` 단계를 확정해 최종 출력 '3'가 됩니다. 이 흐름을 떠올리면
      `널 종료된 문자열 길이 계산` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-37-c-strings-fill
    ...
    answer:
      "#include <stdio.h>\n#include <string.h>\nint main(void) {\n    char word[] = \"cat\";\n    printf(\"\
      %zu\\n\", strlen(word));\n    return 0;\n}"
    hint: 힌트 문장을 완전하게 읽으면 `배열 크기는 널 포함 4입니다` 단계에 필요한 표현이 `strlen(word)`입니다.
    explanation:
      빈칸에 들어갈 표현은 'strlen(word)'입니다. `printf("%zu\n", strlen(word));` 줄을 완성해야 `널 종료된 문자열 길이 계산` 동작이 이어져
      실행 결과 '3'가 됩니다. 힌트의 첫 단계 `배열 크기는 널 포함 4입니다`이 바로 이 줄입니다. 이어서 strlen이 널까지 전진합니다 널 이전 길이 3을 출력합니다 순서로 진행됩니다.
    commonMistakes:
      - 널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-37-c-strings-guided
    ...
    explanation:
      바꾼 뒤 출력은 '4'입니다. 원본 출력 '3'에서 달라졌습니다. 바뀐 줄은 `char word[] = "cat";`에서 `char word[] = "cat!";`로 바뀌었습니다.
      바뀐 프로그램은 `c a t \0` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `4`가 됩니다. 원본 추적 `c a t \0  →  strlen=3  →  출력`와
  ...
  - id: ex-day-37-c-strings-debug
    ...
    answer:
      ...
      %zu\\n\", strlen(word));\n    return 0;\n}"
    hint:
      word에는 c,a,t,\0 네 바이트가 들어 있고 strlen은 널 이전의 글자 세 개만 셉니다. 설명과 어긋나는 줄을 찾으세요. `널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을
      읽음` 상황이 단서가 됩니다.
    explanation:
      틀린 줄은 `printf("%zu\n", sizeof(word));`입니다. 여기서는 `sizeof(word)`을 써서 `strlen(word)` 동작이 깨집니다. 이대로 실행하면
      `널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음` 문제가 생겨 원본 추적 `c a t \0  →  strlen=3  →  출력`대로 '3'가 나오지 않습니다. 고친 줄 `printf("%zu\n",
      strlen(word));`에서는 `strlen(word)`가 `널 종료된 문자열 길이 계산` 동작을 지켜 '3'까지 도달합니다.
    commonMistakes:
      - 널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-37-c-strings-independent
    ...
    answer:
      "#include <stdio.h>\n#include <string.h>\nint main(void) {\n    char word[] = \"cat\";\n    printf(\"\
      %zu\\n\", strlen(word));\n    return 0;\n}"
    hint: word에는 c,a,t,\0 네 바이트가 들어 있고 strlen은 널 이전의 글자 세 개만 셉니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `strlen(word)` 부분이 `널 종료된 문자열 길이 계산` 동작을 지켜 실행 결과 '3'가 됩니다. 같은 개념을 다른 입력으로 바꿔도
      `strlen(word)`부터 `출력`까지 추적할 수 있으면 정답입니다. `널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - 널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-37-c-strings-output
    ...
    answerIndex: 1
    explanation:
      c a t \0  →  strlen=3  →  출력 순서로 실행되어 출력은 '3'입니다. `strlen(word)` 부분이 마지막 단계를 확정하므로 다른 선택지는 이 추적과
      맞지 않습니다.
  - id: quiz-day-37-c-strings-model
    ...
    answerIndex: 2
    explanation:
      word에는 c,a,t,\0 네 바이트가 들어 있고 strlen은 널 이전의 글자 세 개만 셉니다. 이 설명이 맞는 이유는 `널 종료된 문자열 길이 계산` 동작을 지키는 조건과
      같기 때문입니다. `널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-37-c-strings-pitfall
    ...
    answerIndex: 0
    explanation:
      널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음. 이 실수가 나오면 원본 추적 `c a t \0 → strlen=3 → 출력`대로 '3'가 나오지 않으므로 먼저 확인해야
      합니다.
  - id: quiz-day-37-c-strings-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 1
    explanation: 문법은 달라도 `널 종료된 문자열 길이 계산` 목적과 입력·출력은 유지합니다. 실행 결과 '3'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
원본 출력은 `3`이고, 바뀐 코드의 실행 결과는 `4`입니다. 바뀐 줄은 `char word[] = "cat";`에서 `char word[] = "cat!";`로 바뀌었습니다. 바뀐 프로그램은 `c a t \0` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `4`가 됩니다. 원본 추적 `c a t \0  →  strlen=3  →  출력`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
