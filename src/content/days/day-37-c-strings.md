...
exercises:
  ...
  - id: ex-day-37-c-strings-guided
    ...
    explanation:
      예시 답안에서는 첫 문자열을 'cat'에서 'cat!'로 바꿨습니다. 원래 출력은 '3'입니다. 바꾼 줄에서 시작해 중간 값과 마지막 출력을 다시 추적하세요. 출력이 같더라도
...
C 문자열은 단순 char 배열과 달리 끝을 알리는 널 문자까지 공간이 필요합니다. Day 37 "`C 문자열과 널 종료`"에서는 C 코드가 `3`을(를) 만드는 과정을 따라가며, 널 종료된 문자열 길이 계산 동작이 왜 필요한지 확인합니다. 널 종료된 문자열 길이 계산을(를) 빠뜨리면 `널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 36](/learn/day-36-rust-slices)에서는 "Rust 슬라이스와 범위"을(를) 배웠습니다. "Rust 슬라이스와 범위"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`C 문자열과 널 종료`"에서 새로 달라지는 조건을 찾아보세요. Day 37의 답은 `3`이며, 핵심 표현은 `strlen(word)`입니다.

## 머릿속 그림

word에는 c,a,t,\0 네 바이트가 들어 있고 strlen은 널 이전의 글자 세 개만 셉니다. Day 37에서는 아래 흐름 순서대로 상태가 바뀌며, `strlen(word)`이(가) 결과를 가릅니다.

```text
c a t \0  →  strlen=3  →  출력
```

위 흐름에서 `strlen(word)`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `3`입니다.
...
아래 C 코드는 Day 37 "`C 문자열과 널 종료`"의 독립 예제입니다. 전체 7줄에서 `strlen(word)`이(가) 핵심이며, 실행 결과는 `3`입니다.
...
C17은 `gcc -std=c17 -Wall -Wextra`로 컴파일해 실행하면 `3`이(가) 나옵니다. 브라우저에서는 임의 컴파일 대신 하단 실습 칸의 답안 비교를 사용하세요.
...
위 단계에서 `strlen(word)`이(가) 빠지면 `3`이(가) 나오지 않습니다. `첫 문자열을 'cat'에서 'cat!'로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `3`이(가) 됩니다. `c a t \0` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 코드에서 `첫 문자열을 'cat'에서 'cat!'로` 바꾸면 아래와 같이 됩니다.
...
원본 출력은 `3`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `strlen(word)`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 37의 `C 문자열과 널 종료`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음.** 정상 코드에서는 `3`이(가) 출력됩니다.
...
핵심 동작 **널 종료된 문자열 길이 계산**은(는) 세 언어에서 같은 입력과 출력(`3`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `char[]와 '\0', strlen` 규칙으로 "`C 문자열과 널 종료`"의 `3`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `str은 길이를 직접 관리` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `String.len은 UTF-8 바이트 길이` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`3` 맞히기) → 빈칸(`strlen(word)` 채우기) → 변경(`첫 문자열을 'cat'에서 'cat!'로`) → 오류 수정(`널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음` 찾기) → 독립 구현(`C 문자열과 널 종료을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `3` 및 `strlen(word)` 설명과 대조하세요.

## 스스로 설명하기

- "`C 문자열과 널 종료`"이(가) 필요한 상황을 `널 종료된 문자열 길이 계산` 동작으로 설명해 보세요.
- 예제에서 `strlen(word)`이(가) 실행되기 직전의 상태와 직후의 출력 `3`을(를) 말해 보세요.
- "`널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `널 종료된 문자열 길이 계산` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

word에는 c,a,t,\0 네 바이트가 들어 있고 strlen은 널 이전의 글자 세 개만 셉니다. Day 37 "`C 문자열과 널 종료`"의 예제는 `strlen(word)`을(를) 실행해 `3`을(를) 출력합니다. "`널 종료가 없는 버퍼를 strlen에 넘겨 범위 밖을 읽음`" 여부를 확인하고 Day 37을(를) 완료하세요.
