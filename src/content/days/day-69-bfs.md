...
exercises:
  ...
  - id: ex-day-69-bfs-fill
    ...
    answer:
      ...
      \ not in seen: seen.add(next_node); q.append(next_node)\nprint(sorted(seen))"
    hint: A를 꺼냅니다
    explanation: 빈칸에 들어갈 표현은 'q.popleft()'입니다. A를 꺼냅니다 새 이웃 B,C를 방문 표시 후 큐에 넣습니다 둘을 처리하고 seen을 정렬해 출력합니다
    commonMistakes:
      - 방문 표시를 늦춰 같은 노드가 큐에 여러 번 들어감
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-69-bfs-guided
    ...
    explanation:
      바꾼 뒤 출력은 '3'입니다. 원본 출력 "['A', 'B', 'C']"에서 달라졌습니다. 다른 줄(print(sorted(seen)) → print(len(seen)))에서
...
quiz:
  - id: quiz-day-69-bfs-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 0
    explanation: q=[A]  →  q=[B,C]  →  q=[]  →  seen={A,B,C} 순서로 실행되어 출력은 "['A', 'B', 'C']"입니다.
...
연결된 정점을 가까운 순서로 찾으려면 큐와 방문 집합을 함께 사용합니다. Day 69 "`너비 우선 탐색과 방문 집합`"에서는 Python 코드의 실행 결과(`['A', 'B', 'C']`)를 따라가며, `가까운 정점부터 탐색` 동작이 왜 필요한지 확인합니다. 이 동작이 빠지면 "`방문 표시를 늦춰 같은 노드가 큐에 여러 번 들어감`" 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 68](/learn/day-68-graph)에서 배운 "그래프의 인접 목록" 내용을 한 문장으로 말해 보세요. 이번 "`너비 우선 탐색과 방문 집합`"에서 새로 달라지는 조건을 찾아보세요. 핵심 표현은 `q.popleft()`입니다.

## 머릿속 그림

큐는 현재 거리의 정점을 먼저 처리하고 seen은 순환 간선에서 같은 정점을 다시 넣지 않게 합니다. Day 69에서는 아래 흐름 순서대로 상태가 바뀌고, 결과를 가르는 부분은 `q.popleft()`입니다.

```text
q=[A]  →  q=[B,C]  →  q=[]  →  seen={A,B,C}
```

위 흐름에서 `q.popleft()` 부분이 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `['A', 'B', 'C']`입니다.
...
하단 Python 실행 영역에 같은 코드가 들어 있습니다. 먼저 실행 결과(`['A', 'B', 'C']`)를 가리고 예측한 뒤 실행해 비교하세요.
...
`q.popleft()` 부분이 실행될 때 읽는 값과 바뀌는 값을 한 줄씩 적어 보세요. 다음 `결과 예측과 작은 변경`에서는 이 부분이 달라집니다.
...
위 순서대로 실행한 최종 출력은 `['A', 'B', 'C']`입니다. `q=[A]` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본과 달라졌습니다. 원본 출력은 `['A', 'B', 'C']`입니다. 다른 줄(`print(sorted(seen))` → `print(len(seen))`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`q=[A]` → …)과 바뀐 줄부터 대조해 보세요.

## 자주 틀리는 지점

**확인할 실수: 방문 표시를 늦춰 같은 노드가 큐에 여러 번 들어감.** 정상 코드의 실행 결과는 `['A', 'B', 'C']`입니다.
...
세 언어에서 지켜야 할 핵심 동작은 `가까운 정점부터 탐색`입니다. 목표는 같은 입력에 같은 결과를 내는 것으로, 각 언어의 규칙에 맞게 옮겼는지 실행 결과로 대조하세요.
...
C17에서는 `배열 큐와 bool visited` 규칙을 적용합니다. "`너비 우선 탐색과 방문 집합`" 수업의 실행 결과(`['A', 'B', 'C']`)를 기준으로 삼고, C는 범위와 널 검사를 자동으로 해 주지 않으므로, 코드에 사용된 접근마다 유효 범위를 직접 확인해야 합니다. Python에서는 `deque와 set` 규칙을 따릅니다. 실패나 빈 입력은 예외로 드러내며 조용히 넘기지 마세요. Rust에서는 `VecDeque와 HashSet` 규칙을 따릅니다. 실행 결과가 다르면 컴파일 오류(타입, 소유권, 빌림)와 실행 때 패닉(인덱스 범위 등) 중 어느 쪽인지 메시지부터 구분하세요.

## 실습 순서

예측(`['A', 'B', 'C']` 맞히기) → 빈칸(`q.popleft()` 채우기) → 변경(`'print(sorted(seen))' → 'print(len(seen))'`) → 오류 수정(`방문 표시를 늦춰 같은 노드가 큐에 여러 번 들어감` 찾기) → 독립 구현(`'너비 우선 탐색과 방문 집합' 개념을 보여주는 작은 프로그램` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `['A', 'B', 'C']` 및 `q.popleft()` 설명과 대조하세요.

## 스스로 설명하기

- "`너비 우선 탐색과 방문 집합`" 수업이 필요한 이유는 `가까운 정점부터 탐색` 동작으로 설명해 보세요.
- 예제에서 `q.popleft()` 부분 실행 직전의 상태와 직후의 출력(`['A', 'B', 'C']`)을 말해 보세요.
- "`방문 표시를 늦춰 같은 노드가 큐에 여러 번 들어감`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `가까운 정점부터 탐색` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

큐는 현재 거리의 정점을 먼저 처리하고 seen은 순환 간선에서 같은 정점을 다시 넣지 않게 합니다. Day 69 "`너비 우선 탐색과 방문 집합`" 예제의 핵심 부분은 `q.popleft()`이며, 실행 결과는 `['A', 'B', 'C']`입니다. "`방문 표시를 늦춰 같은 노드가 큐에 여러 번 들어감`" 여부를 확인하고 Day 69 수업을 완료하세요.
