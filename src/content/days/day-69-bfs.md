...
두 줄을 나란히 놓으면 정상 줄 `node = q.popleft()`이 `큐는 현재 거리의 정점을 먼저 처리하고 seen은 순환 간선에서 같은 정점을 다시 넣지 않게 합니다.` 설명과 맞고, 잘못된 줄은 `방문 표시를 늦춰 같은 노드가 큐에 여러 번 들어감` 쪽으로 어긋납니다. 결과가 예상과 다르면 `q=[A]`부터 `['A', 'B', 'C']`까지 처음 어긋난 곳을 찾습니다.
