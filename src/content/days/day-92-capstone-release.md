...
exercises:
  ...
  - id: ex-day-92-capstone-release-fill
    ...
    explanation: 빈칸에 들어갈 표현은 'assert total == 60'입니다. 세 기록의 시간을 합해 60을 구합니다 assert로 공통 기대값을 확인합니다 완료 메시지를 출력합니다
    commonMistakes:
      - 한 언어의 결과만 보고 나머지 구현도 같다고 가정함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-92-capstone-release-guided
    ...
    hint:
      ...
      == 3''.'
    explanation:
      "바꾼 뒤 출력은 'Study Log Analyzer 완료: 60'입니다. 원본 출력과 같습니다('Study Log Analyzer 완료: 60'). 추가된 assert도
      통과하므로 print까지 도달합니다. assert는 기대값을 검증할 뿐 출력값 계산과 표시에는 관여하지 않습니다. 출력이 같아도 바뀐 줄(assert len(sessions) == 3) 이후의
      중간 상태가 같은지는 원본 추적과 대조해야 합니다."
    commonMistakes:
      - 한 언어의 결과만 보고 나머지 구현도 같다고 가정함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-92-capstone-release-output
    ...
    answerIndex: 2
    explanation: "C30 + Python20 + Rust10  →  60  →  테스트 통과 순서로 실행되어 출력은 'Study Log Analyzer 완료: 60'입니다."
...
마지막 날은 세 언어 구현의 입력 검증·집계·필터·정렬·오류 메시지를 같은 fixture로 검증합니다. Day 92 "`완성 프로젝트 검증과 확장`"에서는 Python 코드의 실행 결과(`Study Log Analyzer 완료: 60`)를 따라가며, `동일 입력·출력 명세와 테스트 완료` 동작이 왜 필요한지 확인합니다. 이 동작이 빠지면 "`한 언어의 결과만 보고 나머지 구현도 같다고 가정함`" 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 91](/learn/day-91-cross-language-review)에서 배운 "세 구현의 결과 일치 검사" 내용을 한 문장으로 말해 보세요. 이번 "`완성 프로젝트 검증과 확장`"에서 새로 달라지는 조건을 찾아보세요. 핵심 표현은 `assert total == 60`입니다.

## 머릿속 그림

공통 명세가 참인지 테스트로 고정한 후 선호 언어에서 ASCII 시각화 한 가지를 추가합니다. Day 92에서는 아래 흐름 순서대로 상태가 바뀌고, 결과를 가르는 부분은 `assert total == 60`입니다.

```text
C30 + Python20 + Rust10  →  60  →  테스트 통과
```

위 흐름에서 `assert total == 60` 부분이 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `Study Log Analyzer 완료: 60`입니다.
...
하단 Python 실행 영역에 같은 코드가 들어 있습니다. 먼저 실행 결과(`Study Log Analyzer 완료: 60`)를 가리고 예측한 뒤 실행해 비교하세요.
...
`assert total == 60` 부분이 실행될 때 읽는 값과 바뀌는 값을 한 줄씩 적어 보세요. 다음 `결과 예측과 작은 변경`에서는 이 부분이 달라집니다.
...
위 순서대로 실행한 최종 출력은 `Study Log Analyzer 완료: 60`입니다. `C30 + Python20 + Rust10` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
바꾼 뒤 출력도 `Study Log Analyzer 완료: 60`입니다. 추가된 assert도 통과하므로 print까지 도달합니다. assert는 기대값을 검증할 뿐 출력값 계산과 표시에는 관여하지 않습니다. 출력이 같다고 해서 중간 상태까지 같은 것은 아닙니다. 다른 줄(`assert len(sessions) == 3`)부터 원본 추적과 비교해 보세요.

## 자주 틀리는 지점

**확인할 실수: 한 언어의 결과만 보고 나머지 구현도 같다고 가정함.** 정상 코드의 실행 결과는 `Study Log Analyzer 완료: 60`입니다.
...
세 언어에서 지켜야 할 핵심 동작은 `동일 입력·출력 명세와 테스트 완료`입니다. 목표는 같은 입력에 같은 결과를 내는 것으로, 각 언어의 규칙에 맞게 옮겼는지 실행 결과로 대조하세요.
...
C17에서는 `독립 실행 파일의 기준 출력·오류 검증` 규칙을 적용합니다. "`완성 프로젝트 검증과 확장`" 수업의 실행 결과(`Study Log Analyzer 완료: 60`)를 기준으로 삼고, C는 범위와 널 검사를 자동으로 해 주지 않으므로, 코드에 사용된 접근마다 유효 범위를 직접 확인해야 합니다. Python에서는 `pytest/unittest와 선택 확장` 규칙을 따릅니다. 실패나 빈 입력은 예외로 드러내며 조용히 넘기지 마세요. Rust에서는 `cargo test와 동일 fixture 비교` 규칙을 따릅니다. 실행 결과가 다르면 컴파일 오류(타입, 소유권, 빌림)와 실행 때 패닉(인덱스 범위 등) 중 어느 쪽인지 메시지부터 구분하세요.

## 실습 순서

예측(`Study Log Analyzer 완료: 60` 맞히기) → 빈칸(`assert total == 60` 채우기) → 변경(`'assert total == 60' → 'assert total == 60 + 다음 줄 assert len(sessions) == 3'`) → 오류 수정(`한 언어의 결과만 보고 나머지 구현도 같다고 가정함` 찾기) → 독립 구현(`'완성 프로젝트 검증과 확장' 개념을 보여주는 작은 프로그램` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `Study Log Analyzer 완료: 60` 및 `assert total == 60` 설명과 대조하세요.

## 스스로 설명하기

- "`완성 프로젝트 검증과 확장`" 수업이 필요한 이유는 `동일 입력·출력 명세와 테스트 완료` 동작으로 설명해 보세요.
- 예제에서 `assert total == 60` 부분 실행 직전의 상태와 직후의 출력(`Study Log Analyzer 완료: 60`)을 말해 보세요.
- "`한 언어의 결과만 보고 나머지 구현도 같다고 가정함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `동일 입력·출력 명세와 테스트 완료` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

공통 명세가 참인지 테스트로 고정한 후 선호 언어에서 ASCII 시각화 한 가지를 추가합니다. Day 92 "`완성 프로젝트 검증과 확장`" 예제의 핵심 부분은 `assert total == 60`이며, 실행 결과는 `Study Log Analyzer 완료: 60`입니다. "`한 언어의 결과만 보고 나머지 구현도 같다고 가정함`" 여부를 확인하고 Day 92 수업을 완료하세요.
