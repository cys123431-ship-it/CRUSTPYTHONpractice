...
exercises:
  ...
  - id: ex-day-87-c-csv-parse-guided
    ...
    explanation:
      예시 답안에서는 처음 등장하는 숫자를 10에서 11로 바꿨습니다. 원래 출력은 '30'입니다. 바꾼 줄에서 시작해 중간 값과 마지막 출력을 다시 추적하세요. 출력이 같더라도
...
C는 숫자 필드 끝에 글자가 남았는지 직접 확인해야 잘못된 CSV 값을 받아들이지 않습니다. Day 87 "`C 입력 검증과 정수 변환`"에서는 C 코드가 `30`을(를) 만드는 과정을 따라가며, 숫자 필드의 완전한 파싱 검사 동작이 왜 필요한지 확인합니다. 숫자 필드의 완전한 파싱 검사을(를) 빠뜨리면 `atoi로 오류·남은 문자·범위를 구분하지 못함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 86](/learn/day-86-python-aggregate)에서는 "Python 집계·정렬·Top N"을(를) 배웠습니다. "Python 집계·정렬·Top N"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`C 입력 검증과 정수 변환`"에서 새로 달라지는 조건을 찾아보세요. Day 87의 답은 `30`이며, 핵심 표현은 `*end != '\0'`입니다.

## 머릿속 그림

strtol은 숫자를 읽은 마지막 위치를 end에 넣고 *end가 널일 때만 입력 전체가 숫자입니다. Day 87에서는 아래 흐름 순서대로 상태가 바뀌며, `*end != '\0'`이(가) 결과를 가릅니다.

```text
입력 30  →  minutes=30  →  end는 끝  →  성공
```

위 흐름에서 `*end != '\0'`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `30`입니다.
...
아래 C 코드는 Day 87 "`C 입력 검증과 정수 변환`"의 독립 예제입니다. 전체 9줄에서 `*end != '\0'`이(가) 핵심이며, 실행 결과는 `30`입니다.
...
예제의 핵심 표현은 `*end != '\0'`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄을 찾으세요.
...
위 단계에서 `*end != '\0'`이(가) 빠지면 `30`이(가) 나오지 않습니다. `처음 등장하는 숫자를 10에서 11로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `30`이(가) 됩니다. `입력 30` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `30`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `*end != '\0'`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 87의 `C 입력 검증과 정수 변환`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: atoi로 오류·남은 문자·범위를 구분하지 못함.** 정상 코드에서는 `30`이(가) 출력됩니다.
...
핵심 동작 **숫자 필드의 완전한 파싱 검사**은(는) 세 언어에서 같은 입력과 출력(`30`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `strtol + endptr + errno 경계 검사` 규칙으로 "`C 입력 검증과 정수 변환`"의 `30`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `int와 범위 검사` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `parse::<u32>()와 Result` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`30` 맞히기) → 빈칸(`*end != '\0'` 채우기) → 변경(`처음 등장하는 숫자를 10에서 11로`) → 오류 수정(`atoi로 오류·남은 문자·범위를 구분하지 못함` 찾기) → 독립 구현(`C 입력 검증과 정수 변환을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `30` 및 `*end != '\0'` 설명과 대조하세요.

## 스스로 설명하기

- "`C 입력 검증과 정수 변환`"이(가) 필요한 상황을 `숫자 필드의 완전한 파싱 검사` 동작으로 설명해 보세요.
- 예제에서 `*end != '\0'`이(가) 실행되기 직전의 상태와 직후의 출력 `30`을(를) 말해 보세요.
- "`atoi로 오류·남은 문자·범위를 구분하지 못함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `숫자 필드의 완전한 파싱 검사` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

strtol은 숫자를 읽은 마지막 위치를 end에 넣고 *end가 널일 때만 입력 전체가 숫자입니다. Day 87 "`C 입력 검증과 정수 변환`"의 예제는 `*end != '\0'`을(를) 실행해 `30`을(를) 출력합니다. "`atoi로 오류·남은 문자·범위를 구분하지 못함`" 여부를 확인하고 Day 87을(를) 완료하세요.
