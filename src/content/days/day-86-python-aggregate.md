...
exercises:
  - id: ex-day-86-python-aggregate-predict
    ...
    answer: ('C', 45)
    hint: C=30에서 시작해 동률은 이름순 C까지 순서대로 적어 보세요. 사전은 언어별 누적값을 저장하고 정렬 키는 합계 내림차순, 언어 이름 오름차순입니다.
    explanation:
      1. C 30을 기록합니다. 2. Python 45와 C의 추가 15로 둘 다 45입니다. 3. 동률에서 C가 앞서 첫 항목입니다. `C=30 → Python=45 → C=45
      → 동률은 이름순 C` 흐름으로 실제 출력 첫 항목이 됩니다. 핵심 `totals.get(name, 0)`은 없을 때 0에서 시작하는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-86-python-aggregate-fill
    ...
    answer: 'rows = [("C", 30), ("Python", 45), ("C", 15)]
      ...
      print(sorted(totals.items(), key=lambda row: (-row[1], row[0]))[0])'
    hint: 필요한 표현은 그룹별 합계와 결정적 정렬 동작을 잇는 totals.get(name, 0)입니다.
    explanation:
      빈칸에 들어갈 표현은 'totals.get(name, 0)'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. C 30을 기록합니다. 2. Python 45와 C의 추가
      15로 둘 다 45입니다. 3. 동률에서 C가 앞서 첫 항목입니다. `C=30 → Python=45 → C=45 → 동률은 이름순 C` 흐름으로 실제 출력 첫 항목이 됩니다. 핵심 `totals.get(name,
      0)`은 없을 때 0에서 시작하는 자리에 쓰입니다.
    commonMistakes:
      - 동률 순서를 명시하지 않아 구현 언어마다 Top N이 달라짐
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-86-python-aggregate-guided
    ...
    answer: 'rows = [("C", 31), ("Python", 45), ("C", 15)]
      ...
      print(sorted(totals.items(), key=lambda row: (-row[1], row[0]))[0])'
    hint: 처음 등장하는 숫자를 30에서 31로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 "('C', 45)"와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 "('C', 46)"입니다. 원본 첫 항목과 수정본 첫 항목은 합계가 달라진다. 첫 변경 줄의 첫 C분 30에서 31로 바꾸면, C 합계가 46이 되어 동률이
      깨지고 첫 항목이 바뀐다.
    commonMistakes:
      - 동률 순서를 명시하지 않아 구현 언어마다 Top N이 달라짐
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-86-python-aggregate-debug
    ...
    answer: 'rows = [("C", 30), ("Python", 45), ("C", 15)]
      ...
      print(sorted(totals.items(), key=lambda row: (-row[1], row[0]))[0])'
    hint: 동률 순서를 명시하지 않아 구현 언어마다 Top N이 달라짐 상황에서 어긋나는 줄을 사전은 언어별 누적값을 저장하고 정렬 키는 합계 내림차순, 언어 이름 오름차순입니다. 설명과 대조해 보세요.
    explanation:
      '틀린 줄은 `for name, minutes in rows: totals[name] = totals.get(name, 1) + minutes`입니다. 첫 등장부터 1을 더
      얹어 C는 47, Python은 46이 되므로 실행은 되지만 첫 항목이 어긋나 `("C", 46)`이 됩니다. 오류 분류는 잘못된 출력입니다. 합계가 어긋나는 첫 순간은 기본값입니다. 고친 줄
      `for name, minutes in rows: totals[name] = totals.get(name, 0) + minutes`에서는 `그룹별 합계와 결정적 정렬` 동작이 지켜집니다. 동률
      순서를 명시하지 않으면 구현마다 결과가 달라집니다. 정상 코드는 누적→동점 규칙 정렬→첫 항목의 순서로 첫 항목을 출력합니다.'
    commonMistakes:
      - 동률 순서를 명시하지 않아 구현 언어마다 Top N이 달라짐
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-86-python-aggregate-independent
    ...
    answer: 'rows = [("C", 30), ("Python", 45), ("C", 15)]
      ...
      print(sorted(totals.items(), key=lambda row: (-row[1], row[0]))[0])'
    hint: N=0, N이 항목 수보다 클 때도 정해진 결과가 있어야 합니다. 순회 순서에 우연히 의존하지 마세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. N=0, N이 항목 수보다 클 때도 정해진 결과가 있어야 합니다. 순회 순서에 우연히 의존하지 마세요. 같은 그룹별 합계와 결정적
      정렬 동작을 구현하고 실행 결과 "('C', 45)"와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 동률 순서를 명시하지 않아 구현 언어마다 Top N이 달라짐
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-86-python-aggregate-output
    ...
    answerIndex: 2
    explanation:
      먼저 C 30이 기록됩니다. 이어서 Python 45와 C의 추가 15로 둘 다 45가 되고, 동률에서 이름순으로 C가 앞서 첫 항목이 되므로 해당 항목이 출력됩니다. `실행
      전에 반드시 오류가 난다`는 틀린 선택지인데 누적과 정렬이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기 때문입니다. 다른 선택지는 이 추적과
      맞지 않습니다.
  - id: quiz-day-86-python-aggregate-model
    ...
    answerIndex: 0
    explanation:
      사전은 언어별 누적값을 저장한다는 뜻이고, 정렬 키는 합계 내림차순, 언어 이름 오름차순이라는 뜻은 동률까지 규칙으로 정한다는 뜻입니다. `동률 순서를 명시하지 않아 구현 언어마다
      Top N이 달라짐`은 반대 사례인데, 규칙이 없으면 결과가 우연에 좌우되기 때문입니다.
  - id: quiz-day-86-python-aggregate-pitfall
    ...
    answerIndex: 1
    explanation:
      먼저 확인할 실수는 `동률 순서를 명시하지 않아 구현 언어마다 Top N이 달라짐`입니다. 잘못된 코드는 실행은 되지만 `("C", 46)`이라는 잘못된 출력을 냅니다. 기본값과
      동점 규칙을 확인하세요.
  - id: quiz-day-86-python-aggregate-transfer
    ...
    answerIndex: 2
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `그룹별 합계와 결정적 정렬`이라는 의미와 입력·출력 계약입니다. 세 언어의 같은 정렬 키로 동작을 구현하고, 첫 항목 출력으로 대조하면
      옮김이 맞는지 확인할 수 있습니다.
...
예제의 핵심 표현은 `totals.get(name, 0)`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본 실행 결과는 `('C', 45)`입니다. 아래 코드에서 원본과 달라지는 첫 줄을 표시하고, 그 줄의 변경 전후 값을 적어 보세요.
...
원본 첫 항목과 수정본 첫 항목은 합계가 달라진다. 첫 변경 줄의 첫 C분 30에서 31로 바꾸면, C 합계가 46이 되어 동률이 깨지고 첫 항목이 바뀐다.
