...
exercises:
  ...
  - id: ex-day-86-python-aggregate-fill
    ...
    answer: 'rows = [("C", 30), ("Python", 45), ("C", 15)]
      ...
      print(sorted(totals.items(), key=lambda row: (-row[1], row[0]))[0])'
    hint: C 30을 기록합니다
    explanation: 빈칸에 들어갈 표현은 'totals.get(name, 0)'입니다. C 30을 기록합니다 Python 45와 C의 추가 15로 둘 다 45입니다 동률에서 C가 앞서 첫 항목입니다
    commonMistakes:
      - 동률 순서를 명시하지 않아 구현 언어마다 Top N이 달라짐
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-86-python-aggregate-guided
    ...
    explanation:
      바꾼 뒤 출력은 "('C', 46)"입니다. 원본 출력 "('C', 45)"에서 달라졌습니다. 다른 줄(rows = [("C", 30), ("Python", 45), ("C",
      15)] → rows = [("C", 31), ("Python", 45), ("C", 15)])에서 시작한 차이가 최종 출력에 반영되었습니다.
    commonMistakes:
      - 동률 순서를 명시하지 않아 구현 언어마다 Top N이 달라짐
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-86-python-aggregate-output
    ...
    choices:
      ...
      - ('C', 45)
    answerIndex: 2
    explanation: C=30  →  Python=45  →  C=45  →  동률은 이름순 C 순서로 실행되어 출력은 "('C', 45)"입니다.
...
원본과 달라졌습니다. 원본 출력은 `('C', 45)`입니다. 다른 줄(`rows = [("C", 30), ("Python", 45), ("C", 15)]` → `rows = [("C", 31), ("Python", 45), ("C", 15)]`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`C=30` → …)과 바뀐 줄부터 대조해 보세요.
