...
exercises:
  ...
  - id: ex-day-38-unicode-fill
    ...
    explanation: 빈칸에 들어갈 표현은 'encode("utf-8")'입니다. text에는 한·글 두 글자가 있습니다 UTF-8로 각 글자를 3바이트로 인코딩합니다 2와 6을 출력합니다
    commonMistakes:
      - 한글 한 글자를 항상 한 바이트라고 가정함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-38-unicode-guided
    ...
    explanation: 바꾼 뒤 출력은 '3 7'입니다. 원본 출력 '2 6'에서 달라졌습니다. 다른 줄(text = "한글" → text = "한글!")에서 시작한 차이가 최종 출력에 반영되었습니다.
    commonMistakes:
      - 한글 한 글자를 항상 한 바이트라고 가정함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-38-unicode-output
    ...
    choices:
      ...
      - 2 6
    answerIndex: 2
    explanation: 두 글자  →  UTF-8 6바이트  →  2 6 순서로 실행되어 출력은 '2 6'입니다.
...
원본과 달라졌습니다. 원본 출력은 `2 6`입니다. 다른 줄(`text = "한글"` → `text = "한글!"`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`두 글자` → …)과 바뀐 줄부터 대조해 보세요.
