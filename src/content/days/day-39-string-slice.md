...
exercises:
  ...
  - id: ex-day-39-string-slice-guided
    ...
    explanation:
      예시 답안에서는 첫 문자열을 'C'에서 'C!'로 바꿨습니다. 원래 출력은 'C Rust'입니다. 바꾼 줄에서 시작해 중간 값과 마지막 출력을 다시 추적하세요. 출력이 같더라도
...
수정 가능한 소유 문자열과 읽기만 하는 문자열 뷰의 역할을 구분해야 합니다. Day 39 "`Rust String과 &str`"에서는 Rust 코드가 `C Rust`을(를) 만드는 과정을 따라가며, 소유 문자열을 수정하고 읽기 전용 뷰 사용 동작이 왜 필요한지 확인합니다. 소유 문자열을 수정하고 읽기 전용 뷰 사용을(를) 빠뜨리면 `&str 참조로 원본의 크기를 바꿀 수 있다고 생각함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 38](/learn/day-38-unicode)에서는 "Python 문자열과 유니코드"을(를) 배웠습니다. "Python 문자열과 유니코드"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`Rust String과 &str`"에서 새로 달라지는 조건을 찾아보세요. Day 39의 답은 `C Rust`이며, 핵심 표현은 `push_str`입니다.

## 머릿속 그림

String은 가변 힙 데이터를 소유하고 &str은 그 문자열의 유효 구간을 빌립니다. Day 39에서는 아래 흐름 순서대로 상태가 바뀌며, `push_str`이(가) 결과를 가릅니다.

```text
topic=C  →  topic=C Rust  →  view=C Rust
```

위 흐름에서 `push_str`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `C Rust`입니다.
...
아래 Rust 코드는 Day 39 "`Rust String과 &str`"의 독립 예제입니다. 전체 6줄에서 `push_str`이(가) 핵심이며, 실행 결과는 `C Rust`입니다.
...
Rust는 `rustc --edition=2024` 또는 Cargo로 실행하면 `C Rust`이(가) 나옵니다. 브라우저에서는 임의 컴파일 대신 하단 실습 칸의 답안 비교를 사용하세요.
...
위 단계에서 `push_str`이(가) 빠지면 `C Rust`이(가) 나오지 않습니다. `첫 문자열을 'C'에서 'C!'로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `C Rust`이(가) 됩니다. `topic=C` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 코드에서 `첫 문자열을 'C'에서 'C!'로` 바꾸면 아래와 같이 됩니다.
...
원본 출력은 `C Rust`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `push_str`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 39의 `Rust String과 &str`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: &str 참조로 원본의 크기를 바꿀 수 있다고 생각함.** 정상 코드에서는 `C Rust`이(가) 출력됩니다.
...
핵심 동작 **소유 문자열을 수정하고 읽기 전용 뷰 사용**은(는) 세 언어에서 같은 입력과 출력(`C Rust`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `가변 char 버퍼는 용량 검사 필요` 규칙으로 "`Rust String과 &str`"의 `C Rust`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `불변 str 연결은 새 문자열` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `String.push_str 후 &str 빌림` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`C Rust` 맞히기) → 빈칸(`push_str` 채우기) → 변경(`첫 문자열을 'C'에서 'C!'로`) → 오류 수정(`&str 참조로 원본의 크기를 바꿀 수 있다고 생각함` 찾기) → 독립 구현(`Rust String과 &str을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `C Rust` 및 `push_str` 설명과 대조하세요.

## 스스로 설명하기

- "`Rust String과 &str`"이(가) 필요한 상황을 `소유 문자열을 수정하고 읽기 전용 뷰 사용` 동작으로 설명해 보세요.
- 예제에서 `push_str`이(가) 실행되기 직전의 상태와 직후의 출력 `C Rust`을(를) 말해 보세요.
- "`&str 참조로 원본의 크기를 바꿀 수 있다고 생각함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `소유 문자열을 수정하고 읽기 전용 뷰 사용` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

String은 가변 힙 데이터를 소유하고 &str은 그 문자열의 유효 구간을 빌립니다. Day 39 "`Rust String과 &str`"의 예제는 `push_str`을(를) 실행해 `C Rust`을(를) 출력합니다. "`&str 참조로 원본의 크기를 바꿀 수 있다고 생각함`" 여부를 확인하고 Day 39을(를) 완료하세요.
