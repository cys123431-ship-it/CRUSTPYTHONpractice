...
exercises:
  - id: ex-day-50-c-struct-predict
    ...
    starter:
      ...
      \ s = {30, 1};\n    printf(\"%d\\n\", s.minutes);\n    return 0;\n}"
    answer: "30"
    hint: 타입 정의에서 시작해 s.minutes=30까지 순서대로 적어 보세요. Session은 두 필드의 형식이고 s는 그 형식으로 만든 실제 값입니다. 점 연산자가 필드를 선택합니다.
    explanation:
      1. Session 타입을 정의합니다. 2. s에 시간 30과 집중 1을 초기화합니다. 3. s.minutes를 읽어 출력합니다. `타입 정의 → s={30,1} → s.minutes=30`
      흐름으로 실제 출력 `30`이 됩니다. 핵심 `s.minutes`는 묶인 값에서 분 필드를 선택하는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-50-c-struct-fill
    ...
    answer:
      "#include <stdio.h>\ntypedef struct { int minutes; int focused; } Session;\nint main(void) {\n    Session\
      \ s = {30, 1};\n    printf(\"%d\\n\", s.minutes);\n    return 0;\n}"
    hint: 필요한 표현은 관련 필드를 하나의 값으로 묶음 동작을 잇는 s.minutes입니다.
    explanation:
      빈칸에 들어갈 표현은 's.minutes'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. Session 타입을 정의합니다. 2. s에 시간 30과 집중 1을 초기화합니다.
      3. s.minutes를 읽어 출력합니다. `타입 정의 → s={30,1} → s.minutes=30` 흐름으로 실제 출력 `30`이 됩니다. 핵심 `s.minutes`는 묶인 값에서 분 필드를
      선택하는 자리에 쓰입니다.
    commonMistakes:
      - 필드 선언 순서와 초기화 값 순서를 혼동함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-50-c-struct-guided
    ...
    explanation:
      바꾼 뒤 출력은 '31'입니다. 원본은 `30`, 수정본은 `31`이다. 첫 변경 줄 `Session s = {30, 1};`에서 `Session s = {31, 1};`로
      분 값을 바꾸면, 선택하는 필드는 그대로이고 읽히는 값이 달라진다.
    commonMistakes:
      - 필드 선언 순서와 초기화 값 순서를 혼동함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-50-c-struct-debug
    ...
    answer:
      ...
      \ s = {30, 1};\n    printf(\"%d\\n\", s.minutes);\n    return 0;\n}"
    hint:
      필드 선언 순서와 초기화 값 순서를 혼동함 상황에서 어긋나는 줄을 Session은 두 필드의 형식이고 s는 그 형식으로 만든 실제 값입니다. 점 연산자가 필드를 선택합니다. 설명과 대조해
      보세요.
    explanation:
      틀린 줄은 `printf("%d\n", s.focused);`입니다. 분이 아니라 집중도 필드를 읽으므로 실행은 되지만 결과는 `1`이 됩니다. 오류 분류는 잘못된 출력입니다.
      필드 선택이 어긋나는 첫 순간은 점 연산자 뒤 이름입니다. 고친 줄 `printf("%d\n", s.minutes);`에서는 `관련 필드를 하나의 값으로 묶음` 동작이 지켜집니다. 정상 코드는
      형식 정의→초기화→분 필드 선택의 순서로 `30`을 출력합니다.
    commonMistakes:
      - 필드 선언 순서와 초기화 값 순서를 혼동함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-50-c-struct-independent
    ...
    answer:
      "#include <stdio.h>\ntypedef struct { int minutes; int focused; } Session;\nint main(void) {\n    Session\
      \ s = {30, 1};\n    printf(\"%d\\n\", s.minutes);\n    return 0;\n}"
    hint: 읽기 전에 초기화하지 않은 값에는 기본값이 자동으로 들어 있지 않습니다. `s.minutes = 30` 뒤에 읽는 순서를 지키세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 읽기 전에 초기화하지 않은 값에는 기본값이 자동으로 들어 있지 않습니다. `s.minutes = 30` 뒤에 읽는 순서를 지키세요.
      같은 관련 필드를 하나의 값으로 묶음 동작을 구현하고 실행 결과 '30'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 필드 선언 순서와 초기화 값 순서를 혼동함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-50-c-struct-output
    ...
    answerIndex: 2
    explanation:
      먼저 Session 형식에 맞춰 s에 30과 1이 초기화됩니다. 이어서 s.minutes가 분 필드 30을 선택해 출력하므로 `30`이 됩니다. `실행 전에 반드시 오류가 난다`는
      틀린 선택지인데 구조체 선언과 읽기가 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 printf가 실제로 호출되기 때문입니다. 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-50-c-struct-model
    ...
    answerIndex: 0
    explanation:
      Session은 두 필드의 형식이라는 뜻이고, s는 그 형식으로 만든 실제 값이라는 뜻은 형식과 값이 분리된다는 뜻입니다. `필드 선언 순서와 초기화 값 순서를 혼동함`은 반대
      사례인데, 순서가 어긋나면 다른 필드의 값을 읽게 되기 때문입니다.
  - id: quiz-day-50-c-struct-pitfall
    ...
    answerIndex: 1
    explanation: 먼저 확인할 실수는 `필드 선언 순서와 초기화 값 순서를 혼동함`입니다. 잘못된 코드는 실행은 되지만 `1`이라는 잘못된 출력을 냅니다. 점 뒤 필드 이름을 먼저 확인하세요.
  - id: quiz-day-50-c-struct-transfer
    ...
    answerIndex: 2
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `관련 필드를 하나의 값으로 묶음`이라는 의미와 입력·출력 계약입니다. Python의 클래스와 Rust의 구조체로 같은 동작을 구현하고,
...
예제의 핵심 표현은 `s.minutes`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본은 `30`, 수정본은 `31`이다. 첫 변경 줄 `Session s = {30, 1};`에서 `Session s = {31, 1};`로 분 값을 바꾸면, 선택하는 필드는 그대로이고 읽히는 값이 달라진다.
