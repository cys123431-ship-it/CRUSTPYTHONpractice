...
학습 기록의 시간과 집중 여부가 함께 움직여야 하면 이름 있는 구조체로 묶습니다. Day 50 "`C 구조체로 관련 값 묶기`"에서는 C 코드가 `30`을(를) 만드는 과정을 따라가며, 관련 필드를 하나의 값으로 묶음 동작이 왜 필요한지 확인합니다. 관련 필드를 하나의 값으로 묶음을(를) 빠뜨리면 `필드 선언 순서와 초기화 값 순서를 혼동함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 49](/learn/day-49-memory-io-review)에서는 "메모리와 입출력 회상"을(를) 배웠습니다. "메모리와 입출력 회상"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`C 구조체로 관련 값 묶기`"에서 새로 달라지는 조건을 찾아보세요. Day 50의 답은 `30`이며, 핵심 표현은 `s.minutes`입니다.

## 머릿속 그림

Session은 두 필드의 형식이고 s는 그 형식으로 만든 실제 값입니다. 점 연산자가 필드를 선택합니다. Day 50에서는 아래 흐름 순서대로 상태가 바뀌며, `s.minutes`이(가) 결과를 가릅니다.

```text
타입 정의  →  s={30,1}  →  s.minutes=30
```

위 흐름에서 `s.minutes`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `30`입니다.
...
아래 C 코드는 Day 50 "`C 구조체로 관련 값 묶기`"의 독립 예제입니다. 전체 7줄에서 `s.minutes`이(가) 핵심이며, 실행 결과는 `30`입니다.
...
위 단계에서 `s.minutes`이(가) 빠지면 `30`이(가) 나오지 않습니다. `처음 등장하는 숫자를 30에서 31로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `30`이(가) 됩니다. `타입 정의` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `30`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `s.minutes`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 50의 `C 구조체로 관련 값 묶기`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 필드 선언 순서와 초기화 값 순서를 혼동함.** 정상 코드에서는 `30`이(가) 출력됩니다.
...
핵심 동작 **관련 필드를 하나의 값으로 묶음**은(는) 세 언어에서 같은 입력과 출력(`30`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `typedef struct과 s.minutes` 규칙으로 "`C 구조체로 관련 값 묶기`"의 `30`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `dataclass 또는 class 속성` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `struct Session {minutes:i32,...}` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`C 구조체로 관련 값 묶기`"이(가) 필요한 상황을 `관련 필드를 하나의 값으로 묶음` 동작으로 설명해 보세요.
- 예제에서 `s.minutes`이(가) 실행되기 직전의 상태와 직후의 출력 `30`을(를) 말해 보세요.
- "`필드 선언 순서와 초기화 값 순서를 혼동함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `관련 필드를 하나의 값으로 묶음` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

Session은 두 필드의 형식이고 s는 그 형식으로 만든 실제 값입니다. 점 연산자가 필드를 선택합니다. Day 50 "`C 구조체로 관련 값 묶기`"의 예제는 `s.minutes`을(를) 실행해 `30`을(를) 출력합니다. "`필드 선언 순서와 초기화 값 순서를 혼동함`" 여부를 확인하고 Day 50을(를) 완료하세요.
