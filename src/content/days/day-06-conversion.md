...
exercises:
  - id: ex-day-06-conversion-predict
    ...
    starter:
      ...
      \    printf(\"%.1f\\n\", average);\n    return 0;\n}"
    answer: "2.5"
    hint: int 5에서 시작해 5.0/2=2.5까지 순서대로 적어 보세요. 피연산자 하나를 double로 바꾸면 나눗셈 전체가 실수 나눗셈이 됩니다.
    explanation:
      1. total을 int 5로 시작합니다. 2. 캐스트 후 5.0/2.0을 계산합니다. 3. %.1f로 소수 한 자리 2.5를 출력합니다. `int 5 → double 5.0
      → 5.0/2=2.5` 흐름으로 실제 출력 `2.5`가 됩니다. 핵심 `(double)total`은 나누기 전에 피연산자를 실수로 바꾸는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-06-conversion-fill
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int total = 5;\n    double average = (double)total / 2;\n \
      \   printf(\"%.1f\\n\", average);\n    return 0;\n}"
    hint: 필요한 표현은 나누기 전에 실수로 변환함 동작을 잇는 (double)total입니다.
    explanation:
      빈칸에 들어갈 표현은 '(double)total'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. total을 int 5로 시작합니다. 2. 캐스트 후 5.0/2.0을
      계산합니다. 3. %.1f로 소수 한 자리 2.5를 출력합니다. `int 5 → double 5.0 → 5.0/2=2.5` 흐름으로 실제 출력 `2.5`가 됩니다. 핵심 `(double)total`은
      나누기 전에 피연산자를 실수로 바꾸는 자리에 쓰입니다.
    commonMistakes:
      - 정수끼리 먼저 나누고 결과만 double에 저장함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-06-conversion-guided
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int total = 6;\n    double average = (double)total / 2;\n \
      \   printf(\"%.1f\\n\", average);\n    return 0;\n}"
    hint: 처음 등장하는 숫자를 5에서 6로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '2.5'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '3.0'입니다. 원본은 `2.5`, 수정본은 `3.0`이다. 첫 변경 줄 `int total = 5;`에서 `int total = 6;`으로 시작 값을 바꾸면,
      캐스트 뒤 6.0/2.0 계산을 거쳐 3.0이 출력된다.
    commonMistakes:
      - 정수끼리 먼저 나누고 결과만 double에 저장함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-06-conversion-debug
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int total = 5;\n    double average = (double)total / 2;\n \
      \   printf(\"%.1f\\n\", average);\n    return 0;\n}"
    hint: 정수끼리 먼저 나누고 결과만 double에 저장함 상황에서 어긋나는 줄을 피연산자 하나를 double로 바꾸면 나눗셈 전체가 실수 나눗셈이 됩니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `double average = total / 2;`입니다. 캐스트가 없어 정수 나눗셈 `5 / 2`가 먼저 일어나 2가 되고, 그 2가 double에 저장되어 `2.0`이
      출력됩니다. 오류 분류는 잘못된 출력입니다. 0.5를 잃는 첫 순간은 나누기 전 변환을 빠뜨린 나눗셈입니다. 고친 줄 `double average = (double)total / 2;`에서는 나누기
      전에 실수로 바꾸는 동작이 지켜집니다. 정상 코드는 선언→실수 변환→나눗셈→출력의 순서로 `2.5`를 출력합니다.
    commonMistakes:
      - 정수끼리 먼저 나누고 결과만 double에 저장함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-06-conversion-independent
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int total = 5;\n    double average = (double)total / 2;\n \
      \   printf(\"%.1f\\n\", average);\n    return 0;\n}"
    hint: 0으로 나누는 입력은 별도로 막아야 합니다. 나누는 값이 0일 때 어떤 오류 메시지를 보여 줄지도 직접 정해 보세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 0으로 나누는 입력은 별도로 막아야 합니다. 나누는 값이 0일 때 어떤 오류 메시지를 보여 줄지도 직접 정해 보세요. 같은 나누기
      전에 실수로 변환함 동작을 구현하고 실행 결과 '2.5'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 정수끼리 먼저 나누고 결과만 double에 저장함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-06-conversion-output
    ...
    answerIndex: 0
    explanation:
      먼저 total에 정수 5가 저장됩니다. 이어서 캐스트로 5.0이 되어 2.0으로 나누면 2.5가 되고, %.1f가 소수 첫째 자리까지 보여 주므로 출력 `2.5`가 됩니다.
  ...
  - id: quiz-day-06-conversion-model
    ...
    answerIndex: 1
    explanation:
      피연산자 하나를 double로 바꾸면 나눗셈 전체가 실수 나눗셈이 된다는 뜻은, `5.0 / 2.0`처럼 계산 자체가 실수 연산으로 수행된다는 뜻입니다. `정수끼리 먼저 나누고
      결과만 double에 저장함`은 반대 사례인데, 이미 버려진 0.5는 저장 단계에서 되살릴 수 없기 때문입니다.
  - id: quiz-day-06-conversion-pitfall
    ...
    answerIndex: 2
    explanation:
      먼저 확인할 실수는 `정수끼리 먼저 나누고 결과만 double에 저장함`입니다. 잘못된 코드는 실행은 되지만 `2.0`이라는 잘못된 출력을 냅니다. `%.1f`는 보여 주는
      형식일 뿐 계산을 고치지 못합니다.
  - id: quiz-day-06-conversion-transfer
    ...
    answerIndex: 0
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `나누기 전에 실수로 변환함`이라는 의미와 입력·출력 계약입니다. Python 나눗셈은 기본 실수 연산이고 Rust는 타입을 맞춰야 하므로,
      실행 결과 `2.5`로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
예제의 핵심 표현은 `(double)total`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본 실행 결과는 `2.5`입니다. 아래 코드에서 원본과 달라지는 첫 줄을 표시하고, 그 줄의 변경 전후 값을 적어 보세요.
...
원본은 `2.5`, 수정본은 `3.0`이다. 첫 변경 줄 `int total = 5;`에서 `int total = 6;`으로 시작 값을 바꾸면, 캐스트 뒤 6.0/2.0 계산을 거쳐 3.0이 출력된다.
