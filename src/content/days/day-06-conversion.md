...
exercises:
  ...
  - id: ex-day-06-conversion-guided
    ...
    explanation:
      예시 답안에서는 처음 등장하는 숫자를 5에서 6로 바꿨습니다. 원래 출력은 '2.5'입니다. 바꾼 줄에서 시작해 중간 값과 마지막 출력을 다시 추적하세요. 출력이 같더라도 입력·조건·중간
      상태가 달라졌는지 확인해야 합니다.
    commonMistakes:
      - 정수끼리 먼저 나누고 결과만 double에 저장함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
...
정수 나눗셈으로 평균을 내면 소수 부분이 사라질 수 있어 계산 전에 타입을 골라야 합니다. Day 06 "`C 형변환과 나눗셈`"에서는 C 코드가 `2.5`을(를) 만드는 과정을 따라가며, 나누기 전에 실수로 변환함 동작이 왜 필요한지 확인합니다. 나누기 전에 실수로 변환함을(를) 빠뜨리면 `정수끼리 먼저 나누고 결과만 double에 저장함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 05](/learn/day-05-arithmetic)에서는 "산술식과 연산 순서"을(를) 배웠습니다. "산술식과 연산 순서"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`C 형변환과 나눗셈`"에서 새로 달라지는 조건을 찾아보세요. Day 06의 답은 `2.5`이며, 핵심 표현은 `(double)total`입니다.

## 머릿속 그림

피연산자 하나를 double로 바꾸면 나눗셈 전체가 실수 나눗셈이 됩니다. Day 06에서는 아래 흐름 순서대로 상태가 바뀌며, `(double)total`이(가) 결과를 가릅니다.

```text
int 5  →  double 5.0  →  5.0/2=2.5
```

위 흐름에서 `(double)total`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `2.5`입니다.
...
아래 C 코드는 Day 06 "`C 형변환과 나눗셈`"의 독립 예제입니다. 전체 7줄에서 `(double)total`이(가) 핵심이며, 실행 결과는 `2.5`입니다.
...
C17은 `gcc -std=c17 -Wall -Wextra`로 컴파일해 실행하면 `2.5`이(가) 나옵니다. 브라우저에서는 임의 컴파일 대신 하단 실습 칸의 답안 비교를 사용하세요.
...
위 단계에서 `(double)total`이(가) 빠지면 `2.5`이(가) 나오지 않습니다. `처음 등장하는 숫자를 5에서 6로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `2.5`이(가) 됩니다. `int 5` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `2.5`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `(double)total`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 06의 `C 형변환과 나눗셈`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 정수끼리 먼저 나누고 결과만 double에 저장함.** 정상 코드에서는 `2.5`이(가) 출력됩니다.

- 정상 줄: `double average = (double)total / 2;`
- 잘못된 줄: `double average = total / 2;`

두 줄을 나란히 놓고 `피연산자 하나를 double로 바꾸면 나눗셈 전체가 실수 나눗셈이 됩니다.` 기준으로 어느 쪽이 맞는지 설명하세요. 결과가 예상과 다르면 `int 5`부터 `2.5`까지 처음 어긋난 곳을 찾습니다.

## 다른 언어로 옮기기

핵심 동작 **나누기 전에 실수로 변환함**은(는) 세 언어에서 같은 입력과 출력(`2.5`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `(double)total / count` 규칙으로 "`C 형변환과 나눗셈`"의 `2.5`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `total / count는 실수, //는 몫` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `total as f64 / count as f64` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`2.5` 맞히기) → 빈칸(`(double)total` 채우기) → 변경(`처음 등장하는 숫자를 5에서 6로`) → 오류 수정(`정수끼리 먼저 나누고 결과만 double에 저장함` 찾기) → 독립 구현(`C 형변환과 나눗셈을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `2.5` 및 `(double)total` 설명과 대조하세요.

## 스스로 설명하기

- "`C 형변환과 나눗셈`"이(가) 필요한 상황을 `나누기 전에 실수로 변환함` 동작으로 설명해 보세요.
- 예제에서 `(double)total`이(가) 실행되기 직전의 상태와 직후의 출력 `2.5`을(를) 말해 보세요.
- "`정수끼리 먼저 나누고 결과만 double에 저장함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `나누기 전에 실수로 변환함` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

피연산자 하나를 double로 바꾸면 나눗셈 전체가 실수 나눗셈이 됩니다. Day 06 "`C 형변환과 나눗셈`"의 예제는 `(double)total`을(를) 실행해 `2.5`을(를) 출력합니다. "`정수끼리 먼저 나누고 결과만 double에 저장함`" 여부를 확인하고 Day 06을(를) 완료하세요.
