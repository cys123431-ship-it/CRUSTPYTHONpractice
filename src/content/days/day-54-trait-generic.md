...
exercises:
  ...
  - id: ex-day-54-trait-generic-fill
    ...
    explanation: 빈칸에 들어갈 표현은 'Display'입니다. show(30)에서 T가 정수로 정해집니다 Display 구현이 있는지 검사합니다 println!이 30을 표시합니다
    commonMistakes:
      - 지원하지 않는 타입을 제약 없이 출력할 수 있다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-54-trait-generic-guided
    ...
    explanation:
      바꾼 뒤 출력은 '31'입니다. 원본 출력 '30'에서 달라졌습니다. 다른 줄(fn main() { show(30); } → fn main() { show(31); })에서
...
quiz:
  - id: quiz-day-54-trait-generic-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 0
    explanation: T=i32 추론  →  Display 확인  →  30 출력 순서로 실행되어 출력은 '30'입니다.
...
원본과 달라졌습니다. 원본 출력은 `30`입니다. 다른 줄(`fn main() { show(30); }` → `fn main() { show(31); }`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`T=i32 추론` → …)과 바뀐 줄부터 대조해 보세요.
