...
여러 자료형에 같은 출력 기능을 적용할 때 지원할 동작을 계약으로 명시합니다. Day 54 "`Rust trait과 제네릭`"에서는 Rust 코드가 `30`을(를) 만드는 과정을 따라가며, 필요한 동작만 제네릭 타입에 요구함 동작이 왜 필요한지 확인합니다. 필요한 동작만 제네릭 타입에 요구함을(를) 빠뜨리면 `지원하지 않는 타입을 제약 없이 출력할 수 있다고 생각함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 53](/learn/day-53-rust-enum)에서는 "Rust enum으로 상태 표현"을(를) 배웠습니다. "Rust enum으로 상태 표현"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`Rust trait과 제네릭`"에서 새로 달라지는 조건을 찾아보세요. Day 54의 답은 `30`이며, 핵심 표현은 `Display`입니다.

## 머릿속 그림

T는 타입 매개변수이고 Display 제약은 그 값이 사용자용 문자열로 출력될 수 있음을 보장합니다. Day 54에서는 아래 흐름 순서대로 상태가 바뀌며, `Display`이(가) 결과를 가릅니다.

```text
T=i32 추론  →  Display 확인  →  30 출력
```

위 흐름에서 `Display`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `30`입니다.
...
아래 Rust 코드는 Day 54 "`Rust trait과 제네릭`"의 독립 예제입니다. 전체 2줄에서 `Display`이(가) 핵심이며, 실행 결과는 `30`입니다.
...
위 단계에서 `Display`이(가) 빠지면 `30`이(가) 나오지 않습니다. `처음 등장하는 숫자를 30에서 31로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `30`이(가) 됩니다. `T=i32 추론` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `30`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `Display`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 54의 `Rust trait과 제네릭`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 지원하지 않는 타입을 제약 없이 출력할 수 있다고 생각함.** 정상 코드에서는 `30`이(가) 출력됩니다.

- 정상 줄: `fn show<T: std::fmt::Display>(value: T) { println!("{value}"); }`
- 잘못된 줄: `fn show<T: std::fmt::Debug>(value: T) { println!("{value}"); }`

두 줄을 나란히 놓고 `T는 타입 매개변수이고 Display 제약은 그 값이 사용자용 문자열로 출력될 수 있음을 보장합니다.` 기준으로 어느 쪽이 맞는지 설명하세요. 결과가 예상과 다르면 `T=i32 추론`부터 `30`까지 처음 어긋난 곳을 찾습니다.

## 다른 언어로 옮기기

핵심 동작 **필요한 동작만 제네릭 타입에 요구함**은(는) 세 언어에서 같은 입력과 출력(`30`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `_Generic 또는 명시적 타입별 함수` 규칙으로 "`Rust trait과 제네릭`"의 `30`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `덕 타이핑과 런타임 __str__` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `T: Display trait bound` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`30` 맞히기) → 빈칸(`Display` 채우기) → 변경(`처음 등장하는 숫자를 30에서 31로`) → 오류 수정(`지원하지 않는 타입을 제약 없이 출력할 수 있다고 생각함` 찾기) → 독립 구현(`Rust trait과 제네릭을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `30` 및 `Display` 설명과 대조하세요.

## 스스로 설명하기

- "`Rust trait과 제네릭`"이(가) 필요한 상황을 `필요한 동작만 제네릭 타입에 요구함` 동작으로 설명해 보세요.
- 예제에서 `Display`이(가) 실행되기 직전의 상태와 직후의 출력 `30`을(를) 말해 보세요.
- "`지원하지 않는 타입을 제약 없이 출력할 수 있다고 생각함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `필요한 동작만 제네릭 타입에 요구함` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

T는 타입 매개변수이고 Display 제약은 그 값이 사용자용 문자열로 출력될 수 있음을 보장합니다. Day 54 "`Rust trait과 제네릭`"의 예제는 `Display`을(를) 실행해 `30`을(를) 출력합니다. "`지원하지 않는 타입을 제약 없이 출력할 수 있다고 생각함`" 여부를 확인하고 Day 54을(를) 완료하세요.
