...
exercises:
  - id: ex-day-89-rust-record-parse-predict
    ...
    answer: "30"
    hint: raw='30'에서 시작해 Session.minutes=30까지 순서대로 적어 보세요. minutes:u32를 기대하는 필드에서 parse가 숫자 변환을 하고 실패하면 Err가 됩니다.
    explanation:
      1. raw는 문자열 30입니다. 2. parse로 u32 30을 얻습니다. 3. Session의 필드로 저장해 출력합니다. `raw 문자열 → parse Ok(30) → Session.minutes=30`
      흐름으로 실제 출력 `30`이 됩니다. 핵심 `raw.parse()`는 글자를 필드 타입으로 바꾸는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-89-rust-record-parse-fill
    ...
    answer:
      "struct Session { minutes: u32 }\nfn main() {\n    let raw = \"30\";\n    let session = Session { minutes:\
      \ raw.parse().expect(\"분 숫자 오류\") };\n    println!(\"{}\", session.minutes);\n}"
    hint: 필요한 표현은 텍스트 행을 타입 있는 레코드로 변환 동작을 잇는 raw.parse()입니다.
    explanation:
      빈칸에 들어갈 표현은 'raw.parse()'입니다. 이 표현이 없으면 컴파일 단계에서 막혀 실행 자체가 되지 않습니다. 1. raw는 문자열 30입니다. 2. parse로
      u32 30을 얻습니다. 3. Session의 필드로 저장해 출력합니다. `raw 문자열 → parse Ok(30) → Session.minutes=30` 흐름으로 실제 출력 `30`이 됩니다.
      핵심 `raw.parse()`는 글자를 필드 타입으로 바꾸는 자리에 쓰입니다.
    commonMistakes:
      - 실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-89-rust-record-parse-guided
    ...
    answer:
      "struct Session { minutes: u32 }\nfn main() {\n    let raw = \"31\";\n    let session = Session { minutes:\
      \ raw.parse().expect(\"분 숫자 오류\") };\n    println!(\"{}\", session.minutes);\n}"
    hint: 입력 문자열 30을 31로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '30'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '31'입니다. 원본은 `30`, 수정본은 `31`이다. 첫 변경 줄 `let raw = "30";`에서 `let raw = "31";`로 입력 글자를 바꾸면,
      같은 파싱을 거쳐 새 값이 저장되어 출력된다.
    commonMistakes:
      - 실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-89-rust-record-parse-debug
    ...
    answer:
      ...
      \ raw.parse().expect(\"분 숫자 오류\") };\n    println!(\"{}\", session.minutes);\n}"
    hint:
      실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김 상황에서 어긋나는 줄을 minutes:u32를 기대하는 필드에서 parse가 숫자 변환을 하고 실패하면 Err가
      됩니다. 설명과 대조해 보세요.
    explanation:
      '틀린 줄은 `let session = Session { minutes: raw.len().expect("분 숫자 오류") };`입니다. 길이 숫자에 기대한다는 메서드가 없어
      컴파일 단계에서 오류가 나고 실행 파일 자체가 만들어지지 않아 아무것도 출력되지 않습니다. 오류 분류는 컴파일 오류입니다. 흐름이 깨지는 첫 순간은 변환 호출입니다. 고친 줄 `let session
      = Session { minutes: raw.parse().expect("분 숫자 오류") };`에서는 `텍스트 행을 타입 있는 레코드로 변환` 동작이 지켜집니다. 실제 파일 파서에서는 어느 행인지
      숨기고 전체를 끝내지 마세요. 정상 코드는 글자 읽기→u32 변환→필드 저장→출력의 순서로 `30`을 출력합니다.'
    commonMistakes:
      - 실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-89-rust-record-parse-independent
    ...
    answer:
      "struct Session { minutes: u32 }\nfn main() {\n    let raw = \"30\";\n    let session = Session { minutes:\
      \ raw.parse().expect(\"분 숫자 오류\") };\n    println!(\"{}\", session.minutes);\n}"
    hint: 타입을 정하는 것만으로 업무 규칙까지 해결되지는 않습니다. 행 번호와 함께 알리고 계속 읽을지 정하세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 타입을 정하는 것만으로 업무 규칙까지 해결되지는 않습니다. 행 번호와 함께 알리고 계속 읽을지 정하세요. 같은 텍스트 행을 타입
      있는 레코드로 변환 동작을 구현하고 실행 결과 '30'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
...
quiz:
  - id: quiz-day-89-rust-record-parse-output
    ...
    answerIndex: 2
    explanation:
      먼저 raw에 문자열 30이 저장됩니다. 이어서 parse로 u32 30을 얻어 Session 필드에 저장하고 표시하므로 출력 `30`이 됩니다. `실행 전에 반드시 오류가
      난다`는 틀린 선택지인데 파싱과 타입이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 println!이 실제로 호출되기 때문입니다. 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-89-rust-record-parse-model
    ...
    answerIndex: 0
    explanation:
      minutes 필드에서 parse가 숫자 변환을 한다는 뜻이고, 실패하면 오류가 된다는 뜻은 잘못된 기록이 조기에 드러난다는 뜻입니다. `실제 파일 파서에서 expect로 전체
      프로그램을 종료하고 어느 행인지 숨김`은 반대 사례인데, 행 번호 없는 종료는 디버깅을 어렵게 하기 때문입니다.
  - id: quiz-day-89-rust-record-parse-pitfall
    ...
    answerIndex: 1
    explanation:
      먼저 확인할 실수는 `실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김`입니다. 잘못된 코드는 있는 메서드가 아니라 컴파일 오류가 나므로 화면 출력
  ...
  - id: quiz-day-89-rust-record-parse-transfer
    ...
    answerIndex: 2
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `텍스트 행을 타입 있는 레코드로 변환`이라는 의미와 입력·출력 계약입니다. C의 strtol 검사와 Python의 int 변환으로 같은
      동작을 구현하고, 실행 결과 `30`으로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
예제의 핵심 표현은 `raw.parse()`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본은 `30`, 수정본은 `31`이다. 첫 변경 줄 `let raw = "30";`에서 `let raw = "31";`로 입력 글자를 바꾸면, 같은 파싱을 거쳐 새 값이 저장되어 출력된다.
...
두 줄을 나란히 놓으면 정상 줄 `let session = Session { minutes: raw.parse().expect("분 숫자 오류") };`이 `minutes:u32를 기대하는 필드에서 parse가 숫자 변환을 하고 실패하면 Err가 됩니다.` 설명과 맞고, 잘못된 줄은 `실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김` 쪽으로 어긋납니다. 결과가 예상과 다르면 `raw='30'`부터 `30`까지 처음 어긋난 곳을 찾습니다.
