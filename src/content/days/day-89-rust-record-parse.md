...
CSV 문자열을 타입이 있는 Session으로 만들 때 파싱 실패를 호출자에게 전달해야 합니다. Day 89 "`Rust 타입으로 레코드 검증`"에서는 Rust 코드가 `30`을(를) 만드는 과정을 따라가며, 텍스트 행을 타입 있는 레코드로 변환 동작이 왜 필요한지 확인합니다. 텍스트 행을 타입 있는 레코드로 변환을(를) 빠뜨리면 `실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 88](/learn/day-88-c-aggregation)에서는 "C 레코드 집계와 경계 검사"을(를) 배웠습니다. "C 레코드 집계와 경계 검사"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`Rust 타입으로 레코드 검증`"에서 새로 달라지는 조건을 찾아보세요. Day 89의 답은 `30`이며, 핵심 표현은 `raw.parse()`입니다.

## 머릿속 그림

minutes:u32를 기대하는 필드에서 parse가 숫자 변환을 하고 실패하면 Err가 됩니다. Day 89에서는 아래 흐름 순서대로 상태가 바뀌며, `raw.parse()`이(가) 결과를 가릅니다.

```text
raw='30'  →  parse Ok(30)  →  Session.minutes=30
```

위 흐름에서 `raw.parse()`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `30`입니다.
...
아래 Rust 코드는 Day 89 "`Rust 타입으로 레코드 검증`"의 독립 예제입니다. 전체 6줄에서 `raw.parse()`이(가) 핵심이며, 실행 결과는 `30`입니다.
...
위 단계에서 `raw.parse()`이(가) 빠지면 `30`이(가) 나오지 않습니다. `입력 문자열 30을 31로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `30`이(가) 됩니다. `raw='30'` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 코드에서 `입력 문자열 30을 31로` 바꾸면 아래와 같이 됩니다.
...
원본 출력은 `30`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `raw.parse()`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 89의 `Rust 타입으로 레코드 검증`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김.** 정상 코드에서는 `30`이(가) 출력됩니다.
...
핵심 동작 **텍스트 행을 타입 있는 레코드로 변환**은(는) 세 언어에서 같은 입력과 출력(`30`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `struct Session과 검증 함수` 규칙으로 "`Rust 타입으로 레코드 검증`"의 `30`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `dataclass Session과 int 변환` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `struct Session과 Result 반환` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`30` 맞히기) → 빈칸(`raw.parse()` 채우기) → 변경(`입력 문자열 30을 31로`) → 오류 수정(`실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김` 찾기) → 독립 구현(`Rust 타입으로 레코드 검증을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `30` 및 `raw.parse()` 설명과 대조하세요.

## 스스로 설명하기

- "`Rust 타입으로 레코드 검증`"이(가) 필요한 상황을 `텍스트 행을 타입 있는 레코드로 변환` 동작으로 설명해 보세요.
- 예제에서 `raw.parse()`이(가) 실행되기 직전의 상태와 직후의 출력 `30`을(를) 말해 보세요.
...
minutes:u32를 기대하는 필드에서 parse가 숫자 변환을 하고 실패하면 Err가 됩니다. Day 89 "`Rust 타입으로 레코드 검증`"의 예제는 `raw.parse()`을(를) 실행해 `30`을(를) 출력합니다. "`실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김`" 여부를 확인하고 Day 89을(를) 완료하세요.
