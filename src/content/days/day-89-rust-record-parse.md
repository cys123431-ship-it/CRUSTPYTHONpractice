...
exercises:
  - id: ex-day-89-rust-record-parse-predict
    ...
    answer: "30"
    hint: minutes:u32를 기대하는 필드에서 parse가 숫자 변환을 하고 실패하면 Err가 됩니다. 설명을 떠올리고 `raw='30'` 단계부터 순서대로 적어 보세요.
    explanation:
      raw='30'  →  parse Ok(30)  →  Session.minutes=30 순서로 실행됩니다. `raw.parse()` 부분이 `Session.minutes=30`
      단계를 확정해 최종 출력 '30'가 됩니다. 이 흐름을 떠올리면 `텍스트 행을 타입 있는 레코드로 변환` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-89-rust-record-parse-fill
    ...
    answer:
      "struct Session { minutes: u32 }\nfn main() {\n    let raw = \"30\";\n    let session = Session { minutes:\
      \ raw.parse().expect(\"분 숫자 오류\") };\n    println!(\"{}\", session.minutes);\n}"
    hint: 힌트 문장을 완전하게 읽으면 `raw는 문자열 30입니다` 단계에 필요한 표현이 `raw.parse()`입니다.
    explanation:
      '빈칸에 들어갈 표현은 ''raw.parse()''입니다. `let session = Session { minutes: raw.parse().expect("분 숫자 오류")
      };` 줄을 완성해야 `텍스트 행을 타입 있는 레코드로 변환` 동작이 이어져 실행 결과 ''30''가 됩니다. 힌트의 첫 단계 `raw는 문자열 30입니다`이 바로 이 줄입니다. 이어서 parse로
      u32 30을 얻습니다 Session의 필드로 저장해 출력합니다 순서로 진행됩니다.'
    commonMistakes:
      - 실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-89-rust-record-parse-guided
    ...
    explanation:
      바꾼 뒤 출력은 '31'입니다. 원본 출력 '30'에서 달라졌습니다. 바뀐 줄은 `let raw = "30";`에서 `let raw = "31";`로 바뀌었습니다. 바뀐 프로그램은
      `raw='30'` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `31`가 됩니다. 원본 추적 `raw='30'  →  parse Ok(30)  →  Session.minutes=30`와
  ...
  - id: ex-day-89-rust-record-parse-debug
    ...
    answer:
      ...
      \ raw.parse().expect(\"분 숫자 오류\") };\n    println!(\"{}\", session.minutes);\n}"
    hint:
      minutes:u32를 기대하는 필드에서 parse가 숫자 변환을 하고 실패하면 Err가 됩니다. 설명과 어긋나는 줄을 찾으세요. `실제 파일 파서에서 expect로 전체 프로그램을 종료하고
      어느 행인지 숨김` 상황이 단서가 됩니다.
    explanation:
      '틀린 줄은 `let session = Session { minutes: raw.len().expect("분 숫자 오류") };`입니다. 여기서는 `raw.len()`을 써서
      `raw.parse()` 동작이 깨집니다. 이대로 실행하면 `실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김` 문제가 생겨 원본 추적 `raw=''30''  →  parse
      Ok(30)  →  Session.minutes=30`대로 ''30''가 나오지 않습니다. 고친 줄 `let session = Session { minutes: raw.parse().expect("분
      숫자 오류") };`에서는 `raw.parse()`가 `텍스트 행을 타입 있는 레코드로 변환` 동작을 지켜 ''30''까지 도달합니다.'
    commonMistakes:
      - 실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-89-rust-record-parse-independent
    ...
    answer:
      "struct Session { minutes: u32 }\nfn main() {\n    let raw = \"30\";\n    let session = Session { minutes:\
      \ raw.parse().expect(\"분 숫자 오류\") };\n    println!(\"{}\", session.minutes);\n}"
    hint: minutes:u32를 기대하는 필드에서 parse가 숫자 변환을 하고 실패하면 Err가 됩니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `raw.parse()` 부분이 `텍스트 행을 타입 있는 레코드로 변환` 동작을 지켜 실행 결과 '30'가 됩니다. 같은 개념을 다른 입력으로
      바꿔도 `raw.parse()`부터 `Session.minutes=30`까지 추적할 수 있으면 정답입니다. `실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김` 상황과
...
quiz:
  - id: quiz-day-89-rust-record-parse-output
    ...
    answerIndex: 2
    explanation:
      raw='30'  →  parse Ok(30)  →  Session.minutes=30 순서로 실행되어 출력은 '30'입니다. `raw.parse()` 부분이 마지막 단계를
  ...
  - id: quiz-day-89-rust-record-parse-model
    ...
    answerIndex: 0
    explanation:
      minutes:u32를 기대하는 필드에서 parse가 숫자 변환을 하고 실패하면 Err가 됩니다. 이 설명이 맞는 이유는 `텍스트 행을 타입 있는 레코드로 변환` 동작을 지키는
      조건과 같기 때문입니다. `실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-89-rust-record-parse-pitfall
    ...
    answerIndex: 1
    explanation:
      실제 파일 파서에서 expect로 전체 프로그램을 종료하고 어느 행인지 숨김. 이 실수가 나오면 원본 추적 `raw='30' → parse Ok(30) → Session.minutes=30`대로
      '30'가 나오지 않으므로 먼저 확인해야 합니다.
  - id: quiz-day-89-rust-record-parse-transfer
    ...
    answerIndex: 2
    explanation: 문법은 달라도 `텍스트 행을 타입 있는 레코드로 변환` 목적과 입력·출력은 유지합니다. 실행 결과 '30'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
원본 출력은 `30`이고, 바뀐 코드의 실행 결과는 `31`입니다. 바뀐 줄은 `let raw = "30";`에서 `let raw = "31";`로 바뀌었습니다. 바뀐 프로그램은 `raw='30'` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `31`가 됩니다. 원본 추적 `raw='30'  →  parse Ok(30)  →  Session.minutes=30`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
