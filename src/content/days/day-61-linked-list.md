...
노드가 다음 노드의 주소를 보유하면 메모리에서 연속되지 않아도 순서를 만들 수 있습니다. Day 61 "`단일 연결 리스트의 다음 노드`"에서는 C 코드가 `20`을(를) 만드는 과정을 따라가며, 주소 링크를 따라 다음 노드 방문 동작이 왜 필요한지 확인합니다. 주소 링크를 따라 다음 노드 방문을(를) 빠뜨리면 `NULL인 next를 검사하지 않고 역참조함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 60](/learn/day-60-circular-queue)에서는 "원형 큐의 머리·꼬리"을(를) 배웠습니다. "원형 큐의 머리·꼬리"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`단일 연결 리스트의 다음 노드`"에서 새로 달라지는 조건을 찾아보세요. Day 61의 답은 `20`이며, 핵심 표현은 `first.next->value`입니다.

## 머릿속 그림

first.next는 last의 주소이고 ->는 포인터를 통해 last.value를 읽습니다. Day 61에서는 아래 흐름 순서대로 상태가 바뀌며, `first.next->value`이(가) 결과를 가릅니다.

```text
first(10)  →  next=&last  →  last(20)  →  출력
```

위 흐름에서 `first.next->value`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `20`입니다.
...
아래 C 코드는 Day 61 "`단일 연결 리스트의 다음 노드`"의 독립 예제입니다. 전체 7줄에서 `first.next->value`이(가) 핵심이며, 실행 결과는 `20`입니다.
...
위 단계에서 `first.next->value`이(가) 빠지면 `20`이(가) 나오지 않습니다. `처음 등장하는 숫자를 20에서 21로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `20`이(가) 됩니다. `first(10)` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `20`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `first.next->value`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 61의 `단일 연결 리스트의 다음 노드`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: NULL인 next를 검사하지 않고 역참조함.** 정상 코드에서는 `20`이(가) 출력됩니다.
...
핵심 동작 **주소 링크를 따라 다음 노드 방문**은(는) 세 언어에서 같은 입력과 출력(`20`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `Node *next와 NULL 경계 검사` 규칙으로 "`단일 연결 리스트의 다음 노드`"의 `20`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `객체의 next 속성으로 참조 연결` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `Option<Box<Node>>로 단일 소유 연결` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`단일 연결 리스트의 다음 노드`"이(가) 필요한 상황을 `주소 링크를 따라 다음 노드 방문` 동작으로 설명해 보세요.
- 예제에서 `first.next->value`이(가) 실행되기 직전의 상태와 직후의 출력 `20`을(를) 말해 보세요.
- "`NULL인 next를 검사하지 않고 역참조함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `주소 링크를 따라 다음 노드 방문` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

first.next는 last의 주소이고 ->는 포인터를 통해 last.value를 읽습니다. Day 61 "`단일 연결 리스트의 다음 노드`"의 예제는 `first.next->value`을(를) 실행해 `20`을(를) 출력합니다. "`NULL인 next를 검사하지 않고 역참조함`" 여부를 확인하고 Day 61을(를) 완료하세요.
