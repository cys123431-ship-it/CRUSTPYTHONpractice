...
exercises:
  - id: ex-day-24-recursion-predict
    ...
    answer: "6"
    hint: 기저 조건 n=0이 멈추는 바닥이고 각 호출은 n을 1 줄여 그 바닥으로 접근합니다. 설명을 떠올리고 `sum_to(3)` 단계부터 순서대로 적어 보세요.
    explanation:
      sum_to(3)  →  sum_to(2)  →  sum_to(1)  →  sum_to(0)  →  6 순서로 실행됩니다. `n == 0` 부분이 `6` 단계를 확정해 최종
      출력 '6'가 됩니다. 이 흐름을 떠올리면 `문제를 줄여 재귀적으로 합산함` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-24-recursion-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: "'재귀와 종료 조건' 개념의 핵심 표현을 스스로 적는다."
    ...
    answer:
      "#include <stdio.h>\nint sum_to(int n) {\n    if (n == 0) return 0;\n    return n + sum_to(n - 1);\n}\n\
      int main(void) { printf(\"%d\\n\", sum_to(3)); return 0; }"
    hint: 힌트 문장을 완전하게 읽으면 `sum_to(3)이 3+sum_to(2)를 기다립니다` 단계에 필요한 표현이 `n == 0`입니다.
    explanation:
      빈칸에 들어갈 표현은 'n == 0'입니다. `if (n == 0) return 0;` 줄을 완성해야 `문제를 줄여 재귀적으로 합산함` 동작이 이어져 실행 결과 '6'가 됩니다.
      힌트의 첫 단계 `sum_to(3)이 3+sum_to(2)를 기다립니다`이 바로 이 줄입니다. 이어서 2+sum_to(1), 1+sum_to(0)를 거칩니다 0에서 돌아와 1+2+3=6입니다 순서로
      진행됩니다.
    commonMistakes:
      - 기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-24-recursion-guided
    ...
    explanation:
      바꾼 뒤 출력은 '5'입니다. 원본 출력 '6'에서 달라졌습니다. 바뀐 줄은 `if (n == 0) return 0;`에서 `if (n == 1) return 0;`로 바뀌었습니다.
      바뀐 프로그램은 `sum_to(3)` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `5`가 됩니다. 원본 추적 `sum_to(3)  →  sum_to(2)  →  sum_to(1)  →  sum_to(0)  →  6`와
  ...
  - id: ex-day-24-recursion-debug
    ...
    answer:
      ...
      int main(void) { printf(\"%d\\n\", sum_to(3)); return 0; }"
    hint:
      기저 조건 n=0이 멈추는 바닥이고 각 호출은 n을 1 줄여 그 바닥으로 접근합니다. 설명과 어긋나는 줄을 찾으세요. `기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함` 상황이 단서가
      됩니다.
    explanation:
      틀린 줄은 `if (n == -1) return 0;`입니다. 여기서는 `n == -1`을 써서 `n == 0` 동작이 깨집니다. 이대로 실행하면 `기저 조건이 없거나 입력을
      줄이지 않아 끝없이 호출함` 문제가 생겨 원본 추적 `sum_to(3)  →  sum_to(2)  →  sum_to(1)  →  sum_to(0)  →  6`대로 '6'가 나오지 않습니다. 고친
      줄 `if (n == 0) return 0;`에서는 `n == 0`가 `문제를 줄여 재귀적으로 합산함` 동작을 지켜 '6'까지 도달합니다.
    commonMistakes:
      - 기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-24-recursion-independent
    ...
    answer:
      "#include <stdio.h>\nint sum_to(int n) {\n    if (n == 0) return 0;\n    return n + sum_to(n - 1);\n}\n\
      int main(void) { printf(\"%d\\n\", sum_to(3)); return 0; }"
    hint: 기저 조건 n=0이 멈추는 바닥이고 각 호출은 n을 1 줄여 그 바닥으로 접근합니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `n == 0` 부분이 `문제를 줄여 재귀적으로 합산함` 동작을 지켜 실행 결과 '6'가 됩니다. 같은 개념을 다른 입력으로 바꿔도 `n
      == 0`부터 `6`까지 추적할 수 있으면 정답입니다. `기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - 기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-24-recursion-output
    ...
    answerIndex: 0
    explanation:
      sum_to(3)  →  sum_to(2)  →  sum_to(1)  →  sum_to(0)  →  6 순서로 실행되어 출력은 '6'입니다. `n == 0` 부분이 마지막 단계를
      확정하므로 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-24-recursion-model
    question: "'재귀와 종료 조건' 개념을 이해하는 데 맞는 설명은?"
    choices:
    ...
    answerIndex: 1
    explanation:
      기저 조건 n=0이 멈추는 바닥이고 각 호출은 n을 1 줄여 그 바닥으로 접근합니다. 이 설명이 맞는 이유는 `문제를 줄여 재귀적으로 합산함` 동작을 지키는 조건과 같기 때문입니다.
      `기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-24-recursion-pitfall
    ...
    answerIndex: 2
    explanation:
      기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함. 이 실수가 나오면 원본 추적 `sum_to(3) → sum_to(2) → sum_to(1) → sum_to(0) → 6`대로
      '6'가 나오지 않으므로 먼저 확인해야 합니다.
  - id: quiz-day-24-recursion-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 0
    explanation: 문법은 달라도 `문제를 줄여 재귀적으로 합산함` 목적과 입력·출력은 유지합니다. 실행 결과 '6'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
원본 출력은 `6`이고, 바뀐 코드의 실행 결과는 `5`입니다. 바뀐 줄은 `if (n == 0) return 0;`에서 `if (n == 1) return 0;`로 바뀌었습니다. 바뀐 프로그램은 `sum_to(3)` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `5`가 됩니다. 원본 추적 `sum_to(3)  →  sum_to(2)  →  sum_to(1)  →  sum_to(0)  →  6`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
...
두 줄을 나란히 놓으면 정상 줄 `if (n == 0) return 0;`이 `기저 조건 n=0이 멈추는 바닥이고 각 호출은 n을 1 줄여 그 바닥으로 접근합니다.` 설명과 맞고, 잘못된 줄은 `기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `sum_to(3)`부터 `6`까지 처음 어긋난 곳을 찾습니다.
...
예측(`6` 맞히기) → 빈칸(`n == 0` 채우기) → 변경(`처음 등장하는 숫자를 0에서 1로`) → 오류 수정(`기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함` 찾기) → 독립 구현(`'재귀와 종료 조건' 개념을 보여주는 작은 프로그램` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `6` 및 `n == 0` 설명과 대조하세요.
