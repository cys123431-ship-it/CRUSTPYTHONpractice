...
exercises:
  - id: ex-day-24-recursion-predict
    ...
    answer: "6"
    hint: sum_to(3)에서 시작해 6까지 순서대로 적어 보세요. 기저 조건 n=0이 멈추는 바닥이고 각 호출은 n을 1 줄여 그 바닥으로 접근합니다.
    explanation:
      1. sum_to(3)이 3+sum_to(2)를 기다립니다. 2. 2+sum_to(1), 1+sum_to(0)를 거칩니다. 3. 0에서 돌아와 1+2+3=6입니다. `sum_to(3)
      → sum_to(2) → sum_to(1) → sum_to(0) → 6` 흐름으로 실제 출력 `6`이 됩니다. 핵심 `n == 0`은 호출을 멈추는 바닥을 정하는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-24-recursion-fill
    ...
    answer:
      "#include <stdio.h>\nint sum_to(int n) {\n    if (n == 0) return 0;\n    return n + sum_to(n - 1);\n}\n\
      int main(void) { printf(\"%d\\n\", sum_to(3)); return 0; }"
    hint: 필요한 표현은 문제를 줄여 재귀적으로 합산함 동작을 잇는 n == 0입니다.
    explanation:
      빈칸에 들어갈 표현은 'n == 0'입니다. 이 표현이 없어도 이번 화면 출력은 같을 수 있으나 숨은 계약을 어깁니다. 1. sum_to(3)이 3+sum_to(2)를 기다립니다.
      2. 2+sum_to(1), 1+sum_to(0)를 거칩니다. 3. 0에서 돌아와 1+2+3=6입니다. `sum_to(3) → sum_to(2) → sum_to(1) → sum_to(0) → 6`
      흐름으로 실제 출력 `6`이 됩니다. 핵심 `n == 0`은 호출을 멈추는 바닥을 정하는 자리에 쓰입니다.
    commonMistakes:
      - 기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-24-recursion-guided
    ...
    explanation:
      바꾼 뒤 출력은 '5'입니다. 원본은 `6`, 수정본은 `5`이다. 첫 변경 줄 `if (n == 0) return 0;`에서 `if (n == 1) return 0;`으로
      바닥을 바꾸면, 3+2에 1 대신 0이 더해져 5가 된다. 돌아오는 값 1, 3, 5의 순서를 손으로 적어 보세요.
    commonMistakes:
      - 기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-24-recursion-debug
    ...
    answer:
      "#include <stdio.h>\nint sum_to(int n) {\n    if (n == 0) return 0;\n    return n + sum_to(n - 1);\n}\n\
      int main(void) { printf(\"%d\\n\", sum_to(3)); return 0; }"
    hint: 기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함 상황에서 어긋나는 줄을 기저 조건 n=0이 멈추는 바닥이고 각 호출은 n을 1 줄여 그 바닥으로 접근합니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `if (n == -1) return 0;`입니다. 입력 3에 대해서는 3+2+1+0+0으로 우연히 같은 `6`이 출력됩니다. 오류 분류는 같은 출력이지만 숨은 계약
      위반입니다. 0에서 멈춰야 하는 함수 계약이 어긋나 있으므로, 음수 입력이나 일반화된 재귀에서는 종료와 정의가 달라집니다. `6이 나오지 않는다`고 쓰면 안 됩니다. 고친 줄 `if (n ==
      0) return 0;`에서는 `문제를 줄여 재귀적으로 합산함` 동작이 지켜집니다. 정상 코드는 내려가기→바닥 0→돌아오며 더하기의 순서로 `6`을 출력합니다.
    commonMistakes:
      - 기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-24-recursion-independent
    ...
    answer:
      "#include <stdio.h>\nint sum_to(int n) {\n    if (n == 0) return 0;\n    return n + sum_to(n - 1);\n}\n\
      int main(void) { printf(\"%d\\n\", sum_to(3)); return 0; }"
    hint: 음수가 들어오면 1씩 빼는 방식으로는 0에 닿지 않습니다. 입력 조건을 정하고 호출·반환 순서를 따로 적어 보세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 음수가 들어오면 1씩 빼는 방식으로는 0에 닿지 않습니다. 입력 조건을 정하고 호출·반환 순서를 따로 적어 보세요. 같은 문제를
      줄여 재귀적으로 합산함 동작을 구현하고 실행 결과 '6'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-24-recursion-output
    ...
    answerIndex: 0
    explanation:
      먼저 sum_to(3)이 3+sum_to(2)를 기다립니다. 이어서 2+sum_to(1), 1+sum_to(0)를 거쳐 0에서 돌아오며 1, 3, 6이 만들어지므로 출력 `6`이
      됩니다. `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 재귀 호출과 종료가 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 printf가 실제로 호출되기 때문입니다.
  ...
  - id: quiz-day-24-recursion-model
    ...
    answerIndex: 1
    explanation:
      기저 조건 n=0이 멈추는 바닥이라는 뜻은, 그 조건에서 더 이상 호출하지 않는다는 뜻이고, 각 호출은 n을 1 줄여 그 바닥으로 접근한다는 뜻은 호출이 반드시 끝난다는 뜻입니다.
      `기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함`은 반대 사례인데, 바닥이 없으면 호출이 멈추지 않기 때문입니다.
  - id: quiz-day-24-recursion-pitfall
    ...
    answerIndex: 2
    explanation:
      먼저 확인할 실수는 `기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함`입니다. 이번 잘못된 코드는 입력 3에서는 화면 출력이 같습니다. 같다고 해서 맞는 것이 아니라,
      0에서 멈춘다는 계약을 어긴 숨은 계약 위반입니다.
  - id: quiz-day-24-recursion-transfer
    ...
    answerIndex: 0
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `문제를 줄여 재귀적으로 합산함`이라는 의미와 입력·출력 계약입니다. Python의 재귀와 Rust의 재귀 함수로 같은 동작을 구현하고,
      실행 결과 `6`으로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
예제의 핵심 표현은 `n == 0`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본은 `6`, 수정본은 `5`이다. 첫 변경 줄 `if (n == 0) return 0;`에서 `if (n == 1) return 0;`으로 바닥을 바꾸면, 3+2에 1 대신 0이 더해져 5가 된다. 돌아오는 값 1, 3, 5의 순서를 손으로 적어 보세요.
