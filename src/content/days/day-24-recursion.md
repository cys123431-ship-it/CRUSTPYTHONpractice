...
큰 문제를 같은 모양의 작은 문제로 줄일 수 있으면 재귀로 관계를 표현할 수 있습니다. Day 24 "`재귀와 종료 조건`"에서는 C 코드가 `6`을(를) 만드는 과정을 따라가며, 문제를 줄여 재귀적으로 합산함 동작이 왜 필요한지 확인합니다. 문제를 줄여 재귀적으로 합산함을(를) 빠뜨리면 `기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 23](/learn/day-23-scope)에서는 "지역 변수와 이름의 범위"을(를) 배웠습니다. "지역 변수와 이름의 범위"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`재귀와 종료 조건`"에서 새로 달라지는 조건을 찾아보세요. Day 24의 답은 `6`이며, 핵심 표현은 `n == 0`입니다.

## 머릿속 그림

기저 조건 n=0이 멈추는 바닥이고 각 호출은 n을 1 줄여 그 바닥으로 접근합니다. Day 24에서는 아래 흐름 순서대로 상태가 바뀌며, `n == 0`이(가) 결과를 가릅니다.

```text
sum_to(3)  →  sum_to(2)  →  sum_to(1)  →  sum_to(0)  →  6
```

위 흐름에서 `n == 0`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `6`입니다.
...
아래 C 코드는 Day 24 "`재귀와 종료 조건`"의 독립 예제입니다. 전체 6줄에서 `n == 0`이(가) 핵심이며, 실행 결과는 `6`입니다.
...
C17은 `gcc -std=c17 -Wall -Wextra`로 컴파일해 실행하면 `6`이(가) 나옵니다. 브라우저에서는 임의 컴파일 대신 하단 실습 칸의 답안 비교를 사용하세요.
...
위 단계에서 `n == 0`이(가) 빠지면 `6`이(가) 나오지 않습니다. `처음 등장하는 숫자를 0에서 1로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `6`이(가) 됩니다. `sum_to(3)` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `6`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `n == 0`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 24의 `재귀와 종료 조건`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함.** 정상 코드에서는 `6`이(가) 출력됩니다.
...
핵심 동작 **문제를 줄여 재귀적으로 합산함**은(는) 세 언어에서 같은 입력과 출력(`6`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `if (n==0) return 0;` 규칙으로 "`재귀와 종료 조건`"의 `6`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `if n==0: return 0` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `if n==0 {0} else {n+sum_to(n-1)}` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`재귀와 종료 조건`"이(가) 필요한 상황을 `문제를 줄여 재귀적으로 합산함` 동작으로 설명해 보세요.
- 예제에서 `n == 0`이(가) 실행되기 직전의 상태와 직후의 출력 `6`을(를) 말해 보세요.
- "`기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `문제를 줄여 재귀적으로 합산함` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

기저 조건 n=0이 멈추는 바닥이고 각 호출은 n을 1 줄여 그 바닥으로 접근합니다. Day 24 "`재귀와 종료 조건`"의 예제는 `n == 0`을(를) 실행해 `6`을(를) 출력합니다. "`기저 조건이 없거나 입력을 줄이지 않아 끝없이 호출함`" 여부를 확인하고 Day 24을(를) 완료하세요.
