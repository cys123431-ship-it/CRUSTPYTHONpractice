...
exercises:
  - id: ex-day-43-output-parameter-predict
    ...
    answer: "12"
    hint: out은 호출자 n의 주소입니다. *out에 대입해야 호출자 변수의 값이 바뀝니다. 설명을 떠올리고 `n=0` 단계부터 순서대로 적어 보세요.
    explanation:
      n=0  →  &n 전달  →  *out=12  →  n 출력 순서로 실행됩니다. `*out =` 부분이 `n 출력` 단계를 확정해 최종 출력 '12'가 됩니다. 이 흐름을
      떠올리면 `출력 인자로 호출자 값을 수정함` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-43-output-parameter-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: "'C 포인터로 결과 돌려주기' 개념의 핵심 표현을 스스로 적는다."
    ...
    answer:
      "#include <stdio.h>\nint double_into(int input, int *out) {\n    if (!out) return 0;\n    *out = input\
      \ * 2; return 1;\n}\nint main(void) { int n=0; if(double_into(6,&n)) printf(\"%d\\n\",n); return 0; }"
    hint: 힌트 문장을 완전하게 읽으면 `main이 &n을 전달합니다` 단계에 필요한 표현이 `*out =`입니다.
    explanation:
      빈칸에 들어갈 표현은 '*out ='입니다. `*out = input * 2; return 1;` 줄을 완성해야 `출력 인자로 호출자 값을 수정함` 동작이 이어져 실행 결과
      '12'가 됩니다. 힌트의 첫 단계 `main이 &n을 전달합니다`이 바로 이 줄입니다. 이어서 함수가 NULL을 확인합니다 *out으로 n=12를 만들고 성공을 반환합니다 순서로 진행됩니다.
    commonMistakes:
      - out 자체에 대입하고 호출자 변수도 바뀌었다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-43-output-parameter-guided
    ...
    explanation:
      바꾼 뒤 출력은 '18'입니다. 원본 출력 '12'에서 달라졌습니다. 바뀐 줄은 `*out = input * 2; return 1;`에서 `*out = input * 3; return
      1;`로 바뀌었습니다. 바뀐 프로그램은 `n=0` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `18`가 됩니다. 원본 추적 `n=0  →  &n 전달  →  *out=12  →  n
  ...
  - id: ex-day-43-output-parameter-debug
    ...
    answer:
      ...
      \ * 2; return 1;\n}\nint main(void) { int n=0; if(double_into(6,&n)) printf(\"%d\\n\",n); return 0; }"
    hint:
      out은 호출자 n의 주소입니다. *out에 대입해야 호출자 변수의 값이 바뀝니다. 설명과 어긋나는 줄을 찾으세요. `out 자체에 대입하고 호출자 변수도 바뀌었다고 생각함` 상황이 단서가
      됩니다.
    explanation:
      틀린 줄은 `out = input * 2; return 1;`입니다. 여기서는 `out =`을 써서 `*out =` 동작이 깨집니다. 이대로 실행하면 `out 자체에 대입하고
      호출자 변수도 바뀌었다고 생각함` 문제가 생겨 원본 추적 `n=0  →  &n 전달  →  *out=12  →  n 출력`대로 '12'가 나오지 않습니다. 고친 줄 `*out = input *
      2; return 1;`에서는 `*out =`가 `출력 인자로 호출자 값을 수정함` 동작을 지켜 '12'까지 도달합니다.
    commonMistakes:
      - out 자체에 대입하고 호출자 변수도 바뀌었다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-43-output-parameter-independent
    ...
    answer:
      "#include <stdio.h>\nint double_into(int input, int *out) {\n    if (!out) return 0;\n    *out = input\
      \ * 2; return 1;\n}\nint main(void) { int n=0; if(double_into(6,&n)) printf(\"%d\\n\",n); return 0; }"
    hint: out은 호출자 n의 주소입니다. *out에 대입해야 호출자 변수의 값이 바뀝니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `*out =` 부분이 `출력 인자로 호출자 값을 수정함` 동작을 지켜 실행 결과 '12'가 됩니다. 같은 개념을 다른 입력으로 바꿔도
      `*out =`부터 `n 출력`까지 추적할 수 있으면 정답입니다. `out 자체에 대입하고 호출자 변수도 바뀌었다고 생각함` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - out 자체에 대입하고 호출자 변수도 바뀌었다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-43-output-parameter-output
    ...
    answerIndex: 1
    explanation:
      n=0  →  &n 전달  →  *out=12  →  n 출력 순서로 실행되어 출력은 '12'입니다. `*out =` 부분이 마지막 단계를 확정하므로 다른 선택지는 이 추적과
      맞지 않습니다.
  - id: quiz-day-43-output-parameter-model
    ...
    answerIndex: 2
    explanation:
      out은 호출자 n의 주소입니다. *out에 대입해야 호출자 변수의 값이 바뀝니다. 이 설명이 맞는 이유는 `출력 인자로 호출자 값을 수정함` 동작을 지키는 조건과 같기 때문입니다.
      `out 자체에 대입하고 호출자 변수도 바뀌었다고 생각함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-43-output-parameter-pitfall
    ...
    answerIndex: 0
    explanation:
      out 자체에 대입하고 호출자 변수도 바뀌었다고 생각함. 이 실수가 나오면 원본 추적 `n=0 → &n 전달 → *out=12 → n 출력`대로 '12'가 나오지 않으므로 먼저
      확인해야 합니다.
  - id: quiz-day-43-output-parameter-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 1
    explanation: 문법은 달라도 `출력 인자로 호출자 값을 수정함` 목적과 입력·출력은 유지합니다. 실행 결과 '12'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
원본 출력은 `12`이고, 바뀐 코드의 실행 결과는 `18`입니다. 바뀐 줄은 `*out = input * 2; return 1;`에서 `*out = input * 3; return 1;`로 바뀌었습니다. 바뀐 프로그램은 `n=0` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `18`가 됩니다. 원본 추적 `n=0  →  &n 전달  →  *out=12  →  n 출력`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
