...
exercises:
  ...
  - id: ex-day-43-output-parameter-guided
    ...
    answer:
      "#include <stdio.h>\nint double_into(int input, int *out) {\n    if (!out) return 0;\n    *out = input\
      \ * 3; return 1;\n}\nint main(void) { int n=0; if(double_into(6,&n)) printf(\"%d\\n\",n); return 0; }"
    hint: 처음 등장하는 숫자를 2에서 3로 바꾼 뒤 n=0 → &n 전달 → *out=12 → n 출력 순으로 따라가세요.
    explanation: 예시 답안에서는 처음 등장하는 숫자를 2에서 3로 바꿨습니다. 원본 출력은 '12'입니다. 바뀐 코드의 결과는 실행해 확인하세요.
    commonMistakes:
      - out 자체에 대입하고 호출자 변수도 바뀌었다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-43-output-parameter-debug
    ...
    prompt: 아래 코드의 핵심 표현 'out ='을 점검하고 올바른 작동을 복원하세요. out 자체에 대입하고 호출자 변수도 바뀌었다고 생각함 오류도 함께 경계하세요.
    starter:
      "#include <stdio.h>\nint double_into(int input, int *out) {\n    if (!out) return 0;\n    out = input\
      \ * 2; return 1;\n}\nint main(void) { int n=0; if(double_into(6,&n)) printf(\"%d\\n\",n); return 0; }"
    answer:
...
C 함수에서 성공 여부와 계산 결과를 동시에 돌려주려면 주소를 통한 출력 인자를 사용할 수 있습니다. 처음 읽을 때는 결과를 가리고 손으로 예상하세요. 예시를 직접 타이핑한 뒤 한 줄씩 바꾸며 상태를 확인하세요.

## 2. 시작 전에 확인할 것

바로 앞선 [Day 42](/learn/day-42-memory-review)의 핵심을 한 문장으로 설명해 보세요. 모르면 위의 선행 Day 링크에서 다시 확인할 수 있습니다.

## 3. 머릿속 그림

out은 호출자 n의 주소입니다. *out에 대입해야 호출자 변수의 값이 바뀝니다. 다음 흐름을 눈으로 확인하세요.
...
예제의 핵심 표현은 `*out =`입니다. 전체 코드에서 이 표현을 찾아 표시하세요. 선언과 조건, 출력이 연결되는 과정을 순서대로 설명합니다.
...
마지막 상태에서 화면에 표시되는 결과는 `12`입니다. 직접 타이핑할 때는 위의 순서와 실제 출력을 비교하세요.
...
**확인할 실수: out 자체에 대입하고 호출자 변수도 바뀌었다고 생각함.** out은 호출자 n의 주소입니다. *out에 대입해야 호출자 변수의 값이 바뀝니다. 결과가 예상과 다르면 입력 → 중간 상태 → 출력 중 처음 어긋난 곳을 찾습니다.

## 9. 다른 언어로 옮기기

같은 문제를 해결하더라도 선언, 타입, 메모리 관리, 오류 처리는 다릅니다. 핵심 동작은 **출력 인자로 호출자 값을 수정함**입니다. 언어를 옮겨도 이 동작을 유지하세요.
...
- 왜 이 개념이 필요한가? C 함수에서 성공 여부와 계산 결과를 동시에 돌려주려면 주소를 통한 출력 인자를 사용할 수 있습니다.
- 예시에서 가장 먼저 확정되는 값이나 상태는 무엇인가?
- 어떤 실수를 점검해야 하나? out 자체에 대입하고 호출자 변수도 바뀌었다고 생각함
- 다른 두 언어에서는 같은 동작을 어떤 자료형과 오류 처리로 나타내는가?

## 12. 핵심 요약과 복습

out은 호출자 n의 주소입니다. *out에 대입해야 호출자 변수의 값이 바뀝니다. 예시의 출력은 `12`입니다. 오류를 찾을 때는 **out 자체에 대입하고 호출자 변수도 바뀌었다고 생각함** 여부를 확인하세요. 완료 버튼을 누르면 +1·+3·+7·+14·+30일 복습이 이 기기에 등록됩니다.
