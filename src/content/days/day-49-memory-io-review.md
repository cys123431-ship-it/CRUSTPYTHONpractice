...
exercises:
  ...
  - id: ex-day-49-memory-io-review-fill
    ...
    explanation: 빈칸에 들어갈 표현은 '*view'입니다. minutes에 30을 저장합니다 view에 &minutes를 담습니다 *view를 읽어 30을 출력합니다
    commonMistakes:
      - 포인터 값(주소)과 역참조 결과(정수)를 같은 타입으로 혼동함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-49-memory-io-review-guided
    ...
    explanation:
      바꾼 뒤 출력은 '31'입니다. 원본 출력 '30'에서 달라졌습니다. 다른 줄(int minutes = 30; → int minutes = 31;)에서 시작한 차이가 최종 출력에
      반영되었습니다.
    commonMistakes:
      - 포인터 값(주소)과 역참조 결과(정수)를 같은 타입으로 혼동함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
...
quiz:
  - id: quiz-day-49-memory-io-review-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 1
    explanation: minutes=30  →  view=&minutes  →  *view=30 순서로 실행되어 출력은 '30'입니다.
...
원본과 달라졌습니다. 원본 출력은 `30`입니다. 다른 줄(`int minutes = 30;` → `int minutes = 31;`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`minutes=30` → …)과 바뀐 줄부터 대조해 보세요.
