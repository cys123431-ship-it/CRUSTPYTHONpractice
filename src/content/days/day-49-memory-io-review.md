...
exercises:
  - id: ex-day-49-memory-io-review-predict
    ...
    starter:
      ...
      %d\\n\", *view);\n    return 0;\n}"
    answer: "30"
    hint: view의 값은 minutes의 주소이고 *view의 값은 그 주소에 저장된 정수 30입니다. 설명을 떠올리고 `minutes=30` 단계부터 순서대로 적어 보세요.
    explanation:
      minutes=30  →  view=&minutes  →  *view=30 순서로 실행됩니다. `*view` 부분이 `*view=30` 단계를 확정해 최종 출력 '30'가 됩니다.
      이 흐름을 떠올리면 `주소를 통해 저장된 값 읽기` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-49-memory-io-review-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: "'메모리와 입출력 회상' 개념의 핵심 표현을 스스로 적는다."
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int minutes = 30;\n    int *view = &minutes;\n    printf(\"\
      %d\\n\", *view);\n    return 0;\n}"
    hint: 힌트 문장을 완전하게 읽으면 `minutes에 30을 저장합니다` 단계에 필요한 표현이 `*view`입니다.
    explanation:
      빈칸에 들어갈 표현은 '*view'입니다. `int *view = &minutes;` 줄을 완성해야 `주소를 통해 저장된 값 읽기` 동작이 이어져 실행 결과 '30'가 됩니다.
      힌트의 첫 단계 `minutes에 30을 저장합니다`이 바로 이 줄입니다. 이어서 view에 &minutes를 담습니다 *view를 읽어 30을 출력합니다 순서로 진행됩니다.
    commonMistakes:
      - 포인터 값(주소)과 역참조 결과(정수)를 같은 타입으로 혼동함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-49-memory-io-review-guided
    ...
    explanation:
      바꾼 뒤 출력은 '31'입니다. 원본 출력 '30'에서 달라졌습니다. 바뀐 줄은 `int minutes = 30;`에서 `int minutes = 31;`로 바뀌었습니다. 바뀐
      프로그램은 `minutes=30` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `31`가 됩니다. 원본 추적 `minutes=30  →  view=&minutes  →  *view=30`와
  ...
  - id: ex-day-49-memory-io-review-debug
    ...
    answer:
      ...
      %d\\n\", *view);\n    return 0;\n}"
    hint:
      view의 값은 minutes의 주소이고 *view의 값은 그 주소에 저장된 정수 30입니다. 설명과 어긋나는 줄을 찾으세요. `포인터 값(주소)과 역참조 결과(정수)를 같은 타입으로 혼동함`
      상황이 단서가 됩니다.
    explanation:
      틀린 줄은 `int view = &minutes;`입니다. 여기서는 `view`을 써서 `*view` 동작이 깨집니다. 이대로 실행하면 `포인터 값(주소)과 역참조 결과(정수)를
      같은 타입으로 혼동함` 문제가 생겨 원본 추적 `minutes=30  →  view=&minutes  →  *view=30`대로 '30'가 나오지 않습니다. 고친 줄 `int *view = &minutes;`에서는
      `*view`가 `주소를 통해 저장된 값 읽기` 동작을 지켜 '30'까지 도달합니다.
    commonMistakes:
      - 포인터 값(주소)과 역참조 결과(정수)를 같은 타입으로 혼동함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-49-memory-io-review-independent
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int minutes = 30;\n    int *view = &minutes;\n    printf(\"\
      %d\\n\", *view);\n    return 0;\n}"
    hint: view의 값은 minutes의 주소이고 *view의 값은 그 주소에 저장된 정수 30입니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `*view` 부분이 `주소를 통해 저장된 값 읽기` 동작을 지켜 실행 결과 '30'가 됩니다. 같은 개념을 다른 입력으로 바꿔도 `*view`부터
      `*view=30`까지 추적할 수 있으면 정답입니다. `포인터 값(주소)과 역참조 결과(정수)를 같은 타입으로 혼동함` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - 포인터 값(주소)과 역참조 결과(정수)를 같은 타입으로 혼동함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-49-memory-io-review-output
    ...
    answerIndex: 1
    explanation:
      minutes=30  →  view=&minutes  →  *view=30 순서로 실행되어 출력은 '30'입니다. `*view` 부분이 마지막 단계를 확정하므로 다른 선택지는
      이 추적과 맞지 않습니다.
  - id: quiz-day-49-memory-io-review-model
    question: "'메모리와 입출력 회상' 개념을 이해하는 데 맞는 설명은?"
    choices:
    ...
    answerIndex: 2
    explanation:
      view의 값은 minutes의 주소이고 *view의 값은 그 주소에 저장된 정수 30입니다. 이 설명이 맞는 이유는 `주소를 통해 저장된 값 읽기` 동작을 지키는 조건과 같기
      때문입니다. `포인터 값(주소)과 역참조 결과(정수)를 같은 타입으로 혼동함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-49-memory-io-review-pitfall
    ...
    answerIndex: 0
    explanation:
      포인터 값(주소)과 역참조 결과(정수)를 같은 타입으로 혼동함. 이 실수가 나오면 원본 추적 `minutes=30 → view=&minutes → *view=30`대로 '30'가
  ...
  - id: quiz-day-49-memory-io-review-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 1
    explanation: 문법은 달라도 `주소를 통해 저장된 값 읽기` 목적과 입력·출력은 유지합니다. 실행 결과 '30'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
원본 출력은 `30`이고, 바뀐 코드의 실행 결과는 `31`입니다. 바뀐 줄은 `int minutes = 30;`에서 `int minutes = 31;`로 바뀌었습니다. 바뀐 프로그램은 `minutes=30` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `31`가 됩니다. 원본 추적 `minutes=30  →  view=&minutes  →  *view=30`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
...
두 줄을 나란히 놓으면 정상 줄 `int *view = &minutes;`이 `view의 값은 minutes의 주소이고 *view의 값은 그 주소에 저장된 정수 30입니다.` 설명과 맞고, 잘못된 줄은 `포인터 값(주소)과 역참조 결과(정수)를 같은 타입으로 혼동함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `minutes=30`부터 `30`까지 처음 어긋난 곳을 찾습니다.
...
예측(`30` 맞히기) → 빈칸(`*view` 채우기) → 변경(`처음 등장하는 숫자를 30에서 31로`) → 오류 수정(`포인터 값(주소)과 역참조 결과(정수)를 같은 타입으로 혼동함` 찾기) → 독립 구현(`'메모리와 입출력 회상' 개념을 보여주는 작은 프로그램` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `30` 및 `*view` 설명과 대조하세요.
