...
exercises:
  - id: ex-day-78-greedy-predict
    ...
    answer: "[5, 5, 1]"
    hint: 큰 동전을 먼저 고르는 규칙은 5·1 체계에서 11에 3개지만 임의 동전 체계에서 최적을 보장하지 않습니다. 설명을 떠올리고 `11` 단계부터 순서대로 적어 보세요.
    explanation:
      11  →  6  →  1  →  0 순서로 실행됩니다. `remaining >= coin` 부분이 `0` 단계를 확정해 최종 출력 '[5, 5, 1]'가 됩니다. 이 흐름을
      떠올리면 `국소 선택의 최적성 조건 확인` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-78-greedy-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: "'그리디 선택과 반례' 개념의 핵심 표현을 스스로 적는다."
    ...
    answer:
      "coins = [5, 1]\nremaining = 11\nused = []\nfor coin in coins:\n    while remaining >= coin: remaining\
      \ -= coin; used.append(coin)\nprint(used)"
    hint: 힌트 문장을 완전하게 읽으면 `11에서 5를 두 번 빼 1이 남습니다` 단계에 필요한 표현이 `remaining >= coin`입니다.
    explanation:
      "빈칸에 들어갈 표현은 'remaining >= coin'입니다. `while remaining >= coin: remaining -= coin; used.append(coin)`
      줄을 완성해야 `국소 선택의 최적성 조건 확인` 동작이 이어져 실행 결과 '[5, 5, 1]'가 됩니다. 힌트의 첫 단계 `11에서 5를 두 번 빼 1이 남습니다`이 바로 이 줄입니다. 이어서
      1을 한 번 뺍니다 세 동전 [5,5,1]을 출력합니다 순서로 진행됩니다."
    commonMistakes:
      - 모든 동전 체계에서 큰 동전부터 고르면 최적이라고 일반화함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-78-greedy-guided
    ...
    explanation:
      바꾼 뒤 출력은 '[6, 1, 1, 1, 1, 1]'입니다. 원본 출력 '[5, 5, 1]'에서 달라졌습니다. 바뀐 줄은 `coins = [5, 1]`에서 `coins = [6,
      1]`로 바뀌었습니다. 바뀐 프로그램은 `11` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `[6, 1, 1, 1, 1, 1]`가 됩니다. 원본 추적
      `11  →  6  →  1  →  0`와 바뀐 줄 이후를 순서대로 비교하면 처음 달라지는 곳이 보입니다.
    commonMistakes:
      - 모든 동전 체계에서 큰 동전부터 고르면 최적이라고 일반화함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-78-greedy-debug
    ...
    answer:
      ...
      \ -= coin; used.append(coin)\nprint(used)"
    hint:
      큰 동전을 먼저 고르는 규칙은 5·1 체계에서 11에 3개지만 임의 동전 체계에서 최적을 보장하지 않습니다. 설명과 어긋나는 줄을 찾으세요. `모든 동전 체계에서 큰 동전부터 고르면 최적이라고
      일반화함` 상황이 단서가 됩니다.
    explanation:
      "틀린 줄은 `while remaining > coin: remaining -= coin; used.append(coin)`입니다. 여기서는 `remaining > coin`을
      써서 `remaining >= coin` 동작이 깨집니다. 이대로 실행하면 `모든 동전 체계에서 큰 동전부터 고르면 최적이라고 일반화함` 문제가 생겨 원본 추적 `11  →  6  →  1  →  0`대로
      '[5, 5, 1]'가 나오지 않습니다. 고친 줄 `while remaining >= coin: remaining -= coin; used.append(coin)`에서는 `remaining
      >= coin`가 `국소 선택의 최적성 조건 확인` 동작을 지켜 '[5, 5, 1]'까지 도달합니다."
    commonMistakes:
      - 모든 동전 체계에서 큰 동전부터 고르면 최적이라고 일반화함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-78-greedy-independent
    ...
    answer:
      ...
      \ -= coin; used.append(coin)\nprint(used)"
    hint:
      큰 동전을 먼저 고르는 규칙은 5·1 체계에서 11에 3개지만 임의 동전 체계에서 최적을 보장하지 않습니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를
      함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `remaining >= coin` 부분이 `국소 선택의 최적성 조건 확인` 동작을 지켜 실행 결과 '[5, 5, 1]'가 됩니다. 같은
      개념을 다른 입력으로 바꿔도 `remaining >= coin`부터 `0`까지 추적할 수 있으면 정답입니다. `모든 동전 체계에서 큰 동전부터 고르면 최적이라고 일반화함` 상황과 빈 입력 같은
      경계도 함께 설명해 보세요.
    commonMistakes:
      - 모든 동전 체계에서 큰 동전부터 고르면 최적이라고 일반화함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
...
quiz:
  - id: quiz-day-78-greedy-output
    ...
    answerIndex: 0
    explanation:
      11  →  6  →  1  →  0 순서로 실행되어 출력은 '[5, 5, 1]'입니다. `remaining >= coin` 부분이 마지막 단계를 확정하므로 다른 선택지는 이
      추적과 맞지 않습니다.
  - id: quiz-day-78-greedy-model
    ...
    answerIndex: 1
    explanation:
      큰 동전을 먼저 고르는 규칙은 5·1 체계에서 11에 3개지만 임의 동전 체계에서 최적을 보장하지 않습니다. 이 설명이 맞는 이유는 `국소 선택의 최적성 조건 확인` 동작을
      지키는 조건과 같기 때문입니다. `모든 동전 체계에서 큰 동전부터 고르면 최적이라고 일반화함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-78-greedy-pitfall
    ...
    answerIndex: 2
    explanation:
      모든 동전 체계에서 큰 동전부터 고르면 최적이라고 일반화함. 이 실수가 나오면 원본 추적 `11 → 6 → 1 → 0`대로 '[5, 5, 1]'가 나오지 않으므로 먼저 확인해야
      합니다.
  - id: quiz-day-78-greedy-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 0
    explanation: 문법은 달라도 `국소 선택의 최적성 조건 확인` 목적과 입력·출력은 유지합니다. 실행 결과 '[5, 5, 1]'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
playgroundSource:
  "coins = [5, 1]\nremaining = 11\nused = []\nfor coin in coins:\n    while remaining >= coin: remaining\
  \ -= coin; used.append(coin)\nprint(used)"
---
...
원본 실행 결과는 `[5, 5, 1]`입니다. 아래 코드에서 원본과 다른 줄을 먼저 찾으세요.
...
원본 출력은 `[5, 5, 1]`이고, 바뀐 코드의 실행 결과는 `[6, 1, 1, 1, 1, 1]`입니다. 바뀐 줄은 `coins = [5, 1]`에서 `coins = [6, 1]`로 바뀌었습니다. 바뀐 프로그램은 `11` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `[6, 1, 1, 1, 1, 1]`가 됩니다. 원본 추적 `11  →  6  →  1  →  0`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
