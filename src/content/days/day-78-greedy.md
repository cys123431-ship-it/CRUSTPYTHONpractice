...
exercises:
  ...
  - id: ex-day-78-greedy-debug
    ...
    prompt: 아래 코드의 핵심 표현 'remaining > coin'을 점검하고 올바른 작동을 복원하세요. 모든 동전 체계에서 큰 동전부터 고르면 최적이라고 일반화함 오류도 함께 경계하세요.
    starter:
      "coins = [5, 1]\nremaining = 11\nused = []\nfor coin in coins:\n    while remaining > coin: remaining\
      \ -= coin; used.append(coin)\nprint(used)"
    answer:
...
매 순간 가장 큰 선택을 하는 전략이 언제 맞는지 반례와 함께 판단해야 합니다. 처음 읽을 때는 결과를 가리고 손으로 예상하세요. 예시를 직접 타이핑한 뒤 한 줄씩 바꾸며 상태를 확인하세요.

## 2. 시작 전에 확인할 것

바로 앞선 [Day 77](/learn/day-77-search-sort-review)의 핵심을 한 문장으로 설명해 보세요. 모르면 위의 선행 Day 링크에서 다시 확인할 수 있습니다.

## 3. 머릿속 그림

큰 동전을 먼저 고르는 규칙은 5·1 체계에서 11에 3개지만 임의 동전 체계에서 최적을 보장하지 않습니다. 다음 흐름을 눈으로 확인하세요.
...
예제의 핵심 표현은 `remaining >= coin`입니다. 전체 코드에서 이 표현을 찾아 표시하세요. 선언과 조건, 출력이 연결되는 과정을 순서대로 설명합니다.
...
마지막 상태에서 화면에 표시되는 결과는 `[5, 5, 1]`입니다. 직접 타이핑할 때는 위의 순서와 실제 출력을 비교하세요.
...
**확인할 실수: 모든 동전 체계에서 큰 동전부터 고르면 최적이라고 일반화함.** 큰 동전을 먼저 고르는 규칙은 5·1 체계에서 11에 3개지만 임의 동전 체계에서 최적을 보장하지 않습니다. 결과가 예상과 다르면 입력 → 중간 상태 → 출력 중 처음 어긋난 곳을 찾습니다.

## 9. 다른 언어로 옮기기

같은 문제를 해결하더라도 선언, 타입, 메모리 관리, 오류 처리는 다릅니다. 핵심 동작은 **국소 선택의 최적성 조건 확인**입니다. 언어를 옮겨도 이 동작을 유지하세요.
...
- 왜 이 개념이 필요한가? 매 순간 가장 큰 선택을 하는 전략이 언제 맞는지 반례와 함께 판단해야 합니다.
- 예시에서 가장 먼저 확정되는 값이나 상태는 무엇인가?
- 어떤 실수를 점검해야 하나? 모든 동전 체계에서 큰 동전부터 고르면 최적이라고 일반화함
- 다른 두 언어에서는 같은 동작을 어떤 자료형과 오류 처리로 나타내는가?

## 12. 핵심 요약과 복습

큰 동전을 먼저 고르는 규칙은 5·1 체계에서 11에 3개지만 임의 동전 체계에서 최적을 보장하지 않습니다. 예시의 출력은 `[5, 5, 1]`입니다. 오류를 찾을 때는 **모든 동전 체계에서 큰 동전부터 고르면 최적이라고 일반화함** 여부를 확인하세요. 완료 버튼을 누르면 +1·+3·+7·+14·+30일 복습이 이 기기에 등록됩니다.
