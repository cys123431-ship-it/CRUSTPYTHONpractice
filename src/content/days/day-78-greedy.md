...
exercises:
  ...
  - id: ex-day-78-greedy-fill
    ...
    explanation: 빈칸에 들어갈 표현은 'remaining >= coin'입니다. 11에서 5를 두 번 빼 1이 남습니다 1을 한 번 뺍니다 세 동전 [5,5,1]을 출력합니다
    commonMistakes:
      - 모든 동전 체계에서 큰 동전부터 고르면 최적이라고 일반화함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-78-greedy-guided
    ...
    explanation:
      바꾼 뒤 출력은 '[6, 1, 1, 1, 1, 1]'입니다. 원본 출력 '[5, 5, 1]'에서 달라졌습니다. 다른 줄(coins = [5, 1] → coins = [6, 1])에서
...
quiz:
  - id: quiz-day-78-greedy-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 0
    explanation: 11  →  6  →  1  →  0 순서로 실행되어 출력은 '[5, 5, 1]'입니다.
...
원본과 달라졌습니다. 원본 출력은 `[5, 5, 1]`입니다. 다른 줄(`coins = [5, 1]` → `coins = [6, 1]`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`11` → …)과 바뀐 줄부터 대조해 보세요.
