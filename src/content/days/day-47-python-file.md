...
exercises:
  ...
  - id: ex-day-47-python-file-fill
    ...
    explanation: 빈칸에 들어갈 표현은 'int(line)'입니다. 첫 줄 10을 문자열로 읽습니다 각 줄을 int로 바꾸어 [10,20]을 만듭니다 sum이 30입니다
    commonMistakes:
      - 문자열을 숫자로 바꾸지 않고 합하려고 함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-47-python-file-guided
    ...
    explanation:
      바꾼 뒤 출력은 '60'입니다. 원본 출력 '30'에서 달라졌습니다. 다른 줄(stream = StringIO("10\n20\n") → stream = StringIO("10\n20\n30\n"))에서
...
quiz:
  - id: quiz-day-47-python-file-output
    ...
    choices:
      ...
      - "30"
    answerIndex: 2
    explanation: 문자열 줄  →  int 변환  →  [10,20]  →  합 30 순서로 실행되어 출력은 '30'입니다.
...
원본과 달라졌습니다. 원본 출력은 `30`입니다. 다른 줄(`stream = StringIO("10\n20\n")` → `stream = StringIO("10\n20\n30\n")`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`문자열 줄` → …)과 바뀐 줄부터 대조해 보세요.
