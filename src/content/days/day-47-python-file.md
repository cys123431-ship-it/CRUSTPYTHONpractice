...
exercises:
  ...
  - id: ex-day-47-python-file-guided
    ...
    answer: 'from io import StringIO
      ...
      print(sum(values))'
    hint: '''10\\n20\\n''을 ''10\\n20\\n30\\n''로 바꾼 뒤 문자열 줄 → int 변환 → [10,20] → 합 30 순으로 따라가세요.'
    explanation: 예시 답안에서는 '10\\n20\\n'을 '10\\n20\\n30\\n'로 바꿨습니다. 원본 출력은 '30'입니다. 바뀐 코드의 결과는 실행해 확인하세요.
    commonMistakes:
      - 문자열을 숫자로 바꾸지 않고 합하려고 함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-47-python-file-debug
    ...
    prompt: 아래 코드의 핵심 표현 'line'을 점검하고 올바른 작동을 복원하세요. 문자열을 숫자로 바꾸지 않고 합하려고 함 오류도 함께 경계하세요.
    starter: 'from io import StringIO
