...
Rust에서는 마지막 표현식이 함수 반환값이 될 수 있어 문장과 표현식의 차이를 알아야 합니다. 처음 읽을 때는 결과를 가리고 손으로 예상하세요. 예시를 직접 타이핑한 뒤 한 줄씩 바꾸며 상태를 확인하세요.

## 시작 전에 확인할 것

바로 앞선 [Day 21](/learn/day-21-functions-review)의 핵심을 한 문장으로 설명해 보세요. 모르면 위의 선행 Day 링크에서 다시 확인할 수 있습니다.

## 머릿속 그림

본문 마지막 n*2는 세미콜론이 없어서 i32 값을 돌려줍니다. 다음 흐름을 눈으로 확인하세요.
...
예제의 핵심 표현은 `n * 2`입니다. 전체 코드에서 이 표현을 찾아 표시하세요. 선언과 조건, 출력이 연결되는 과정을 순서대로 설명합니다.
...
마지막 상태에서 화면에 표시되는 결과는 `14`입니다. 직접 타이핑할 때는 위의 순서와 실제 출력을 비교하세요.
...
**확인할 실수: 마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨.** 본문 마지막 n*2는 세미콜론이 없어서 i32 값을 돌려줍니다. 결과가 예상과 다르면 입력 → 중간 상태 → 출력 중 처음 어긋난 곳을 찾습니다.

## 다른 언어로 옮기기

같은 문제를 해결하더라도 선언, 타입, 메모리 관리, 오류 처리는 다릅니다. 핵심 동작은 **표현식 결과를 반환함**입니다. 언어를 옮겨도 이 동작을 유지하세요.
...
- 왜 이 개념이 필요한가? Rust에서는 마지막 표현식이 함수 반환값이 될 수 있어 문장과 표현식의 차이를 알아야 합니다.
- 예시에서 가장 먼저 확정되는 값이나 상태는 무엇인가?
- 어떤 실수를 점검해야 하나? 마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨
- 다른 두 언어에서는 같은 동작을 어떤 자료형과 오류 처리로 나타내는가?

## 핵심 요약과 복습

본문 마지막 n*2는 세미콜론이 없어서 i32 값을 돌려줍니다. 예시의 출력은 `14`입니다. 오류를 찾을 때는 **마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨** 여부를 확인하세요. 완료 버튼을 누르면 +1·+3·+7·+14·+30일 복습이 이 기기에 등록됩니다.
