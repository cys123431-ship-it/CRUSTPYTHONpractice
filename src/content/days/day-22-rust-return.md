...
exercises:
  ...
  - id: ex-day-22-rust-return-fill
    ...
    explanation: 빈칸에는 'n * 2'이 들어갑니다. doubled(7)에 n=7을 묶습니다 마지막 표현식이 14입니다 main의 println!이 14를 출력합니다
    commonMistakes:
      - 마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-22-rust-return-guided
    ...
    explanation:
      예시 답안에서는 처음 등장하는 숫자를 2에서 3로 바꿨습니다. 원래 출력은 '14'입니다. 바꾼 줄에서 시작해 중간 값과 마지막 출력을 다시 추적하세요. 출력이 같더라도 입력·조건·중간
      상태가 달라졌는지 확인해야 합니다.
    commonMistakes:
      - 마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
...
quiz:
  - id: quiz-day-22-rust-return-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 1
    explanation: n=7  →  7*2=14  →  반환 14 순서로 실행되어 '14'을 출력합니다.
  - id: quiz-day-22-rust-return-model
    question: Rust 표현식과 함수 반환을 이해하는 데 맞는 설명은?
    choices:
...
Rust에서는 마지막 표현식이 함수 반환값이 될 수 있어 문장과 표현식의 차이를 알아야 합니다. Day 22 "`Rust 표현식과 함수 반환`"에서는 Rust 코드가 `14`을(를) 만드는 과정을 따라가며, 표현식 결과를 반환함 동작이 왜 필요한지 확인합니다. 표현식 결과를 반환함을(를) 빠뜨리면 `마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 21](/learn/day-21-functions-review)에서는 "반복과 함수 회상"을(를) 배웠습니다. "반복과 함수 회상"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`Rust 표현식과 함수 반환`"에서 새로 달라지는 조건을 찾아보세요. Day 22의 답은 `14`이며, 핵심 표현은 `n * 2`입니다.

## 머릿속 그림

본문 마지막 n*2는 세미콜론이 없어서 i32 값을 돌려줍니다. Day 22에서는 아래 흐름 순서대로 상태가 바뀌며, `n * 2`이(가) 결과를 가릅니다.

```text
n=7  →  7*2=14  →  반환 14
```

위 흐름에서 `n * 2`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `14`입니다.
...
아래 Rust 코드는 Day 22 "`Rust 표현식과 함수 반환`"의 독립 예제입니다. 전체 4줄에서 `n * 2`이(가) 핵심이며, 실행 결과는 `14`입니다.
...
Rust는 `rustc --edition=2024` 또는 Cargo로 실행하면 `14`이(가) 나옵니다. 브라우저에서는 임의 컴파일 대신 하단 실습 칸의 답안 비교를 사용하세요.
...
위 단계에서 `n * 2`이(가) 빠지면 `14`이(가) 나오지 않습니다. `처음 등장하는 숫자를 2에서 3로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `14`이(가) 됩니다. `n=7` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `14`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `n * 2`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 22의 `Rust 표현식과 함수 반환`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨.** 정상 코드에서는 `14`이(가) 출력됩니다.
...
핵심 동작 **표현식 결과를 반환함**은(는) 세 언어에서 같은 입력과 출력(`14`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `return n * 2; 필수` 규칙으로 "`Rust 표현식과 함수 반환`"의 `14`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `return n * 2 명시` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `마지막 표현식 n * 2` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`14` 맞히기) → 빈칸(`n * 2` 채우기) → 변경(`처음 등장하는 숫자를 2에서 3로`) → 오류 수정(`마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨` 찾기) → 독립 구현(`Rust 표현식과 함수 반환을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `14` 및 `n * 2` 설명과 대조하세요.

## 스스로 설명하기

- "`Rust 표현식과 함수 반환`"이(가) 필요한 상황을 `표현식 결과를 반환함` 동작으로 설명해 보세요.
- 예제에서 `n * 2`이(가) 실행되기 직전의 상태와 직후의 출력 `14`을(를) 말해 보세요.
- "`마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `표현식 결과를 반환함` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

본문 마지막 n*2는 세미콜론이 없어서 i32 값을 돌려줍니다. Day 22 "`Rust 표현식과 함수 반환`"의 예제는 `n * 2`을(를) 실행해 `14`을(를) 출력합니다. "`마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨`" 여부를 확인하고 Day 22을(를) 완료하세요.
