...
exercises:
  - id: ex-day-22-rust-return-predict
    ...
    answer: "14"
    hint: 본문 마지막 n*2는 세미콜론이 없어서 i32 값을 돌려줍니다. 설명을 떠올리고 `n=7` 단계부터 순서대로 적어 보세요.
    explanation:
      n=7  →  7*2=14  →  반환 14 순서로 실행됩니다. `n * 2` 부분이 `반환 14` 단계를 확정해 최종 출력 '14'가 됩니다. 이 흐름을 떠올리면 `표현식
      결과를 반환함` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-22-rust-return-fill
    ...
    hint: 힌트 문장을 완전하게 읽으면 `doubled(7)에 n=7을 묶습니다` 단계에 필요한 표현이 `n * 2`입니다.
    explanation:
      빈칸에 들어갈 표현은 'n * 2'입니다. `n * 2` 줄을 완성해야 `표현식 결과를 반환함` 동작이 이어져 실행 결과 '14'가 됩니다. 힌트의 첫 단계 `doubled(7)에
      n=7을 묶습니다`이 바로 이 줄입니다. 이어서 마지막 표현식이 14입니다 main의 println!이 14를 출력합니다 순서로 진행됩니다.
    commonMistakes:
      - 마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-22-rust-return-guided
    ...
    explanation:
      바꾼 뒤 출력은 '21'입니다. 원본 출력 '14'에서 달라졌습니다. 바뀐 줄은 `n * 2`에서 `n * 3`로 바뀌었습니다. 바뀐 프로그램은 `n=7` 단계에서 시작해 바뀐
      줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `21`가 됩니다. 원본 추적 `n=7  →  7*2=14  →  반환 14`와 바뀐 줄 이후를 순서대로 비교하면 처음 달라지는
      곳이 보입니다.
    commonMistakes:
      - 마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-22-rust-return-debug
    ...
    starter: "fn doubled(n: i32) -> i32 {\n    n * 3\n}\nfn main() { println!(\"{}\", doubled(7)); }"
    answer: "fn doubled(n: i32) -> i32 {\n    n * 2\n}\nfn main() { println!(\"{}\", doubled(7)); }"
    hint: 본문 마지막 n*2는 세미콜론이 없어서 i32 값을 돌려줍니다. 설명과 어긋나는 줄을 찾으세요. `마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨` 상황이 단서가 됩니다.
    explanation:
      틀린 줄은 `n * 3`입니다. 여기서는 `n * 3`을 써서 `n * 2` 동작이 깨집니다. 이대로 실행하면 `마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨` 문제가
      생겨 원본 추적 `n=7  →  7*2=14  →  반환 14`대로 '14'가 나오지 않습니다. 고친 줄 `n * 2`에서는 `n * 2`가 `표현식 결과를 반환함` 동작을 지켜 '14'까지 도달합니다.
    commonMistakes:
      - 마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-22-rust-return-independent
    ...
    answer: "fn doubled(n: i32) -> i32 {\n    n * 2\n}\nfn main() { println!(\"{}\", doubled(7)); }"
    hint: 본문 마지막 n*2는 세미콜론이 없어서 i32 값을 돌려줍니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `n * 2` 부분이 `표현식 결과를 반환함` 동작을 지켜 실행 결과 '14'가 됩니다. 같은 개념을 다른 입력으로 바꿔도 `n * 2`부터
      `반환 14`까지 추적할 수 있으면 정답입니다. `마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - 마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
...
quiz:
  - id: quiz-day-22-rust-return-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 1
    explanation: n=7  →  7*2=14  →  반환 14 순서로 실행되어 출력은 '14'입니다. `n * 2` 부분이 마지막 단계를 확정하므로 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-22-rust-return-model
    ...
    answerIndex: 2
    explanation:
      본문 마지막 n*2는 세미콜론이 없어서 i32 값을 돌려줍니다. 이 설명이 맞는 이유는 `표현식 결과를 반환함` 동작을 지키는 조건과 같기 때문입니다. `마지막 표현식에 세미콜론을
      붙여 반환형이 ()가 됨` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-22-rust-return-pitfall
    ...
    answerIndex: 0
    explanation: 마지막 표현식에 세미콜론을 붙여 반환형이 ()가 됨. 이 실수가 나오면 원본 추적 `n=7 → 7*2=14 → 반환 14`대로 '14'가 나오지 않으므로 먼저 확인해야 합니다.
  - id: quiz-day-22-rust-return-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 1
    explanation: 문법은 달라도 `표현식 결과를 반환함` 목적과 입력·출력은 유지합니다. 실행 결과 '14'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
원본 출력은 `14`이고, 바뀐 코드의 실행 결과는 `21`입니다. 바뀐 줄은 `n * 2`에서 `n * 3`로 바뀌었습니다. 바뀐 프로그램은 `n=7` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `21`가 됩니다. 원본 추적 `n=7  →  7*2=14  →  반환 14`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
