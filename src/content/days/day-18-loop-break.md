...
exercises:
  - id: ex-day-18-loop-break-predict
    ...
    starter:
      ...
      \ println!(\"{answer}\");\n}"
    answer: "6"
    hint: n=0에서 시작해 answer=6까지 순서대로 적어 보세요. Rust의 loop는 명시적 break가 실행될 때까지 돌며 break 뒤 값이 loop 표현식의 값입니다.
    explanation:
      1. n은 0에서 출발합니다. 2. 1·2는 계속 반복합니다. 3. 3에서 break 6으로 종료하여 answer=6입니다. `n=0 → 1 → 2 → 3 → answer=6`
      흐름으로 실제 출력 `6`이 됩니다. 핵심 `break n * 2`는 탈출과 동시에 결과값을 정하는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-18-loop-break-fill
    ...
    answer:
      "fn main() {\n    let mut n = 0;\n    let answer = loop { n += 1; if n == 3 { break n * 2; } };\n    println!(\"\
      {answer}\");\n}"
    hint: 필요한 표현은 반복 종료 값을 사용함 동작을 잇는 break n * 2입니다.
    explanation:
      빈칸에 들어갈 표현은 'break n * 2'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. n은 0에서 출발합니다. 2. 1·2는 계속 반복합니다. 3. 3에서 break
      6으로 종료하여 answer=6입니다. `n=0 → 1 → 2 → 3 → answer=6` 흐름으로 실제 출력 `6`이 됩니다. 핵심 `break n * 2`는 탈출과 동시에 결과값을 정하는 자리에
      쓰입니다.
    commonMistakes:
      - break 뒤의 값을 무시하고 loop는 언제나 빈 값을 준다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-18-loop-break-guided
    ...
    answer:
      "fn main() {\n    let mut n = 1;\n    let answer = loop { n += 1; if n == 3 { break n * 2; } };\n    println!(\"\
      {answer}\");\n}"
    hint: 처음 등장하는 숫자를 0에서 1로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '6'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '6'입니다. 원본과 수정본 모두 `6`을 출력한다. 멈추는 조건은 n이 3이 되는 순간이라 시작값과 무관하게 break 값 3 곱하기 2로 6이 됩니다. 시작값만
      달라지고 종료식이 그대로이므로 최종 출력은 그대로라고 명시한다.
    commonMistakes:
      - break 뒤의 값을 무시하고 loop는 언제나 빈 값을 준다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-18-loop-break-debug
    ...
    answer:
      ...
      {answer}\");\n}"
    hint:
      break 뒤의 값을 무시하고 loop는 언제나 빈 값을 준다고 생각함 상황에서 어긋나는 줄을 Rust의 loop는 명시적 break가 실행될 때까지 돌며 break 뒤 값이 loop 표현식의
      값입니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `let answer = loop { n += 1; if n == 3 { break n; } };`입니다. break 값이 `n * 2`가 아니라 `n`이므로 탈출
      순간 answer에는 3이 들어가 `3`이 출력됩니다. 오류 분류는 잘못된 출력입니다. 결과값이 달라지는 첫 순간은 break 뒤 식입니다. 고친 줄의 `break n * 2`에서는 `반복 종료
      값을 사용함` 동작이 지켜집니다. 정상 코드는 n 증가→3에서 탈출→6 전달의 순서로 `6`을 출력합니다.
    commonMistakes:
      - break 뒤의 값을 무시하고 loop는 언제나 빈 값을 준다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-18-loop-break-independent
    ...
    answer:
      "fn main() {\n    let mut n = 0;\n    let answer = loop { n += 1; if n == 3 { break n * 2; } };\n    println!(\"\
      {answer}\");\n}"
    hint: "`break;`처럼 값 없이 빠져나오면 결과값이 없고, `break n * 2`처럼 적으면 결과가 있습니다. 두 경우를 구별하세요. 예상 출력과 경계 조건도 함께 적어 보세요."
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. `break;`처럼 값 없이 빠져나오면 결과값이 없고, `break n * 2`처럼 적으면 결과가 있습니다. 두 경우를 구별하세요.
      같은 반복 종료 값을 사용함 동작을 구현하고 실행 결과 '6'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - break 뒤의 값을 무시하고 loop는 언제나 빈 값을 준다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
...
quiz:
  - id: quiz-day-18-loop-break-output
    ...
    answerIndex: 0
    explanation:
      먼저 n이 0에서 출발합니다. 이어서 1·2 회차는 조건이 맞지 않아 계속 돌고, 3에서 break를 만나 `n * 2` 값 6이 loop 전체의 결과가 되어 answer에
      들어가므로 출력 `6`이 됩니다. `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 컴파일과 실행이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 println!이
      실제로 호출되기 때문입니다. 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-18-loop-break-model
    ...
    answerIndex: 1
    explanation:
      Rust의 loop는 명시적 break가 실행될 때까지 돈다는 뜻이고, break 뒤 값이 loop 표현식의 값이라는 뜻은 탈출 값이 변수에 저장된다는 뜻입니다. `break
      뒤의 값을 무시하고 loop는 언제나 빈 값을 준다고 생각함`은 반대 사례인데, 값을 적으면 그 값이 결과가 되기 때문입니다.
  - id: quiz-day-18-loop-break-pitfall
    ...
    answerIndex: 2
    explanation:
      먼저 확인할 실수는 `break 뒤의 값을 무시하고 loop는 언제나 빈 값을 준다고 생각함`입니다. 잘못된 코드는 실행은 되지만 `3`이라는 잘못된 출력을 냅니다. break
      뒤 식을 먼저 확인하세요.
  - id: quiz-day-18-loop-break-transfer
    ...
    answerIndex: 0
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `반복 종료 값을 사용함`이라는 의미와 입력·출력 계약입니다. C의 break 탈출값 패턴과 Python의 루프 결과 변수로 같은 동작을
      구현하고, 실행 결과 `6`으로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
예제의 핵심 표현은 `break n * 2`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본 실행 결과는 `6`입니다. 아래 코드에서 원본과 달라지는 첫 줄을 표시하고, 그 줄의 변경 전후 값을 적어 보세요.
...
원본과 수정본 모두 `6`을 출력한다. 멈추는 조건은 n이 3이 되는 순간이라 시작값과 무관하게 break 값 3 곱하기 2로 6이 됩니다. 시작값만 달라지고 종료식이 그대로이므로 최종 출력은 그대로라고 명시한다.
