...
반복을 마칠 때 계산 결과를 바로 바깥 표현식에 전달할 수 있습니다. Day 18 "`Rust loop와 break 값`"에서는 Rust 코드가 `6`을(를) 만드는 과정을 따라가며, 반복 종료 값을 사용함 동작이 왜 필요한지 확인합니다. 반복 종료 값을 사용함을(를) 빠뜨리면 `break 뒤의 값을 무시하고 loop는 언제나 빈 값을 준다고 생각함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 17](/learn/day-17-while-loop)에서는 "while과 종료 조건"을(를) 배웠습니다. "while과 종료 조건"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`Rust loop와 break 값`"에서 새로 달라지는 조건을 찾아보세요. Day 18의 답은 `6`이며, 핵심 표현은 `break n * 2`입니다.

## 머릿속 그림

Rust의 loop는 명시적 break가 실행될 때까지 돌며 break 뒤 값이 loop 표현식의 값입니다. Day 18에서는 아래 흐름 순서대로 상태가 바뀌며, `break n * 2`이(가) 결과를 가릅니다.

```text
n=0  →  1  →  2  →  3  →  answer=6
```

위 흐름에서 `break n * 2`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `6`입니다.
...
아래 Rust 코드는 Day 18 "`Rust loop와 break 값`"의 독립 예제입니다. 전체 5줄에서 `break n * 2`이(가) 핵심이며, 실행 결과는 `6`입니다.
...
Rust는 `rustc --edition=2024` 또는 Cargo로 실행하면 `6`이(가) 나옵니다. 브라우저에서는 임의 컴파일 대신 하단 실습 칸의 답안 비교를 사용하세요.
...
위 단계에서 `break n * 2`이(가) 빠지면 `6`이(가) 나오지 않습니다. `처음 등장하는 숫자를 0에서 1로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `6`이(가) 됩니다. `n=0` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `6`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `break n * 2`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 18의 `Rust loop와 break 값`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: break 뒤의 값을 무시하고 loop는 언제나 빈 값을 준다고 생각함.** 정상 코드에서는 `6`이(가) 출력됩니다.
...
핵심 동작 **반복 종료 값을 사용함**은(는) 세 언어에서 같은 입력과 출력(`6`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `while(1)과 별도의 결과 변수` 규칙으로 "`Rust loop와 break 값`"의 `6`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `while True와 결과 변수, break` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `let answer = loop { break 값; }` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`Rust loop와 break 값`"이(가) 필요한 상황을 `반복 종료 값을 사용함` 동작으로 설명해 보세요.
- 예제에서 `break n * 2`이(가) 실행되기 직전의 상태와 직후의 출력 `6`을(를) 말해 보세요.
- "`break 뒤의 값을 무시하고 loop는 언제나 빈 값을 준다고 생각함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `반복 종료 값을 사용함` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

Rust의 loop는 명시적 break가 실행될 때까지 돌며 break 뒤 값이 loop 표현식의 값입니다. Day 18 "`Rust loop와 break 값`"의 예제는 `break n * 2`을(를) 실행해 `6`을(를) 출력합니다. "`break 뒤의 값을 무시하고 loop는 언제나 빈 값을 준다고 생각함`" 여부를 확인하고 Day 18을(를) 완료하세요.
