...
두 줄을 나란히 놓으면 정상 줄 `print(actual == expected)`이 `사전의 ==는 항목의 순서가 달라도 키와 값이 모두 같으면 참입니다.` 설명과 맞고, 잘못된 줄은 `객체 동일성 is로 독립된 결과의 값 일치를 검사함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `두 결과의 키·값 일치`부터 `True`까지 처음 어긋난 곳을 찾습니다.
