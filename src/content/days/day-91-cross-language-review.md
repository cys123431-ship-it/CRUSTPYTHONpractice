...
동일 입력에서 출력이 같아야 하므로 공백보다 먼저 구조화된 결과의 값과 정렬 규칙을 비교합니다. Day 91 "`세 구현의 결과 일치 검사`"에서는 Python 코드가 `True`을(를) 만드는 과정을 따라가며, 공통 fixture 기대값 비교 동작이 왜 필요한지 확인합니다. 공통 fixture 기대값 비교을(를) 빠뜨리면 `객체 동일성 is로 독립된 결과의 값 일치를 검사함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 90](/learn/day-90-rust-aggregate)에서는 "Rust iterator 집계와 정렬"을(를) 배웠습니다. "Rust iterator 집계와 정렬"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`세 구현의 결과 일치 검사`"에서 새로 달라지는 조건을 찾아보세요. Day 91의 답은 `True`이며, 핵심 표현은 `actual == expected`입니다.

## 머릿속 그림

사전의 ==는 항목의 순서가 달라도 키와 값이 모두 같으면 참입니다. Day 91에서는 아래 흐름 순서대로 상태가 바뀌며, `actual == expected`이(가) 결과를 가릅니다.

```text
두 결과의 키·값 일치  →  True
```

위 흐름에서 `actual == expected`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `True`입니다.
...
아래 Python 코드는 Day 91 "`세 구현의 결과 일치 검사`"의 독립 예제입니다. 전체 3줄에서 `actual == expected`이(가) 핵심이며, 실행 결과는 `True`입니다.
...
위 단계에서 `actual == expected`이(가) 빠지면 `True`이(가) 나오지 않습니다. `처음 등장하는 숫자를 60에서 61로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `True`이(가) 됩니다. `두 결과의 키·값 일치` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 코드에서 `처음 등장하는 숫자를 60에서 61로` 바꾸면 아래와 같이 됩니다.
...
원본 출력은 `True`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `actual == expected`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 91의 `세 구현의 결과 일치 검사`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 객체 동일성 is로 독립된 결과의 값 일치를 검사함.** 정상 코드에서는 `True`이(가) 출력됩니다.
...
핵심 동작 **공통 fixture 기대값 비교**은(는) 세 언어에서 같은 입력과 출력(`True`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `출력 파싱 또는 기준 출력 파일 비교` 규칙으로 "`세 구현의 결과 일치 검사`"의 `True`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `dict 값 비교와 테스트` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `PartialEq를 가진 struct 비교` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`True` 맞히기) → 빈칸(`actual == expected` 채우기) → 변경(`처음 등장하는 숫자를 60에서 61로`) → 오류 수정(`객체 동일성 is로 독립된 결과의 값 일치를 검사함` 찾기) → 독립 구현(`세 구현의 결과 일치 검사을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `True` 및 `actual == expected` 설명과 대조하세요.

## 스스로 설명하기

- "`세 구현의 결과 일치 검사`"이(가) 필요한 상황을 `공통 fixture 기대값 비교` 동작으로 설명해 보세요.
- 예제에서 `actual == expected`이(가) 실행되기 직전의 상태와 직후의 출력 `True`을(를) 말해 보세요.
- "`객체 동일성 is로 독립된 결과의 값 일치를 검사함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `공통 fixture 기대값 비교` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

사전의 ==는 항목의 순서가 달라도 키와 값이 모두 같으면 참입니다. Day 91 "`세 구현의 결과 일치 검사`"의 예제는 `actual == expected`을(를) 실행해 `True`을(를) 출력합니다. "`객체 동일성 is로 독립된 결과의 값 일치를 검사함`" 여부를 확인하고 Day 91을(를) 완료하세요.
