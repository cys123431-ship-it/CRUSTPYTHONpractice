...
exercises:
  - id: ex-day-71-complexity-predict
    ...
    answer: "4"
    hint: 원소 n개를 한 번씩 방문하면 작업 수가 n에 비례하므로 O(n)입니다. 실제 시간과 상수는 따로 측정합니다. 설명을 떠올리고 `0` 단계부터 순서대로 적어 보세요.
    explanation:
      0  →  1  →  2  →  3  →  4 순서로 실행됩니다. `steps += 1` 부분이 `4` 단계를 확정해 최종 출력 '4'가 됩니다. 이 흐름을 떠올리면 `입력
      크기와 작업 수의 증가율 파악` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-71-complexity-fill
    ...
    hint: 힌트 문장을 완전하게 읽으면 `배열 길이는 4입니다` 단계에 필요한 표현이 `steps += 1`입니다.
    explanation:
      빈칸에 들어갈 표현은 'steps += 1'입니다. `steps += 1` 줄을 완성해야 `입력 크기와 작업 수의 증가율 파악` 동작이 이어져 실행 결과 '4'가 됩니다. 힌트의
      첫 단계 `배열 길이는 4입니다`이 바로 이 줄입니다. 이어서 각 원소를 방문할 때 steps를 1 올립니다 4번 방문 후 4를 출력합니다 순서로 진행됩니다.
    commonMistakes:
      - O(n)을 실제 실행 시간이 정확히 n초라는 뜻으로 착각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-71-complexity-guided
    ...
    explanation:
      바꾼 뒤 출력은 '4'입니다. 원본 출력 '4'와 같습니다. 바뀐 줄은 `values = [3, 4, 6, 8]`입니다. 바뀐 프로그램도 `0` 단계에서 시작해 바뀐 줄을 지나
      최종 `4`로 끝납니다. 세는 것은 원소 개수라 들어 있는 값과 무관해 결과가 같습니다. 출력은 같지만 바뀐 줄 부분의 중간 값은 달라졌다가 같은 최종 값으로 이어진다는 점을 위 추적 순서와 대조해
      확인할 수 있습니다.
    commonMistakes:
      - O(n)을 실제 실행 시간이 정확히 n초라는 뜻으로 착각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-71-complexity-debug
    ...
    answer: "values = [2, 4, 6, 8]\nsteps = 0\nfor value in values:\n    steps += 1\nprint(steps)"
    hint:
      원소 n개를 한 번씩 방문하면 작업 수가 n에 비례하므로 O(n)입니다. 실제 시간과 상수는 따로 측정합니다. 설명과 어긋나는 줄을 찾으세요. `O(n)을 실제 실행 시간이 정확히 n초라는
      뜻으로 착각함` 상황이 단서가 됩니다.
    explanation:
      틀린 줄은 `steps += 2`입니다. 여기서는 `steps += 2`을 써서 `steps += 1` 동작이 깨집니다. 이대로 실행하면 `O(n)을 실제 실행 시간이 정확히
      n초라는 뜻으로 착각함` 문제가 생겨 원본 추적 `0  →  1  →  2  →  3  →  4`대로 '4'가 나오지 않습니다. 고친 줄 `steps += 1`에서는 `steps += 1`가 `입력
      크기와 작업 수의 증가율 파악` 동작을 지켜 '4'까지 도달합니다.
    commonMistakes:
      - O(n)을 실제 실행 시간이 정확히 n초라는 뜻으로 착각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-71-complexity-independent
    ...
    answer: "values = [2, 4, 6, 8]\nsteps = 0\nfor value in values:\n    steps += 1\nprint(steps)"
    hint:
      원소 n개를 한 번씩 방문하면 작업 수가 n에 비례하므로 O(n)입니다. 실제 시간과 상수는 따로 측정합니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를
      함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `steps += 1` 부분이 `입력 크기와 작업 수의 증가율 파악` 동작을 지켜 실행 결과 '4'가 됩니다. 같은 개념을 다른 입력으로
      바꿔도 `steps += 1`부터 `4`까지 추적할 수 있으면 정답입니다. `O(n)을 실제 실행 시간이 정확히 n초라는 뜻으로 착각함` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - O(n)을 실제 실행 시간이 정확히 n초라는 뜻으로 착각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-71-complexity-output
    ...
    choices:
      ...
      - "4"
    answerIndex: 2
    explanation: 0  →  1  →  2  →  3  →  4 순서로 실행되어 출력은 '4'입니다. `steps += 1` 부분이 마지막 단계를 확정하므로 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-71-complexity-model
    ...
    answerIndex: 0
    explanation:
      원소 n개를 한 번씩 방문하면 작업 수가 n에 비례하므로 O(n)입니다. 실제 시간과 상수는 따로 측정합니다. 이 설명이 맞는 이유는 `입력 크기와 작업 수의 증가율 파악`
      동작을 지키는 조건과 같기 때문입니다. `O(n)을 실제 실행 시간이 정확히 n초라는 뜻으로 착각함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-71-complexity-pitfall
    ...
    answerIndex: 1
    explanation: O(n)을 실제 실행 시간이 정확히 n초라는 뜻으로 착각함. 이 실수가 나오면 원본 추적 `0 → 1 → 2 → 3 → 4`대로 '4'가 나오지 않으므로 먼저 확인해야 합니다.
  - id: quiz-day-71-complexity-transfer
    ...
    answerIndex: 2
    explanation: 문법은 달라도 `입력 크기와 작업 수의 증가율 파악` 목적과 입력·출력은 유지합니다. 실행 결과 '4'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
...
바꾼 뒤 출력도 `4`입니다. 세는 것은 원소 개수라 들어 있는 값과 무관해 결과가 같습니다. 바뀐 프로그램도 `0` 단계에서 시작해 `values = [3, 4, 6, 8]` 줄을 지나 최종 `4`로 끝납니다. 원본 추적 `0  →  1  →  2  →  3  →  4`와 바뀐 줄 이후를 순서대로 놓으면 출력은 같아도 중간 값이 어디서 달라졌다가 합쳐지는지 확인할 수 있습니다.
