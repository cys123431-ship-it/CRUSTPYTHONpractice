...
입력 크기가 커질 때 작업 수가 어떻게 늘어나는지 봐야 자료구조와 알고리즘을 비교할 수 있습니다. Day 71 "`시간 복잡도와 입력 크기`"에서는 Python 코드가 `4`을(를) 만드는 과정을 따라가며, 입력 크기와 작업 수의 증가율 파악 동작이 왜 필요한지 확인합니다. 입력 크기와 작업 수의 증가율 파악을(를) 빠뜨리면 `O(n)을 실제 실행 시간이 정확히 n초라는 뜻으로 착각함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 70](/learn/day-70-structures-review)에서는 "자료구조 선택 회상"을(를) 배웠습니다. "자료구조 선택 회상"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`시간 복잡도와 입력 크기`"에서 새로 달라지는 조건을 찾아보세요. Day 71의 답은 `4`이며, 핵심 표현은 `steps += 1`입니다.

## 머릿속 그림

원소 n개를 한 번씩 방문하면 작업 수가 n에 비례하므로 O(n)입니다. 실제 시간과 상수는 따로 측정합니다. Day 71에서는 아래 흐름 순서대로 상태가 바뀌며, `steps += 1`이(가) 결과를 가릅니다.

```text
0  →  1  →  2  →  3  →  4
```

위 흐름에서 `steps += 1`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `4`입니다.
...
아래 Python 코드는 Day 71 "`시간 복잡도와 입력 크기`"의 독립 예제입니다. 전체 5줄에서 `steps += 1`이(가) 핵심이며, 실행 결과는 `4`입니다.
...
하단 Python 실행 영역에 같은 코드가 들어 있습니다. 먼저 `4`을(를) 가리고 예측한 뒤 실행해 비교하세요.
...
위 단계에서 `steps += 1`이(가) 빠지면 `4`이(가) 나오지 않습니다. `처음 등장하는 숫자를 2에서 3로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `4`이(가) 됩니다. `0` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `4`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `steps += 1`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 71의 `시간 복잡도와 입력 크기`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: O(n)을 실제 실행 시간이 정확히 n초라는 뜻으로 착각함.** 정상 코드에서는 `4`이(가) 출력됩니다.
...
핵심 동작 **입력 크기와 작업 수의 증가율 파악**은(는) 세 언어에서 같은 입력과 출력(`4`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `루프 반복 횟수 n을 세기` 규칙으로 "`시간 복잡도와 입력 크기`"의 `4`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `for 순회 횟수 n` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `iterator가 n개 원소를 방문` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`시간 복잡도와 입력 크기`"이(가) 필요한 상황을 `입력 크기와 작업 수의 증가율 파악` 동작으로 설명해 보세요.
- 예제에서 `steps += 1`이(가) 실행되기 직전의 상태와 직후의 출력 `4`을(를) 말해 보세요.
- "`O(n)을 실제 실행 시간이 정확히 n초라는 뜻으로 착각함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `입력 크기와 작업 수의 증가율 파악` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

원소 n개를 한 번씩 방문하면 작업 수가 n에 비례하므로 O(n)입니다. 실제 시간과 상수는 따로 측정합니다. Day 71 "`시간 복잡도와 입력 크기`"의 예제는 `steps += 1`을(를) 실행해 `4`을(를) 출력합니다. "`O(n)을 실제 실행 시간이 정확히 n초라는 뜻으로 착각함`" 여부를 확인하고 Day 71을(를) 완료하세요.
