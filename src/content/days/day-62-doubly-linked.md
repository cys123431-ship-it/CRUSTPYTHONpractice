...
exercises:
  - id: ex-day-62-doubly-linked-predict
    ...
    answer: B
    hint: B.prev=A에서 시작해 일치까지 순서대로 적어 보세요. B.prev=A라면 A.next=B여야 합니다. 두 방향이 서로를 가리키는 것이 불변식입니다.
    explanation:
      1. B의 prev를 읽으면 A입니다. 2. A의 next를 읽으면 B입니다. 3. 왕복 관계가 맞는지 출력합니다. `B.prev=A → A.next=B → 일치` 흐름으로
      실제 출력 `B`가 됩니다. 핵심 `["prev"]`는 뒤쪽 링크를 먼저 읽는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-62-doubly-linked-fill
    ...
    answer:
      ...
      print(nodes[nodes["B"]["prev"]]["next"])'
    hint: 필요한 표현은 앞뒤 링크 일치 유지 동작을 잇는 ["prev"]입니다.
    explanation:
      빈칸에 들어갈 표현은 '["prev"]'입니다. 이 표현이 없으면 실행 중 오류로 멈춥니다. 1. B의 prev를 읽으면 A입니다. 2. A의 next를 읽으면 B입니다. 3.
      왕복 관계가 맞는지 출력합니다. `B.prev=A → A.next=B → 일치` 흐름으로 실제 출력 `B`가 됩니다. 핵심 `["prev"]`는 뒤쪽 링크를 먼저 읽는 자리에 쓰입니다.
    commonMistakes:
      - 삽입·삭제 때 next만 바꾸고 prev를 갱신하지 않음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-62-doubly-linked-guided
    ...
    prompt: 예시를 직접 타이핑한 뒤 '["B"]["prev"]' → '["A"]["next"]' 바꿔 보세요. 출력이 바뀌는지 먼저 예측하고, 그대로라면 그 이유도 설명하세요.
    starter:
      'nodes = {"A": {"next": "B", "prev": None}, "B": {"next": None, "prev": "A"}}
    ...
    hint: '''["B"]["prev"]'' → ''["A"]["next"]'' 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 ''B''와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.'
    explanation:
      바꾼 뒤 출력은 'None'입니다. 원본은 `B`, 수정본은 `None`이다. 첫 변경 줄의 `["B"]["prev"]`에서 `["A"]["next"]`로 읽기 시작점을 바꾸면,
      A의 next인 B의 next(None)를 읽게 되어 `None`이 출력된다.
    commonMistakes:
      - 삽입·삭제 때 next만 바꾸고 prev를 갱신하지 않음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-62-doubly-linked-debug
    ...
    answer:
      ...
      print(nodes[nodes["B"]["prev"]]["next"])'
    hint:
      삽입·삭제 때 next만 바꾸고 prev를 갱신하지 않음 상황에서 어긋나는 줄을 B.prev=A라면 A.next=B여야 합니다. 두 방향이 서로를 가리키는 것이 불변식입니다. 설명과 대조해
      보세요.
    explanation:
      틀린 줄은 `print(nodes[nodes["B"]["next"]]["next"])`입니다. `nodes["B"]["next"]`가 `None`이므로 `nodes[None]`을
      읽으려다 KeyError라는 런타임 오류가 나고 프로그램이 멈춥니다. 오류 분류는 런타임 오류입니다. 흐름이 깨지는 첫 순간은 존재하지 않는 키 읽기입니다. 고친 줄 `print(nodes[nodes["B"]["prev"]]["next"])`에서는
      `앞뒤 링크 일치 유지` 동작이 지켜집니다. 정상 코드는 뒤 링크 읽기→앞 링크 읽기→왕복 확인의 순서로 `B`를 출력합니다.
    commonMistakes:
      - 삽입·삭제 때 next만 바꾸고 prev를 갱신하지 않음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-62-doubly-linked-independent
    ...
    answer:
      ...
      print(nodes[nodes["B"]["prev"]]["next"])'
    hint: 머리·꼬리에서는 `None` 처리가 필요합니다. 추가·삭제 때마다 왕복 검사를 하세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 머리·꼬리에서는 `None` 처리가 필요합니다. 추가·삭제 때마다 왕복 검사를 하세요. 같은 앞뒤 링크 일치 유지 동작을 구현하고
      실행 결과 'B'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 삽입·삭제 때 next만 바꾸고 prev를 갱신하지 않음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-62-doubly-linked-output
    ...
    answerIndex: 2
    explanation:
      먼저 B의 prev를 읽으면 A입니다. 이어서 A의 next를 읽으면 B가 되어 왕복이 맞으므로 출력 `B`가 됩니다. `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 딕셔너리
      읽기가 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기 때문입니다. 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-62-doubly-linked-model
    question: "'양방향 연결과 불변식' 개념을 이해하는 데 맞는 설명은?"
    choices:
    ...
    answerIndex: 0
    explanation:
      B.prev=A라면 A.next=B여야 한다는 뜻이고, 두 방향이 서로를 가리키는 것이 불변식이라는 뜻은 왕복하면 제자리라는 뜻입니다. `삽입·삭제 때 next만 바꾸고 prev를
      갱신하지 않음`은 반대 사례인데, 한쪽만 고치면 돌아올 때 어긋나기 때문입니다.
  - id: quiz-day-62-doubly-linked-pitfall
    ...
    answerIndex: 1
    explanation:
      먼저 확인할 실수는 `삽입·삭제 때 next만 바꾸고 prev를 갱신하지 않음`입니다. 잘못된 코드는 존재하지 않는 키를 읽어 KeyError로 멈춥니다. 양쪽 링크를 함께
      갱신했는지 확인하세요.
  - id: quiz-day-62-doubly-linked-transfer
    ...
    answerIndex: 2
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `앞뒤 링크 일치 유지`라는 의미와 입력·출력 계약입니다. C의 양방향 포인터와 Rust의 링크 구조로 같은 동작을 구현하고, 실행 결과
      `B`로 대조하면 옮김이 맞는지 확인할 수 있습니다.
playgroundSource:
  'nodes = {"A": {"next": "B", "prev": None}, "B": {"next": None, "prev": "A"}}
...
예제의 핵심 표현은 `["prev"]`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본 실행 결과는 `B`입니다. 아래 코드에서 원본과 달라지는 첫 줄을 표시하고, 그 줄의 변경 전후 값을 적어 보세요.
...
원본은 `B`, 수정본은 `None`이다. 첫 변경 줄의 `["B"]["prev"]`에서 `["A"]["next"]`로 읽기 시작점을 바꾸면, A의 next인 B의 next(None)를 읽게 되어 `None`이 출력된다.
...
두 줄을 나란히 놓으면 정상 줄 `print(nodes[nodes["B"]["prev"]]["next"])`이 `B.prev=A라면 A.next=B여야 합니다. 두 방향이 서로를 가리키는 것이 불변식입니다.` 설명과 맞고, 잘못된 줄은 `삽입·삭제 때 next만 바꾸고 prev를 갱신하지 않음` 쪽으로 어긋납니다. 결과가 예상과 다르면 `B.prev=A`부터 `B`까지 처음 어긋난 곳을 찾습니다.
...
예측(`B` 맞히기) → 빈칸(`["prev"]` 채우기) → 변경(`'["B"]["prev"]' → '["A"]["next"]'`) → 오류 수정(`삽입·삭제 때 next만 바꾸고 prev를 갱신하지 않음` 찾기) → 독립 구현(`'양방향 연결과 불변식' 개념을 보여주는 작은 프로그램` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `B` 및 `["prev"]` 설명과 대조하세요.
