...
이전과 다음 방향 모두 이동하려면 두 링크를 같은 관계로 갱신해야 합니다. Day 62 "`양방향 연결과 불변식`"에서는 Python 코드가 `B`을(를) 만드는 과정을 따라가며, 앞뒤 링크 일치 유지 동작이 왜 필요한지 확인합니다. 앞뒤 링크 일치 유지을(를) 빠뜨리면 `삽입·삭제 때 next만 바꾸고 prev를 갱신하지 않음` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 61](/learn/day-61-linked-list)에서는 "단일 연결 리스트의 다음 노드"을(를) 배웠습니다. "단일 연결 리스트의 다음 노드"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`양방향 연결과 불변식`"에서 새로 달라지는 조건을 찾아보세요. Day 62의 답은 `B`이며, 핵심 표현은 `["prev"]`입니다.

## 머릿속 그림

B.prev=A라면 A.next=B여야 합니다. 두 방향이 서로를 가리키는 것이 불변식입니다. Day 62에서는 아래 흐름 순서대로 상태가 바뀌며, `["prev"]`이(가) 결과를 가릅니다.

```text
B.prev=A  →  A.next=B  →  일치
```

위 흐름에서 `["prev"]`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `B`입니다.
...
아래 Python 코드는 Day 62 "`양방향 연결과 불변식`"의 독립 예제입니다. 전체 2줄에서 `["prev"]`이(가) 핵심이며, 실행 결과는 `B`입니다.
...
하단 Python 실행 영역에 같은 코드가 들어 있습니다. 먼저 `B`을(를) 가리고 예측한 뒤 실행해 비교하세요.
...
위 단계에서 `["prev"]`이(가) 빠지면 `B`이(가) 나오지 않습니다. `'["B"]["prev"]'을 '["A"]["next"]'로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `B`이(가) 됩니다. `B.prev=A` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 코드에서 `'["B"]["prev"]'을 '["A"]["next"]'로` 바꾸면 아래와 같이 됩니다.
...
원본 출력은 `B`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `["prev"]`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 62의 `양방향 연결과 불변식`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 삽입·삭제 때 next만 바꾸고 prev를 갱신하지 않음.** 정상 코드에서는 `B`이(가) 출력됩니다.
...
핵심 동작 **앞뒤 링크 일치 유지**은(는) 세 언어에서 같은 입력과 출력(`B`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `Node *prev,*next를 함께 수정` 규칙으로 "`양방향 연결과 불변식`"의 `B`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `참조 속성 양쪽 갱신` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `소유권 때문에 인덱스 기반 Vec나 Rc<RefCell> 고려` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`양방향 연결과 불변식`"이(가) 필요한 상황을 `앞뒤 링크 일치 유지` 동작으로 설명해 보세요.
- 예제에서 `["prev"]`이(가) 실행되기 직전의 상태와 직후의 출력 `B`을(를) 말해 보세요.
- "`삽입·삭제 때 next만 바꾸고 prev를 갱신하지 않음`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `앞뒤 링크 일치 유지` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

B.prev=A라면 A.next=B여야 합니다. 두 방향이 서로를 가리키는 것이 불변식입니다. Day 62 "`양방향 연결과 불변식`"의 예제는 `["prev"]`을(를) 실행해 `B`을(를) 출력합니다. "`삽입·삭제 때 next만 바꾸고 prev를 갱신하지 않음`" 여부를 확인하고 Day 62을(를) 완료하세요.
