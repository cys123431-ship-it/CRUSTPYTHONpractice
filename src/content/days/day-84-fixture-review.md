...
exercises:
  - id: ex-day-84-fixture-review-predict
    ...
    answer: "40"
    hint: C 행 두 개의 분을 더하면 40이고 Rust 행은 필터에서 제외됩니다. 설명을 떠올리고 `C 30` 단계부터 순서대로 적어 보세요.
    explanation:
      C 30  →  Rust 제외  →  C 10  →  40 순서로 실행됩니다. `language == "C"` 부분이 `40` 단계를 확정해 최종 출력 '40'가 됩니다. 이
      흐름을 떠올리면 `공통 fixture의 필터 기대값 계산` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-84-fixture-review-fill
    ...
    answer: 'rows = [("C", 30), ("Rust", 20), ("C", 10)]
      ...
      print(sum(minutes for language, minutes in rows if language == "C"))'
    hint: 힌트 문장을 완전하게 읽으면 `첫 C에서 30을 얻습니다` 단계에 필요한 표현이 `language == "C"`입니다.
    explanation:
      빈칸에 들어갈 표현은 'language == "C"'입니다. `print(sum(minutes for language, minutes in rows if language ==
      "C"))` 줄을 완성해야 `공통 fixture의 필터 기대값 계산` 동작이 이어져 실행 결과 '40'가 됩니다. 힌트의 첫 단계 `첫 C에서 30을 얻습니다`이 바로 이 줄입니다. 이어서 Rust는
      건너뜁니다 두 번째 C의 10을 더해 40입니다 순서로 진행됩니다.
    commonMistakes:
      - 필터를 적용한 결과와 전체 합계를 같은 값으로 비교함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-84-fixture-review-guided
    ...
    explanation:
      바꾼 뒤 출력은 '41'입니다. 원본 출력 '40'에서 달라졌습니다. 바뀐 줄은 `rows = [("C", 30), ("Rust", 20), ("C", 10)]`에서 `rows
      = [("C", 31), ("Rust", 20), ("C", 10)]`로 바뀌었습니다. 바뀐 프로그램은 `C 30` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로
      이어져 최종 `41`가 됩니다. 원본 추적 `C 30  →  Rust 제외  →  C 10  →  40`와 바뀐 줄 이후를 순서대로 비교하면 처음 달라지는 곳이 보입니다.
    commonMistakes:
      - 필터를 적용한 결과와 전체 합계를 같은 값으로 비교함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-84-fixture-review-debug
    ...
    answer: 'rows = [("C", 30), ("Rust", 20), ("C", 10)]
      ...
      print(sum(minutes for language, minutes in rows if language == "C"))'
    hint: C 행 두 개의 분을 더하면 40이고 Rust 행은 필터에서 제외됩니다. 설명과 어긋나는 줄을 찾으세요. `필터를 적용한 결과와 전체 합계를 같은 값으로 비교함` 상황이 단서가 됩니다.
    explanation:
      틀린 줄은 `print(sum(minutes for language, minutes in rows if language != "C"))`입니다. 여기서는 `language !=
      "C"`을 써서 `language == "C"` 동작이 깨집니다. 이대로 실행하면 `필터를 적용한 결과와 전체 합계를 같은 값으로 비교함` 문제가 생겨 원본 추적 `C 30  →  Rust 제외  →  C
      10  →  40`대로 '40'가 나오지 않습니다. 고친 줄 `print(sum(minutes for language, minutes in rows if language == "C"))`에서는
      `language == "C"`가 `공통 fixture의 필터 기대값 계산` 동작을 지켜 '40'까지 도달합니다.
    commonMistakes:
      - 필터를 적용한 결과와 전체 합계를 같은 값으로 비교함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-84-fixture-review-independent
    ...
    answer: 'rows = [("C", 30), ("Rust", 20), ("C", 10)]
      ...
      print(sum(minutes for language, minutes in rows if language == "C"))'
    hint: C 행 두 개의 분을 더하면 40이고 Rust 행은 필터에서 제외됩니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `language == "C"` 부분이 `공통 fixture의 필터 기대값 계산` 동작을 지켜 실행 결과 '40'가 됩니다. 같은 개념을
      다른 입력으로 바꿔도 `language == "C"`부터 `40`까지 추적할 수 있으면 정답입니다. `필터를 적용한 결과와 전체 합계를 같은 값으로 비교함` 상황과 빈 입력 같은 경계도 함께 설명해
      보세요.
    commonMistakes:
      - 필터를 적용한 결과와 전체 합계를 같은 값으로 비교함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
...
quiz:
  - id: quiz-day-84-fixture-review-output
    ...
    answerIndex: 0
    explanation:
      C 30  →  Rust 제외  →  C 10  →  40 순서로 실행되어 출력은 '40'입니다. `language == "C"` 부분이 마지막 단계를 확정하므로 다른 선택지는
      이 추적과 맞지 않습니다.
  - id: quiz-day-84-fixture-review-model
    ...
    answerIndex: 1
    explanation:
      C 행 두 개의 분을 더하면 40이고 Rust 행은 필터에서 제외됩니다. 이 설명이 맞는 이유는 `공통 fixture의 필터 기대값 계산` 동작을 지키는 조건과 같기 때문입니다.
      `필터를 적용한 결과와 전체 합계를 같은 값으로 비교함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-84-fixture-review-pitfall
    ...
    answerIndex: 2
    explanation:
      필터를 적용한 결과와 전체 합계를 같은 값으로 비교함. 이 실수가 나오면 원본 추적 `C 30 → Rust 제외 → C 10 → 40`대로 '40'가 나오지 않으므로 먼저 확인해야
      합니다.
  - id: quiz-day-84-fixture-review-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 0
    explanation: 문법은 달라도 `공통 fixture의 필터 기대값 계산` 목적과 입력·출력은 유지합니다. 실행 결과 '40'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
...
원본 출력은 `40`이고, 바뀐 코드의 실행 결과는 `41`입니다. 바뀐 줄은 `rows = [("C", 30), ("Rust", 20), ("C", 10)]`에서 `rows = [("C", 31), ("Rust", 20), ("C", 10)]`로 바뀌었습니다. 바뀐 프로그램은 `C 30` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `41`가 됩니다. 원본 추적 `C 30  →  Rust 제외  →  C 10  →  40`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
