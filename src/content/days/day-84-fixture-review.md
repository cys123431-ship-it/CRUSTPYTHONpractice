...
exercises:
  - id: ex-day-84-fixture-review-predict
    ...
    answer: "40"
    hint: C 30에서 시작해 40까지 순서대로 적어 보세요. C 행 두 개의 분을 더하면 40이고 Rust 행은 필터에서 제외됩니다.
    explanation:
      1. 첫 C에서 30을 얻습니다. 2. Rust는 건너뜁니다. 3. 두 번째 C의 10을 더해 40입니다. `C 30 → Rust 제외 → C 10 → 40` 흐름으로 실제
      출력 `40`이 됩니다. 핵심 `language == "C"`는 고를 행을 정하는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-84-fixture-review-fill
    ...
    answer: 'rows = [("C", 30), ("Rust", 20), ("C", 10)]
      ...
      print(sum(minutes for language, minutes in rows if language == "C"))'
    hint: 필요한 표현은 공통 fixture의 필터 기대값 계산 동작을 잇는 language == "C"입니다.
    explanation:
      빈칸에 들어갈 표현은 'language == "C"'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. 첫 C에서 30을 얻습니다. 2. Rust는 건너뜁니다. 3. 두
      번째 C의 10을 더해 40입니다. `C 30 → Rust 제외 → C 10 → 40` 흐름으로 실제 출력 `40`이 됩니다. 핵심 `language == "C"`는 고를 행을 정하는 자리에 쓰입니다.
    commonMistakes:
      - 필터를 적용한 결과와 전체 합계를 같은 값으로 비교함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-84-fixture-review-guided
    ...
    answer: 'rows = [("C", 31), ("Rust", 20), ("C", 10)]
      ...
      print(sum(minutes for language, minutes in rows if language == "C"))'
    hint: 처음 등장하는 숫자를 30에서 31로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '40'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation: 바꾼 뒤 출력은 '41'입니다. 원본은 `40`, 수정본은 `41`이다. 첫 변경 줄의 첫 C분 30에서 31로 바꾸면, 필터를 통과하는 합이 그대로 1만큼 커진다.
    commonMistakes:
      - 필터를 적용한 결과와 전체 합계를 같은 값으로 비교함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-84-fixture-review-debug
    ...
    answer: 'rows = [("C", 30), ("Rust", 20), ("C", 10)]
      ...
      print(sum(minutes for language, minutes in rows if language == "C"))'
    hint: 필터를 적용한 결과와 전체 합계를 같은 값으로 비교함 상황에서 어긋나는 줄을 C 행 두 개의 분을 더하면 40이고 Rust 행은 필터에서 제외됩니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `print(sum(minutes for language, minutes in rows if language != "C"))`입니다. C가 아니라 Rust 행만 골라
      20이 되므로 실행은 되지만 결과는 `20`이 됩니다. 오류 분류는 잘못된 출력입니다. 고를 행이 뒤집히는 첫 순간이 원인입니다. 고친 줄 `print(sum(minutes for language,
      minutes in rows if language == "C"))`에서는 `공통 fixture의 필터 기대값 계산` 동작이 지켜집니다. 정상 코드는 C 선택→30+10 합산→Rust 제외의 순서로
      `40`을 출력합니다.
    commonMistakes:
      - 필터를 적용한 결과와 전체 합계를 같은 값으로 비교함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-84-fixture-review-independent
    ...
    answer: 'rows = [("C", 30), ("Rust", 20), ("C", 10)]
      ...
      print(sum(minutes for language, minutes in rows if language == "C"))'
    hint: 빠진 열, 숫자가 아닌 분, 빈 파일, 중복 주제도 입력으로 만들어 기대 결과를 미리 정하세요. 정답을 나중에 맞추지 마세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 빠진 열, 숫자가 아닌 분, 빈 파일, 중복 주제도 입력으로 만들어 기대 결과를 미리 정하세요. 정답을 나중에 맞추지 마세요.
      같은 공통 fixture의 필터 기대값 계산 동작을 구현하고 실행 결과 '40'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 필터를 적용한 결과와 전체 합계를 같은 값으로 비교함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-84-fixture-review-output
    ...
    answerIndex: 0
    explanation:
      먼저 첫 C에서 30을 얻습니다. 이어서 Rust는 건너뛰고 두 번째 C의 10을 더해 40이 되므로 출력 `40`이 됩니다. `실행 전에 반드시 오류가 난다`는 틀린 선택지인데
      필터와 합산이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기 때문입니다. 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-84-fixture-review-model
    ...
    answerIndex: 1
    explanation:
      C 행 두 개의 분을 더하면 40이고 Rust 행은 필터에서 제외된다는 뜻은, 고르는 조건이 결과를 정한다는 뜻입니다. `필터를 적용한 결과와 전체 합계를 같은 값으로 비교함`은
      반대 사례인데, 필터를 씌운 값과 전체 합은 다르기 때문입니다.
  - id: quiz-day-84-fixture-review-pitfall
    ...
    choices:
      ...
      - 필터를 적용한 결과와 전체 합계를 같은 값으로 비교함
    answerIndex: 2
    explanation: 먼저 확인할 실수는 `필터를 적용한 결과와 전체 합계를 같은 값으로 비교함`입니다. 잘못된 코드는 실행은 되지만 `20`이라는 잘못된 출력을 냅니다. 고르는 조건을 먼저 확인하세요.
  - id: quiz-day-84-fixture-review-transfer
    ...
    answerIndex: 0
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `공통 fixture의 필터 기대값 계산`이라는 의미와 입력·출력 계약입니다. 세 언어의 같은 필터 조건으로 동작을 구현하고, 실행 결과
      `40`으로 대조하면 옮김이 맞는지 확인할 수 있습니다.
...
예제의 핵심 표현은 `language == "C"`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본 실행 결과는 `40`입니다. 아래 코드에서 원본과 달라지는 첫 줄을 표시하고, 그 줄의 변경 전후 값을 적어 보세요.
...
원본은 `40`, 수정본은 `41`이다. 첫 변경 줄의 첫 C분 30에서 31로 바꾸면, 필터를 통과하는 합이 그대로 1만큼 커진다.
...
두 줄을 나란히 놓으면 정상 줄 `print(sum(minutes for language, minutes in rows if language == "C"))`이 `C 행 두 개의 분을 더하면 40이고 Rust 행은 필터에서 제외됩니다.` 설명과 맞고, 잘못된 줄은 `필터를 적용한 결과와 전체 합계를 같은 값으로 비교함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `C 30`부터 `40`까지 처음 어긋난 곳을 찾습니다.
