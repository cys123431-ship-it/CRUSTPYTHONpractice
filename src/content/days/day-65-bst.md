...
exercises:
  ...
  - id: ex-day-65-bst-guided
    ...
    prompt: 예시를 직접 타이핑한 뒤 'key = 10'을 'key = 9'로 바꿔 보세요. 출력이 바뀌는지 먼저 예측하고, 그대로라면 그 이유도 설명하세요.
    starter: "tree = {8: (3, 10), 3: (None, None), 10: (None, None)}
    ...
    explanation:
      예시 답안에서는 'key = 10'을 'key = 9'로 바꿨습니다. 원래 출력은 'True'입니다. 바꾼 줄에서 시작해 중간 값과 마지막 출력을 다시 추적하세요. 출력이 같더라도
...
정렬 규칙이 있는 트리라면 한 번의 비교로 다음 탐색 방향을 정합니다. Day 65 "`이진 탐색 트리의 비교 경로`"에서는 Python 코드가 `True`을(를) 만드는 과정을 따라가며, 키 비교로 가지 하나 선택 동작이 왜 필요한지 확인합니다. 키 비교로 가지 하나 선택을(를) 빠뜨리면 `정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 64](/learn/day-64-tree)에서는 "트리의 부모·자식 순회"을(를) 배웠습니다. "트리의 부모·자식 순회"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`이진 탐색 트리의 비교 경로`"에서 새로 달라지는 조건을 찾아보세요. Day 65의 답은 `True`이며, 핵심 표현은 `key > node`입니다.

## 머릿속 그림

현재 노드보다 작은 키는 왼쪽, 큰 키는 오른쪽입니다. 중복 키 정책은 따로 정해야 합니다. Day 65에서는 아래 흐름 순서대로 상태가 바뀌며, `key > node`이(가) 결과를 가릅니다.

```text
root=8  →  오른쪽=10  →  발견
```

위 흐름에서 `key > node`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `True`입니다.
...
아래 Python 코드는 Day 65 "`이진 탐색 트리의 비교 경로`"의 독립 예제입니다. 전체 5줄에서 `key > node`이(가) 핵심이며, 실행 결과는 `True`입니다.
...
위 단계에서 `key > node`이(가) 빠지면 `True`이(가) 나오지 않습니다. `'key = 10'을 'key = 9'로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `True`이(가) 됩니다. `root=8` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 코드에서 `'key = 10'을 'key = 9'로` 바꾸면 아래와 같이 됩니다.
...
원본 출력은 `True`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `key > node`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 65의 `이진 탐색 트리의 비교 경로`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함.** 정상 코드에서는 `True`이(가) 출력됩니다.

- 정상 줄: `if key > node: node = tree[node][1]`
- 잘못된 줄: `if key < node: node = tree[node][1]`

두 줄을 나란히 놓고 `현재 노드보다 작은 키는 왼쪽, 큰 키는 오른쪽입니다. 중복 키 정책은 따로 정해야 합니다.` 기준으로 어느 쪽이 맞는지 설명하세요. 결과가 예상과 다르면 `root=8`부터 `True`까지 처음 어긋난 곳을 찾습니다.

## 다른 언어로 옮기기

핵심 동작 **키 비교로 가지 하나 선택**은(는) 세 언어에서 같은 입력과 출력(`True`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `struct Node*와 값 비교` 규칙으로 "`이진 탐색 트리의 비교 경로`"의 `True`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `노드 객체와 왼쪽/오른쪽 참조` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `Option<Box<Node>>와 Ord` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`True` 맞히기) → 빈칸(`key > node` 채우기) → 변경(`'key = 10'을 'key = 9'로`) → 오류 수정(`정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함` 찾기) → 독립 구현(`이진 탐색 트리의 비교 경로을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `True` 및 `key > node` 설명과 대조하세요.

## 스스로 설명하기

- "`이진 탐색 트리의 비교 경로`"이(가) 필요한 상황을 `키 비교로 가지 하나 선택` 동작으로 설명해 보세요.
- 예제에서 `key > node`이(가) 실행되기 직전의 상태와 직후의 출력 `True`을(를) 말해 보세요.
- "`정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `키 비교로 가지 하나 선택` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

현재 노드보다 작은 키는 왼쪽, 큰 키는 오른쪽입니다. 중복 키 정책은 따로 정해야 합니다. Day 65 "`이진 탐색 트리의 비교 경로`"의 예제는 `key > node`을(를) 실행해 `True`을(를) 출력합니다. "`정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함`" 여부를 확인하고 Day 65을(를) 완료하세요.
