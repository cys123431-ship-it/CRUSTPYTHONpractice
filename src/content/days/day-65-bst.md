...
exercises:
  ...
  - id: ex-day-65-bst-fill
    ...
    explanation: 빈칸에 들어갈 표현은 'key > node'입니다. 루트 8과 목표 10을 비교합니다 10>8이라 오른쪽 자식 10으로 이동합니다 목표와 같아 True입니다
    commonMistakes:
      - 정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-65-bst-guided
    ...
    explanation: 바꾼 뒤 출력은 'False'입니다. 원본 출력 'True'에서 달라졌습니다. 다른 줄(key = 10 → key = 9)에서 시작한 차이가 최종 출력에 반영되었습니다.
    commonMistakes:
      - 정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-65-bst-output
    ...
    choices:
      ...
      - "True"
    answerIndex: 2
    explanation: root=8  →  오른쪽=10  →  발견 순서로 실행되어 출력은 'True'입니다.
...
원본과 달라졌습니다. 원본 출력은 `True`입니다. 다른 줄(`key = 10` → `key = 9`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`root=8` → …)과 바뀐 줄부터 대조해 보세요.
