...
exercises:
  - id: ex-day-65-bst-predict
    ...
    starter: "tree = {8: (3, 10), 3: (None, None), 10: (None, None)}
      ...
      print(node == key)"
    answer: "True"
    hint: root=8에서 시작해 발견까지 순서대로 적어 보세요. 현재 노드보다 작은 키는 왼쪽, 큰 키는 오른쪽입니다. 중복 키 정책은 따로 정해야 합니다.
    explanation:
      1. 루트 8과 목표 10을 비교합니다. 2. 10>8이라 오른쪽 자식 10으로 이동합니다. 3. 목표와 같아 True입니다. `root=8 → 오른쪽=10 → 발견` 흐름으로
      실제 출력 `True`가 됩니다. 핵심 `key > node`는 어느 가지로 갈지 정하는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-65-bst-fill
    ...
    answer: "tree = {8: (3, 10), 3: (None, None), 10: (None, None)}
      ...
      print(node == key)"
    hint: 필요한 표현은 키 비교로 가지 하나 선택 동작을 잇는 key > node입니다.
    explanation:
      빈칸에 들어갈 표현은 'key > node'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. 루트 8과 목표 10을 비교합니다. 2. 10>8이라 오른쪽 자식 10으로
      이동합니다. 3. 목표와 같아 True입니다. `root=8 → 오른쪽=10 → 발견` 흐름으로 실제 출력 `True`가 됩니다. 핵심 `key > node`는 어느 가지로 갈지 정하는 자리에
      쓰입니다.
    commonMistakes:
      - 정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-65-bst-guided
    ...
    prompt: 예시를 직접 타이핑한 뒤 'key = 10' → 'key = 9' 바꿔 보세요. 출력이 바뀌는지 먼저 예측하고, 그대로라면 그 이유도 설명하세요.
    starter: "tree = {8: (3, 10), 3: (None, None), 10: (None, None)}
    ...
    answer: "tree = {8: (3, 10), 3: (None, None), 10: (None, None)}
      ...
      print(node == key)"
    hint: "'key = 10' → 'key = 9' 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 'True'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다."
    explanation:
      바꾼 뒤 출력은 'False'입니다. 원본은 `True`, 수정본은 `False`이다. 첫 변경 줄 `key = 10`에서 `key = 9`로 목표를 바꾸면, 오른쪽으로 이동한
      10과 목표 9가 달라 거짓이 된다.
    commonMistakes:
      - 정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-65-bst-debug
    ...
    answer: "tree = {8: (3, 10), 3: (None, None), 10: (None, None)}
      ...
      print(node == key)"
    hint: 정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함 상황에서 어긋나는 줄을 현재 노드보다 작은 키는 왼쪽, 큰 키는 오른쪽입니다. 중복 키 정책은 따로 정해야 합니다. 설명과 대조해 보세요.
    explanation:
      "틀린 줄은 `if key < node: node = tree[node][1]`입니다. 10<8이 거짓이므로 이동하지 않고 루트 8이 목표 10과 달라 `False`가 출력됩니다.
      오류 분류는 잘못된 출력입니다. 가지 선택이 어긋나는 첫 순간은 비교 방향입니다. 고친 줄 `if key > node: node = tree[node][1]`에서는 `키 비교로 가지 하나 선택`
      동작이 지켜집니다. 정렬 규칙이 깨진 트리에 이분 탐색을 그대로 쓰면 답을 보장하지 못합니다. 정상 코드는 비교→오른쪽 이동→일치 확인의 순서로 `True`를 출력합니다."
    commonMistakes:
      - 정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-65-bst-independent
    ...
    answer: "tree = {8: (3, 10), 3: (None, None), 10: (None, None)}
      ...
      print(node == key)"
    hint: 찾는 값이 같을 때 멈추고, 자식이 `None`이면 없다고 답합니다. 중복 키 정책을 트리 규칙에 넣으세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 찾는 값이 같을 때 멈추고, 자식이 `None`이면 없다고 답합니다. 중복 키 정책을 트리 규칙에 넣으세요. 같은 키 비교로
      가지 하나 선택 동작을 구현하고 실행 결과 'True'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-65-bst-output
    ...
    answerIndex: 2
    explanation:
      먼저 루트 8과 목표 10을 비교합니다. 이어서 10>8이라 오른쪽 자식 10으로 이동하고, 목표와 같아 출력 `True`가 됩니다. `실행 전에 반드시 오류가 난다`는 틀린
      선택지인데 비교와 이동이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기 때문입니다. 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-65-bst-model
    ...
    answerIndex: 0
    explanation:
      현재 노드보다 작은 키는 왼쪽, 큰 키는 오른쪽이라는 뜻은 비교 한 번에 한쪽 가지를 버린다는 뜻입니다. `정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함`은 반대 사례인데,
      규칙이 깨진 위치의 값은 경로 탐색이 보장하지 못하기 때문입니다.
  - id: quiz-day-65-bst-pitfall
    ...
    answerIndex: 1
    explanation: 먼저 확인할 실수는 `정렬 불변식 없이 아무 트리에도 이분 탐색을 적용함`입니다. 잘못된 코드는 실행은 되지만 `False`라는 잘못된 출력을 냅니다. 비교 방향을 먼저 확인하세요.
  - id: quiz-day-65-bst-transfer
    ...
    answerIndex: 2
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `키 비교로 가지 하나 선택`이라는 의미와 입력·출력 계약입니다. C의 포인터 이동과 Rust의 매칭 이동으로 같은 동작을 구현하고,
      실행 결과 `True`로 대조하면 옮김이 맞는지 확인할 수 있습니다.
...
예제의 핵심 표현은 `key > node`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본은 `True`, 수정본은 `False`이다. 첫 변경 줄 `key = 10`에서 `key = 9`로 목표를 바꾸면, 오른쪽으로 이동한 10과 목표 9가 달라 거짓이 된다.
