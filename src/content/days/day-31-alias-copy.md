...
exercises:
  - id: ex-day-31-alias-copy-predict
    ...
    starter: "source = [10, 20]
      ...
      print(source[0])"
    answer: "10"
    hint: copy()는 새 바깥 리스트를 만들고, 단순 대입은 같은 리스트에 새 이름만 붙입니다. 설명을 떠올리고 `source=[10,20]` 단계부터 순서대로 적어 보세요.
    explanation:
      source=[10,20]  →  copy=[10,20]  →  copy=[99,20]  →  source[0]=10 순서로 실행됩니다. `source.copy()` 부분이
      `source[0]=10` 단계를 확정해 최종 출력 '10'가 됩니다. 이 흐름을 떠올리면 `원본과 새 리스트를 구분함` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-31-alias-copy-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: "'Python 별칭과 얕은 복사' 개념의 핵심 표현을 스스로 적는다."
    ...
    answer: "source = [10, 20]
      ...
      print(source[0])"
    hint: 힌트 문장을 완전하게 읽으면 `source에 두 원소가 있습니다` 단계에 필요한 표현이 `source.copy()`입니다.
    explanation:
      빈칸에 들어갈 표현은 'source.copy()'입니다. `copy = source.copy()` 줄을 완성해야 `원본과 새 리스트를 구분함` 동작이 이어져 실행 결과 '10'가
      됩니다. 힌트의 첫 단계 `source에 두 원소가 있습니다`이 바로 이 줄입니다. 이어서 copy()가 다른 리스트를 만듭니다 copy[0]만 99가 되어 source[0]은 10입니다 순서로
      진행됩니다.
    commonMistakes:
      - copy = source를 독립된 리스트 복사라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-31-alias-copy-guided
    ...
    explanation:
      바꾼 뒤 출력은 '11'입니다. 원본 출력 '10'에서 달라졌습니다. 바뀐 줄은 `source = [10, 20]`에서 `source = [11, 20]`로 바뀌었습니다. 바뀐
      프로그램은 `source=[10,20]` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `11`가 됩니다. 원본 추적 `source=[10,20]  →  copy=[10,20]  →  copy=[99,20]  →  source[0]=10`와
  ...
  - id: ex-day-31-alias-copy-debug
    ...
    answer: "source = [10, 20]
      ...
      print(source[0])"
    hint:
      copy()는 새 바깥 리스트를 만들고, 단순 대입은 같은 리스트에 새 이름만 붙입니다. 설명과 어긋나는 줄을 찾으세요. `copy = source를 독립된 리스트 복사라고 생각함` 상황이
      단서가 됩니다.
    explanation:
      틀린 줄은 `copy = source`입니다. 여기서는 `source`을 써서 `source.copy()` 동작이 깨집니다. 이대로 실행하면 `copy = source를 독립된
      리스트 복사라고 생각함` 문제가 생겨 원본 추적 `source=[10,20]  →  copy=[10,20]  →  copy=[99,20]  →  source[0]=10`대로 '10'가 나오지 않습니다.
      고친 줄 `copy = source.copy()`에서는 `source.copy()`가 `원본과 새 리스트를 구분함` 동작을 지켜 '10'까지 도달합니다.
    commonMistakes:
      - copy = source를 독립된 리스트 복사라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-31-alias-copy-independent
    ...
    answer: "source = [10, 20]
      ...
      print(source[0])"
    hint: copy()는 새 바깥 리스트를 만들고, 단순 대입은 같은 리스트에 새 이름만 붙입니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `source.copy()` 부분이 `원본과 새 리스트를 구분함` 동작을 지켜 실행 결과 '10'가 됩니다. 같은 개념을 다른 입력으로
      바꿔도 `source.copy()`부터 `source[0]=10`까지 추적할 수 있으면 정답입니다. `copy = source를 독립된 리스트 복사라고 생각함` 상황과 빈 입력 같은 경계도 함께
      설명해 보세요.
    commonMistakes:
      - copy = source를 독립된 리스트 복사라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
...
quiz:
  - id: quiz-day-31-alias-copy-output
    ...
    answerIndex: 1
    explanation:
      source=[10,20]  →  copy=[10,20]  →  copy=[99,20]  →  source[0]=10 순서로 실행되어 출력은 '10'입니다. `source.copy()`
  ...
  - id: quiz-day-31-alias-copy-model
    ...
    answerIndex: 2
    explanation:
      copy()는 새 바깥 리스트를 만들고, 단순 대입은 같은 리스트에 새 이름만 붙입니다. 이 설명이 맞는 이유는 `원본과 새 리스트를 구분함` 동작을 지키는 조건과 같기 때문입니다.
      `copy = source를 독립된 리스트 복사라고 생각함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-31-alias-copy-pitfall
    ...
    answerIndex: 0
    explanation:
      copy = source를 독립된 리스트 복사라고 생각함. 이 실수가 나오면 원본 추적 `source=[10,20] → copy=[10,20] → copy=[99,20] →
      source[0]=10`대로 '10'가 나오지 않으므로 먼저 확인해야 합니다.
  - id: quiz-day-31-alias-copy-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 1
    explanation: 문법은 달라도 `원본과 새 리스트를 구분함` 목적과 입력·출력은 유지합니다. 실행 결과 '10'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
...
원본 출력은 `10`이고, 바뀐 코드의 실행 결과는 `11`입니다. 바뀐 줄은 `source = [10, 20]`에서 `source = [11, 20]`로 바뀌었습니다. 바뀐 프로그램은 `source=[10,20]` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `11`가 됩니다. 원본 추적 `source=[10,20]  →  copy=[10,20]  →  copy=[99,20]  →  source[0]=10`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
