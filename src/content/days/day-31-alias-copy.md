...
두 변수가 같은 리스트를 공유하면 한쪽 수정이 다른 쪽에 보이므로 복사 여부를 판단해야 합니다. Day 31 "`Python 별칭과 얕은 복사`"에서는 Python 코드가 `10`을(를) 만드는 과정을 따라가며, 원본과 새 리스트를 구분함 동작이 왜 필요한지 확인합니다. 원본과 새 리스트를 구분함을(를) 빠뜨리면 `copy = source를 독립된 리스트 복사라고 생각함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 30](/learn/day-30-pointer-arithmetic)에서는 "C 포인터 이동과 배열"을(를) 배웠습니다. "C 포인터 이동과 배열"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`Python 별칭과 얕은 복사`"에서 새로 달라지는 조건을 찾아보세요. Day 31의 답은 `10`이며, 핵심 표현은 `source.copy()`입니다.

## 머릿속 그림

copy()는 새 바깥 리스트를 만들고, 단순 대입은 같은 리스트에 새 이름만 붙입니다. Day 31에서는 아래 흐름 순서대로 상태가 바뀌며, `source.copy()`이(가) 결과를 가릅니다.

```text
source=[10,20]  →  copy=[10,20]  →  copy=[99,20]  →  source[0]=10
```

위 흐름에서 `source.copy()`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `10`입니다.
...
아래 Python 코드는 Day 31 "`Python 별칭과 얕은 복사`"의 독립 예제입니다. 전체 4줄에서 `source.copy()`이(가) 핵심이며, 실행 결과는 `10`입니다.
...
하단 Python 실행 영역에 같은 코드가 들어 있습니다. 먼저 `10`을(를) 가리고 예측한 뒤 실행해 비교하세요.
...
위 단계에서 `source.copy()`이(가) 빠지면 `10`이(가) 나오지 않습니다. `처음 등장하는 숫자를 10에서 11로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `10`이(가) 됩니다. `source=[10,20]` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `10`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `source.copy()`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 31의 `Python 별칭과 얕은 복사`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: copy = source를 독립된 리스트 복사라고 생각함.** 정상 코드에서는 `10`이(가) 출력됩니다.
...
핵심 동작 **원본과 새 리스트를 구분함**은(는) 세 언어에서 같은 입력과 출력(`10`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `배열을 원소별 복사하거나 memcpy 길이 확인` 규칙으로 "`Python 별칭과 얕은 복사`"의 `10`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `list.copy는 얕은 복사` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `Vec::clone은 새 소유 데이터` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`Python 별칭과 얕은 복사`"이(가) 필요한 상황을 `원본과 새 리스트를 구분함` 동작으로 설명해 보세요.
- 예제에서 `source.copy()`이(가) 실행되기 직전의 상태와 직후의 출력 `10`을(를) 말해 보세요.
- "`copy = source를 독립된 리스트 복사라고 생각함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `원본과 새 리스트를 구분함` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

copy()는 새 바깥 리스트를 만들고, 단순 대입은 같은 리스트에 새 이름만 붙입니다. Day 31 "`Python 별칭과 얕은 복사`"의 예제는 `source.copy()`을(를) 실행해 `10`을(를) 출력합니다. "`copy = source를 독립된 리스트 복사라고 생각함`" 여부를 확인하고 Day 31을(를) 완료하세요.
