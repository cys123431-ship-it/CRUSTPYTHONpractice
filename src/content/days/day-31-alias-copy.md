...
exercises:
  - id: ex-day-31-alias-copy-predict
    ...
    starter: "source = [10, 20]
      ...
      print(source[0])"
    answer: "10"
    hint: source=[10,20]에서 시작해 source[0]=10까지 순서대로 적어 보세요. copy()는 새 바깥 리스트를 만들고, 단순 대입은 같은 리스트에 새 이름만 붙입니다.
    explanation:
      1. source에 두 원소가 있습니다. 2. copy()가 다른 리스트를 만듭니다. 3. copy[0]만 99가 되어 source[0]은 10입니다. `source=[10,20]
      → copy=[10,20] → copy=[99,20] → source[0]=10` 흐름으로 실제 출력 `10`이 됩니다. 핵심 `source.copy()`는 별도의 바깥 목록을 만드는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-31-alias-copy-fill
    ...
    answer: "source = [10, 20]
      ...
      print(source[0])"
    hint: 필요한 표현은 원본과 새 리스트를 구분함 동작을 잇는 source.copy()입니다.
    explanation:
      빈칸에 들어갈 표현은 'source.copy()'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. source에 두 원소가 있습니다. 2. copy()가 다른 리스트를
      만듭니다. 3. copy[0]만 99가 되어 source[0]은 10입니다. `source=[10,20] → copy=[10,20] → copy=[99,20] → source[0]=10` 흐름으로
      실제 출력 `10`이 됩니다. 핵심 `source.copy()`는 별도의 바깥 목록을 만드는 자리에 쓰입니다.
    commonMistakes:
      - copy = source를 독립된 리스트 복사라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-31-alias-copy-guided
    ...
    answer: "source = [11, 20]
      ...
      print(source[0])"
    hint: 처음 등장하는 숫자를 10에서 11로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '10'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '11'입니다. 원본은 `10`, 수정본은 `11`이다. 첫 변경 줄 `source = [10, 20]`에서 `source = [11, 20]`으로 첫 값을
      바꾸면, 복사본의 0번만 99가 되고 원본의 0번이 그대로 출력되어 11이 된다.
    commonMistakes:
      - copy = source를 독립된 리스트 복사라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-31-alias-copy-debug
    ...
    answer: "source = [10, 20]
      ...
      print(source[0])"
    hint: copy = source를 독립된 리스트 복사라고 생각함 상황에서 어긋나는 줄을 copy()는 새 바깥 리스트를 만들고, 단순 대입은 같은 리스트에 새 이름만 붙입니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `copy = source`입니다. 새 리스트가 아니라 같은 객체에 새 이름만 붙으므로, 뒤의 대입이 원본까지 바꿔 `99`가 출력됩니다. 오류 분류는 잘못된 출력입니다.
      원본과 복사가 갈라지지 않는 첫 순간은 복사 줄입니다. 고친 줄 `copy = source.copy()`에서는 `원본과 새 리스트를 구분함` 동작이 지켜집니다. 정상 코드는 원본 생성→별도 복사→복사본만
      수정→원본 읽기의 순서로 `10`을 출력합니다.
    commonMistakes:
      - copy = source를 독립된 리스트 복사라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-31-alias-copy-independent
    ...
    answer: "source = [10, 20]
      ...
      print(source[0])"
    hint: 안에 또 리스트가 있으면 얕은 복사는 안쪽을 공유합니다. 바깥 목록과 안쪽 값 중 무엇을 복사할지 먼저 물으세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 안에 또 리스트가 있으면 얕은 복사는 안쪽을 공유합니다. 바깥 목록과 안쪽 값 중 무엇을 복사할지 먼저 물으세요. 같은 원본과
      새 리스트를 구분함 동작을 구현하고 실행 결과 '10'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - copy = source를 독립된 리스트 복사라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-31-alias-copy-output
    ...
    answerIndex: 1
    explanation:
      먼저 source에 [10,20]이 저장됩니다. 이어서 copy()가 다른 리스트 [10,20]을 만들고, copy[0]만 99가 되므로 source[0]은 그대로 10이어서
      출력 `10`이 됩니다. `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 대입과 메서드 호출이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기
      때문입니다. 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-31-alias-copy-model
    question: "'Python 별칭과 얕은 복사' 개념을 이해하는 데 맞는 설명은?"
    choices:
    ...
    answerIndex: 2
    explanation:
      copy()는 새 바깥 리스트를 만든다는 뜻이고, 단순 대입은 같은 리스트에 새 이름만 붙인다는 뜻은 이름은 둘이라도 객체는 하나라는 뜻입니다. `copy = source를
      독립된 리스트 복사라고 생각함`은 반대 사례인데, 대입은 객체를 공유하므로 원본까지 바뀌기 때문입니다.
  - id: quiz-day-31-alias-copy-pitfall
    ...
    answerIndex: 0
    explanation:
      먼저 확인할 실수는 `copy = source를 독립된 리스트 복사라고 생각함`입니다. 잘못된 코드는 실행은 되지만 `99`라는 잘못된 출력을 냅니다. 복사인지 별칭인지 먼저
      확인하세요.
  - id: quiz-day-31-alias-copy-transfer
    ...
    answerIndex: 1
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `원본과 새 리스트를 구분함`이라는 의미와 입력·출력 계약입니다. Rust의 clone과 C의 배열 복사로 같은 동작을 구현하고, 실행
      결과 `10`으로 대조하면 옮김이 맞는지 확인할 수 있습니다.
...
예제의 핵심 표현은 `source.copy()`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본은 `10`, 수정본은 `11`이다. 첫 변경 줄 `source = [10, 20]`에서 `source = [11, 20]`으로 첫 값을 바꾸면, 복사본의 0번만 99가 되고 원본의 0번이 그대로 출력되어 11이 된다.
...
두 줄을 나란히 놓으면 정상 줄 `copy = source.copy()`이 `copy()는 새 바깥 리스트를 만들고, 단순 대입은 같은 리스트에 새 이름만 붙입니다.` 설명과 맞고, 잘못된 줄은 `copy = source를 독립된 리스트 복사라고 생각함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `source=[10,20]`부터 `10`까지 처음 어긋난 곳을 찾습니다.
...
예측(`10` 맞히기) → 빈칸(`source.copy()` 채우기) → 변경(`처음 등장하는 숫자를 10에서 11로`) → 오류 수정(`copy = source를 독립된 리스트 복사라고 생각함` 찾기) → 독립 구현(`'Python 별칭과 얕은 복사' 개념을 보여주는 작은 프로그램` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `10` 및 `source.copy()` 설명과 대조하세요.
