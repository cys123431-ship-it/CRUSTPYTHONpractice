...
exercises:
  ...
  - id: ex-day-31-alias-copy-fill
    ...
    explanation:
      빈칸에 들어갈 표현은 'source.copy()'입니다. source에 두 원소가 있습니다 copy()가 다른 리스트를 만듭니다 copy[0]만 99가 되어 source[0]은
      10입니다
    commonMistakes:
      - copy = source를 독립된 리스트 복사라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-31-alias-copy-guided
    ...
    explanation:
      바꾼 뒤 출력은 '11'입니다. 원본 출력 '10'에서 달라졌습니다. 다른 줄(source = [10, 20] → source = [11, 20])에서 시작한 차이가 최종 출력에
      반영되었습니다.
    commonMistakes:
      - copy = source를 독립된 리스트 복사라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
...
quiz:
  - id: quiz-day-31-alias-copy-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 1
    explanation: source=[10,20]  →  copy=[10,20]  →  copy=[99,20]  →  source[0]=10 순서로 실행되어 출력은 '10'입니다.
...
원본과 달라졌습니다. 원본 출력은 `10`입니다. 다른 줄(`source = [10, 20]` → `source = [11, 20]`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`source=[10,20]` → …)과 바뀐 줄부터 대조해 보세요.
