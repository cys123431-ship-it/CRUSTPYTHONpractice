...
두 줄을 나란히 놓으면 정상 줄 `result += value`이 `함수 내부 result는 호출마다 새로 시작하며 return이 최종 합계를 호출자에게 전달합니다.` 설명과 맞고, 잘못된 줄은 `반복 안에서 result를 덮어써 마지막 값만 반환함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `입력 [10,20]`부터 `30`까지 처음 어긋난 곳을 찾습니다.
