...
한 주의 반복·함수를 직접 결합해 보면 상태 누적과 반환의 차이를 확인할 수 있습니다. Day 21 "`반복과 함수 회상`"에서는 Python 코드가 `30`을(를) 만드는 과정을 따라가며, 반복 계산을 함수로 묶음 동작이 왜 필요한지 확인합니다. 반복 계산을 함수로 묶음을(를) 빠뜨리면 `반복 안에서 result를 덮어써 마지막 값만 반환함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 20](/learn/day-20-c-functions)에서는 "C 함수 매개변수와 반환값"을(를) 배웠습니다. "C 함수 매개변수와 반환값"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`반복과 함수 회상`"에서 새로 달라지는 조건을 찾아보세요. Day 21의 답은 `30`이며, 핵심 표현은 `result += value`입니다.

## 머릿속 그림

함수 내부 result는 호출마다 새로 시작하며 return이 최종 합계를 호출자에게 전달합니다. Day 21에서는 아래 흐름 순서대로 상태가 바뀌며, `result += value`이(가) 결과를 가릅니다.

```text
입력 [10,20]  →  result=10  →  result=30  →  출력
```

위 흐름에서 `result += value`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `30`입니다.
...
아래 Python 코드는 Day 21 "`반복과 함수 회상`"의 독립 예제입니다. 전체 6줄에서 `result += value`이(가) 핵심이며, 실행 결과는 `30`입니다.
...
위 단계에서 `result += value`이(가) 빠지면 `30`이(가) 나오지 않습니다. `처음 등장하는 숫자를 0에서 1로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `30`이(가) 됩니다. `입력 [10,20]` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `30`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `result += value`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 21의 `반복과 함수 회상`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 반복 안에서 result를 덮어써 마지막 값만 반환함.** 정상 코드에서는 `30`이(가) 출력됩니다.
...
핵심 동작 **반복 계산을 함수로 묶음**은(는) 세 언어에서 같은 입력과 출력(`30`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `배열과 길이를 인자로 받는 함수` 규칙으로 "`반복과 함수 회상`"의 `30`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `iterable을 매개변수로 받아 return` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `slice를 빌려 iterator.sum()` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`반복과 함수 회상`"이(가) 필요한 상황을 `반복 계산을 함수로 묶음` 동작으로 설명해 보세요.
- 예제에서 `result += value`이(가) 실행되기 직전의 상태와 직후의 출력 `30`을(를) 말해 보세요.
- "`반복 안에서 result를 덮어써 마지막 값만 반환함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `반복 계산을 함수로 묶음` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

함수 내부 result는 호출마다 새로 시작하며 return이 최종 합계를 호출자에게 전달합니다. Day 21 "`반복과 함수 회상`"의 예제는 `result += value`을(를) 실행해 `30`을(를) 출력합니다. "`반복 안에서 result를 덮어써 마지막 값만 반환함`" 여부를 확인하고 Day 21을(를) 완료하세요.
