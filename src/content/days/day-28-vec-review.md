...
exercises:
  ...
  - id: ex-day-28-vec-review-fill
    ...
    explanation: 빈칸에 들어갈 표현은 'push(30)'입니다. Vec [10,20]을 만듭니다 push(30)으로 [10,20,30]이 됩니다 순회해 합 60을 출력합니다
    commonMistakes:
      - Vec의 원소 추가에 불변 바인딩을 사용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-28-vec-review-guided
    ...
    explanation:
      바꾼 뒤 출력은 '61'입니다. 원본 출력 '60'에서 달라졌습니다. 다른 줄(let mut minutes = vec![10, 20]; → let mut minutes = vec![11,
      20];)에서 시작한 차이가 최종 출력에 반영되었습니다.
    commonMistakes:
      - Vec의 원소 추가에 불변 바인딩을 사용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
...
원본과 달라졌습니다. 원본 출력은 `60`입니다. 다른 줄(`let mut minutes = vec![10, 20];` → `let mut minutes = vec![11, 20];`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`[10,20]` → …)과 바뀐 줄부터 대조해 보세요.
