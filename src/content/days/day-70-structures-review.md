...
exercises:
  ...
  - id: ex-day-70-structures-review-guided
    ...
    prompt: 예시를 직접 타이핑한 뒤 첫 문자열을 'first'에서 'first!'로 바꿔 보세요. 출력도 먼저 예측하세요.
    starter: 'from collections import deque
  ...
  - id: ex-day-70-structures-review-debug
    ...
    prompt: 아래 코드의 핵심 표현 'pop()'을 점검하고 올바른 작동을 복원하세요. 최근 항목 pop을 써서 접수 순서를 뒤집음 오류도 함께 경계하세요.
    starter: 'from collections import deque
...
문제의 입력·제거 순서를 보고 스택·큐·트리·해시 중 무엇을 고를지 말로 정리합니다. 이번 Day는 새 문법을 많이 추가하는 대신 앞선 개념을 떠올리고 작은 변경 실험으로 오개념을 확인합니다. 코드를 가린 채 먼저 답하고, 모른 부분만 선행 Day로 돌아가 보세요.

## 2. 시작 전에 확인할 것

바로 앞선 [Day 69](/learn/day-69-bfs)의 핵심을 한 문장으로 설명해 보세요. 모르면 위의 선행 Day 링크에서 다시 확인할 수 있습니다.

## 3. 머릿속 그림

접수 순서 보존이 불변식이라면 앞에서 꺼내는 큐가 맞습니다. 다음 흐름을 눈으로 확인하세요.
...
마지막 상태에서 화면에 표시되는 결과는 `first`입니다. 직접 타이핑할 때는 위의 순서와 실제 출력을 비교하세요.

## 7. 결과 예측과 작은 변경

1. 코드를 가리고 결과를 먼저 적으세요.
2. 그 결과를 만든 핵심 줄을 찾아 밑줄을 그으세요.
3. 첫 문자열을 'first'에서 'first!'로 바꾼 뒤 어느 단계부터 결과가 달라질지 예측하세요.
4. 실행할 수 있는 환경에서 확인하고 틀린 예측의 이유를 한 문장으로 적으세요.

## 8. 자주 틀리는 지점

**확인할 실수: 최근 항목 pop을 써서 접수 순서를 뒤집음.** 접수 순서 보존이 불변식이라면 앞에서 꺼내는 큐가 맞습니다. 결과가 예상과 다르면 입력 → 중간 상태 → 출력 중 처음 어긋난 곳을 찾습니다.

## 9. 다른 언어로 옮기기

같은 문제를 해결하더라도 선언, 타입, 메모리 관리, 오류 처리는 다릅니다. 핵심 동작은 **처리 순서에 맞는 구조 선택**입니다. 언어를 옮겨도 이 동작을 유지하세요.
...
- 왜 이 개념이 필요한가? 문제의 입력·제거 순서를 보고 스택·큐·트리·해시 중 무엇을 고를지 말로 정리합니다.
- 예시에서 가장 먼저 확정되는 값이나 상태는 무엇인가?
- 어떤 실수를 점검해야 하나? 최근 항목 pop을 써서 접수 순서를 뒤집음
- 다른 두 언어에서는 같은 동작을 어떤 자료형과 오류 처리로 나타내는가?

## 12. 핵심 요약과 복습

접수 순서 보존이 불변식이라면 앞에서 꺼내는 큐가 맞습니다. 예시의 출력은 `first`입니다. 오류를 찾을 때는 **최근 항목 pop을 써서 접수 순서를 뒤집음** 여부를 확인하세요. 완료 버튼을 누르면 +1·+3·+7·+14·+30일 복습이 이 기기에 등록됩니다.
