...
문제의 입력·제거 순서를 보고 스택·큐·트리·해시 중 무엇을 고를지 말로 정리합니다. Day 70 "`자료구조 선택 회상`"에서는 Python 코드가 `first`을(를) 만드는 과정을 따라가며, 처리 순서에 맞는 구조 선택 동작이 왜 필요한지 확인합니다. 처리 순서에 맞는 구조 선택을(를) 빠뜨리면 `최근 항목 pop을 써서 접수 순서를 뒤집음` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 69](/learn/day-69-bfs)에서는 "너비 우선 탐색과 방문 집합"을(를) 배웠습니다. "너비 우선 탐색과 방문 집합"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`자료구조 선택 회상`"에서 새로 달라지는 조건을 찾아보세요. Day 70의 답은 `first`이며, 핵심 표현은 `popleft()`입니다.

## 머릿속 그림

접수 순서 보존이 불변식이라면 앞에서 꺼내는 큐가 맞습니다. Day 70에서는 아래 흐름 순서대로 상태가 바뀌며, `popleft()`이(가) 결과를 가릅니다.

```text
first 대기  →  second 대기  →  first 처리
```

위 흐름에서 `popleft()`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `first`입니다.
...
아래 Python 코드는 Day 70 "`자료구조 선택 회상`"의 독립 예제입니다. 전체 3줄에서 `popleft()`이(가) 핵심이며, 실행 결과는 `first`입니다.
...
하단 Python 실행 영역에 같은 코드가 들어 있습니다. 먼저 `first`을(를) 가리고 예측한 뒤 실행해 비교하세요.
...
위 단계에서 `popleft()`이(가) 빠지면 `first`이(가) 나오지 않습니다. `첫 문자열을 'first'에서 'first!'로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `first`이(가) 됩니다. `first 대기` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 코드에서 `첫 문자열을 'first'에서 'first!'로` 바꾸면 아래와 같이 됩니다.
...
원본 출력은 `first`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `popleft()`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 70의 `자료구조 선택 회상`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 최근 항목 pop을 써서 접수 순서를 뒤집음.** 정상 코드에서는 `first`이(가) 출력됩니다.
...
핵심 동작 **처리 순서에 맞는 구조 선택**은(는) 세 언어에서 같은 입력과 출력(`first`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `원형 큐로 FIFO 구현` 규칙으로 "`자료구조 선택 회상`"의 `first`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `deque.popleft` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `VecDeque::pop_front` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`자료구조 선택 회상`"이(가) 필요한 상황을 `처리 순서에 맞는 구조 선택` 동작으로 설명해 보세요.
- 예제에서 `popleft()`이(가) 실행되기 직전의 상태와 직후의 출력 `first`을(를) 말해 보세요.
- "`최근 항목 pop을 써서 접수 순서를 뒤집음`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `처리 순서에 맞는 구조 선택` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

접수 순서 보존이 불변식이라면 앞에서 꺼내는 큐가 맞습니다. Day 70 "`자료구조 선택 회상`"의 예제는 `popleft()`을(를) 실행해 `first`을(를) 출력합니다. "`최근 항목 pop을 써서 접수 순서를 뒤집음`" 여부를 확인하고 Day 70을(를) 완료하세요.
