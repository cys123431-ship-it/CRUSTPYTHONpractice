...
코드를 함수와 모듈로 나누더라도 예상 동작을 검사하는 자동 테스트가 있어야 안전하게 고칩니다. Day 55 "`Python 모듈화와 assert`"에서는 Python 코드가 `검증 완료`을(를) 만드는 과정을 따라가며, 예상 결과를 작은 테스트로 고정 동작이 왜 필요한지 확인합니다. 예상 결과를 작은 테스트로 고정을(를) 빠뜨리면 `assert가 실패해도 다음 print가 실행된다고 생각함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 54](/learn/day-54-trait-generic)에서는 "Rust trait과 제네릭"을(를) 배웠습니다. "Rust trait과 제네릭"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`Python 모듈화와 assert`"에서 새로 달라지는 조건을 찾아보세요. Day 55의 답은 `검증 완료`이며, 핵심 표현은 `assert`입니다.

## 머릿속 그림

assert는 조건이 거짓일 때 예외를 내고, 조건이 참일 때는 다음 줄로 진행합니다. Day 55에서는 아래 흐름 순서대로 상태가 바뀌며, `assert`이(가) 결과를 가릅니다.

```text
[10,20]  →  total=30  →  조건 참  →  출력
```

위 흐름에서 `assert`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `검증 완료`입니다.
...
아래 Python 코드는 Day 55 "`Python 모듈화와 assert`"의 독립 예제입니다. 전체 4줄에서 `assert`이(가) 핵심이며, 실행 결과는 `검증 완료`입니다.
...
하단 Python 실행 영역에 같은 코드가 들어 있습니다. 먼저 `검증 완료`을(를) 가리고 예측한 뒤 실행해 비교하세요.
...
위 단계에서 `assert`이(가) 빠지면 `검증 완료`이(가) 나오지 않습니다. `'assert total([10, 20]) == 30'을 'assert total([11, 20]) == 31'로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `검증 완료`이(가) 됩니다. `[10,20]` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 코드에서 `'assert total([10, 20]) == 30'을 'assert total([11, 20]) == 31'로` 바꾸면 아래와 같이 됩니다.
...
원본 출력은 `검증 완료`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `assert`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 55의 `Python 모듈화와 assert`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: assert가 실패해도 다음 print가 실행된다고 생각함.** 정상 코드에서는 `검증 완료`이(가) 출력됩니다.
...
핵심 동작 **예상 결과를 작은 테스트로 고정**은(는) 세 언어에서 같은 입력과 출력(`검증 완료`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `테스트 main과 return 상태 검사` 규칙으로 "`Python 모듈화와 assert`"의 `검증 완료`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `별도 test_*.py 또는 assert` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `#[test]와 cargo test` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`Python 모듈화와 assert`"이(가) 필요한 상황을 `예상 결과를 작은 테스트로 고정` 동작으로 설명해 보세요.
- 예제에서 `assert`이(가) 실행되기 직전의 상태와 직후의 출력 `검증 완료`을(를) 말해 보세요.
- "`assert가 실패해도 다음 print가 실행된다고 생각함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `예상 결과를 작은 테스트로 고정` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

assert는 조건이 거짓일 때 예외를 내고, 조건이 참일 때는 다음 줄로 진행합니다. Day 55 "`Python 모듈화와 assert`"의 예제는 `assert`을(를) 실행해 `검증 완료`을(를) 출력합니다. "`assert가 실패해도 다음 print가 실행된다고 생각함`" 여부를 확인하고 Day 55을(를) 완료하세요.
