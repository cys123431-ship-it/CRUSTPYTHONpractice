...
exercises:
  - id: ex-day-55-module-test-predict
    ...
    answer: 검증 완료
    hint: "[10,20]에서 시작해 출력까지 순서대로 적어 보세요. assert는 조건이 거짓일 때 예외를 내고, 조건이 참일 때는 다음 줄로 진행합니다."
    explanation:
      1. total에 두 값을 전달합니다. 2. sum 결과 30을 기대값과 비교합니다. 3. 참이므로 검증 완료를 출력합니다. `[10,20] → total=30 → 조건 참
      → 출력` 흐름으로 실제 출력 `검증 완료`가 됩니다. 핵심 `assert`는 기대값을 고정하고 어긋나면 멈추는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-55-module-test-fill
    ...
    hint: 필요한 표현은 예상 결과를 작은 테스트로 고정 동작을 잇는 assert입니다.
    explanation:
      빈칸에 들어갈 표현은 'assert'입니다. 이 표현이 없으면 컴파일 단계에서 막혀 실행 자체가 되지 않습니다. 1. total에 두 값을 전달합니다. 2. sum 결과 30을
      기대값과 비교합니다. 3. 참이므로 검증 완료를 출력합니다. `[10,20] → total=30 → 조건 참 → 출력` 흐름으로 실제 출력 `검증 완료`가 됩니다. 핵심 `assert`는 기대값을
      고정하고 어긋나면 멈추는 자리에 쓰입니다.
    commonMistakes:
      - assert가 실패해도 다음 print가 실행된다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-55-module-test-guided
    ...
    prompt:
      예시를 직접 타이핑한 뒤 'assert total([10, 20]) == 30' → 'assert total([11, 20]) == 31' 바꿔 보세요. 출력이 바뀌는지 먼저 예측하고,
      그대로라면 그 이유도 설명하세요.
    starter: "def total(values):\n    return sum(values)\nassert total([10, 20]) == 30\nprint(\"검증 완료\")"
    answer: "def total(values):\n    return sum(values)\nassert total([11, 20]) == 31\nprint(\"검증 완료\")"
    hint:
      "'assert total([10, 20]) == 30' → 'assert total([11, 20]) == 31' 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '검증 완료'와
      비교해 달라지는 첫 중간 값을 찾으면 됩니다."
    explanation:
      바꾼 뒤 출력은 '검증 완료'입니다. 원본과 수정본 모두 `검증 완료`를 출력한다. 기대값을 함께 31로 바꿨으므로 assert가 통과해 출력까지 도달합니다. 검사 대상과 기대값이
      함께 바뀌어 통과 여부는 그대로이므로 중간 상태가 달라졌다고 쓰지 않는다.
    commonMistakes:
      - assert가 실패해도 다음 print가 실행된다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-55-module-test-debug
    ...
    answer: "def total(values):\n    return sum(values)\nassert total([10, 20]) == 30\nprint(\"검증 완료\")"
    hint: assert가 실패해도 다음 print가 실행된다고 생각함 상황에서 어긋나는 줄을 assert는 조건이 거짓일 때 예외를 내고, 조건이 참일 때는 다음 줄로 진행합니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `print total([10, 20]) == 30`입니다. `print` 뒤에 괄호가 없어 Python 3 문법에 맞지 않으므로 실행 전에 SyntaxError라는
      컴파일 오류가 나고 프로그램이 시작되지 않아 아무것도 출력되지 않습니다. 오류 분류는 컴파일 오류입니다. 흐름이 깨지는 첫 순간은 검사 줄 자체입니다. 한 걸음 더 들어가면, 괄호를 넣어 `print(...)`로
      고쳐도 검사가 실패했을 때 멈추지 않고 참·거짓을 화면에 표시할 뿐이라 마지막 검증 완료가 계속 나올 수 있습니다. 실패를 탐지해 멈추는 기능이 사라진다는 점이 핵심입니다. 고친 줄 `assert
      total([10, 20]) == 30`에서는 `예상 결과를 작은 테스트로 고정` 동작이 지켜집니다. 정상 코드는 계산→기대값 비교→통과 후 출력의 순서로 `검증 완료`를 출력합니다.
    commonMistakes:
      - assert가 실패해도 다음 print가 실행된다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-55-module-test-independent
    ...
    answer: "def total(values):\n    return sum(values)\nassert total([10, 20]) == 30\nprint(\"검증 완료\")"
    hint: 빈 입력 결과가 0인지도 별도로 시험하세요. 배포 중 상시 검증을 `assert`에만 의존하면 안 됩니다. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 빈 입력 결과가 0인지도 별도로 시험하세요. 배포 중 상시 검증을 `assert`에만 의존하면 안 됩니다. 같은 예상 결과를
      작은 테스트로 고정 동작을 구현하고 실행 결과 '검증 완료'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - assert가 실패해도 다음 print가 실행된다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-55-module-test-output
    ...
    answerIndex: 1
    explanation:
      먼저 total에 [10,20]이 전달되어 sum 30이 계산됩니다. 이어서 assert가 기대값 30과 비교해 참이므로 통과하고, print가 표시하므로 출력 `검증 완료`가
      됩니다. `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 함수와 검사가 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기 때문입니다. 다른
      선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-55-module-test-model
    ...
    answerIndex: 2
    explanation:
      assert는 조건이 거짓일 때 예외를 낸다는 뜻이고, 조건이 참일 때는 다음 줄로 진행한다는 뜻은 통과가 조용하다는 뜻입니다. `assert가 실패해도 다음 print가 실행된다고
      생각함`은 반대 사례인데, 실패하면 예외로 멈추기 때문입니다.
  - id: quiz-day-55-module-test-pitfall
    ...
    answerIndex: 0
    explanation:
      먼저 확인할 실수는 `assert가 실패해도 다음 print가 실행된다고 생각함`입니다. 잘못된 코드는 괄호 없는 print라 컴파일 오류가 나므로 화면 출력 비교 이전에 실행
      자체가 막힙니다. 검사와 표시의 역할을 구별하세요.
  - id: quiz-day-55-module-test-transfer
    ...
    answerIndex: 1
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `예상 결과를 작은 테스트로 고정`이라는 의미와 입력·출력 계약입니다. C의 assert와 Rust의 assert_eq!로 같은 동작을
...
예제의 핵심 표현은 `assert`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본과 수정본 모두 `검증 완료`를 출력한다. 기대값을 함께 31로 바꿨으므로 assert가 통과해 출력까지 도달합니다. 검사 대상과 기대값이 함께 바뀌어 통과 여부는 그대로이므로 중간 상태가 달라졌다고 쓰지 않는다.
