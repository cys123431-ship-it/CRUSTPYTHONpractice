...
exercises:
  ...
  - id: ex-day-55-module-test-guided
    ...
    hint:
      "'assert total([10, 20]) == 30'을 'assert total([11, 20]) == 31'로 바꾼 뒤 [10,20] → total=30 → 조건 참 → 출력
      순으로 따라가세요."
    explanation:
  ...
  - id: ex-day-55-module-test-debug
    ...
    prompt: 아래 코드의 핵심 표현 'print'을 점검하고 올바른 작동을 복원하세요. assert가 실패해도 다음 print가 실행된다고 생각함 오류도 함께 경계하세요.
