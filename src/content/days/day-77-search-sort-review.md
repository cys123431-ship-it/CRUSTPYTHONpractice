...
exercises:
  - id: ex-day-77-search-sort-review-predict
    ...
    answer: "2"
    hint: sorted는 새 정렬 리스트를 만들고 index는 그 리스트에서 목표의 위치를 찾습니다. 설명을 떠올리고 `원본` 단계부터 순서대로 적어 보세요.
    explanation:
      원본  →  [2,4,6,8]  →  6은 2번 순서로 실행됩니다. `sorted(` 부분이 `6은 2번` 단계를 확정해 최종 출력 '2'가 됩니다. 이 흐름을 떠올리면 `정렬
      전후 탐색 위치 구분` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-77-search-sort-review-fill
    ...
    answer: "values = sorted([8, 2, 6, 4])
      ...
      print(values.index(6))"
    hint: 힌트 문장을 완전하게 읽으면 `입력 [8,2,6,4]가 정렬되어 [2,4,6,8]이 됩니다` 단계에 필요한 표현이 `sorted(`입니다.
    explanation:
      빈칸에 들어갈 표현은 'sorted('입니다. `values = sorted([8, 2, 6, 4])` 줄을 완성해야 `정렬 전후 탐색 위치 구분` 동작이 이어져 실행 결과
      '2'가 됩니다. 힌트의 첫 단계 `입력 [8,2,6,4]가 정렬되어 [2,4,6,8]이 됩니다`이 바로 이 줄입니다. 이어서 6을 찾으면 인덱스 2입니다 순서로 진행됩니다.
    commonMistakes:
      - 원본도 sorted 호출만으로 바뀐다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-77-search-sort-review-guided
    ...
    explanation:
      바꾼 뒤 출력은 '2'입니다. 원본 출력 '2'와 같습니다. 바뀐 줄은 `values = sorted([9, 2, 6, 4])`입니다. 바뀐 프로그램도 `원본` 단계에서 시작해
      바뀐 줄을 지나 최종 `2`로 끝납니다. 6의 위치는 그대로 2라 결과가 같습니다. 출력은 같지만 바뀐 줄 부분의 중간 값은 달라졌다가 같은 최종 값으로 이어진다는 점을 위 추적 순서와 대조해
      확인할 수 있습니다.
    commonMistakes:
      - 원본도 sorted 호출만으로 바뀐다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-77-search-sort-review-debug
    ...
    answer: "values = sorted([8, 2, 6, 4])
      ...
      print(values.index(6))"
    hint:
      sorted는 새 정렬 리스트를 만들고 index는 그 리스트에서 목표의 위치를 찾습니다. 설명과 어긋나는 줄을 찾으세요. `원본도 sorted 호출만으로 바뀐다고 생각함` 상황이 단서가
      됩니다.
    explanation:
      틀린 줄은 `values = list([8, 2, 6, 4])`입니다. 여기서는 `list(`을 써서 `sorted(` 동작이 깨집니다. 이대로 실행하면 `원본도 sorted
      호출만으로 바뀐다고 생각함` 문제가 생겨 원본 추적 `원본  →  [2,4,6,8]  →  6은 2번`대로 '2'가 나오지 않습니다. 고친 줄 `values = sorted([8, 2, 6, 4])`에서는
      `sorted(`가 `정렬 전후 탐색 위치 구분` 동작을 지켜 '2'까지 도달합니다.
    commonMistakes:
      - 원본도 sorted 호출만으로 바뀐다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-77-search-sort-review-independent
    ...
    answer: "values = sorted([8, 2, 6, 4])
      ...
      print(values.index(6))"
    hint: sorted는 새 정렬 리스트를 만들고 index는 그 리스트에서 목표의 위치를 찾습니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `sorted(` 부분이 `정렬 전후 탐색 위치 구분` 동작을 지켜 실행 결과 '2'가 됩니다. 같은 개념을 다른 입력으로 바꿔도 `sorted(`부터
      `6은 2번`까지 추적할 수 있으면 정답입니다. `원본도 sorted 호출만으로 바뀐다고 생각함` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - 원본도 sorted 호출만으로 바뀐다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-77-search-sort-review-output
    ...
    choices:
      ...
      - "2"
    answerIndex: 2
    explanation: 원본  →  [2,4,6,8]  →  6은 2번 순서로 실행되어 출력은 '2'입니다. `sorted(` 부분이 마지막 단계를 확정하므로 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-77-search-sort-review-model
    ...
    answerIndex: 0
    explanation:
      sorted는 새 정렬 리스트를 만들고 index는 그 리스트에서 목표의 위치를 찾습니다. 이 설명이 맞는 이유는 `정렬 전후 탐색 위치 구분` 동작을 지키는 조건과 같기 때문입니다.
      `원본도 sorted 호출만으로 바뀐다고 생각함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-77-search-sort-review-pitfall
    ...
    answerIndex: 1
    explanation: 원본도 sorted 호출만으로 바뀐다고 생각함. 이 실수가 나오면 원본 추적 `원본 → [2,4,6,8] → 6은 2번`대로 '2'가 나오지 않으므로 먼저 확인해야 합니다.
  - id: quiz-day-77-search-sort-review-transfer
    ...
    answerIndex: 2
    explanation: 문법은 달라도 `정렬 전후 탐색 위치 구분` 목적과 입력·출력은 유지합니다. 실행 결과 '2'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
...
바꾼 뒤 출력도 `2`입니다. 6의 위치는 그대로 2라 결과가 같습니다. 바뀐 프로그램도 `원본` 단계에서 시작해 `values = sorted([9, 2, 6, 4])` 줄을 지나 최종 `2`로 끝납니다. 원본 추적 `원본  →  [2,4,6,8]  →  6은 2번`와 바뀐 줄 이후를 순서대로 놓으면 출력은 같아도 중간 값이 어디서 달라졌다가 합쳐지는지 확인할 수 있습니다.
