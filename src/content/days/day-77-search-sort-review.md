...
exercises:
  ...
  - id: ex-day-77-search-sort-review-guided
    ...
    prompt: 예시를 직접 타이핑한 뒤 처음 등장하는 숫자를 8에서 9로 바꿔 보세요. 출력도 먼저 예측하세요.
    starter: "values = sorted([8, 2, 6, 4])
    ...
    explanation: 예시 답안에서는 처음 등장하는 숫자를 8에서 9로 바꿨습니다. 원본 출력은 '2'입니다. 바뀐 코드의 결과는 실행해 확인하세요.
    commonMistakes:
      - 원본도 sorted 호출만으로 바뀐다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-77-search-sort-review-debug
    ...
    prompt: 아래 코드의 핵심 표현 'list('을 점검하고 올바른 작동을 복원하세요. 원본도 sorted 호출만으로 바뀐다고 생각함 오류도 함께 경계하세요.
...
탐색 알고리즘의 전제인 정렬 상태와 정렬 결과를 직접 점검합니다. 이번 Day는 새 문법을 많이 추가하는 대신 앞선 개념을 떠올리고 작은 변경 실험으로 오개념을 확인합니다. 코드를 가린 채 먼저 답하고, 모른 부분만 선행 Day로 돌아가 보세요.

## 2. 시작 전에 확인할 것

바로 앞선 [Day 76](/learn/day-76-divide-conquer)의 핵심을 한 문장으로 설명해 보세요. 모르면 위의 선행 Day 링크에서 다시 확인할 수 있습니다.

## 3. 머릿속 그림

sorted는 새 정렬 리스트를 만들고 index는 그 리스트에서 목표의 위치를 찾습니다. 다음 흐름을 눈으로 확인하세요.
...
예제의 핵심 표현은 `sorted(`입니다. 전체 코드에서 이 표현을 찾아 표시하세요. 선언과 조건, 출력이 연결되는 과정을 순서대로 설명합니다.
...
3. 처음 등장하는 숫자를 8에서 9로 바꾼 뒤 어느 단계부터 결과가 달라질지 예측하세요.
4. 실행할 수 있는 환경에서 확인하고 틀린 예측의 이유를 한 문장으로 적으세요.

## 8. 자주 틀리는 지점

**확인할 실수: 원본도 sorted 호출만으로 바뀐다고 생각함.** sorted는 새 정렬 리스트를 만들고 index는 그 리스트에서 목표의 위치를 찾습니다. 결과가 예상과 다르면 입력 → 중간 상태 → 출력 중 처음 어긋난 곳을 찾습니다.

## 9. 다른 언어로 옮기기

같은 문제를 해결하더라도 선언, 타입, 메모리 관리, 오류 처리는 다릅니다. 핵심 동작은 **정렬 전후 탐색 위치 구분**입니다. 언어를 옮겨도 이 동작을 유지하세요.
...
- 왜 이 개념이 필요한가? 탐색 알고리즘의 전제인 정렬 상태와 정렬 결과를 직접 점검합니다.
- 예시에서 가장 먼저 확정되는 값이나 상태는 무엇인가?
- 어떤 실수를 점검해야 하나? 원본도 sorted 호출만으로 바뀐다고 생각함
- 다른 두 언어에서는 같은 동작을 어떤 자료형과 오류 처리로 나타내는가?

## 12. 핵심 요약과 복습

sorted는 새 정렬 리스트를 만들고 index는 그 리스트에서 목표의 위치를 찾습니다. 예시의 출력은 `2`입니다. 오류를 찾을 때는 **원본도 sorted 호출만으로 바뀐다고 생각함** 여부를 확인하세요. 완료 버튼을 누르면 +1·+3·+7·+14·+30일 복습이 이 기기에 등록됩니다.
