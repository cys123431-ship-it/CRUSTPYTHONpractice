...
여러 학습 기록에 같은 데이터 형식과 동작을 적용하려면 클래스로 설계합니다. Day 51 "`Python 클래스와 인스턴스`"에서는 Python 코드가 `30`을(를) 만드는 과정을 따라가며, 인스턴스 속성 초기화 동작이 왜 필요한지 확인합니다. 인스턴스 속성 초기화을(를) 빠뜨리면 `self를 빼고 지역 변수만 저장해 객체에 값이 남지 않음` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 50](/learn/day-50-c-struct)에서는 "C 구조체로 관련 값 묶기"을(를) 배웠습니다. "C 구조체로 관련 값 묶기"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`Python 클래스와 인스턴스`"에서 새로 달라지는 조건을 찾아보세요. Day 51의 답은 `30`이며, 핵심 표현은 `self.minutes`입니다.

## 머릿속 그림

클래스는 객체의 설계도이고 self는 지금 만들고 있는 바로 그 객체입니다. Day 51에서는 아래 흐름 순서대로 상태가 바뀌며, `self.minutes`이(가) 결과를 가릅니다.

```text
클래스 정의  →  s 생성  →  s.minutes=30  →  출력
```

위 흐름에서 `self.minutes`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `30`입니다.
...
아래 Python 코드는 Day 51 "`Python 클래스와 인스턴스`"의 독립 예제입니다. 전체 5줄에서 `self.minutes`이(가) 핵심이며, 실행 결과는 `30`입니다.
...
위 단계에서 `self.minutes`이(가) 빠지면 `30`이(가) 나오지 않습니다. `처음 등장하는 숫자를 30에서 31로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `30`이(가) 됩니다. `클래스 정의` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `30`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `self.minutes`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 51의 `Python 클래스와 인스턴스`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: self를 빼고 지역 변수만 저장해 객체에 값이 남지 않음.** 정상 코드에서는 `30`이(가) 출력됩니다.

- 정상 줄: `self.minutes = minutes`
- 잘못된 줄: `minutes = minutes`

두 줄을 나란히 놓고 `클래스는 객체의 설계도이고 self는 지금 만들고 있는 바로 그 객체입니다.` 기준으로 어느 쪽이 맞는지 설명하세요. 결과가 예상과 다르면 `클래스 정의`부터 `30`까지 처음 어긋난 곳을 찾습니다.

## 다른 언어로 옮기기

핵심 동작 **인스턴스 속성 초기화**은(는) 세 언어에서 같은 입력과 출력(`30`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `struct와 초기화 함수` 규칙으로 "`Python 클래스와 인스턴스`"의 `30`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `__init__(self)와 self.field` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `struct와 impl 생성 함수` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`Python 클래스와 인스턴스`"이(가) 필요한 상황을 `인스턴스 속성 초기화` 동작으로 설명해 보세요.
- 예제에서 `self.minutes`이(가) 실행되기 직전의 상태와 직후의 출력 `30`을(를) 말해 보세요.
- "`self를 빼고 지역 변수만 저장해 객체에 값이 남지 않음`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `인스턴스 속성 초기화` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

클래스는 객체의 설계도이고 self는 지금 만들고 있는 바로 그 객체입니다. Day 51 "`Python 클래스와 인스턴스`"의 예제는 `self.minutes`을(를) 실행해 `30`을(를) 출력합니다. "`self를 빼고 지역 변수만 저장해 객체에 값이 남지 않음`" 여부를 확인하고 Day 51을(를) 완료하세요.
