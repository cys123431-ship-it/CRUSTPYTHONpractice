...
exercises:
  - id: ex-day-51-python-class-predict
    ...
    starter:
      ...
      print(s.minutes)"
    answer: "30"
    hint: 클래스는 객체의 설계도이고 self는 지금 만들고 있는 바로 그 객체입니다. 설명을 떠올리고 `클래스 정의` 단계부터 순서대로 적어 보세요.
    explanation:
      클래스 정의  →  s 생성  →  s.minutes=30  →  출력 순서로 실행됩니다. `self.minutes` 부분이 `출력` 단계를 확정해 최종 출력 '30'가 됩니다.
      이 흐름을 떠올리면 `인스턴스 속성 초기화` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-51-python-class-fill
    ...
    hint: 힌트 문장을 완전하게 읽으면 `Session(30)이 새 인스턴스를 만듭니다` 단계에 필요한 표현이 `self.minutes`입니다.
    explanation:
      빈칸에 들어갈 표현은 'self.minutes'입니다. `self.minutes = minutes` 줄을 완성해야 `인스턴스 속성 초기화` 동작이 이어져 실행 결과 '30'가
      됩니다. 힌트의 첫 단계 `Session(30)이 새 인스턴스를 만듭니다`이 바로 이 줄입니다. 이어서 __init__에서 그 객체의 minutes에 30을 저장합니다 s.minutes로 읽습니다
      순서로 진행됩니다.
    commonMistakes:
      - self를 빼고 지역 변수만 저장해 객체에 값이 남지 않음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-51-python-class-guided
    ...
    explanation:
      바꾼 뒤 출력은 '31'입니다. 원본 출력 '30'에서 달라졌습니다. 바뀐 줄은 `s = Session(30)`에서 `s = Session(31)`로 바뀌었습니다. 바뀐 프로그램은
      `클래스 정의` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `31`가 됩니다. 원본 추적 `클래스 정의  →  s 생성  →  s.minutes=30  →  출력`와
  ...
  - id: ex-day-51-python-class-debug
    ...
    answer: "class Session:\n    def __init__(self, minutes):\n        self.minutes = minutes\ns = Session(30)\nprint(s.minutes)"
    hint:
      클래스는 객체의 설계도이고 self는 지금 만들고 있는 바로 그 객체입니다. 설명과 어긋나는 줄을 찾으세요. `self를 빼고 지역 변수만 저장해 객체에 값이 남지 않음` 상황이 단서가
      됩니다.
    explanation:
      틀린 줄은 `minutes = minutes`입니다. 여기서는 `minutes`을 써서 `self.minutes` 동작이 깨집니다. 이대로 실행하면 `self를 빼고 지역 변수만
      저장해 객체에 값이 남지 않음` 문제가 생겨 원본 추적 `클래스 정의  →  s 생성  →  s.minutes=30  →  출력`대로 '30'가 나오지 않습니다. 고친 줄 `self.minutes
      = minutes`에서는 `self.minutes`가 `인스턴스 속성 초기화` 동작을 지켜 '30'까지 도달합니다.
    commonMistakes:
      - self를 빼고 지역 변수만 저장해 객체에 값이 남지 않음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-51-python-class-independent
    ...
    answer: "class Session:\n    def __init__(self, minutes):\n        self.minutes = minutes\ns = Session(30)\nprint(s.minutes)"
    hint: 클래스는 객체의 설계도이고 self는 지금 만들고 있는 바로 그 객체입니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `self.minutes` 부분이 `인스턴스 속성 초기화` 동작을 지켜 실행 결과 '30'가 됩니다. 같은 개념을 다른 입력으로 바꿔도
      `self.minutes`부터 `출력`까지 추적할 수 있으면 정답입니다. `self를 빼고 지역 변수만 저장해 객체에 값이 남지 않음` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - self를 빼고 지역 변수만 저장해 객체에 값이 남지 않음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-51-python-class-output
    ...
    answerIndex: 0
    explanation:
      클래스 정의  →  s 생성  →  s.minutes=30  →  출력 순서로 실행되어 출력은 '30'입니다. `self.minutes` 부분이 마지막 단계를 확정하므로 다른
      선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-51-python-class-model
    ...
    answerIndex: 1
    explanation:
      클래스는 객체의 설계도이고 self는 지금 만들고 있는 바로 그 객체입니다. 이 설명이 맞는 이유는 `인스턴스 속성 초기화` 동작을 지키는 조건과 같기 때문입니다. `self를
      빼고 지역 변수만 저장해 객체에 값이 남지 않음` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-51-python-class-pitfall
    ...
    answerIndex: 2
    explanation:
      self를 빼고 지역 변수만 저장해 객체에 값이 남지 않음. 이 실수가 나오면 원본 추적 `클래스 정의 → s 생성 → s.minutes=30 → 출력`대로 '30'가 나오지
      않으므로 먼저 확인해야 합니다.
  - id: quiz-day-51-python-class-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 0
    explanation: 문법은 달라도 `인스턴스 속성 초기화` 목적과 입력·출력은 유지합니다. 실행 결과 '30'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
playgroundSource:
  "class Session:\n    def __init__(self, minutes):\n        self.minutes = minutes\ns = Session(30)\n\
  print(s.minutes)"
---
...
예제의 핵심 표현은 `self.minutes`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄을 찾으세요.
...
원본 출력은 `30`이고, 바뀐 코드의 실행 결과는 `31`입니다. 바뀐 줄은 `s = Session(30)`에서 `s = Session(31)`로 바뀌었습니다. 바뀐 프로그램은 `클래스 정의` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `31`가 됩니다. 원본 추적 `클래스 정의  →  s 생성  →  s.minutes=30  →  출력`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
