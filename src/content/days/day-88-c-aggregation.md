...
C 구현에서도 공통 fixture의 전체 공부 시간과 필터별 시간을 정확히 맞춰야 합니다. Day 88 "`C 레코드 집계와 경계 검사`"에서는 C 코드가 `60`을(를) 만드는 과정을 따라가며, 레코드 시간 합계 누적 동작이 왜 필요한지 확인합니다. 레코드 시간 합계 누적을(를) 빠뜨리면 `배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 87](/learn/day-87-c-csv-parse)에서는 "C 입력 검증과 정수 변환"을(를) 배웠습니다. "C 입력 검증과 정수 변환"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`C 레코드 집계와 경계 검사`"에서 새로 달라지는 조건을 찾아보세요. Day 88의 답은 `60`이며, 핵심 표현은 `i < 3`입니다.

## 머릿속 그림

현재 합계 total에 각 유효 인덱스의 값을 한 번씩 더합니다. 마지막 유효 인덱스는 2입니다. Day 88에서는 아래 흐름 순서대로 상태가 바뀌며, `i < 3`이(가) 결과를 가릅니다.

```text
0  →  30  →  50  →  60
```

위 흐름에서 `i < 3`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `60`입니다.
...
아래 C 코드는 Day 88 "`C 레코드 집계와 경계 검사`"의 독립 예제입니다. 전체 8줄에서 `i < 3`이(가) 핵심이며, 실행 결과는 `60`입니다.
...
C17은 `gcc -std=c17 -Wall -Wextra`로 컴파일해 실행하면 `60`이(가) 나옵니다. 브라우저에서는 임의 컴파일 대신 하단 실습 칸의 답안 비교를 사용하세요.
...
위 단계에서 `i < 3`이(가) 빠지면 `60`이(가) 나오지 않습니다. `처음 등장하는 숫자를 30에서 31로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `60`이(가) 됩니다. `0` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `60`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `i < 3`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 88의 `C 레코드 집계와 경계 검사`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음.** 정상 코드에서는 `60`이(가) 출력됩니다.
...
핵심 동작 **레코드 시간 합계 누적**은(는) 세 언어에서 같은 입력과 출력(`60`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `정수 overflow 검사와 입력 배열 길이` 규칙으로 "`C 레코드 집계와 경계 검사`"의 `60`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `sum(row.minutes for row in rows)` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `checked_add로 합계 overflow 처리` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`60` 맞히기) → 빈칸(`i < 3` 채우기) → 변경(`처음 등장하는 숫자를 30에서 31로`) → 오류 수정(`배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음` 찾기) → 독립 구현(`C 레코드 집계와 경계 검사을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `60` 및 `i < 3` 설명과 대조하세요.

## 스스로 설명하기

- "`C 레코드 집계와 경계 검사`"이(가) 필요한 상황을 `레코드 시간 합계 누적` 동작으로 설명해 보세요.
- 예제에서 `i < 3`이(가) 실행되기 직전의 상태와 직후의 출력 `60`을(를) 말해 보세요.
- "`배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `레코드 시간 합계 누적` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

현재 합계 total에 각 유효 인덱스의 값을 한 번씩 더합니다. 마지막 유효 인덱스는 2입니다. Day 88 "`C 레코드 집계와 경계 검사`"의 예제는 `i < 3`을(를) 실행해 `60`을(를) 출력합니다. "`배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음`" 여부를 확인하고 Day 88을(를) 완료하세요.
