...
exercises:
  - id: ex-day-88-c-aggregation-predict
    ...
    answer: "60"
    hint: 현재 합계 total에 각 유효 인덱스의 값을 한 번씩 더합니다. 마지막 유효 인덱스는 2입니다. 설명을 떠올리고 `0` 단계부터 순서대로 적어 보세요.
    explanation:
      0  →  30  →  50  →  60 순서로 실행됩니다. `i < 3` 부분이 `60` 단계를 확정해 최종 출력 '60'가 됩니다. 이 흐름을 떠올리면 `레코드 시간 합계
      누적` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-88-c-aggregation-fill
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int minutes[] = {30, 20, 10};\n    int total = 0;\n    for\
      \ (int i = 0; i < 3; i++) total += minutes[i];\n    printf(\"%d\\n\", total);\n    return 0;\n}"
    hint: 힌트 문장을 완전하게 읽으면 `total=0입니다` 단계에 필요한 표현이 `i < 3`입니다.
    explanation:
      빈칸에 들어갈 표현은 'i < 3'입니다. `for (int i = 0; i < 3; i++) total += minutes[i];` 줄을 완성해야 `레코드 시간 합계 누적`
      동작이 이어져 실행 결과 '60'가 됩니다. 힌트의 첫 단계 `total=0입니다`이 바로 이 줄입니다. 이어서 30,20,10을 순서대로 누적합니다 합계 60을 출력합니다 순서로 진행됩니다.
    commonMistakes:
      - 배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-88-c-aggregation-guided
    ...
    explanation:
      바꾼 뒤 출력은 '61'입니다. 원본 출력 '60'에서 달라졌습니다. 바뀐 줄은 `int minutes[] = {30, 20, 10};`에서 `int minutes[] = {31,
      20, 10};`로 바뀌었습니다. 바뀐 프로그램은 `0` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `61`가 됩니다. 원본 추적 `0  →  30  →  50  →  60`와
  ...
  - id: ex-day-88-c-aggregation-debug
    ...
    answer:
      ...
      \ (int i = 0; i < 3; i++) total += minutes[i];\n    printf(\"%d\\n\", total);\n    return 0;\n}"
    hint:
      현재 합계 total에 각 유효 인덱스의 값을 한 번씩 더합니다. 마지막 유효 인덱스는 2입니다. 설명과 어긋나는 줄을 찾으세요. `배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음`
      상황이 단서가 됩니다.
    explanation:
      틀린 줄은 `for (int i = 0; i <= 3; i++) total += minutes[i];`입니다. 여기서는 `i <= 3`을 써서 `i < 3` 동작이 깨집니다.
      이대로 실행하면 `배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음` 문제가 생겨 원본 추적 `0  →  30  →  50  →  60`대로 '60'가 나오지 않습니다. 고친 줄 `for
      (int i = 0; i < 3; i++) total += minutes[i];`에서는 `i < 3`가 `레코드 시간 합계 누적` 동작을 지켜 '60'까지 도달합니다.
    commonMistakes:
      - 배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-88-c-aggregation-independent
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int minutes[] = {30, 20, 10};\n    int total = 0;\n    for\
      \ (int i = 0; i < 3; i++) total += minutes[i];\n    printf(\"%d\\n\", total);\n    return 0;\n}"
    hint: 현재 합계 total에 각 유효 인덱스의 값을 한 번씩 더합니다. 마지막 유효 인덱스는 2입니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `i < 3` 부분이 `레코드 시간 합계 누적` 동작을 지켜 실행 결과 '60'가 됩니다. 같은 개념을 다른 입력으로 바꿔도 `i < 3`부터
      `60`까지 추적할 수 있으면 정답입니다. `배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - 배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-88-c-aggregation-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 1
    explanation: 0  →  30  →  50  →  60 순서로 실행되어 출력은 '60'입니다. `i < 3` 부분이 마지막 단계를 확정하므로 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-88-c-aggregation-model
    ...
    answerIndex: 2
    explanation:
      현재 합계 total에 각 유효 인덱스의 값을 한 번씩 더합니다. 마지막 유효 인덱스는 2입니다. 이 설명이 맞는 이유는 `레코드 시간 합계 누적` 동작을 지키는 조건과 같기
      때문입니다. `배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-88-c-aggregation-pitfall
    ...
    answerIndex: 0
    explanation: 배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음. 이 실수가 나오면 원본 추적 `0 → 30 → 50 → 60`대로 '60'가 나오지 않으므로 먼저 확인해야 합니다.
  - id: quiz-day-88-c-aggregation-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 1
    explanation: 문법은 달라도 `레코드 시간 합계 누적` 목적과 입력·출력은 유지합니다. 실행 결과 '60'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
원본 출력은 `60`이고, 바뀐 코드의 실행 결과는 `61`입니다. 바뀐 줄은 `int minutes[] = {30, 20, 10};`에서 `int minutes[] = {31, 20, 10};`로 바뀌었습니다. 바뀐 프로그램은 `0` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `61`가 됩니다. 원본 추적 `0  →  30  →  50  →  60`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
