...
exercises:
  - id: ex-day-88-c-aggregation-predict
    ...
    answer: "60"
    hint: 0에서 시작해 60까지 순서대로 적어 보세요. 현재 합계 total에 각 유효 인덱스의 값을 한 번씩 더합니다. 마지막 유효 인덱스는 2입니다.
    explanation:
      1. total=0입니다. 2. 30,20,10을 순서대로 누적합니다. 3. 합계 60을 출력합니다. `0 → 30 → 50 → 60` 흐름으로 실제 출력 `60`이 됩니다.
      핵심 `i < 3`은 유효 번호 0,1,2만 방문하게 하는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-88-c-aggregation-fill
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int minutes[] = {30, 20, 10};\n    int total = 0;\n    for\
      \ (int i = 0; i < 3; i++) total += minutes[i];\n    printf(\"%d\\n\", total);\n    return 0;\n}"
    hint: 필요한 표현은 레코드 시간 합계 누적 동작을 잇는 i < 3입니다.
    explanation:
      빈칸에 들어갈 표현은 'i < 3'입니다. 이 표현이 없으면 정의되지 않은 동작이 되어 어떤 결과도 보장할 수 없습니다. 1. total=0입니다. 2. 30,20,10을 순서대로
      누적합니다. 3. 합계 60을 출력합니다. `0 → 30 → 50 → 60` 흐름으로 실제 출력 `60`이 됩니다. 핵심 `i < 3`은 유효 번호 0,1,2만 방문하게 하는 자리에 쓰입니다.
    commonMistakes:
      - 배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-88-c-aggregation-guided
    ...
    answer:
      ...
      \ (int i = 0; i < 3; i++) total += minutes[i];\n    printf(\"%d\\n\", total);\n    return 0;\n}"
    hint: 처음 등장하는 숫자를 30에서 31로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '60'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation: 바꾼 뒤 출력은 '61'입니다. 원본은 `60`, 수정본은 `61`이다. 첫 변경 줄의 첫 값 30에서 31로 바꾸면, 이후 누적에 그대로 더해져 최종 합이 1만큼 커진다.
    commonMistakes:
      - 배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-88-c-aggregation-debug
    ...
    answer:
      ...
      \ (int i = 0; i < 3; i++) total += minutes[i];\n    printf(\"%d\\n\", total);\n    return 0;\n}"
    hint:
      배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음 상황에서 어긋나는 줄을 현재 합계 total에 각 유효 인덱스의 값을 한 번씩 더합니다. 마지막 유효 인덱스는 2입니다. 설명과
      대조해 보세요.
    explanation:
      틀린 줄은 `for (int i = 0; i <= 3; i++) total += minutes[i];`입니다. 마지막 반복에서 네 번째 원소를 읽으므로 C의 정의되지 않은 동작이
      됩니다. 합계가 특정 숫자가 된다고 단정하지 않습니다. 어떤 값이 보일 수도 있고 비정상 종료될 수도 있으며, 어떤 결과도 보장할 수 없습니다. 오류 분류는 정의되지 않은 동작입니다. 유효 인덱스는
      0,1,2뿐입니다. 고친 줄 `for (int i = 0; i < 3; i++) total += minutes[i];`에서는 `레코드 시간 합계 누적` 동작이 지켜집니다. 정상 코드는 0 시작→30→50→60
      누적→출력의 순서로 `60`을 출력합니다.
    commonMistakes:
      - 배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-88-c-aggregation-independent
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int minutes[] = {30, 20, 10};\n    int total = 0;\n    for\
      \ (int i = 0; i < 3; i++) total += minutes[i];\n    printf(\"%d\\n\", total);\n    return 0;\n}"
    hint: 잘못된 분을 건너뛸지 오류로 종료할지는 공통 계약에 맞추세요. 포인터와 길이를 항상 함께 보세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 잘못된 분을 건너뛸지 오류로 종료할지는 공통 계약에 맞추세요. 포인터와 길이를 항상 함께 보세요. 같은 레코드 시간 합계 누적
      동작을 구현하고 실행 결과 '60'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-88-c-aggregation-output
    ...
    answerIndex: 1
    explanation:
      먼저 total이 0에서 시작됩니다. 이어서 30,20,10을 순서대로 누적해 60이 되고, 출력하므로 `60`이 됩니다. `실행 전에 반드시 오류가 난다`는 틀린 선택지인데
      유효 범위 접근이라 컴파일과 실행이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 printf가 실제로 호출되기 때문입니다. 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-88-c-aggregation-model
    ...
    answerIndex: 2
    explanation:
      현재 합계 total에 각 유효 인덱스의 값을 한 번씩 더한다는 뜻이고, 마지막 유효 인덱스는 2라는 뜻은 쓸 수 있는 번호가 0,1,2뿐이라는 뜻입니다. `배열 길이와 인덱스의
      경계를 혼동해 원소를 하나 더 읽음`은 반대 사례인데, 하나 더 읽으면 범위 밖이라 정의되지 않은 동작이 되기 때문입니다.
  - id: quiz-day-88-c-aggregation-pitfall
    ...
    answerIndex: 0
    explanation:
      먼저 확인할 실수는 `배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음`입니다. 범위 밖 읽기는 정의되지 않은 동작이므로, 합계가 특정 숫자가 된다고 약속하지 마세요.
      길이와 인덱스를 함께 전달하세요.
  - id: quiz-day-88-c-aggregation-transfer
    ...
    answerIndex: 1
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `레코드 시간 합계 누적`이라는 의미와 입력·출력 계약입니다. Python의 합산과 Rust의 iterator 합으로 같은 동작을 구현하고,
...
예제의 핵심 표현은 `i < 3`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본은 `60`, 수정본은 `61`이다. 첫 변경 줄의 첫 값 30에서 31로 바꾸면, 이후 누적에 그대로 더해져 최종 합이 1만큼 커진다.
...
두 줄을 나란히 놓으면 정상 줄 `for (int i = 0; i < 3; i++) total += minutes[i];`이 `현재 합계 total에 각 유효 인덱스의 값을 한 번씩 더합니다. 마지막 유효 인덱스는 2입니다.` 설명과 맞고, 잘못된 줄은 `배열 길이와 인덱스의 경계를 혼동해 원소를 하나 더 읽음` 쪽으로 어긋납니다. 결과가 예상과 다르면 `0`부터 `60`까지 처음 어긋난 곳을 찾습니다.
