...
exercises:
  ...
  - id: ex-day-75-partition-fill
    ...
    explanation:
      빈칸에 들어갈 표현은 'v >= pivot'입니다. 원소가 하나면 그대로 반환합니다 피벗 4 아래에 1,3,2,3이 모입니다 다음 피벗 1·3이 각각 부분을 나눕니다 재귀 결과를
      합쳐 중복 3을 모두 유지합니다
    commonMistakes:
      - 피벗과 같은 값을 두 그룹 모두에서 제외해 중복 원소를 잃음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-75-partition-guided
    ...
    explanation:
      "바꾼 뒤 출력은 '[1, 2, 3, 3, 4]'입니다. 원본 출력과 같습니다('[1, 2, 3, 3, 4]'). 길이 3인 부분도 그대로 분할되므로 정렬 결과가 같습니다.
      출력이 같아도 바뀐 줄(if len(values) < 3: return values) 이후의 중간 상태가 같은지는 원본 추적과 대조해야 합니다."
    commonMistakes:
      - 피벗과 같은 값을 두 그룹 모두에서 제외해 중복 원소를 잃음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-75-partition-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 0
    explanation: "[4,1,3,2,3]  →  피벗4  →  피벗3  →  [1,2,3,3,4] 순서로 실행되어 출력은 '[1, 2, 3, 3, 4]'입니다."
...
바꾼 뒤 출력도 `[1, 2, 3, 3, 4]`입니다. 길이 3인 부분도 그대로 분할되므로 정렬 결과가 같습니다. 출력이 같다고 해서 중간 상태까지 같은 것은 아닙니다. 다른 줄(`if len(values) < 3: return values`)부터 원본 추적과 비교해 보세요.
