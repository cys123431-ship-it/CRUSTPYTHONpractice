...
exercises:
  - id: ex-day-75-partition-predict
    ...
    answer: "[1, 2, 3, 3, 4]"
    hint:
      "[4,1,3,2,3]에서 시작해 [1,2,3,3,4]까지 순서대로 적어 보세요. 첫 값을 피벗으로 뽑고 나머지를 작은 값과 이상인 값으로 나눕니다. 각 부분을 다시 정렬해 피벗 양쪽에
      붙입니다."
    explanation:
      1. 원소가 하나면 그대로 반환합니다. 2. 피벗 4 아래에 1,3,2,3이 모입니다. 3. 다음 피벗 1·3이 각각 부분을 나눕니다. 4. 재귀 결과를 합쳐 중복 3을 모두
      유지합니다. `[4,1,3,2,3] → 피벗4 → 피벗3 → [1,2,3,3,4]` 흐름으로 실제 출력 `[1, 2, 3, 3, 4]`가 됩니다. 핵심 `v >= pivot`은 같은 값을 큰 쪽에
      남기는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-75-partition-fill
    ...
    answer:
      ...
      \ for v in values[1:] if v < pivot]\n    more = [v for v in values[1:] if v >= pivot]\n    return quick_sort(less)\
      \ + [pivot] + quick_sort(more)\nprint(quick_sort([4, 1, 3, 2, 3]))"
    hint: 필요한 표현은 피벗 기준 분할 후 재귀적으로 정렬 동작을 잇는 v >= pivot입니다.
    explanation:
      빈칸에 들어갈 표현은 'v >= pivot'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. 원소가 하나면 그대로 반환합니다. 2. 피벗 4 아래에 1,3,2,3이 모입니다.
      3. 다음 피벗 1·3이 각각 부분을 나눕니다. 4. 재귀 결과를 합쳐 중복 3을 모두 유지합니다. `[4,1,3,2,3] → 피벗4 → 피벗3 → [1,2,3,3,4]` 흐름으로 실제 출력 `[1,
      2, 3, 3, 4]`가 됩니다. 핵심 `v >= pivot`은 같은 값을 큰 쪽에 남기는 자리에 쓰입니다.
    commonMistakes:
      - 피벗과 같은 값을 두 그룹 모두에서 제외해 중복 원소를 잃음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-75-partition-guided
    ...
    answer:
      ...
      \ for v in values[1:] if v < pivot]\n    more = [v for v in values[1:] if v >= pivot]\n    return quick_sort(less)\
      \ + [pivot] + quick_sort(more)\nprint(quick_sort([4, 1, 3, 2, 3]))"
    hint: 처음 등장하는 숫자를 2에서 3로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '[1, 2, 3, 3, 4]'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '[1, 2, 3, 3, 4]'입니다. 원본과 수정본 모두 `[1, 2, 3, 3, 4]`를 출력한다. 길이 3인 부분도 그대로 분할되므로 정렬 결과가 같습니다.
      기저 조건만 달라지고 분할은 그대로이므로 중간 상태가 달라졌다고 쓰지 않는다.
    commonMistakes:
      - 피벗과 같은 값을 두 그룹 모두에서 제외해 중복 원소를 잃음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-75-partition-debug
    ...
    answer:
      ...
      \ + [pivot] + quick_sort(more)\nprint(quick_sort([4, 1, 3, 2, 3]))"
    hint:
      피벗과 같은 값을 두 그룹 모두에서 제외해 중복 원소를 잃음 상황에서 어긋나는 줄을 첫 값을 피벗으로 뽑고 나머지를 작은 값과 이상인 값으로 나눕니다. 각 부분을 다시 정렬해 피벗 양쪽에
      붙입니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `more = [v for v in values[1:] if v > pivot]`입니다. 피벗과 같은 값이 어느 쪽에도 들어가지 못해 중복 3 하나가 사라지고 `[1,
      2, 3, 4]`가 됩니다. 오류 분류는 잘못된 출력입니다. 값이 사라지는 첫 순간은 같은 값의 분류입니다. 고친 줄 `more = [v for v in values[1:] if v >= pivot]`에서는
      `피벗 기준 분할 후 재귀적으로 정렬` 동작이 지켜집니다. 정상 코드는 피벗 선택→양쪽 분할→재귀 정렬→합치기의 순서로 `[1, 2, 3, 3, 4]`를 출력합니다.
    commonMistakes:
      - 피벗과 같은 값을 두 그룹 모두에서 제외해 중복 원소를 잃음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-75-partition-independent
    ...
    answer:
      ...
      \ for v in values[1:] if v < pivot]\n    more = [v for v in values[1:] if v >= pivot]\n    return quick_sort(less)\
      \ + [pivot] + quick_sort(more)\nprint(quick_sort([4, 1, 3, 2, 3]))"
    hint: 빈 부분과 한 원소 부분에서 재귀가 끝나는지 확인하세요. 피벗이 매번 최솟값이면 최악 `O(n²)`이 됩니다. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 빈 부분과 한 원소 부분에서 재귀가 끝나는지 확인하세요. 피벗이 매번 최솟값이면 최악 `O(n²)`이 됩니다. 같은 피벗 기준
      분할 후 재귀적으로 정렬 동작을 구현하고 실행 결과 '[1, 2, 3, 3, 4]'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 피벗과 같은 값을 두 그룹 모두에서 제외해 중복 원소를 잃음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-75-partition-output
    ...
    answerIndex: 0
    explanation:
      먼저 피벗 4 아래에 1,3,2,3이 모입니다. 이어서 다음 피벗들이 부분을 나누고, 재귀 결과를 합쳐 중복 3을 모두 유지하므로 출력 `[1, 2, 3, 3, 4]`가 됩니다.
      `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 분할과 재귀가 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기 때문입니다. 다른 선택지는
      이 추적과 맞지 않습니다.
  - id: quiz-day-75-partition-model
    ...
    answerIndex: 1
    explanation:
      첫 값을 피벗으로 뽑고 나머지를 작은 값과 이상인 값으로 나눈다는 뜻은, 같은 값을 큰 쪽에 남긴다는 뜻입니다. `피벗과 같은 값을 두 그룹 모두에서 제외해 중복 원소를 잃음`은
      반대 사례인데, 어느 쪽에도 못 들어가면 값이 사라지기 때문입니다.
  - id: quiz-day-75-partition-pitfall
    ...
    answerIndex: 2
    explanation:
      먼저 확인할 실수는 `피벗과 같은 값을 두 그룹 모두에서 제외해 중복 원소를 잃음`입니다. 잘못된 코드는 실행은 되지만 `[1, 2, 3, 4]`라는 잘못된 출력을 냅니다.
      같은 값의 행방을 먼저 확인하세요.
  - id: quiz-day-75-partition-transfer
    ...
    answerIndex: 0
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `피벗 기준 분할 후 재귀적으로 정렬`이라는 의미와 입력·출력 계약입니다. C의 제자리 분할과 Rust의 분할 수집으로 같은 동작을 구현하고,
      실행 결과 `[1, 2, 3, 3, 4]`로 대조하면 옮김이 맞는지 확인할 수 있습니다.
playgroundSource:
  "def quick_sort(values):\n    if len(values) < 2: return values\n    pivot = values[0]\n    less\
...
예제의 핵심 표현은 `v >= pivot`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본 실행 결과는 `[1, 2, 3, 3, 4]`입니다. 아래 코드에서 원본과 달라지는 첫 줄을 표시하고, 그 줄의 변경 전후 값을 적어 보세요.
...
원본과 수정본 모두 `[1, 2, 3, 3, 4]`를 출력한다. 길이 3인 부분도 그대로 분할되므로 정렬 결과가 같습니다. 기저 조건만 달라지고 분할은 그대로이므로 중간 상태가 달라졌다고 쓰지 않는다.
