...
퀵 정렬은 기준보다 작은 구역과 나머지를 나누는 분할 연산이 핵심입니다. Day 75 "`퀵 정렬의 분할 기준`"에서는 Python 코드가 `[1, 2, 3, 3, 4]`을(를) 만드는 과정을 따라가며, 피벗 기준 분할 후 재귀적으로 정렬 동작이 왜 필요한지 확인합니다. 피벗 기준 분할 후 재귀적으로 정렬을(를) 빠뜨리면 `피벗과 같은 값을 두 그룹 모두에서 제외해 중복 원소를 잃음` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 74](/learn/day-74-merge-sort)에서는 "병합 정렬의 분할·병합"을(를) 배웠습니다. "병합 정렬의 분할·병합"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`퀵 정렬의 분할 기준`"에서 새로 달라지는 조건을 찾아보세요. Day 75의 답은 `[1, 2, 3, 3, 4]`이며, 핵심 표현은 `v >= pivot`입니다.

## 머릿속 그림

첫 값을 피벗으로 뽑고 나머지를 작은 값과 이상인 값으로 나눕니다. 각 부분을 다시 정렬해 피벗 양쪽에 붙입니다. Day 75에서는 아래 흐름 순서대로 상태가 바뀌며, `v >= pivot`이(가) 결과를 가릅니다.

```text
[4,1,3,2,3]  →  피벗4  →  피벗3  →  [1,2,3,3,4]
```

위 흐름에서 `v >= pivot`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `[1, 2, 3, 3, 4]`입니다.
...
아래 Python 코드는 Day 75 "`퀵 정렬의 분할 기준`"의 독립 예제입니다. 전체 7줄에서 `v >= pivot`이(가) 핵심이며, 실행 결과는 `[1, 2, 3, 3, 4]`입니다.
...
하단 Python 실행 영역에 같은 코드가 들어 있습니다. 먼저 `[1, 2, 3, 3, 4]`을(를) 가리고 예측한 뒤 실행해 비교하세요.
...
위 단계에서 `v >= pivot`이(가) 빠지면 `[1, 2, 3, 3, 4]`이(가) 나오지 않습니다. `처음 등장하는 숫자를 2에서 3로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `[1, 2, 3, 3, 4]`이(가) 됩니다. `[4,1,3,2,3]` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `[1, 2, 3, 3, 4]`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `v >= pivot`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 75의 `퀵 정렬의 분할 기준`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 피벗과 같은 값을 두 그룹 모두에서 제외해 중복 원소를 잃음.** 정상 코드에서는 `[1, 2, 3, 3, 4]`이(가) 출력됩니다.
...
핵심 동작 **피벗 기준 분할 후 재귀적으로 정렬**은(는) 세 언어에서 같은 입력과 출력(`[1, 2, 3, 3, 4]`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `인덱스 교환으로 제자리 분할` 규칙으로 "`퀵 정렬의 분할 기준`"의 `[1, 2, 3, 3, 4]`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `학습용 새 리스트 분할, 평균 O(n log n)·최악 O(n²)` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `partition 또는 두 Vec로 분할` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`퀵 정렬의 분할 기준`"이(가) 필요한 상황을 `피벗 기준 분할 후 재귀적으로 정렬` 동작으로 설명해 보세요.
- 예제에서 `v >= pivot`이(가) 실행되기 직전의 상태와 직후의 출력 `[1, 2, 3, 3, 4]`을(를) 말해 보세요.
- "`피벗과 같은 값을 두 그룹 모두에서 제외해 중복 원소를 잃음`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `피벗 기준 분할 후 재귀적으로 정렬` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

첫 값을 피벗으로 뽑고 나머지를 작은 값과 이상인 값으로 나눕니다. 각 부분을 다시 정렬해 피벗 양쪽에 붙입니다. Day 75 "`퀵 정렬의 분할 기준`"의 예제는 `v >= pivot`을(를) 실행해 `[1, 2, 3, 3, 4]`을(를) 출력합니다. "`피벗과 같은 값을 두 그룹 모두에서 제외해 중복 원소를 잃음`" 여부를 확인하고 Day 75을(를) 완료하세요.
