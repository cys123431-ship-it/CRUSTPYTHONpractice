...
exercises:
  - id: ex-day-44-mutable-arguments-predict
    ...
    answer: "[10, 30]"
    hint: values에서 시작해 [10,30]까지 순서대로 적어 보세요. 함수 매개변수 items와 호출자 values가 같은 가변 리스트를 참조합니다. append는 객체를 바꿉니다.
    explanation:
      1. values가 [10]을 참조합니다. 2. items가 같은 객체를 가리킵니다. 3. append 뒤 [10,30]이 됩니다. `values→[10] → items→같은
      객체 → append → [10,30]` 흐름으로 실제 출력 `[10, 30]`이 됩니다. 핵심 `items.append(30)`은 공유 객체 자체를 바꾸는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-44-mutable-arguments-fill
    ...
    hint: 필요한 표현은 가변 객체의 공유 부작용 구분 동작을 잇는 items.append(30)입니다.
    explanation:
      빈칸에 들어갈 표현은 'items.append(30)'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. values가 [10]을 참조합니다. 2. items가 같은 객체를
      가리킵니다. 3. append 뒤 [10,30]이 됩니다. `values→[10] → items→같은 객체 → append → [10,30]` 흐름으로 실제 출력 `[10, 30]`이 됩니다.
      핵심 `items.append(30)`은 공유 객체 자체를 바꾸는 자리에 쓰입니다.
    commonMistakes:
      - 매개변수 대입과 객체 내부 수정을 같은 동작으로 혼동함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-44-mutable-arguments-guided
    ...
    starter: "def add_session(items):\n    items.append(30)\nvalues = [10]\nadd_session(values)\nprint(values)"
    answer: "def add_session(items):\n    items.append(31)\nvalues = [10]\nadd_session(values)\nprint(values)"
    hint: 처음 등장하는 숫자를 30에서 31로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '[10, 30]'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '[10, 31]'입니다. 원본은 `[10, 30]`, 수정본은 `[10, 31]`이다. 첫 변경 줄 `items.append(30)`에서 `items.append(31)`로
      추가 값을 바꾸면, 공유 객체에 31이 들어가 호출자 리스트가 그대로 보인다.
    commonMistakes:
      - 매개변수 대입과 객체 내부 수정을 같은 동작으로 혼동함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-44-mutable-arguments-debug
    ...
    answer: "def add_session(items):\n    items.append(30)\nvalues = [10]\nadd_session(values)\nprint(values)"
    hint:
      매개변수 대입과 객체 내부 수정을 같은 동작으로 혼동함 상황에서 어긋나는 줄을 함수 매개변수 items와 호출자 values가 같은 가변 리스트를 참조합니다. append는 객체를 바꿉니다.
      설명과 대조해 보세요.
    explanation:
      틀린 줄은 `items = [30]`입니다. 지역 이름이 새 리스트를 가리킬 뿐 원래 공유 객체는 그대로이므로, 호출자 `values`는 `[10]` 그대로 출력됩니다. 오류
      분류는 잘못된 출력입니다. 공유가 끊기는 첫 순간은 재할당 줄입니다. 고친 줄 `items.append(30)`에서는 `가변 객체의 공유 부작용 구분` 동작이 지켜집니다. 정상 코드는 공유 참조→객체
      변경→호출자 읽기의 순서로 `[10, 30]`을 출력합니다.
    commonMistakes:
      - 매개변수 대입과 객체 내부 수정을 같은 동작으로 혼동함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-44-mutable-arguments-independent
    ...
    answer: "def add_session(items):\n    items.append(30)\nvalues = [10]\nadd_session(values)\nprint(values)"
    hint: 원래 입력을 보존해야 한다면 `items.copy()`로 별도 리스트를 만드세요. 안에 다른 리스트가 있으면 한계를 기억하세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 원래 입력을 보존해야 한다면 `items.copy()`로 별도 리스트를 만드세요. 안에 다른 리스트가 있으면 한계를 기억하세요.
      같은 가변 객체의 공유 부작용 구분 동작을 구현하고 실행 결과 '[10, 30]'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 매개변수 대입과 객체 내부 수정을 같은 동작으로 혼동함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-44-mutable-arguments-output
    ...
    answerIndex: 2
    explanation:
      먼저 values가 [10]을 참조합니다. 이어서 items가 같은 객체를 가리키고 append가 그 객체를 바꿔 [10,30]이 되므로, print가 호출자 리스트를 읽어
      출력 `[10, 30]`이 됩니다. `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 참조와 메서드 호출이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가
  ...
  - id: quiz-day-44-mutable-arguments-model
    ...
    answerIndex: 0
    explanation:
      함수 매개변수 items와 호출자 values가 같은 가변 리스트를 참조한다는 뜻은, 이름은 둘이라도 객체는 하나라는 뜻이고, append는 객체를 바꾼다는 뜻은 호출자 쪽에서도
      보인다는 뜻입니다. `매개변수 대입과 객체 내부 수정을 같은 동작으로 혼동함`은 반대 사례인데, 재할당은 지역 이름만 바꾸기 때문입니다.
  - id: quiz-day-44-mutable-arguments-pitfall
    ...
    answerIndex: 1
    explanation:
      먼저 확인할 실수는 `매개변수 대입과 객체 내부 수정을 같은 동작으로 혼동함`입니다. 잘못된 코드는 실행은 되지만 `[10]`이라는 잘못된 출력을 냅니다. 대입과 내부 수정을
      구별하세요.
  - id: quiz-day-44-mutable-arguments-transfer
    ...
    answerIndex: 2
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `가변 객체의 공유 부작용 구분`이라는 의미와 입력·출력 계약입니다. C의 포인터 전달과 Rust의 가변 빌림으로 같은 동작을 구현하고,
      실행 결과 `[10, 30]`으로 대조하면 옮김이 맞는지 확인할 수 있습니다.
...
예제의 핵심 표현은 `items.append(30)`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본 실행 결과는 `[10, 30]`입니다. 아래 코드에서 원본과 달라지는 첫 줄을 표시하고, 그 줄의 변경 전후 값을 적어 보세요.
...
원본은 `[10, 30]`, 수정본은 `[10, 31]`이다. 첫 변경 줄 `items.append(30)`에서 `items.append(31)`로 추가 값을 바꾸면, 공유 객체에 31이 들어가 호출자 리스트가 그대로 보인다.
