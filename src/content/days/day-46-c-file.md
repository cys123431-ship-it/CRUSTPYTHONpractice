...
exercises:
  ...
  - id: ex-day-46-c-file-fill
    ...
    explanation: 빈칸에 들어갈 표현은 'rewind(f)'입니다. tmpfile이 임시 파일을 엽니다 30을 쓰고 위치를 처음으로 돌립니다 fscanf 성공 후 출력하고 닫습니다
    commonMistakes:
      - 쓰기 직후 위치를 되돌리지 않고 읽어 EOF를 만남
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-46-c-file-guided
    ...
    explanation:
      바꾼 뒤 출력은 '30'입니다. 원본 출력과 같습니다('30'). 바꾼 return 2는 파일 열기 실패 갈래에 있어 정상 흐름에서는 실행되지 않습니다. 출력이 같아도 바뀐
      줄(if (!f) return 2;) 이후의 중간 상태가 같은지는 원본 추적과 대조해야 합니다.
    commonMistakes:
      - 쓰기 직후 위치를 되돌리지 않고 읽어 EOF를 만남
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-46-c-file-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 1
    explanation: 열기  →  쓰기  →  rewind  →  읽기  →  닫기 순서로 실행되어 출력은 '30'입니다.
...
바꾼 뒤 출력도 `30`입니다. 바꾼 return 2는 파일 열기 실패 갈래에 있어 정상 흐름에서는 실행되지 않습니다. 출력이 같다고 해서 중간 상태까지 같은 것은 아닙니다. 다른 줄(`if (!f) return 2;`)부터 원본 추적과 비교해 보세요.
