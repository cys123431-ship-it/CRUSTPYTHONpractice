...
exercises:
  ...
  - id: ex-day-74-merge-sort-fill
    ...
    explanation:
      빈칸에 들어갈 표현은 'a[i] <= b[j]'입니다. 길이 2 미만을 기저로 둡니다 [4,1]과 [3,2]를 각각 [1,4], [2,3]으로 정렬합니다 두 부분의 i,j 인덱스를
  ...
  - id: ex-day-74-merge-sort-guided
    ...
    explanation:
      "바꾼 뒤 출력은 '[3, 2, 4, 1]'입니다. 원본 출력 '[1, 2, 3, 4]'에서 달라졌습니다. 다른 줄(if len(values) < 2: return values
      → if len(values) < 3: return values)에서 시작한 차이가 최종 출력에 반영되었습니다."
    commonMistakes:
      - 부분 배열의 정렬이 끝나기 전에 병합하거나 남은 꼬리를 빠뜨림
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-74-merge-sort-output
    ...
    choices:
      ...
      - "[1, 2, 3, 4]"
    answerIndex: 2
    explanation: "[4,1,3,2]  →  [1,4]+[2,3]  →  [1,2,3,4] 순서로 실행되어 출력은 '[1, 2, 3, 4]'입니다."
...
원본과 달라졌습니다. 원본 출력은 `[1, 2, 3, 4]`입니다. 다른 줄(`if len(values) < 2: return values` → `if len(values) < 3: return values`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`[4,1,3,2]` → …)과 바뀐 줄부터 대조해 보세요.
