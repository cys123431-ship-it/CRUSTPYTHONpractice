...
exercises:
  - id: ex-day-20-c-functions-predict
    ...
    starter:
      ...
      , add(2, 3));\n    return 0;\n}"
    answer: "5"
    hint: 인수 2,3 전달에서 시작해 호출 지점에 5 반환까지 순서대로 적어 보세요. 매개변수 a,b는 호출 시 전달된 값이고 return은 계산 결과를 호출한 위치에 돌려줍니다.
    explanation:
      1. main에서 add(2,3)을 호출합니다. 2. 함수의 a=2,b=3으로 계산합니다. 3. 반환 5를 printf가 출력합니다. `인수 2,3 전달 → a+b=5 → 호출
      지점에 5 반환` 흐름으로 실제 출력 `5`가 됩니다. 핵심 `return a + b`는 계산 결과를 호출한 자리로 보내는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-20-c-functions-fill
    ...
    answer:
      "#include <stdio.h>\nint add(int a, int b) { return a + b; }\nint main(void) {\n    printf(\"%d\\n\",\
      \ add(2, 3));\n    return 0;\n}"
    hint: 필요한 표현은 입력값을 받아 반환값을 계산함 동작을 잇는 return a + b입니다.
    explanation:
      빈칸에 들어갈 표현은 'return a + b'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. main에서 add(2,3)을 호출합니다. 2. 함수의 a=2,b=3으로
      계산합니다. 3. 반환 5를 printf가 출력합니다. `인수 2,3 전달 → a+b=5 → 호출 지점에 5 반환` 흐름으로 실제 출력 `5`가 됩니다. 핵심 `return a + b`는 계산
      결과를 호출한 자리로 보내는 자리에 쓰입니다.
    commonMistakes:
      - 함수 안의 a를 main의 변수와 같은 저장공간이라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-20-c-functions-guided
    ...
    answer:
      "#include <stdio.h>\nint add(int a, int b) { return a + b; }\nint main(void) {\n    printf(\"%d\\n\",\
      \ add(3, 3));\n    return 0;\n}"
    hint: 처음 등장하는 숫자를 2에서 3로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '5'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '6'입니다. 원본은 `5`, 수정본은 `6`이다. 첫 변경 줄 `printf("%d\n", add(2, 3));`에서 `printf("%d\n", add(3,
      3));`로 인수를 바꾸면, 함수 안에서 3+3이 계산되어 반환된 6이 출력된다.
    commonMistakes:
      - 함수 안의 a를 main의 변수와 같은 저장공간이라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-20-c-functions-debug
    ...
    answer:
      ...
      \ add(2, 3));\n    return 0;\n}"
    hint:
      함수 안의 a를 main의 변수와 같은 저장공간이라고 생각함 상황에서 어긋나는 줄을 매개변수 a,b는 호출 시 전달된 값이고 return은 계산 결과를 호출한 위치에 돌려줍니다. 설명과
      대조해 보세요.
    explanation:
      틀린 줄은 `int add(int a, int b) { return a - b; }`입니다. 2-3을 계산하므로 실행은 되지만 결과는 -1이 됩니다. 오류 분류는 잘못된 출력입니다.
      약속과 달라지는 첫 순간은 반환식의 연산자입니다. 고친 줄의 `return a + b`에서는 `입력값을 받아 반환값을 계산함` 동작이 지켜집니다. 정상 코드는 인수 전달→덧셈→반환→출력의 순서로
      `5`를 출력합니다.
    commonMistakes:
      - 함수 안의 a를 main의 변수와 같은 저장공간이라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-20-c-functions-independent
    ...
    answer:
      "#include <stdio.h>\nint add(int a, int b) { return a + b; }\nint main(void) {\n    printf(\"%d\\n\",\
      \ add(2, 3));\n    return 0;\n}"
    hint: 뺄셈 함수처럼 순서가 결과에 영향을 주면 인수 순서를 바꿔 시험하세요. `return`은 화면 출력이 아니므로 호출 쪽 표시를 확인하세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 뺄셈 함수처럼 순서가 결과에 영향을 주면 인수 순서를 바꿔 시험하세요. `return`은 화면 출력이 아니므로 호출 쪽 표시를
      확인하세요. 같은 입력값을 받아 반환값을 계산함 동작을 구현하고 실행 결과 '5'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 함수 안의 a를 main의 변수와 같은 저장공간이라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-20-c-functions-output
    ...
    answerIndex: 2
    explanation:
      먼저 main에서 add(2,3)이 호출되어 a=2,b=3이 됩니다. 이어서 a+b=5가 계산되어 호출 지점에 반환되고, printf가 표시하므로 출력 `5`가 됩니다. `실행
      전에 반드시 오류가 난다`는 틀린 선택지인데 컴파일과 실행이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 printf가 실제로 호출되기 때문입니다. 다른 선택지는 이
      추적과 맞지 않습니다.
  - id: quiz-day-20-c-functions-model
    ...
    answerIndex: 0
    explanation:
      매개변수 a,b는 호출 시 전달된 값이라는 뜻이고, return은 계산 결과를 호출한 위치에 돌려준다는 뜻은 반환값이 호출한 식의 값이 된다는 뜻입니다. `함수 안의 a를 main의
      변수와 같은 저장공간이라고 생각함`은 반대 사례인데, 정수 인수는 값이 복사되어 전달되기 때문입니다.
  - id: quiz-day-20-c-functions-pitfall
    ...
    answerIndex: 1
    explanation:
      먼저 확인할 실수는 `함수 안의 a를 main의 변수와 같은 저장공간이라고 생각함`입니다. 잘못된 코드는 실행은 되지만 `-1`이라는 잘못된 출력을 냅니다. 반환식의 연산자를
      먼저 확인하세요.
  - id: quiz-day-20-c-functions-transfer
    ...
    answerIndex: 2
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `입력값을 받아 반환값을 계산함`이라는 의미와 입력·출력 계약입니다. Python의 return과 Rust의 반환 표현식으로 같은 동작을
      구현하고, 실행 결과 `5`로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
예제의 핵심 표현은 `return a + b`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
