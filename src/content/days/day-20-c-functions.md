...
exercises:
  ...
  - id: ex-day-20-c-functions-fill
    ...
    explanation: 빈칸에 들어갈 표현은 'return a + b'입니다. main에서 add(2,3)을 호출합니다 함수의 a=2,b=3으로 계산합니다 반환 5를 printf가 출력합니다
    commonMistakes:
      - 함수 안의 a를 main의 변수와 같은 저장공간이라고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-20-c-functions-guided
    ...
    explanation:
      바꾼 뒤 출력은 '6'입니다. 원본 출력 '5'에서 달라졌습니다. 다른 줄(printf("%d\n", add(2, 3)); → printf("%d\n", add(3, 3));)에서
...
quiz:
  - id: quiz-day-20-c-functions-output
    ...
    choices:
      ...
      - "5"
    answerIndex: 2
    explanation: 인수 2,3 전달  →  a+b=5  →  호출 지점에 5 반환 순서로 실행되어 출력은 '5'입니다.
...
원본과 달라졌습니다. 원본 출력은 `5`입니다. 다른 줄(`printf("%d\n", add(2, 3));` → `printf("%d\n", add(3, 3));`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`인수 2,3 전달` → …)과 바뀐 줄부터 대조해 보세요.
