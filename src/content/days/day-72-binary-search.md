...
exercises:
  ...
  - id: ex-day-72-binary-search-fill
    ...
    explanation: 빈칸에 들어갈 표현은 'lo = mid + 1'입니다. 중간 인덱스 2의 값 6이 작아 lo=3입니다 새 중간 4의 값 10은 크므로 hi=4입니다 인덱스 3의 8에서 수렴합니다
    commonMistakes:
      - 정렬되지 않은 배열에 이진 탐색을 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-72-binary-search-guided
    ...
    explanation:
      바꾼 뒤 출력은 '3'입니다. 원본 출력과 같습니다('3'). 목표 8의 위치는 그대로 3이라 결과가 같습니다. 출력이 같아도 바뀐 줄(values = [3, 4, 6, 8,
      10]) 이후의 중간 상태가 같은지는 원본 추적과 대조해야 합니다.
    commonMistakes:
      - 정렬되지 않은 배열에 이진 탐색을 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-72-binary-search-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 0
    explanation: "[0,5)  →  [3,5)  →  [3,4)  →  위치 3 순서로 실행되어 출력은 '3'입니다."
...
바꾼 뒤 출력도 `3`입니다. 목표 8의 위치는 그대로 3이라 결과가 같습니다. 출력이 같다고 해서 중간 상태까지 같은 것은 아닙니다. 다른 줄(`values = [3, 4, 6, 8, 10]`)부터 원본 추적과 비교해 보세요.
