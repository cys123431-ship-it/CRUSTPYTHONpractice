...
exercises:
  - id: ex-day-72-binary-search-predict
    ...
    answer: "3"
    hint: "[0,5)에서 시작해 위치 3까지 순서대로 적어 보세요. 반열린 범위 [lo,hi)에서 target 이상이 처음 나타나는 위치를 유지합니다."
    explanation:
      1. 중간 인덱스 2의 값 6이 작아 lo=3입니다. 2. 새 중간 4의 값 10은 크므로 hi=4입니다. 3. 인덱스 3의 8에서 수렴합니다. `[0,5) → [3,5) →
      [3,4) → 위치 3` 흐름으로 실제 출력 `3`이 됩니다. 핵심 `lo = mid + 1`은 작은 쪽 절반을 버리는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-72-binary-search-fill
    ...
    answer:
      "values = [2, 4, 6, 8, 10]\ntarget = 8\nlo, hi = 0, len(values)\nwhile lo < hi:\n    mid = (lo + hi) //\
      \ 2\n    if values[mid] < target: lo = mid + 1\n    else: hi = mid\nprint(lo)"
    hint: 필요한 표현은 정렬된 범위를 반으로 축소 동작을 잇는 lo = mid + 1입니다.
    explanation:
      빈칸에 들어갈 표현은 'lo = mid + 1'입니다. 이 표현이 없으면 반복이 끝나지 않습니다. 1. 중간 인덱스 2의 값 6이 작아 lo=3입니다. 2. 새 중간 4의 값
      10은 크므로 hi=4입니다. 3. 인덱스 3의 8에서 수렴합니다. `[0,5) → [3,5) → [3,4) → 위치 3` 흐름으로 실제 출력 `3`이 됩니다. 핵심 `lo = mid + 1`은
      작은 쪽 절반을 버리는 자리에 쓰입니다.
    commonMistakes:
      - 정렬되지 않은 배열에 이진 탐색을 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-72-binary-search-guided
    ...
    explanation:
      바꾼 뒤 출력은 '3'입니다. 원본과 수정본 모두 `3`을 출력한다. 목표 8의 위치는 그대로 3이라 결과가 같습니다. 목표 위치가 달라지지 않았으므로 중간 상태가 달라졌다고
      쓰지 않는다.
    commonMistakes:
      - 정렬되지 않은 배열에 이진 탐색을 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-72-binary-search-debug
    ...
    answer:
      "values = [2, 4, 6, 8, 10]\ntarget = 8\nlo, hi = 0, len(values)\nwhile lo < hi:\n    mid = (lo + hi) //\
      \ 2\n    if values[mid] < target: lo = mid + 1\n    else: hi = mid\nprint(lo)"
    hint: 정렬되지 않은 배열에 이진 탐색을 적용함 상황에서 어긋나는 줄을 반열린 범위 [lo,hi)에서 target 이상이 처음 나타나는 위치를 유지합니다. 설명과 대조해 보세요.
    explanation:
      "틀린 줄은 `if values[mid] < target: lo = mid`입니다. mid가 lo와 같아지는 순간 탐색 범위가 줄지 않아 같은 계산을 영원히 반복하므로 끝나지
      않습니다. 오류 분류는 무한 반복입니다. 단순히 출력값만 다르다고 쓰면 안 됩니다. lo·hi·mid 표를 보면, 범위가 감소하지 않는 지점이 원인입니다. 고친 줄 `if values[mid]
      < target: lo = mid + 1`에서는 `정렬된 범위를 반으로 축소` 동작이 지켜집니다. 정상 코드는 절반 버리기→범위 축소→위치 수렴의 순서로 `3`을 출력합니다."
    commonMistakes:
      - 정렬되지 않은 배열에 이진 탐색을 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-72-binary-search-independent
    ...
    answer:
      "values = [2, 4, 6, 8, 10]\ntarget = 8\nlo, hi = 0, len(values)\nwhile lo < hi:\n    mid = (lo + hi) //\
      \ 2\n    if values[mid] < target: lo = mid + 1\n    else: hi = mid\nprint(lo)"
    hint: 비어 있는 배열에서는 처음부터 `lo==hi==0`입니다. 끝난 뒤 그 칸이 실제 목표인지 따로 확인하세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 비어 있는 배열에서는 처음부터 `lo==hi==0`입니다. 끝난 뒤 그 칸이 실제 목표인지 따로 확인하세요. 같은 정렬된 범위를
      반으로 축소 동작을 구현하고 실행 결과 '3'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 정렬되지 않은 배열에 이진 탐색을 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-72-binary-search-output
    ...
    answerIndex: 0
    explanation:
      먼저 [0,5) 범위에서 중간 2의 값 6이 작아 lo=3이 됩니다. 이어서 [3,5)에서 중간 4의 값 10이 크므로 hi=4가 되고, 인덱스 3의 8에서 수렴하므로 출력
      `3`이 됩니다. `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 범위 축소가 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기 때문입니다. 다른
      선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-72-binary-search-model
    ...
    answerIndex: 1
    explanation:
      반열린 범위 [lo,hi)에서 target 이상이 처음 나타나는 위치를 유지한다는 뜻은, 매 단계마다 답의 후보 범위를 좁힌다는 뜻입니다. `정렬되지 않은 배열에 이진 탐색을
      적용함`은 반대 사례인데, 정렬이 깨지면 절반 버리기가 정답을 보장하지 못하기 때문입니다.
  - id: quiz-day-72-binary-search-pitfall
    ...
    choices:
      ...
      - 정렬되지 않은 배열에 이진 탐색을 적용함
    answerIndex: 2
    explanation: 먼저 확인할 실수는 `정렬되지 않은 배열에 이진 탐색을 적용함`입니다. 잘못된 코드는 범위가 줄지 않아 끝나지 않습니다. lo·hi·mid 표로 구간 감소를 확인하세요.
  - id: quiz-day-72-binary-search-transfer
    ...
    answerIndex: 0
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `정렬된 범위를 반으로 축소`라는 의미와 입력·출력 계약입니다. C의 인덱스 이동과 Rust의 슬라이스 축소로 같은 동작을 구현하고,
...
예제의 핵심 표현은 `lo = mid + 1`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본과 수정본 모두 `3`을 출력한다. 목표 8의 위치는 그대로 3이라 결과가 같습니다. 목표 위치가 달라지지 않았으므로 중간 상태가 달라졌다고 쓰지 않는다.
