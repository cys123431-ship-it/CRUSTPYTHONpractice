...
exercises:
  ...
  - id: ex-day-25-result-error-debug
    ...
    prompt: 아래 코드의 핵심 표현 '"x"'을 점검하고 올바른 작동을 복원하세요. unwrap으로 잘못된 입력에서 프로그램을 종료시킴 오류도 함께 경계하세요.
    starter:
      "fn parse_minutes(text: &str) -> Result<u32, std::num::ParseIntError> {\n    text.parse::<u32>()\n}\n\
...
외부에서 받은 글자는 잘못된 숫자일 수 있어 정상 값과 실패 이유를 분리해야 합니다. 처음 읽을 때는 결과를 가리고 손으로 예상하세요. 예시를 직접 타이핑한 뒤 한 줄씩 바꾸며 상태를 확인하세요.

## 2. 시작 전에 확인할 것

바로 앞선 [Day 24](/learn/day-24-recursion)의 핵심을 한 문장으로 설명해 보세요. 모르면 위의 선행 Day 링크에서 다시 확인할 수 있습니다.

## 3. 머릿속 그림

Result는 성공 Ok(값) 또는 실패 Err(오류) 중 하나이며 match가 양쪽을 처리합니다. 다음 흐름을 눈으로 확인하세요.
...
**확인할 실수: unwrap으로 잘못된 입력에서 프로그램을 종료시킴.** Result는 성공 Ok(값) 또는 실패 Err(오류) 중 하나이며 match가 양쪽을 처리합니다. 결과가 예상과 다르면 입력 → 중간 상태 → 출력 중 처음 어긋난 곳을 찾습니다.

## 9. 다른 언어로 옮기기

같은 문제를 해결하더라도 선언, 타입, 메모리 관리, 오류 처리는 다릅니다. 핵심 동작은 **파싱 성공과 실패를 분리함**입니다. 언어를 옮겨도 이 동작을 유지하세요.
...
- 왜 이 개념이 필요한가? 외부에서 받은 글자는 잘못된 숫자일 수 있어 정상 값과 실패 이유를 분리해야 합니다.
- 예시에서 가장 먼저 확정되는 값이나 상태는 무엇인가?
- 어떤 실수를 점검해야 하나? unwrap으로 잘못된 입력에서 프로그램을 종료시킴
- 다른 두 언어에서는 같은 동작을 어떤 자료형과 오류 처리로 나타내는가?

## 12. 핵심 요약과 복습

Result는 성공 Ok(값) 또는 실패 Err(오류) 중 하나이며 match가 양쪽을 처리합니다. 예시의 출력은 `30`입니다. 오류를 찾을 때는 **unwrap으로 잘못된 입력에서 프로그램을 종료시킴** 여부를 확인하세요. 완료 버튼을 누르면 +1·+3·+7·+14·+30일 복습이 이 기기에 등록됩니다.
