...
exercises:
  ...
  - id: ex-day-25-result-error-fill
    ...
    explanation: 빈칸에는 '"30"'이 들어갑니다. 문자열 30을 u32로 파싱합니다 Ok(30)이 생성됩니다 match의 Ok arm에서 30을 출력합니다
    commonMistakes:
      - unwrap으로 잘못된 입력에서 프로그램을 종료시킴
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
...
quiz:
  - id: quiz-day-25-result-error-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 1
    explanation: 문자열 30  →  Ok(30)  →  30 출력 순서로 실행되어 '30'을 출력합니다.
  - id: quiz-day-25-result-error-model
    question: Rust Result와 실패 전달을 이해하는 데 맞는 설명은?
    choices:
...
외부에서 받은 글자는 잘못된 숫자일 수 있어 정상 값과 실패 이유를 분리해야 합니다. Day 25 "`Rust Result와 실패 전달`"에서는 Rust 코드가 `30`을(를) 만드는 과정을 따라가며, 파싱 성공과 실패를 분리함 동작이 왜 필요한지 확인합니다. 파싱 성공과 실패를 분리함을(를) 빠뜨리면 `unwrap으로 잘못된 입력에서 프로그램을 종료시킴` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 24](/learn/day-24-recursion)에서는 "재귀와 종료 조건"을(를) 배웠습니다. "재귀와 종료 조건"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`Rust Result와 실패 전달`"에서 새로 달라지는 조건을 찾아보세요. Day 25의 답은 `30`이며, 핵심 표현은 `"30"`입니다.

## 머릿속 그림

Result는 성공 Ok(값) 또는 실패 Err(오류) 중 하나이며 match가 양쪽을 처리합니다. Day 25에서는 아래 흐름 순서대로 상태가 바뀌며, `"30"`이(가) 결과를 가릅니다.

```text
문자열 30  →  Ok(30)  →  30 출력
```

위 흐름에서 `"30"`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `30`입니다.
...
아래 Rust 코드는 Day 25 "`Rust Result와 실패 전달`"의 독립 예제입니다. 전체 6줄에서 `"30"`이(가) 핵심이며, 실행 결과는 `30`입니다.
...
위 단계에서 `"30"`이(가) 빠지면 `30`이(가) 나오지 않습니다. `첫 문자열을 '30'에서 '30!'로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `30`이(가) 됩니다. `문자열 30` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 코드에서 `첫 문자열을 '30'에서 '30!'로` 바꾸면 아래와 같이 됩니다.
...
원본 출력은 `30`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `"30"`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 25의 `Rust Result와 실패 전달`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: unwrap으로 잘못된 입력에서 프로그램을 종료시킴.** 정상 코드에서는 `30`이(가) 출력됩니다.
...
핵심 동작 **파싱 성공과 실패를 분리함**은(는) 세 언어에서 같은 입력과 출력(`30`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `strtoul와 endptr/errno 검증` 규칙으로 "`Rust Result와 실패 전달`"의 `30`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `int(text)와 ValueError 처리` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `parse::<u32>()의 Result match` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`30` 맞히기) → 빈칸(`"30"` 채우기) → 변경(`첫 문자열을 '30'에서 '30!'로`) → 오류 수정(`unwrap으로 잘못된 입력에서 프로그램을 종료시킴` 찾기) → 독립 구현(`Rust Result와 실패 전달을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `30` 및 `"30"` 설명과 대조하세요.

## 스스로 설명하기

- "`Rust Result와 실패 전달`"이(가) 필요한 상황을 `파싱 성공과 실패를 분리함` 동작으로 설명해 보세요.
- 예제에서 `"30"`이(가) 실행되기 직전의 상태와 직후의 출력 `30`을(를) 말해 보세요.
- "`unwrap으로 잘못된 입력에서 프로그램을 종료시킴`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `파싱 성공과 실패를 분리함` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

Result는 성공 Ok(값) 또는 실패 Err(오류) 중 하나이며 match가 양쪽을 처리합니다. Day 25 "`Rust Result와 실패 전달`"의 예제는 `"30"`을(를) 실행해 `30`을(를) 출력합니다. "`unwrap으로 잘못된 입력에서 프로그램을 종료시킴`" 여부를 확인하고 Day 25을(를) 완료하세요.
