...
원본(`30`)과 달라졌습니다. 다른 줄(`match parse_minutes("30") { Ok(n) => println!("{n}"), Err(_) => println!("오류") }` → `match parse_minutes("30!") { Ok(n) => println!("{n}"), Err(_) => println!("오류") }`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`문자열 30` → …)과 바뀐 줄부터 대조해 보세요.
