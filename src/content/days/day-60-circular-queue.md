...
exercises:
  ...
  - id: ex-day-60-circular-queue-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: "'원형 큐의 머리·꼬리' 개념의 핵심 표현을 스스로 적는다."
    ...
    explanation: 빈칸에 들어갈 표현은 '(head + 1) % 3'입니다. head는 마지막 인덱스 2입니다 3%3으로 위치 0을 얻습니다 buffer[0]=10을 출력합니다
    commonMistakes:
      - 나머지 연산을 빼먹어 배열 경계를 넘음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-60-circular-queue-guided
    ...
    explanation:
      바꾼 뒤 출력은 '10'입니다. 원본 출력과 같습니다('10'). 바뀐 것은 배열 크기뿐이며 읽는 위치 (2+1)%3=0은 그대로라 결과가 같습니다. 출력이 같아도 바뀐 줄(int
      buffer[4] = {10,20,30};) 이후의 중간 상태가 같은지는 원본 추적과 대조해야 합니다.
    commonMistakes:
      - 나머지 연산을 빼먹어 배열 경계를 넘음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-60-circular-queue-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 0
    explanation: head=2  →  다음 인덱스=0  →  값 10 순서로 실행되어 출력은 '10'입니다.
...
바꾼 뒤 출력도 `10`입니다. 바뀐 것은 배열 크기뿐이며 읽는 위치 (2+1)%3=0은 그대로라 결과가 같습니다. 출력이 같다고 해서 중간 상태까지 같은 것은 아닙니다. 다른 줄(`int buffer[4] = {10,20,30};`)부터 원본 추적과 비교해 보세요.
