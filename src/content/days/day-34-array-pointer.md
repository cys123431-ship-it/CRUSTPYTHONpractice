...
exercises:
  ...
  - id: ex-day-34-array-pointer-fill
    ...
    explanation: 빈칸에 들어갈 표현은 'i<n'입니다. a의 첫 주소와 길이 3을 전달합니다 i=0,1,2에서 2+4+6을 누적합니다 i=3에서 멈추고 12를 반환합니다
    commonMistakes:
      - 배열 끝 인덱스를 n으로 착각해 한 칸 넘겨 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-34-array-pointer-guided
    ...
    explanation:
      바꾼 뒤 출력은 '13'입니다. 원본 출력 '12'에서 달라졌습니다. 다른 줄(int sum(const int *a, size_t n) { int out=0; for (size_t
      i=0;i<n;i++) out+=a[i]; return out; } → int sum(const int *a, size_t n) { int out=1; for (size_t i=0;i<n;i++)
      out+=a[i]; return out; })에서 시작한 차이가 최종 출력에 반영되었습니다.
    commonMistakes:
      - 배열 끝 인덱스를 n으로 착각해 한 칸 넘겨 읽음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-34-array-pointer-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 1
    explanation: i=0 out=2  →  i=1 out=6  →  i=2 out=12 순서로 실행되어 출력은 '12'입니다.
...
원본과 달라졌습니다. 원본 출력은 `12`입니다. 다른 줄(`int sum(const int *a, size_t n) { int out=0; for (size_t i=0;i<n;i++) out+=a[i]; return out; }` → `int sum(const int *a, size_t n) { int out=1; for (size_t i=0;i<n;i++) out+=a[i]; return out; }`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`i=0 out=2` → …)과 바뀐 줄부터 대조해 보세요.
