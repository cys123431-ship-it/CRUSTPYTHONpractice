...
원본(`12`)과 달라졌습니다. 다른 줄(`int sum(const int *a, size_t n) { int out=0; for (size_t i=0;i<n;i++) out+=a[i]; return out; }` → `int sum(const int *a, size_t n) { int out=1; for (size_t i=0;i<n;i++) out+=a[i]; return out; }`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`i=0 out=2` → …)과 바뀐 줄부터 대조해 보세요.
