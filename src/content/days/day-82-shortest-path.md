...
exercises:
  - id: ex-day-82-shortest-path-predict
    ...
    starter:
      ...
      \ next_node))\nprint(dist[\"C\"])"
    answer: "3"
    hint: 음수가 없는 간선에서는 현재까지 가장 짧은 후보부터 꺼냅니다. 이웃 경로가 기존 거리보다 짧으면 거리와 우선순위 큐를 갱신합니다. 설명을 떠올리고 `A=0` 단계부터 순서대로 적어 보세요.
    explanation:
      A=0  →  B=2,C=5  →  B 경유 C=3  →  C=3 순서로 실행됩니다. `cost + weight < dist[next_node]` 부분이 `C=3` 단계를 확정해
      최종 출력 '3'가 됩니다. 이 흐름을 떠올리면 `더 짧은 후보로 거리 완화` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-82-shortest-path-fill
    ...
    answer:
      ...
      \ dist[next_node]:\n            dist[next_node] = cost + weight\n            heapq.heappush(heap, (dist[next_node],\
      \ next_node))\nprint(dist[\"C\"])"
    hint: 힌트 문장을 완전하게 읽으면 `모든 거리를 무한대로 시작하고 A만 0으로 둡니다` 단계에 필요한 표현이 `cost + weight < dist[next_node]`입니다.
    explanation:
      빈칸에 들어갈 표현은 'cost + weight < dist[next_node]'입니다. `if cost + weight < dist[next_node]:` 줄을 완성해야 `더
      짧은 후보로 거리 완화` 동작이 이어져 실행 결과 '3'가 됩니다. 힌트의 첫 단계 `모든 거리를 무한대로 시작하고 A만 0으로 둡니다`이 바로 이 줄입니다. 이어서 큐에서 A를 꺼내 B=2,C=5로
      기록합니다 B에서 C의 새 후보 3을 발견해 갱신합니다 오래된 C=5 후보는 건너뛰고 3을 출력합니다 순서로 진행됩니다.
    commonMistakes:
      - 음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-82-shortest-path-guided
    ...
    explanation:
      '바꾼 뒤 출력은 ''4''입니다. 원본 출력 ''3''에서 달라졌습니다. 바뀐 줄은 `graph = {"A": [("B", 2), ("C", 5)], "B": [("C",
      1)], "C": []}`에서 `graph = {"A": [("B", 3), ("C", 5)], "B": [("C", 1)], "C": []}`로 바뀌었습니다. 바뀐 프로그램은 `A=0` 단계에서
      시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `4`가 됩니다. 원본 추적 `A=0  →  B=2,C=5  →  B 경유 C=3  →  C=3`와 바뀐 줄 이후를
      순서대로 비교하면 처음 달라지는 곳이 보입니다.'
    commonMistakes:
      - 음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-82-shortest-path-debug
    ...
    answer:
      ...
      \ next_node))\nprint(dist[\"C\"])"
    hint:
      음수가 없는 간선에서는 현재까지 가장 짧은 후보부터 꺼냅니다. 이웃 경로가 기존 거리보다 짧으면 거리와 우선순위 큐를 갱신합니다. 설명과 어긋나는 줄을 찾으세요. `음수 간선에도 다익스트라의
      확정 규칙을 그대로 적용함` 상황이 단서가 됩니다.
    explanation:
      틀린 줄은 `if cost + weight > dist[next_node]:`입니다. 여기서는 `cost + weight > dist[next_node]`을 써서 `cost
      + weight < dist[next_node]` 동작이 깨집니다. 이대로 실행하면 `음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함` 문제가 생겨 원본 추적 `A=0  →  B=2,C=5  →  B
      경유 C=3  →  C=3`대로 '3'가 나오지 않습니다. 고친 줄 `if cost + weight < dist[next_node]:`에서는 `cost + weight < dist[next_node]`가
      `더 짧은 후보로 거리 완화` 동작을 지켜 '3'까지 도달합니다.
    commonMistakes:
      - 음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-82-shortest-path-independent
    ...
    answer:
      ...
      \ next_node))\nprint(dist[\"C\"])"
    hint:
      음수가 없는 간선에서는 현재까지 가장 짧은 후보부터 꺼냅니다. 이웃 경로가 기존 거리보다 짧으면 거리와 우선순위 큐를 갱신합니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과
      빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `cost + weight < dist[next_node]` 부분이 `더 짧은 후보로 거리 완화` 동작을 지켜 실행 결과 '3'가 됩니다.
      같은 개념을 다른 입력으로 바꿔도 `cost + weight < dist[next_node]`부터 `C=3`까지 추적할 수 있으면 정답입니다. `음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함`
      상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - 음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-82-shortest-path-output
    ...
    answerIndex: 1
    explanation:
      A=0  →  B=2,C=5  →  B 경유 C=3  →  C=3 순서로 실행되어 출력은 '3'입니다. `cost + weight < dist[next_node]` 부분이 마지막
  ...
  - id: quiz-day-82-shortest-path-model
    ...
    answerIndex: 2
    explanation:
      음수가 없는 간선에서는 현재까지 가장 짧은 후보부터 꺼냅니다. 이웃 경로가 기존 거리보다 짧으면 거리와 우선순위 큐를 갱신합니다. 이 설명이 맞는 이유는 `더 짧은 후보로 거리
      완화` 동작을 지키는 조건과 같기 때문입니다. `음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-82-shortest-path-pitfall
    ...
    answerIndex: 0
    explanation:
      음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함. 이 실수가 나오면 원본 추적 `A=0 → B=2,C=5 → B 경유 C=3 → C=3`대로 '3'가 나오지 않으므로 먼저
      확인해야 합니다.
  - id: quiz-day-82-shortest-path-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 1
    explanation: 문법은 달라도 `더 짧은 후보로 거리 완화` 목적과 입력·출력은 유지합니다. 실행 결과 '3'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
playgroundSource:
  "import heapq\ngraph = {\"A\": [(\"B\", 2), (\"C\", 5)], \"B\": [(\"C\", 1)], \"C\": []}\ndist\
...
원본 출력은 `3`이고, 바뀐 코드의 실행 결과는 `4`입니다. 바뀐 줄은 `graph = {"A": [("B", 2), ("C", 5)], "B": [("C", 1)], "C": []}`에서 `graph = {"A": [("B", 3), ("C", 5)], "B": [("C", 1)], "C": []}`로 바뀌었습니다. 바뀐 프로그램은 `A=0` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `4`가 됩니다. 원본 추적 `A=0  →  B=2,C=5  →  B 경유 C=3  →  C=3`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
