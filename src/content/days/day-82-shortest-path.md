...
exercises:
  - id: ex-day-82-shortest-path-predict
    ...
    starter:
      ...
      \ next_node))\nprint(dist[\"C\"])"
    answer: "3"
    hint: A=0에서 시작해 C=3까지 순서대로 적어 보세요. 음수가 없는 간선에서는 현재까지 가장 짧은 후보부터 꺼냅니다. 이웃 경로가 기존 거리보다 짧으면 거리와 우선순위 큐를 갱신합니다.
    explanation:
      1. 모든 거리를 무한대로 시작하고 A만 0으로 둡니다. 2. 큐에서 A를 꺼내 B=2,C=5로 기록합니다. 3. B에서 C의 새 후보 3을 발견해 갱신합니다. 4. 오래된
      C=5 후보는 건너뛰고 3을 출력합니다. `A=0 → B=2,C=5 → B 경유 C=3 → C=3` 흐름으로 실제 출력 `3`이 됩니다. 핵심 `cost + weight < dist[next_node]`는
      더 짧을 때만 갱신하는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-82-shortest-path-fill
    ...
    answer:
      ...
      \ dist[next_node]:\n            dist[next_node] = cost + weight\n            heapq.heappush(heap, (dist[next_node],\
      \ next_node))\nprint(dist[\"C\"])"
    hint: 필요한 표현은 더 짧은 후보로 거리 완화 동작을 잇는 cost + weight < dist[next_node]입니다.
    explanation:
      빈칸에 들어갈 표현은 'cost + weight < dist[next_node]'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. 모든 거리를 무한대로 시작하고 A만
      0으로 둡니다. 2. 큐에서 A를 꺼내 B=2,C=5로 기록합니다. 3. B에서 C의 새 후보 3을 발견해 갱신합니다. 4. 오래된 C=5 후보는 건너뛰고 3을 출력합니다. `A=0 → B=2,C=5
      → B 경유 C=3 → C=3` 흐름으로 실제 출력 `3`이 됩니다. 핵심 `cost + weight < dist[next_node]`는 더 짧을 때만 갱신하는 자리에 쓰입니다.
    commonMistakes:
      - 음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-82-shortest-path-guided
    ...
    answer:
      ...
      \ dist[next_node]:\n            dist[next_node] = cost + weight\n            heapq.heappush(heap, (dist[next_node],\
      \ next_node))\nprint(dist[\"C\"])"
    hint: 처음 등장하는 숫자를 2에서 3로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '3'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '4'입니다. 원본은 `3`, 수정본은 `4`이다. 첫 변경 줄의 B 간선 비용 2에서 3으로 바꾸면, B 경유 C 후보가 3+1=4가 되어 직접 간선 5보다
      여전히 짧지만 최종 거리가 4가 된다.
    commonMistakes:
      - 음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-82-shortest-path-debug
    ...
    answer:
      ...
      \ next_node))\nprint(dist[\"C\"])"
    hint:
      음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함 상황에서 어긋나는 줄을 음수가 없는 간선에서는 현재까지 가장 짧은 후보부터 꺼냅니다. 이웃 경로가 기존 거리보다 짧으면 거리와 우선순위
      큐를 갱신합니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `if cost + weight > dist[next_node]:`입니다. 더 길 때만 갱신하므로 어떤 거리도 짧아지지 않아 C는 무한대 그대로 남아 `inf`가
      출력됩니다. 오류 분류는 잘못된 출력입니다. 완화가 일어나지 않는 첫 순간이 원인입니다. 고친 줄 `if cost + weight < dist[next_node]:`에서는 `더 짧은 후보로 거리
      완화` 동작이 지켜집니다. 음수 간선에는 다익스트라의 확정 규칙을 그대로 적용하면 안 됩니다. 정상 코드는 0 시작→이웃 기록→B 경유 갱신→3 출력의 순서로 `3`을 출력합니다.
    commonMistakes:
      - 음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-82-shortest-path-independent
    ...
    answer:
      ...
      \ dist[next_node]:\n            dist[next_node] = cost + weight\n            heapq.heappush(heap, (dist[next_node],\
      \ next_node))\nprint(dist[\"C\"])"
    hint: 도달할 수 없는 정점은 거리가 무한대로 남습니다. 음수 간선이 있으면 다른 알고리즘을 고르세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 도달할 수 없는 정점은 거리가 무한대로 남습니다. 음수 간선이 있으면 다른 알고리즘을 고르세요. 같은 더 짧은 후보로 거리 완화
...
quiz:
  - id: quiz-day-82-shortest-path-output
    ...
    answerIndex: 1
    explanation:
      먼저 모든 거리가 무한대이고 A만 0입니다. 이어서 A를 꺼내 B=2,C=5를 기록하고, B에서 C의 새 후보 3을 발견해 갱신한 뒤, 오래된 후보를 건너뛰고 3을 출력하므로
      `3`이 됩니다. `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 힙 연산과 갱신이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기 때문입니다.
  ...
  - id: quiz-day-82-shortest-path-model
    ...
    answerIndex: 2
    explanation:
      음수가 없는 간선에서는 현재까지 가장 짧은 후보부터 꺼낸다는 뜻이고, 이웃 경로가 기존 거리보다 짧으면 거리와 우선순위 큐를 갱신한다는 뜻은 더 짧을 때만 바꾼다는 뜻입니다.
      `음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함`은 반대 사례인데, 음수가 있으면 확정 순서가 깨지기 때문입니다.
  - id: quiz-day-82-shortest-path-pitfall
    ...
    answerIndex: 0
    explanation: 먼저 확인할 실수는 `음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함`입니다. 잘못된 코드는 실행은 되지만 `inf`라는 잘못된 출력을 냅니다. 비교 방향을 먼저 확인하세요.
  - id: quiz-day-82-shortest-path-transfer
    ...
    answerIndex: 1
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `더 짧은 후보로 거리 완화`라는 의미와 입력·출력 계약입니다. C의 거리 배열과 Rust의 거리 맵으로 같은 동작을 구현하고, 실행
...
예제의 핵심 표현은 `cost + weight < dist[next_node]`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본은 `3`, 수정본은 `4`이다. 첫 변경 줄의 B 간선 비용 2에서 3으로 바꾸면, B 경유 C 후보가 3+1=4가 되어 직접 간선 5보다 여전히 짧지만 최종 거리가 4가 된다.
...
두 줄을 나란히 놓으면 정상 줄 `if cost + weight < dist[next_node]:`이 `음수가 없는 간선에서는 현재까지 가장 짧은 후보부터 꺼냅니다. 이웃 경로가 기존 거리보다 짧으면 거리와 우선순위 큐를 갱신합니다.` 설명과 맞고, 잘못된 줄은 `음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `A=0`부터 `3`까지 처음 어긋난 곳을 찾습니다.
