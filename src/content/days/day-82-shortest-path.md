...
exercises:
  ...
  - id: ex-day-82-shortest-path-guided
    ...
    explanation:
      예시 답안에서는 처음 등장하는 숫자를 2에서 3로 바꿨습니다. 원래 출력은 '3'입니다. 바꾼 줄에서 시작해 중간 값과 마지막 출력을 다시 추적하세요. 출력이 같더라도 입력·조건·중간
      상태가 달라졌는지 확인해야 합니다.
    commonMistakes:
      - 음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
...
더 짧은 경로를 발견하면 기존 거리보다 작은 값으로 갱신해야 합니다. Day 82 "`최단 경로의 거리 갱신`"에서는 Python 코드가 `3`을(를) 만드는 과정을 따라가며, 더 짧은 후보로 거리 완화 동작이 왜 필요한지 확인합니다. 더 짧은 후보로 거리 완화을(를) 빠뜨리면 `음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 81](/learn/day-81-dfs)에서는 "깊이 우선 탐색과 순환"을(를) 배웠습니다. "깊이 우선 탐색과 순환"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`최단 경로의 거리 갱신`"에서 새로 달라지는 조건을 찾아보세요. Day 82의 답은 `3`이며, 핵심 표현은 `cost + weight < dist[next_node]`입니다.

## 머릿속 그림

음수가 없는 간선에서는 현재까지 가장 짧은 후보부터 꺼냅니다. 이웃 경로가 기존 거리보다 짧으면 거리와 우선순위 큐를 갱신합니다. Day 82에서는 아래 흐름 순서대로 상태가 바뀌며, `cost + weight < dist[next_node]`이(가) 결과를 가릅니다.

```text
A=0  →  B=2,C=5  →  B 경유 C=3  →  C=3
```

위 흐름에서 `cost + weight < dist[next_node]`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `3`입니다.
...
아래 Python 코드는 Day 82 "`최단 경로의 거리 갱신`"의 독립 예제입니다. 전체 12줄에서 `cost + weight < dist[next_node]`이(가) 핵심이며, 실행 결과는 `3`입니다.
...
위 단계에서 `cost + weight < dist[next_node]`이(가) 빠지면 `3`이(가) 나오지 않습니다. `처음 등장하는 숫자를 2에서 3로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `3`이(가) 됩니다. `A=0` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `3`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `cost + weight < dist[next_node]`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 82의 `최단 경로의 거리 갱신`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함.** 정상 코드에서는 `3`이(가) 출력됩니다.
...
핵심 동작 **더 짧은 후보로 거리 완화**은(는) 세 언어에서 같은 입력과 출력(`3`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `거리 배열과 INF 처리` 규칙으로 "`최단 경로의 거리 갱신`"의 `3`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `dict 거리와 min 갱신` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `우선순위 큐에 Reverse 비용 사용` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`3` 맞히기) → 빈칸(`cost + weight < dist[next_node]` 채우기) → 변경(`처음 등장하는 숫자를 2에서 3로`) → 오류 수정(`음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함` 찾기) → 독립 구현(`최단 경로의 거리 갱신을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `3` 및 `cost + weight < dist[next_node]` 설명과 대조하세요.

## 스스로 설명하기

- "`최단 경로의 거리 갱신`"이(가) 필요한 상황을 `더 짧은 후보로 거리 완화` 동작으로 설명해 보세요.
- 예제에서 `cost + weight < dist[next_node]`이(가) 실행되기 직전의 상태와 직후의 출력 `3`을(를) 말해 보세요.
- "`음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `더 짧은 후보로 거리 완화` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

음수가 없는 간선에서는 현재까지 가장 짧은 후보부터 꺼냅니다. 이웃 경로가 기존 거리보다 짧으면 거리와 우선순위 큐를 갱신합니다. Day 82 "`최단 경로의 거리 갱신`"의 예제는 `cost + weight < dist[next_node]`을(를) 실행해 `3`을(를) 출력합니다. "`음수 간선에도 다익스트라의 확정 규칙을 그대로 적용함`" 여부를 확인하고 Day 82을(를) 완료하세요.
