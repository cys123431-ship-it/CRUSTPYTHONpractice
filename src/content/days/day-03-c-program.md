...
exercises:
  ...
  - id: ex-day-03-c-program-fill
    ...
    explanation: 빈칸에 들어갈 표현은 'printf'입니다. stdio.h가 printf 선언을 제공합니다 main 내부에서 %d에 정수 30을 채웁니다 return 0은 정상 종료를 알립니다
    commonMistakes:
      - 형식 지정자 %d에 정수 인수를 전달하지 않음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-03-c-program-guided
    ...
    explanation:
      '바꾼 뒤 출력은 ''study: 31''입니다. 원본 출력 ''study: 30''에서 달라졌습니다. 다른 줄(printf("study: %d\n", 30); → printf("study:
...
quiz:
  - id: quiz-day-03-c-program-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 0
    explanation: "main 시작  →  %d에 30 대입  →  study: 30 출력  →  종료 순서로 실행되어 출력은 'study: 30'입니다."
