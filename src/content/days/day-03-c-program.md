...
exercises:
  - id: ex-day-03-c-program-predict
    ...
    answer: "study: 30"
    hint: main은 프로그램 진입문이고 printf는 형식 문자열의 빈자리에 값을 채워 출력합니다. 설명을 떠올리고 `main 시작` 단계부터 순서대로 적어 보세요.
    explanation:
      "main 시작  →  %d에 30 대입  →  study: 30 출력  →  종료 순서로 실행됩니다. `printf` 부분이 `종료` 단계를 확정해 최종 출력 'study:
      30'가 됩니다. 이 흐름을 떠올리면 `정수를 지정한 형식으로 출력함` 동작이 왜 필요한지 알 수 있습니다."
    commonMistakes:
  ...
  - id: ex-day-03-c-program-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: "'C 프로그램의 시작점과 printf' 개념의 핵심 표현을 스스로 적는다."
    prompt: "빈칸을 채워 예시와 같은 결과를 만드세요. 필요한 표현: printf"
    starter: "#include <stdio.h>\nint main(void) {\n    _____(\"study: %d\\n\", 30);\n    return 0;\n}"
    answer: "#include <stdio.h>\nint main(void) {\n    printf(\"study: %d\\n\", 30);\n    return 0;\n}"
    hint: 힌트 문장을 완전하게 읽으면 `stdio.h가 printf 선언을 제공합니다` 단계에 필요한 표현이 `printf`입니다.
    explanation:
      '빈칸에 들어갈 표현은 ''printf''입니다. `printf("study: %d\n", 30);` 줄을 완성해야 `정수를 지정한 형식으로 출력함` 동작이 이어져 실행 결과
      ''study: 30''가 됩니다. 힌트의 첫 단계 `stdio.h가 printf 선언을 제공합니다`이 바로 이 줄입니다. 이어서 main 내부에서 %d에 정수 30을 채웁니다 return 0은
      정상 종료를 알립니다 순서로 진행됩니다.'
    commonMistakes:
      - 형식 지정자 %d에 정수 인수를 전달하지 않음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-03-c-program-guided
    ...
    explanation:
      '바꾼 뒤 출력은 ''study: 31''입니다. 원본 출력 ''study: 30''에서 달라졌습니다. 바뀐 줄은 `printf("study: %d\n", 30);`에서 `printf("study:
      %d\n", 31);`로 바뀌었습니다. 바뀐 프로그램은 `main 시작` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `study: 31`가 됩니다.
      원본 추적 `main 시작  →  %d에 30 대입  →  study: 30 출력  →  종료`와 바뀐 줄 이후를 순서대로 비교하면 처음 달라지는 곳이 보입니다.'
    commonMistakes:
      - 형식 지정자 %d에 정수 인수를 전달하지 않음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-03-c-program-debug
    ...
    answer: "#include <stdio.h>\nint main(void) {\n    printf(\"study: %d\\n\", 30);\n    return 0;\n}"
    hint:
      main은 프로그램 진입문이고 printf는 형식 문자열의 빈자리에 값을 채워 출력합니다. 설명과 어긋나는 줄을 찾으세요. `형식 지정자 %d에 정수 인수를 전달하지 않음` 상황이 단서가
      됩니다.
    explanation:
      '틀린 줄은 `puts("study: %d\n", 30);`입니다. 여기서는 `puts`을 써서 `printf` 동작이 깨집니다. 이대로 실행하면 `형식 지정자 %d에 정수
      인수를 전달하지 않음` 문제가 생겨 원본 추적 `main 시작  →  %d에 30 대입  →  study: 30 출력  →  종료`대로 ''study: 30''가 나오지 않습니다. 고친 줄 `printf("study:
      %d\n", 30);`에서는 `printf`가 `정수를 지정한 형식으로 출력함` 동작을 지켜 ''study: 30''까지 도달합니다.'
    commonMistakes:
      - 형식 지정자 %d에 정수 인수를 전달하지 않음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-03-c-program-independent
    ...
    answer: "#include <stdio.h>\nint main(void) {\n    printf(\"study: %d\\n\", 30);\n    return 0;\n}"
    hint: main은 프로그램 진입문이고 printf는 형식 문자열의 빈자리에 값을 채워 출력합니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      "한 가지 예시 해법은 위 코드입니다. `printf` 부분이 `정수를 지정한 형식으로 출력함` 동작을 지켜 실행 결과 'study: 30'가 됩니다. 같은 개념을 다른
      입력으로 바꿔도 `printf`부터 `종료`까지 추적할 수 있으면 정답입니다. `형식 지정자 %d에 정수 인수를 전달하지 않음` 상황과 빈 입력 같은 경계도 함께 설명해 보세요."
    commonMistakes:
      - 형식 지정자 %d에 정수 인수를 전달하지 않음
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-03-c-program-output
    ...
    answerIndex: 0
    explanation:
      "main 시작  →  %d에 30 대입  →  study: 30 출력  →  종료 순서로 실행되어 출력은 'study: 30'입니다. `printf` 부분이 마지막 단계를
  ...
  - id: quiz-day-03-c-program-model
    ...
    answerIndex: 1
    explanation:
      main은 프로그램 진입문이고 printf는 형식 문자열의 빈자리에 값을 채워 출력합니다. 이 설명이 맞는 이유는 `정수를 지정한 형식으로 출력함` 동작을 지키는 조건과 같기
      때문입니다. `형식 지정자 %d에 정수 인수를 전달하지 않음` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-03-c-program-pitfall
    ...
    answerIndex: 2
    explanation:
      "형식 지정자 %d에 정수 인수를 전달하지 않음. 이 실수가 나오면 원본 추적 `main 시작 → %d에 30 대입 → study: 30 출력 → 종료`대로 'study:
  ...
  - id: quiz-day-03-c-program-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 0
    explanation: "문법은 달라도 `정수를 지정한 형식으로 출력함` 목적과 입력·출력은 유지합니다. 실행 결과 'study: 30'로 대조하면 옮김이 맞는지 확인할 수 있습니다."
---
...
원본 실행 결과는 `study: 30`입니다. 아래 코드에서 원본과 다른 줄을 먼저 찾으세요.
...
원본 출력은 `study: 30`이고, 바뀐 코드의 실행 결과는 `study: 31`입니다. 바뀐 줄은 `printf("study: %d\n", 30);`에서 `printf("study: %d\n", 31);`로 바뀌었습니다. 바뀐 프로그램은 `main 시작` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `study: 31`가 됩니다. 원본 추적 `main 시작  →  %d에 30 대입  →  study: 30 출력  →  종료`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
