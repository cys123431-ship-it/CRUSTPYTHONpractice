...
exercises:
  ...
  - id: ex-day-79-dynamic-programming-fill
    ...
    answer:
      ...
      \ if step >= 2 else 0)\nprint(ways[4])"
    hint: ways[0]=1입니다
    explanation: 빈칸에 들어갈 표현은 'ways[step-1]'입니다. ways[0]=1입니다 ways[1..4]가 1,2,3,5가 됩니다 4칸 경우 수 5를 출력합니다
    commonMistakes:
      - 기저 경우 수 0을 0으로 두어 모든 값이 0이 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-79-dynamic-programming-guided
    ...
    explanation:
      바꾼 뒤 출력은 '5'입니다. 원본 출력과 같습니다('5'). ways의 0번을 1로 고정한 뒤에는 같은 점화식으로 쌓으므로 결과가 같습니다. 출력이 같아도 바뀐 줄(ways
      = [1] * 5) 이후의 중간 상태가 같은지는 원본 추적과 대조해야 합니다.
    commonMistakes:
      - 기저 경우 수 0을 0으로 두어 모든 값이 0이 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-79-dynamic-programming-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 1
    explanation: ways[0]=1  →  1  →  2  →  3  →  5 순서로 실행되어 출력은 '5'입니다.
...
바꾼 뒤 출력도 `5`입니다. ways의 0번을 1로 고정한 뒤에는 같은 점화식으로 쌓으므로 결과가 같습니다. 출력이 같다고 해서 중간 상태까지 같은 것은 아닙니다. 다른 줄(`ways = [1] * 5`)부터 원본 추적과 비교해 보세요.
