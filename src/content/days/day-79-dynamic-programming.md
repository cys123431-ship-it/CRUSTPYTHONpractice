...
계단을 한 칸·두 칸 오르는 경우 수처럼 동일한 작은 문제를 여러 번 쓰면 결과를 저장해 재사용합니다. Day 79 "`DP로 중복 계산 줄이기`"에서는 Python 코드가 `5`을(를) 만드는 과정을 따라가며, 작은 문제의 답을 저장해 결합 동작이 왜 필요한지 확인합니다. 작은 문제의 답을 저장해 결합을(를) 빠뜨리면 `기저 경우 수 0을 0으로 두어 모든 값이 0이 됨` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 78](/learn/day-78-greedy)에서는 "그리디 선택과 반례"을(를) 배웠습니다. "그리디 선택과 반례"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`DP로 중복 계산 줄이기`"에서 새로 달라지는 조건을 찾아보세요. Day 79의 답은 `5`이며, 핵심 표현은 `ways[step-1]`입니다.

## 머릿속 그림

ways[n]은 n칸까지 오는 경우 수이며 마지막 이동은 n-1 또는 n-2에서 옵니다. Day 79에서는 아래 흐름 순서대로 상태가 바뀌며, `ways[step-1]`이(가) 결과를 가릅니다.

```text
ways[0]=1  →  1  →  2  →  3  →  5
```

위 흐름에서 `ways[step-1]`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `5`입니다.
...
아래 Python 코드는 Day 79 "`DP로 중복 계산 줄이기`"의 독립 예제입니다. 전체 5줄에서 `ways[step-1]`이(가) 핵심이며, 실행 결과는 `5`입니다.
...
하단 Python 실행 영역에 같은 코드가 들어 있습니다. 먼저 `5`을(를) 가리고 예측한 뒤 실행해 비교하세요.
...
위 단계에서 `ways[step-1]`이(가) 빠지면 `5`이(가) 나오지 않습니다. `처음 등장하는 숫자를 0에서 1로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `5`이(가) 됩니다. `ways[0]=1` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `5`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `ways[step-1]`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 79의 `DP로 중복 계산 줄이기`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 기저 경우 수 0을 0으로 두어 모든 값이 0이 됨.** 정상 코드에서는 `5`이(가) 출력됩니다.
...
핵심 동작 **작은 문제의 답을 저장해 결합**은(는) 세 언어에서 같은 입력과 출력(`5`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `크기 n+1 배열과 경계 검사` 규칙으로 "`DP로 중복 계산 줄이기`"의 `5`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `리스트를 위에서 아래로 채움` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `Vec<usize>와 인덱스 검사` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`DP로 중복 계산 줄이기`"이(가) 필요한 상황을 `작은 문제의 답을 저장해 결합` 동작으로 설명해 보세요.
- 예제에서 `ways[step-1]`이(가) 실행되기 직전의 상태와 직후의 출력 `5`을(를) 말해 보세요.
- "`기저 경우 수 0을 0으로 두어 모든 값이 0이 됨`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `작은 문제의 답을 저장해 결합` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

ways[n]은 n칸까지 오는 경우 수이며 마지막 이동은 n-1 또는 n-2에서 옵니다. Day 79 "`DP로 중복 계산 줄이기`"의 예제는 `ways[step-1]`을(를) 실행해 `5`을(를) 출력합니다. "`기저 경우 수 0을 0으로 두어 모든 값이 0이 됨`" 여부를 확인하고 Day 79을(를) 완료하세요.
