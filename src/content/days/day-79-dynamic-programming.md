...
exercises:
  - id: ex-day-79-dynamic-programming-predict
    ...
    starter:
      ...
      \ if step >= 2 else 0)\nprint(ways[4])"
    answer: "5"
    hint: ways[n]은 n칸까지 오는 경우 수이며 마지막 이동은 n-1 또는 n-2에서 옵니다. 설명을 떠올리고 `ways[0]=1` 단계부터 순서대로 적어 보세요.
    explanation:
      ways[0]=1  →  1  →  2  →  3  →  5 순서로 실행됩니다. `ways[step-1]` 부분이 `5` 단계를 확정해 최종 출력 '5'가 됩니다. 이 흐름을
      떠올리면 `작은 문제의 답을 저장해 결합` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-79-dynamic-programming-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: "'DP로 중복 계산 줄이기' 개념의 핵심 표현을 스스로 적는다."
    ...
    answer:
      "ways = [0] * 5\nways[0] = 1\nfor step in range(1, 5):\n    ways[step] = ways[step-1] + (ways[step-2]\
      \ if step >= 2 else 0)\nprint(ways[4])"
    hint: 힌트 문장을 완전하게 읽으면 `ways[0]=1입니다` 단계에 필요한 표현이 `ways[step-1]`입니다.
    explanation:
      빈칸에 들어갈 표현은 'ways[step-1]'입니다. `ways[step] = ways[step-1] + (ways[step-2] if step >= 2 else 0)` 줄을
      완성해야 `작은 문제의 답을 저장해 결합` 동작이 이어져 실행 결과 '5'가 됩니다. 힌트의 첫 단계 `ways[0]=1입니다`이 바로 이 줄입니다. 이어서 ways[1..4]가 1,2,3,5가
      됩니다 4칸 경우 수 5를 출력합니다 순서로 진행됩니다.
    commonMistakes:
      - 기저 경우 수 0을 0으로 두어 모든 값이 0이 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-79-dynamic-programming-guided
    ...
    explanation:
      바꾼 뒤 출력은 '5'입니다. 원본 출력 '5'와 같습니다. 바뀐 줄은 `ways = [1] * 5`입니다. 바뀐 프로그램도 `ways[0]=1` 단계에서 시작해 바뀐 줄을
      지나 최종 `5`로 끝납니다. ways의 0번을 1로 고정한 뒤에는 같은 점화식으로 쌓으므로 결과가 같습니다. 출력은 같지만 바뀐 줄 부분의 중간 값은 달라졌다가 같은 최종 값으로 이어진다는 점을
      위 추적 순서와 대조해 확인할 수 있습니다.
    commonMistakes:
      - 기저 경우 수 0을 0으로 두어 모든 값이 0이 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-79-dynamic-programming-debug
    ...
    answer:
      ...
      \ if step >= 2 else 0)\nprint(ways[4])"
    hint:
      ways[n]은 n칸까지 오는 경우 수이며 마지막 이동은 n-1 또는 n-2에서 옵니다. 설명과 어긋나는 줄을 찾으세요. `기저 경우 수 0을 0으로 두어 모든 값이 0이 됨` 상황이 단서가
      됩니다.
    explanation:
      틀린 줄은 `ways[step] = ways[step-2] + (ways[step-2] if step >= 2 else 0)`입니다. 여기서는 `ways[step-2]`을 써서
      `ways[step-1]` 동작이 깨집니다. 이대로 실행하면 `기저 경우 수 0을 0으로 두어 모든 값이 0이 됨` 문제가 생겨 원본 추적 `ways[0]=1  →  1  →  2  →  3  →  5`대로
      '5'가 나오지 않습니다. 고친 줄 `ways[step] = ways[step-1] + (ways[step-2] if step >= 2 else 0)`에서는 `ways[step-1]`가 `작은
      문제의 답을 저장해 결합` 동작을 지켜 '5'까지 도달합니다.
    commonMistakes:
      - 기저 경우 수 0을 0으로 두어 모든 값이 0이 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-79-dynamic-programming-independent
    ...
    answer:
      "ways = [0] * 5\nways[0] = 1\nfor step in range(1, 5):\n    ways[step] = ways[step-1] + (ways[step-2]\
      \ if step >= 2 else 0)\nprint(ways[4])"
    hint: ways[n]은 n칸까지 오는 경우 수이며 마지막 이동은 n-1 또는 n-2에서 옵니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `ways[step-1]` 부분이 `작은 문제의 답을 저장해 결합` 동작을 지켜 실행 결과 '5'가 됩니다. 같은 개념을 다른 입력으로
      바꿔도 `ways[step-1]`부터 `5`까지 추적할 수 있으면 정답입니다. `기저 경우 수 0을 0으로 두어 모든 값이 0이 됨` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - 기저 경우 수 0을 0으로 두어 모든 값이 0이 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-79-dynamic-programming-output
    ...
    answerIndex: 1
    explanation:
      ways[0]=1  →  1  →  2  →  3  →  5 순서로 실행되어 출력은 '5'입니다. `ways[step-1]` 부분이 마지막 단계를 확정하므로 다른 선택지는 이
      추적과 맞지 않습니다.
  - id: quiz-day-79-dynamic-programming-model
    ...
    answerIndex: 2
    explanation:
      ways[n]은 n칸까지 오는 경우 수이며 마지막 이동은 n-1 또는 n-2에서 옵니다. 이 설명이 맞는 이유는 `작은 문제의 답을 저장해 결합` 동작을 지키는 조건과 같기
      때문입니다. `기저 경우 수 0을 0으로 두어 모든 값이 0이 됨` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-79-dynamic-programming-pitfall
    ...
    answerIndex: 0
    explanation:
      기저 경우 수 0을 0으로 두어 모든 값이 0이 됨. 이 실수가 나오면 원본 추적 `ways[0]=1 → 1 → 2 → 3 → 5`대로 '5'가 나오지 않으므로 먼저 확인해야
      합니다.
  - id: quiz-day-79-dynamic-programming-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 1
    explanation: 문법은 달라도 `작은 문제의 답을 저장해 결합` 목적과 입력·출력은 유지합니다. 실행 결과 '5'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
playgroundSource:
  "ways = [0] * 5\nways[0] = 1\nfor step in range(1, 5):\n    ways[step] = ways[step-1] + (ways[step-2]\
  \ if step >= 2 else 0)\nprint(ways[4])"
---
...
바꾼 뒤 출력도 `5`입니다. ways의 0번을 1로 고정한 뒤에는 같은 점화식으로 쌓으므로 결과가 같습니다. 바뀐 프로그램도 `ways[0]=1` 단계에서 시작해 `ways = [1] * 5` 줄을 지나 최종 `5`로 끝납니다. 원본 추적 `ways[0]=1  →  1  →  2  →  3  →  5`와 바뀐 줄 이후를 순서대로 놓으면 출력은 같아도 중간 값이 어디서 달라졌다가 합쳐지는지 확인할 수 있습니다.
