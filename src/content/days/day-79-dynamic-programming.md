...
exercises:
  - id: ex-day-79-dynamic-programming-predict
    ...
    starter:
      ...
      \ if step >= 2 else 0)\nprint(ways[4])"
    answer: "5"
    hint: ways[0]=1에서 시작해 5까지 순서대로 적어 보세요. ways[n]은 n칸까지 오는 경우 수이며 마지막 이동은 n-1 또는 n-2에서 옵니다.
    explanation:
      1. ways[0]=1입니다. 2. ways[1..4]가 1,2,3,5가 됩니다. 3. 4칸 경우 수 5를 출력합니다. `ways[0]=1 → 1 → 2 → 3 → 5` 흐름으로
      실제 출력 `5`가 됩니다. 핵심 `ways[step-1]`은 바로 앞 칸의 경우 수를 읽는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-79-dynamic-programming-fill
    ...
    answer:
      "ways = [0] * 5\nways[0] = 1\nfor step in range(1, 5):\n    ways[step] = ways[step-1] + (ways[step-2]\
      \ if step >= 2 else 0)\nprint(ways[4])"
    hint: 필요한 표현은 작은 문제의 답을 저장해 결합 동작을 잇는 ways[step-1]입니다.
    explanation:
      빈칸에 들어갈 표현은 'ways[step-1]'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. ways[0]=1입니다. 2. ways[1..4]가 1,2,3,5가 됩니다.
      3. 4칸 경우 수 5를 출력합니다. `ways[0]=1 → 1 → 2 → 3 → 5` 흐름으로 실제 출력 `5`가 됩니다. 핵심 `ways[step-1]`은 바로 앞 칸의 경우 수를 읽는 자리에
      쓰입니다.
    commonMistakes:
      - 기저 경우 수 0을 0으로 두어 모든 값이 0이 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-79-dynamic-programming-guided
    ...
    answer:
      "ways = [1] * 5\nways[0] = 1\nfor step in range(1, 5):\n    ways[step] = ways[step-1] + (ways[step-2]\
      \ if step >= 2 else 0)\nprint(ways[4])"
    hint: 처음 등장하는 숫자를 0에서 1로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '5'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '5'입니다. 원본과 수정본 모두 `5`를 출력한다. ways의 0번을 1로 고정한 뒤에는 같은 점화식으로 쌓으므로 결과가 같습니다. 시작값 고정 뒤 쌓는 과정은
      그대로이므로 중간 상태가 달라졌다고 쓰지 않는다.
    commonMistakes:
      - 기저 경우 수 0을 0으로 두어 모든 값이 0이 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-79-dynamic-programming-debug
    ...
    answer:
      "ways = [0] * 5\nways[0] = 1\nfor step in range(1, 5):\n    ways[step] = ways[step-1] + (ways[step-2]\
      \ if step >= 2 else 0)\nprint(ways[4])"
    hint: 기저 경우 수 0을 0으로 두어 모든 값이 0이 됨 상황에서 어긋나는 줄을 ways[n]은 n칸까지 오는 경우 수이며 마지막 이동은 n-1 또는 n-2에서 옵니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `ways[step] = ways[step-2] + (ways[step-2] if step >= 2 else 0)`입니다. 바로 앞 칸 대신 두 칸 앞만 더하므로,
      step=1에서는 ways[-1]인 마지막 칸을 읽어 어긋나고 최종 `4`가 됩니다. 오류 분류는 잘못된 출력입니다. 앞 칸 읽기가 빠진 첫 순간이 원인입니다. 고친 줄 `ways[step] =
      ways[step-1] + (ways[step-2] if step >= 2 else 0)`에서는 `작은 문제의 답을 저장해 결합` 동작이 지켜집니다. 정상 코드는 기저 고정→앞 두 칸 합산→5
      출력의 순서로 `5`를 출력합니다.
    commonMistakes:
      - 기저 경우 수 0을 0으로 두어 모든 값이 0이 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-79-dynamic-programming-independent
    ...
    answer:
      "ways = [0] * 5\nways[0] = 1\nfor step in range(1, 5):\n    ways[step] = ways[step-1] + (ways[step-2]\
      \ if step >= 2 else 0)\nprint(ways[4])"
    hint: "`ways[0]=1`로 두는 이유를 설명하세요. n이 음수일 때 규칙도 정하세요. 예상 출력과 경계 조건도 함께 적어 보세요."
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. `ways[0]=1`로 두는 이유를 설명하세요. n이 음수일 때 규칙도 정하세요. 같은 작은 문제의 답을 저장해 결합 동작을
      구현하고 실행 결과 '5'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 기저 경우 수 0을 0으로 두어 모든 값이 0이 됨
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-79-dynamic-programming-output
    ...
    answerIndex: 1
    explanation:
      먼저 ways[0]=1로 고정됩니다. 이어서 앞 두 칸을 더해 1,2,3,5가 쌓이고, 4칸 경우 수 5를 출력하므로 `5`가 됩니다. `실행 전에 반드시 오류가 난다`는 틀린
      선택지인데 점화식 실행이 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기 때문입니다. 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-79-dynamic-programming-model
    question: "'DP로 중복 계산 줄이기' 개념을 이해하는 데 맞는 설명은?"
    choices:
    ...
    answerIndex: 2
    explanation:
      ways[n]은 n칸까지 오는 경우 수이며 마지막 이동은 n-1 또는 n-2에서 온다는 뜻은, 두 앞 칸의 합이 답이라는 뜻입니다. `기저 경우 수 0을 0으로 두어 모든 값이
      0이 됨`은 반대 사례인데, 시작값이 어긋나면 뒤가 전부 어긋나기 때문입니다.
  - id: quiz-day-79-dynamic-programming-pitfall
    ...
    answerIndex: 0
    explanation: 먼저 확인할 실수는 `기저 경우 수 0을 0으로 두어 모든 값이 0이 됨`입니다. 잘못된 코드는 실행은 되지만 `4`라는 잘못된 출력을 냅니다. 앞 칸 읽기를 먼저 확인하세요.
  - id: quiz-day-79-dynamic-programming-transfer
    ...
    answerIndex: 1
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `작은 문제의 답을 저장해 결합`이라는 의미와 입력·출력 계약입니다. C의 배열 점화와 Rust의 벡터 점화로 같은 동작을 구현하고,
      실행 결과 `5`로 대조하면 옮김이 맞는지 확인할 수 있습니다.
playgroundSource:
  "ways = [0] * 5\nways[0] = 1\nfor step in range(1, 5):\n    ways[step] = ways[step-1] + (ways[step-2]\
  \ if step >= 2 else 0)\nprint(ways[4])"
---
...
예제의 핵심 표현은 `ways[step-1]`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본 실행 결과는 `5`입니다. 아래 코드에서 원본과 달라지는 첫 줄을 표시하고, 그 줄의 변경 전후 값을 적어 보세요.
...
원본과 수정본 모두 `5`를 출력한다. ways의 0번을 1로 고정한 뒤에는 같은 점화식으로 쌓으므로 결과가 같습니다. 시작값 고정 뒤 쌓는 과정은 그대로이므로 중간 상태가 달라졌다고 쓰지 않는다.
...
두 줄을 나란히 놓으면 정상 줄 `ways[step] = ways[step-1] + (ways[step-2] if step >= 2 else 0)`이 `ways[n]은 n칸까지 오는 경우 수이며 마지막 이동은 n-1 또는 n-2에서 옵니다.` 설명과 맞고, 잘못된 줄은 `기저 경우 수 0을 0으로 두어 모든 값이 0이 됨` 쪽으로 어긋납니다. 결과가 예상과 다르면 `ways[0]=1`부터 `5`까지 처음 어긋난 곳을 찾습니다.
...
예측(`5` 맞히기) → 빈칸(`ways[step-1]` 채우기) → 변경(`처음 등장하는 숫자를 0에서 1로`) → 오류 수정(`기저 경우 수 0을 0으로 두어 모든 값이 0이 됨` 찾기) → 독립 구현(`'DP로 중복 계산 줄이기' 개념을 보여주는 작은 프로그램` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `5` 및 `ways[step-1]` 설명과 대조하세요.
