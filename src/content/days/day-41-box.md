...
exercises:
  ...
  - id: ex-day-41-box-guided
    ...
    prompt: 예시를 직접 타이핑한 뒤 처음 등장하는 숫자를 40에서 41로 바꿔 보세요. 출력도 먼저 예측하세요.
    starter:
      "fn main() {\n    let boxed = Box::new(40);\n    let answer = *boxed + 2;\n    println!(\"{answer}\"\
      );\n}"
    answer:
  ...
  - id: ex-day-41-box-debug
    ...
    prompt: 아래 코드의 핵심 표현 'boxed'을 점검하고 올바른 작동을 복원하세요. Box를 C의 raw pointer처럼 직접 free해야 한다고 생각함 오류도 함께 경계하세요.
    starter:
      "fn main() {\n    let boxed = Box::new(40);\n    let answer = boxed + 2;\n    println!(\"{answer}\");\n\
      }"
    answer:
...
크기가 큰 값이나 재귀적 구조를 힙에 저장할 때 누가 소유하는지 알아야 합니다. 처음 읽을 때는 결과를 가리고 손으로 예상하세요. 예시를 직접 타이핑한 뒤 한 줄씩 바꾸며 상태를 확인하세요.

## 2. 시작 전에 확인할 것

바로 앞선 [Day 40](/learn/day-40-malloc-free)의 핵심을 한 문장으로 설명해 보세요. 모르면 위의 선행 Day 링크에서 다시 확인할 수 있습니다.

## 3. 머릿속 그림

Box<T>는 힙 값의 유일한 소유자이고 *는 안쪽 값을 읽습니다. scope가 끝나면 자동으로 해제합니다. 다음 흐름을 눈으로 확인하세요.
...
예제의 핵심 표현은 `*boxed`입니다. 전체 코드에서 이 표현을 찾아 표시하세요. 선언과 조건, 출력이 연결되는 과정을 순서대로 설명합니다.
...
3. 처음 등장하는 숫자를 40에서 41로 바꾼 뒤 어느 단계부터 결과가 달라질지 예측하세요.
4. 실행할 수 있는 환경에서 확인하고 틀린 예측의 이유를 한 문장으로 적으세요.

## 8. 자주 틀리는 지점

**확인할 실수: Box를 C의 raw pointer처럼 직접 free해야 한다고 생각함.** Box<T>는 힙 값의 유일한 소유자이고 *는 안쪽 값을 읽습니다. scope가 끝나면 자동으로 해제합니다. 결과가 예상과 다르면 입력 → 중간 상태 → 출력 중 처음 어긋난 곳을 찾습니다.

## 9. 다른 언어로 옮기기

같은 문제를 해결하더라도 선언, 타입, 메모리 관리, 오류 처리는 다릅니다. 핵심 동작은 **힙 값의 안전한 소유**입니다. 언어를 옮겨도 이 동작을 유지하세요.
...
- 왜 이 개념이 필요한가? 크기가 큰 값이나 재귀적 구조를 힙에 저장할 때 누가 소유하는지 알아야 합니다.
- 예시에서 가장 먼저 확정되는 값이나 상태는 무엇인가?
- 어떤 실수를 점검해야 하나? Box를 C의 raw pointer처럼 직접 free해야 한다고 생각함
- 다른 두 언어에서는 같은 동작을 어떤 자료형과 오류 처리로 나타내는가?

## 12. 핵심 요약과 복습

Box<T>는 힙 값의 유일한 소유자이고 *는 안쪽 값을 읽습니다. scope가 끝나면 자동으로 해제합니다. 예시의 출력은 `42`입니다. 오류를 찾을 때는 **Box를 C의 raw pointer처럼 직접 free해야 한다고 생각함** 여부를 확인하세요. 완료 버튼을 누르면 +1·+3·+7·+14·+30일 복습이 이 기기에 등록됩니다.
