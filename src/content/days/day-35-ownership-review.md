...
exercises:
  - id: ex-day-35-ownership-review-predict
    ...
    starter: "items = [1, 2]
      ...
      print(items)"
    answer: "[1, 2, 3]"
    hint: 두 이름이 같은 가변 리스트를 가리키면 어느 이름을 통해 바꿔도 같은 객체가 수정됩니다. 설명을 떠올리고 `items` 단계부터 순서대로 적어 보세요.
    explanation:
      items → 리스트  →  alias → 같은 리스트  →  append  →  [1,2,3] 순서로 실행됩니다. `alias = items` 부분이 `[1,2,3]` 단계를
      확정해 최종 출력 '[1, 2, 3]'가 됩니다. 이 흐름을 떠올리면 `공유 참조와 복사를 구별함` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-35-ownership-review-fill
    ...
    answer: "items = [1, 2]
      ...
      print(items)"
    hint: 힌트 문장을 완전하게 읽으면 `items가 리스트를 가리킵니다` 단계에 필요한 표현이 `alias = items`입니다.
    explanation:
      빈칸에 들어갈 표현은 'alias = items'입니다. `alias = items` 줄을 완성해야 `공유 참조와 복사를 구별함` 동작이 이어져 실행 결과 '[1, 2, 3]'가
      됩니다. 힌트의 첫 단계 `items가 리스트를 가리킵니다`이 바로 이 줄입니다. 이어서 alias가 동일 객체를 가리킵니다 append가 그 객체를 변경해 items에서도 3이 보입니다 순서로
      진행됩니다.
    commonMistakes:
      - 새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-35-ownership-review-guided
    ...
    explanation:
      바꾼 뒤 출력은 '[2, 2, 3]'입니다. 원본 출력 '[1, 2, 3]'에서 달라졌습니다. 바뀐 줄은 `items = [1, 2]`에서 `items = [2, 2]`로 바뀌었습니다.
      바뀐 프로그램은 `items` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `[2, 2, 3]`가 됩니다. 원본 추적 `items → 리스트  →  alias
      → 같은 리스트  →  append  →  [1,2,3]`와 바뀐 줄 이후를 순서대로 비교하면 처음 달라지는 곳이 보입니다.
    commonMistakes:
      - 새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-35-ownership-review-debug
    ...
    answer: "items = [1, 2]
      ...
      print(items)"
    hint:
      두 이름이 같은 가변 리스트를 가리키면 어느 이름을 통해 바꿔도 같은 객체가 수정됩니다. 설명과 어긋나는 줄을 찾으세요. `새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함` 상황이
      단서가 됩니다.
    explanation:
      틀린 줄은 `alias = items.copy()`입니다. 여기서는 `alias = items.copy()`을 써서 `alias = items` 동작이 깨집니다. 이대로 실행하면
      `새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함` 문제가 생겨 원본 추적 `items → 리스트  →  alias → 같은 리스트  →  append  →  [1,2,3]`대로 '[1,
      2, 3]'가 나오지 않습니다. 고친 줄 `alias = items`에서는 `alias = items`가 `공유 참조와 복사를 구별함` 동작을 지켜 '[1, 2, 3]'까지 도달합니다.
    commonMistakes:
      - 새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-35-ownership-review-independent
    ...
    answer: "items = [1, 2]
      ...
      print(items)"
    hint: 두 이름이 같은 가변 리스트를 가리키면 어느 이름을 통해 바꿔도 같은 객체가 수정됩니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `alias = items` 부분이 `공유 참조와 복사를 구별함` 동작을 지켜 실행 결과 '[1, 2, 3]'가 됩니다. 같은 개념을 다른
      입력으로 바꿔도 `alias = items`부터 `[1,2,3]`까지 추적할 수 있으면 정답입니다. `새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함` 상황과 빈 입력 같은 경계도 함께
      설명해 보세요.
    commonMistakes:
      - 새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
...
quiz:
  - id: quiz-day-35-ownership-review-output
    ...
    answerIndex: 2
    explanation:
      items → 리스트  →  alias → 같은 리스트  →  append  →  [1,2,3] 순서로 실행되어 출력은 '[1, 2, 3]'입니다. `alias = items`
  ...
  - id: quiz-day-35-ownership-review-model
    ...
    answerIndex: 0
    explanation:
      두 이름이 같은 가변 리스트를 가리키면 어느 이름을 통해 바꿔도 같은 객체가 수정됩니다. 이 설명이 맞는 이유는 `공유 참조와 복사를 구별함` 동작을 지키는 조건과 같기 때문입니다.
      `새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-35-ownership-review-pitfall
    ...
    answerIndex: 1
    explanation:
      새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함. 이 실수가 나오면 원본 추적 `items→리스트 → alias→같은 리스트 → append → [1,2,3]`대로 '[1,
      2, 3]'가 나오지 않으므로 먼저 확인해야 합니다.
  - id: quiz-day-35-ownership-review-transfer
    ...
    answerIndex: 2
    explanation: 문법은 달라도 `공유 참조와 복사를 구별함` 목적과 입력·출력은 유지합니다. 실행 결과 '[1, 2, 3]'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
...
원본 출력은 `[1, 2, 3]`이고, 바뀐 코드의 실행 결과는 `[2, 2, 3]`입니다. 바뀐 줄은 `items = [1, 2]`에서 `items = [2, 2]`로 바뀌었습니다. 바뀐 프로그램은 `items` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `[2, 2, 3]`가 됩니다. 원본 추적 `items → 리스트  →  alias → 같은 리스트  →  append  →  [1,2,3]`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
...
두 줄을 나란히 놓으면 정상 줄 `alias = items`이 `두 이름이 같은 가변 리스트를 가리키면 어느 이름을 통해 바꿔도 같은 객체가 수정됩니다.` 설명과 맞고, 잘못된 줄은 `새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `items`부터 `[1, 2, 3]`까지 처음 어긋난 곳을 찾습니다.
