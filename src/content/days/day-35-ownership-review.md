...
exercises:
  ...
  - id: ex-day-35-ownership-review-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: 주소와 소유권 회상의 핵심 표현을 스스로 적는다.
    ...
    explanation:
      빈칸에는 'alias = items'이 들어갑니다. items가 리스트를 가리킵니다 alias가 동일 객체를 가리킵니다 append가 그 객체를 변경해 items에서도 3이
      보입니다
    commonMistakes:
      - 새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-35-ownership-review-guided
    ...
    explanation:
      예시 답안에서는 처음 등장하는 숫자를 1에서 2로 바꿨습니다. 원래 출력은 '[1, 2, 3]'입니다. 바꾼 줄에서 시작해 중간 값과 마지막 출력을 다시 추적하세요. 출력이
...
quiz:
  - id: quiz-day-35-ownership-review-output
    ...
    choices:
      ...
      - "[1, 2, 3]"
    answerIndex: 2
    explanation: items → 리스트  →  alias → 같은 리스트  →  append  →  [1,2,3] 순서로 실행되어 '[1, 2, 3]'을 출력합니다.
  - id: quiz-day-35-ownership-review-model
    question: 주소와 소유권 회상을 이해하는 데 맞는 설명은?
    choices:
...
포인터·참조·복사의 차이를 스스로 설명할 수 있어야 공유 상태의 버그를 찾아냅니다. Day 35 "`주소와 소유권 회상`"에서는 Python 코드가 `[1, 2, 3]`을(를) 만드는 과정을 따라가며, 공유 참조와 복사를 구별함 동작이 왜 필요한지 확인합니다. 공유 참조와 복사를 구별함을(를) 빠뜨리면 `새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 34](/learn/day-34-array-pointer)에서는 "C 배열 매개변수와 길이"을(를) 배웠습니다. "C 배열 매개변수와 길이"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`주소와 소유권 회상`"에서 새로 달라지는 조건을 찾아보세요. Day 35의 답은 `[1, 2, 3]`이며, 핵심 표현은 `alias = items`입니다.

## 머릿속 그림

두 이름이 같은 가변 리스트를 가리키면 어느 이름을 통해 바꿔도 같은 객체가 수정됩니다. Day 35에서는 아래 흐름 순서대로 상태가 바뀌며, `alias = items`이(가) 결과를 가릅니다.

```text
items → 리스트  →  alias → 같은 리스트  →  append  →  [1,2,3]
```

위 흐름에서 `alias = items`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `[1, 2, 3]`입니다.
...
아래 Python 코드는 Day 35 "`주소와 소유권 회상`"의 독립 예제입니다. 전체 4줄에서 `alias = items`이(가) 핵심이며, 실행 결과는 `[1, 2, 3]`입니다.
...
위 단계에서 `alias = items`이(가) 빠지면 `[1, 2, 3]`이(가) 나오지 않습니다. `처음 등장하는 숫자를 1에서 2로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `[1, 2, 3]`이(가) 됩니다. `items` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `[1, 2, 3]`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `alias = items`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 35의 `주소와 소유권 회상`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함.** 정상 코드에서는 `[1, 2, 3]`이(가) 출력됩니다.
...
핵심 동작 **공유 참조와 복사를 구별함**은(는) 세 언어에서 같은 입력과 출력(`[1, 2, 3]`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `포인터 복사는 주소 공유, 별도 malloc 필요` 규칙으로 "`주소와 소유권 회상`"의 `[1, 2, 3]`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `대입은 별칭, copy는 새 리스트` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `move·&·clone 중 의도 선택` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`[1, 2, 3]` 맞히기) → 빈칸(`alias = items` 채우기) → 변경(`처음 등장하는 숫자를 1에서 2로`) → 오류 수정(`새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함` 찾기) → 독립 구현(`주소와 소유권 회상을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `[1, 2, 3]` 및 `alias = items` 설명과 대조하세요.

## 스스로 설명하기

- "`주소와 소유권 회상`"이(가) 필요한 상황을 `공유 참조와 복사를 구별함` 동작으로 설명해 보세요.
- 예제에서 `alias = items`이(가) 실행되기 직전의 상태와 직후의 출력 `[1, 2, 3]`을(를) 말해 보세요.
- "`새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `공유 참조와 복사를 구별함` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

두 이름이 같은 가변 리스트를 가리키면 어느 이름을 통해 바꿔도 같은 객체가 수정됩니다. Day 35 "`주소와 소유권 회상`"의 예제는 `alias = items`을(를) 실행해 `[1, 2, 3]`을(를) 출력합니다. "`새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함`" 여부를 확인하고 Day 35을(를) 완료하세요.
