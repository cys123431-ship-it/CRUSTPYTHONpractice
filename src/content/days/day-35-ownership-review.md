...
exercises:
  - id: ex-day-35-ownership-review-predict
    ...
    starter: "items = [1, 2]
      ...
      print(items)"
    answer: "[1, 2, 3]"
    hint: items에서 시작해 [1,2,3]까지 순서대로 적어 보세요. 두 이름이 같은 가변 리스트를 가리키면 어느 이름을 통해 바꿔도 같은 객체가 수정됩니다.
    explanation:
      1. items가 리스트를 가리킵니다. 2. alias가 동일 객체를 가리킵니다. 3. append가 그 객체를 변경해 items에서도 3이 보입니다. `items→리스트 →
      alias→같은 리스트 → append → [1,2,3]` 흐름으로 실제 출력 `[1, 2, 3]`이 됩니다. 핵심 `alias = items`는 새 객체가 아니라 같은 객체를 가리키는 자리에
      쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-35-ownership-review-fill
    ...
    answer: "items = [1, 2]
      ...
      print(items)"
    hint: 필요한 표현은 공유 참조와 복사를 구별함 동작을 잇는 alias = items입니다.
    explanation:
      빈칸에 들어갈 표현은 'alias = items'입니다. 이 표현이 없으면 실행 결과는 달라집니다. 1. items가 리스트를 가리킵니다. 2. alias가 동일 객체를 가리킵니다.
      3. append가 그 객체를 변경해 items에서도 3이 보입니다. `items→리스트 → alias→같은 리스트 → append → [1,2,3]` 흐름으로 실제 출력 `[1, 2, 3]`이
      됩니다. 핵심 `alias = items`는 새 객체가 아니라 같은 객체를 가리키는 자리에 쓰입니다.
    commonMistakes:
      - 새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-35-ownership-review-guided
    ...
    answer: "items = [2, 2]
      ...
      print(items)"
    hint: 처음 등장하는 숫자를 1에서 2로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '[1, 2, 3]'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '[2, 2, 3]'입니다. 원본은 `[1, 2, 3]`, 수정본은 `[2, 2, 3]`이다. 첫 변경 줄 `items = [1, 2]`에서 `items =
      [2, 2]`로 첫 값을 바꾸면, 별칭을 통한 추가 뒤에도 첫 자리는 바뀐 값 그대로 남는다.
    commonMistakes:
      - 새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-35-ownership-review-debug
    ...
    answer: "items = [1, 2]
      ...
      print(items)"
    hint: 새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함 상황에서 어긋나는 줄을 두 이름이 같은 가변 리스트를 가리키면 어느 이름을 통해 바꿔도 같은 객체가 수정됩니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `alias = items.copy()`입니다. 별도의 바깥 리스트가 생겨 뒤의 추가가 원본에 닿지 않으므로 `items`는 `[1, 2]` 그대로 출력됩니다. 오류
      분류는 잘못된 출력입니다. 공유가 끊기는 첫 순간은 복사 줄입니다. 고친 줄 `alias = items`에서는 `공유 참조와 복사를 구별함` 동작이 지켜집니다. 정상 코드는 원본 생성→별칭 공유→공유
      객체 변경→원본 읽기의 순서로 `[1, 2, 3]`을 출력합니다.
    commonMistakes:
      - 새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-35-ownership-review-independent
    ...
    answer: "items = [1, 2]
      ...
      print(items)"
    hint: "`items.copy()`는 바깥만 따로 만들고 안의 가변 객체는 공유할 수 있습니다. 무엇을 복사할지 먼저 물으세요. 예상 출력과 경계 조건도 함께 적어 보세요."
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. `items.copy()`는 바깥만 따로 만들고 안의 가변 객체는 공유할 수 있습니다. 무엇을 복사할지 먼저 물으세요. 같은
      공유 참조와 복사를 구별함 동작을 구현하고 실행 결과 '[1, 2, 3]'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-35-ownership-review-output
    ...
    answerIndex: 2
    explanation:
      먼저 items가 [1,2] 리스트를 가리킵니다. 이어서 alias가 같은 객체를 가리키고, append가 그 객체를 바꿔 items에서도 3이 보이므로 출력 `[1, 2,
      3]`이 됩니다. `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 별칭과 메서드 호출이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기
  ...
  - id: quiz-day-35-ownership-review-model
    ...
    answerIndex: 0
    explanation:
      두 이름이 같은 가변 리스트를 가리키면 어느 이름을 통해 바꿔도 같은 객체가 수정된다는 뜻은, 이름은 둘이라도 객체는 하나라는 뜻입니다. `새 이름을 붙이면 무조건 별도의 객체가
      생긴다고 생각함`은 반대 사례인데, 대입은 객체를 공유하기 때문입니다.
  - id: quiz-day-35-ownership-review-pitfall
    ...
    answerIndex: 1
    explanation:
      먼저 확인할 실수는 `새 이름을 붙이면 무조건 별도의 객체가 생긴다고 생각함`입니다. 잘못된 코드는 실행은 되지만 `[1, 2]`라는 잘못된 출력을 냅니다. 별칭인지 복사인지
      먼저 확인하세요.
  - id: quiz-day-35-ownership-review-transfer
    ...
    answerIndex: 2
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `공유 참조와 복사를 구별함`이라는 의미와 입력·출력 계약입니다. C의 포인터 공유와 Rust의 참조·복제로 같은 동작을 구현하고, 실행
...
예제의 핵심 표현은 `alias = items`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본은 `[1, 2, 3]`, 수정본은 `[2, 2, 3]`이다. 첫 변경 줄 `items = [1, 2]`에서 `items = [2, 2]`로 첫 값을 바꾸면, 별칭을 통한 추가 뒤에도 첫 자리는 바뀐 값 그대로 남는다.
