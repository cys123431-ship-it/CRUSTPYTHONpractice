...
exercises:
  ...
  - id: ex-day-76-divide-conquer-fill
    ...
    answer:
      ...
      \ max(maximum(values[:mid]), maximum(values[mid:]))\nprint(maximum([3, 9, 2, 7]))"
    hint: "[3,9]의 최대는 9입니다"
    explanation: 빈칸에는 'len(values) == 1'이 들어갑니다. [3,9]의 최대는 9입니다 [2,7]의 최대는 7입니다 둘을 비교해 9를 반환합니다
    commonMistakes:
      - 빈 입력 처리 정책 없이 길이 0의 배열도 호출함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-76-divide-conquer-guided
    ...
    explanation:
      예시 답안에서는 처음 등장하는 숫자를 1에서 2로 바꿨습니다. 원래 출력은 '9'입니다. 바꾼 줄에서 시작해 중간 값과 마지막 출력을 다시 추적하세요. 출력이 같더라도 입력·조건·중간
      상태가 달라졌는지 확인해야 합니다.
    commonMistakes:
      - 빈 입력 처리 정책 없이 길이 0의 배열도 호출함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
...
quiz:
  - id: quiz-day-76-divide-conquer-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 1
    explanation: "[3,9,2,7]  →  max(3,9)=9 + max(2,7)=7  →  9 순서로 실행되어 '9'을 출력합니다."
  - id: quiz-day-76-divide-conquer-model
    question: 분할 정복의 재귀 결합을 이해하는 데 맞는 설명은?
    choices:
...
하나의 큰 문제를 두 하위 문제로 나누고 각각의 답을 결합하는 사고가 여러 알고리즘의 공통 뼈대입니다. Day 76 "`분할 정복의 재귀 결합`"에서는 Python 코드가 `9`을(를) 만드는 과정을 따라가며, 부분 답을 결합해 전체 답 생성 동작이 왜 필요한지 확인합니다. 부분 답을 결합해 전체 답 생성을(를) 빠뜨리면 `빈 입력 처리 정책 없이 길이 0의 배열도 호출함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 75](/learn/day-75-partition)에서는 "퀵 정렬의 분할 기준"을(를) 배웠습니다. "퀵 정렬의 분할 기준"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`분할 정복의 재귀 결합`"에서 새로 달라지는 조건을 찾아보세요. Day 76의 답은 `9`이며, 핵심 표현은 `len(values) == 1`입니다.

## 머릿속 그림

원소 하나인 부분은 답이 바로 그 값이고 두 절반의 최대 중 큰 것이 전체 최대입니다. Day 76에서는 아래 흐름 순서대로 상태가 바뀌며, `len(values) == 1`이(가) 결과를 가릅니다.

```text
[3,9,2,7]  →  max(3,9)=9 + max(2,7)=7  →  9
```

위 흐름에서 `len(values) == 1`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `9`입니다.
...
아래 Python 코드는 Day 76 "`분할 정복의 재귀 결합`"의 독립 예제입니다. 전체 5줄에서 `len(values) == 1`이(가) 핵심이며, 실행 결과는 `9`입니다.
...
하단 Python 실행 영역에 같은 코드가 들어 있습니다. 먼저 `9`을(를) 가리고 예측한 뒤 실행해 비교하세요.
...
위 단계에서 `len(values) == 1`이(가) 빠지면 `9`이(가) 나오지 않습니다. `처음 등장하는 숫자를 1에서 2로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `9`이(가) 됩니다. `[3,9,2,7]` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `9`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `len(values) == 1`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 76의 `분할 정복의 재귀 결합`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 빈 입력 처리 정책 없이 길이 0의 배열도 호출함.** 정상 코드에서는 `9`이(가) 출력됩니다.
...
핵심 동작 **부분 답을 결합해 전체 답 생성**은(는) 세 언어에서 같은 입력과 출력(`9`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `시작·끝 인덱스를 재귀에 전달` 규칙으로 "`분할 정복의 재귀 결합`"의 `9`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `슬라이스 복사는 추가 비용` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `&slice[..mid]로 복사 없이 분할` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`9` 맞히기) → 빈칸(`len(values) == 1` 채우기) → 변경(`처음 등장하는 숫자를 1에서 2로`) → 오류 수정(`빈 입력 처리 정책 없이 길이 0의 배열도 호출함` 찾기) → 독립 구현(`분할 정복의 재귀 결합을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `9` 및 `len(values) == 1` 설명과 대조하세요.

## 스스로 설명하기

- "`분할 정복의 재귀 결합`"이(가) 필요한 상황을 `부분 답을 결합해 전체 답 생성` 동작으로 설명해 보세요.
- 예제에서 `len(values) == 1`이(가) 실행되기 직전의 상태와 직후의 출력 `9`을(를) 말해 보세요.
- "`빈 입력 처리 정책 없이 길이 0의 배열도 호출함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `부분 답을 결합해 전체 답 생성` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

원소 하나인 부분은 답이 바로 그 값이고 두 절반의 최대 중 큰 것이 전체 최대입니다. Day 76 "`분할 정복의 재귀 결합`"의 예제는 `len(values) == 1`을(를) 실행해 `9`을(를) 출력합니다. "`빈 입력 처리 정책 없이 길이 0의 배열도 호출함`" 여부를 확인하고 Day 76을(를) 완료하세요.
