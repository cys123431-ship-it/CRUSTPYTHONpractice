...
exercises:
  - id: ex-day-76-divide-conquer-predict
    ...
    answer: "9"
    hint: "[3,9,2,7]에서 시작해 9까지 순서대로 적어 보세요. 원소 하나인 부분은 답이 바로 그 값이고 두 절반의 최대 중 큰 것이 전체 최대입니다."
    explanation:
      1. [3,9]의 최대는 9입니다. 2. [2,7]의 최대는 7입니다. 3. 둘을 비교해 9를 반환합니다. `[3,9,2,7] → max(3,9)=9 + max(2,7)=7
      → 9` 흐름으로 실제 출력 `9`가 됩니다. 핵심 `len(values) == 1`은 더 나눌 수 없는 바닥을 정하는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-76-divide-conquer-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: 분할 정복의 재귀 결합의 핵심 표현을 스스로 적는다.
    ...
    answer:
      "def maximum(values):\n    if len(values) == 1: return values[0]\n    mid = len(values) // 2\n    return\
      \ max(maximum(values[:mid]), maximum(values[mid:]))\nprint(maximum([3, 9, 2, 7]))"
    hint: 필요한 표현은 부분 답을 결합해 전체 답 생성 동작을 잇는 len(values) == 1입니다.
    explanation:
      빈칸에 들어갈 표현은 'len(values) == 1'입니다. 이 표현이 없으면 실행 중 오류로 멈춥니다. 1. [3,9]의 최대는 9입니다. 2. [2,7]의 최대는 7입니다.
      3. 둘을 비교해 9를 반환합니다. `[3,9,2,7] → max(3,9)=9 + max(2,7)=7 → 9` 흐름으로 실제 출력 `9`가 됩니다. 핵심 `len(values) == 1`은 더
      나눌 수 없는 바닥을 정하는 자리에 쓰입니다.
    commonMistakes:
      - 빈 입력 처리 정책 없이 길이 0의 배열도 호출함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-76-divide-conquer-guided
    ...
    answer:
      ...
      \ max(maximum(values[:mid]), maximum(values[mid:]))\nprint(maximum([3, 9, 2, 7]))"
    hint: 처음 등장하는 숫자를 1에서 2로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '9'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '3'입니다. 원본은 `9`, 수정본은 `3`이다. 첫 변경 줄의 기저 조건 `== 1`에서 `== 2`로 바꾸면, 길이 2 부분도 그대로 첫 원소를 돌려주므로
      각 절반의 첫 값 3과 2 중 큰 3이 된다.
    commonMistakes:
      - 빈 입력 처리 정책 없이 길이 0의 배열도 호출함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-76-divide-conquer-debug
    ...
    answer:
      "def maximum(values):\n    if len(values) == 1: return values[0]\n    mid = len(values) // 2\n    return\
      \ max(maximum(values[:mid]), maximum(values[mid:]))\nprint(maximum([3, 9, 2, 7]))"
    hint: 빈 입력 처리 정책 없이 길이 0의 배열도 호출함 상황에서 어긋나는 줄을 원소 하나인 부분은 답이 바로 그 값이고 두 절반의 최대 중 큰 것이 전체 최대입니다. 설명과 대조해 보세요.
    explanation:
      "틀린 줄은 `if len(values) == 0: return values[0]`입니다. 길이가 0인 호출에서 빈 목록의 0번을 읽으려 하므로 IndexError라는 런타임
      오류가 나고 프로그램이 멈춥니다. 오류 분류는 런타임 오류입니다. 흐름이 깨지는 첫 순간은 빈 입력의 기저 조건입니다. 고친 줄 `if len(values) == 1: return values[0]`에서는
      `부분 답을 결합해 전체 답 생성` 동작이 지켜집니다. 빈 입력 처리 정책 없이 길이 0의 배열도 호출하면 안 됩니다. 정상 코드는 절반 분할→바닥 값→큰 값 선택의 순서로 `9`를 출력합니다."
    commonMistakes:
      - 빈 입력 처리 정책 없이 길이 0의 배열도 호출함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-76-divide-conquer-independent
    ...
    answer:
      "def maximum(values):\n    if len(values) == 1: return values[0]\n    mid = len(values) // 2\n    return\
      \ max(maximum(values[:mid]), maximum(values[mid:]))\nprint(maximum([3, 9, 2, 7]))"
    hint: 빈 입력에는 최댓값이 없으니 오류나 별도 결과를 정하세요. 분할이 무조건 빠른 것은 아닙니다. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 빈 입력에는 최댓값이 없으니 오류나 별도 결과를 정하세요. 분할이 무조건 빠른 것은 아닙니다. 같은 부분 답을 결합해 전체 답
      생성 동작을 구현하고 실행 결과 '9'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 빈 입력 처리 정책 없이 길이 0의 배열도 호출함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-76-divide-conquer-output
    ...
    answerIndex: 1
    explanation:
      먼저 [3,9]와 [2,7]로 나뉩니다. 이어서 각 답 9와 7을 구한 뒤 큰 값 9를 택하므로 출력 `9`가 됩니다. `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 분할과
      기저가 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기 때문입니다. 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-76-divide-conquer-model
    ...
    answerIndex: 2
    explanation:
      원소 하나인 부분은 답이 바로 그 값이라는 뜻이고, 두 절반의 최대 중 큰 것이 전체 최대라는 뜻은 합치기가 비교라는 뜻입니다. `빈 입력 처리 정책 없이 길이 0의 배열도
      호출함`은 반대 사례인데, 빈 입력에는 최댓값이 없기 때문입니다.
  - id: quiz-day-76-divide-conquer-pitfall
    ...
    answerIndex: 0
    explanation: 먼저 확인할 실수는 `빈 입력 처리 정책 없이 길이 0의 배열도 호출함`입니다. 잘못된 코드는 빈 목록을 읽어 IndexError로 멈춥니다. 바닥 조건을 먼저 확인하세요.
  - id: quiz-day-76-divide-conquer-transfer
    ...
    answerIndex: 1
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `부분 답을 결합해 전체 답 생성`이라는 의미와 입력·출력 계약입니다. C의 재귀 분할과 Rust의 재귀 결합으로 같은 동작을 구현하고,
      실행 결과 `9`로 대조하면 옮김이 맞는지 확인할 수 있습니다.
playgroundSource:
  "def maximum(values):\n    if len(values) == 1: return values[0]\n    mid = len(values) // 2\n\
...
예제의 핵심 표현은 `len(values) == 1`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본 실행 결과는 `9`입니다. 아래 코드에서 원본과 달라지는 첫 줄을 표시하고, 그 줄의 변경 전후 값을 적어 보세요.
...
원본은 `9`, 수정본은 `3`이다. 첫 변경 줄의 기저 조건 `== 1`에서 `== 2`로 바꾸면, 길이 2 부분도 그대로 첫 원소를 돌려주므로 각 절반의 첫 값 3과 2 중 큰 3이 된다.
...
두 줄을 나란히 놓으면 정상 줄 `if len(values) == 1: return values[0]`이 `원소 하나인 부분은 답이 바로 그 값이고 두 절반의 최대 중 큰 것이 전체 최대입니다.` 설명과 맞고, 잘못된 줄은 `빈 입력 처리 정책 없이 길이 0의 배열도 호출함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `[3,9,2,7]`부터 `9`까지 처음 어긋난 곳을 찾습니다.
