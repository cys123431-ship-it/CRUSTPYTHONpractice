...
그래프에는 사이클이 있으므로 깊게 따라가더라도 방문 표시 없이는 끝나지 않을 수 있습니다. Day 81 "`깊이 우선 탐색과 순환`"에서는 Python 코드가 `3`을(를) 만드는 과정을 따라가며, 사이클을 차단하며 깊이 탐색 동작이 왜 필요한지 확인합니다. 사이클을 차단하며 깊이 탐색을(를) 빠뜨리면 `방문 표시를 빼고 A→B→C→A 사이클에서 무한 재귀함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 80](/learn/day-80-backtracking)에서는 "백트래킹과 되돌리기"을(를) 배웠습니다. "백트래킹과 되돌리기"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`깊이 우선 탐색과 순환`"에서 새로 달라지는 조건을 찾아보세요. Day 81의 답은 `3`이며, 핵심 표현은 `seen.add(node)`입니다.

## 머릿속 그림

DFS는 이웃을 재귀적으로 끝까지 따라가고 이미 방문한 정점은 즉시 반환합니다. Day 81에서는 아래 흐름 순서대로 상태가 바뀌며, `seen.add(node)`이(가) 결과를 가릅니다.

```text
A 방문  →  B 방문  →  C 방문  →  A 재방문 차단
```

위 흐름에서 `seen.add(node)`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `3`입니다.
...
아래 Python 코드는 Day 81 "`깊이 우선 탐색과 순환`"의 독립 예제입니다. 전체 8줄에서 `seen.add(node)`이(가) 핵심이며, 실행 결과는 `3`입니다.
...
위 단계에서 `seen.add(node)`이(가) 빠지면 `3`이(가) 나오지 않습니다. `'dfs("A")'을 'dfs("B")'로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `3`이(가) 됩니다. `A 방문` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 코드에서 `'dfs("A")'을 'dfs("B")'로` 바꾸면 아래와 같이 됩니다.
...
원본 출력은 `3`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `seen.add(node)`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 81의 `깊이 우선 탐색과 순환`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 방문 표시를 빼고 A→B→C→A 사이클에서 무한 재귀함.** 정상 코드에서는 `3`이(가) 출력됩니다.
...
핵심 동작 **사이클을 차단하며 깊이 탐색**은(는) 세 언어에서 같은 입력과 출력(`3`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `visited 배열과 재귀 또는 명시적 스택` 규칙으로 "`깊이 우선 탐색과 순환`"의 `3`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `seen 집합과 재귀` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `HashSet과 &graph 빌림` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`깊이 우선 탐색과 순환`"이(가) 필요한 상황을 `사이클을 차단하며 깊이 탐색` 동작으로 설명해 보세요.
- 예제에서 `seen.add(node)`이(가) 실행되기 직전의 상태와 직후의 출력 `3`을(를) 말해 보세요.
- "`방문 표시를 빼고 A→B→C→A 사이클에서 무한 재귀함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `사이클을 차단하며 깊이 탐색` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

DFS는 이웃을 재귀적으로 끝까지 따라가고 이미 방문한 정점은 즉시 반환합니다. Day 81 "`깊이 우선 탐색과 순환`"의 예제는 `seen.add(node)`을(를) 실행해 `3`을(를) 출력합니다. "`방문 표시를 빼고 A→B→C→A 사이클에서 무한 재귀함`" 여부를 확인하고 Day 81을(를) 완료하세요.
