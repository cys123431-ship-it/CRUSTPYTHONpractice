...
exercises:
  - id: ex-day-81-dfs-predict
    ...
    answer: "3"
    hint: A 방문에서 시작해 A 재방문 차단까지 순서대로 적어 보세요. DFS는 이웃을 재귀적으로 끝까지 따라가고 이미 방문한 정점은 즉시 반환합니다.
    explanation:
      1. A를 표시하고 B로 갑니다. 2. B를 표시하고 C로 갑니다. 3. C의 다음 A는 이미 방문해 돌아옵니다. `A 방문 → B 방문 → C 방문 → A 재방문 차단` 흐름으로
      실제 출력 `3`이 됩니다. 핵심 `seen.add(node)`는 방문 사실을 기록하는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-81-dfs-fill
    ...
    answer:
      "graph = {\"A\": [\"B\"], \"B\": [\"C\"], \"C\": [\"A\"]}\nseen = set()\ndef dfs(node):\n    if node in\
      \ seen: return\n    seen.add(node)\n    for nxt in graph[node]: dfs(nxt)\ndfs(\"A\")\nprint(len(seen))"
    hint: 필요한 표현은 사이클을 차단하며 깊이 탐색 동작을 잇는 seen.add(node)입니다.
    explanation:
      빈칸에 들어갈 표현은 'seen.add(node)'입니다. 이 표현이 없으면 실행 중 오류로 멈춥니다. 1. A를 표시하고 B로 갑니다. 2. B를 표시하고 C로 갑니다. 3.
      C의 다음 A는 이미 방문해 돌아옵니다. `A 방문 → B 방문 → C 방문 → A 재방문 차단` 흐름으로 실제 출력 `3`이 됩니다. 핵심 `seen.add(node)`는 방문 사실을 기록하는
      자리에 쓰입니다.
    commonMistakes:
      - 방문 표시를 빼고 A→B→C→A 사이클에서 무한 재귀함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-81-dfs-guided
    ...
    prompt: 예시를 직접 타이핑한 뒤 'dfs("A")' → 'dfs("B")' 바꿔 보세요. 출력이 바뀌는지 먼저 예측하고, 그대로라면 그 이유도 설명하세요.
    starter:
      "graph = {\"A\": [\"B\"], \"B\": [\"C\"], \"C\": [\"A\"]}\nseen = set()\ndef dfs(node):\n    if node\
      \ in seen: return\n    seen.add(node)\n    for nxt in graph[node]: dfs(nxt)\ndfs(\"A\")\nprint(len(seen))"
    answer:
      "graph = {\"A\": [\"B\"], \"B\": [\"C\"], \"C\": [\"A\"]}\nseen = set()\ndef dfs(node):\n    if node in\
      \ seen: return\n    seen.add(node)\n    for nxt in graph[node]: dfs(nxt)\ndfs(\"B\")\nprint(len(seen))"
    hint: '''dfs("A")'' → ''dfs("B")'' 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 ''3''와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.'
    explanation:
      바꾼 뒤 출력은 '3'입니다. 원본과 수정본 모두 `3`을 출력한다. 순환 구조라 시작점이 달라도 방문 집합은 같은 3개라 결과가 같습니다. 시작점만 달라지고 모이는 집합은
  ...
  - id: ex-day-81-dfs-debug
    ...
    answer:
      "graph = {\"A\": [\"B\"], \"B\": [\"C\"], \"C\": [\"A\"]}\nseen = set()\ndef dfs(node):\n    if node in\
      \ seen: return\n    seen.add(node)\n    for nxt in graph[node]: dfs(nxt)\ndfs(\"A\")\nprint(len(seen))"
    hint: 방문 표시를 빼고 A→B→C→A 사이클에서 무한 재귀함 상황에서 어긋나는 줄을 DFS는 이웃을 재귀적으로 끝까지 따라가고 이미 방문한 정점은 즉시 반환합니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `seen.remove(node)`입니다. A는 집합에 없으므로 첫 호출에서 KeyError라는 런타임 오류가 나고 프로그램이 멈춥니다. 오류 분류는 런타임 오류입니다.
      흐름이 깨지는 첫 순간은 방문 기록 줄입니다. 고친 줄 `seen.add(node)`에서는 `사이클을 차단하며 깊이 탐색` 동작이 지켜집니다. 표시를 빼면 A→B→C→A 사이클에서 끝나지 않으므로,
      기록은 필수입니다. 정상 코드는 표시→이웃 따라가기→재방문 차단의 순서로 `3`을 출력합니다.
    commonMistakes:
      - 방문 표시를 빼고 A→B→C→A 사이클에서 무한 재귀함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-81-dfs-independent
    ...
    answer:
      "graph = {\"A\": [\"B\"], \"B\": [\"C\"], \"C\": [\"A\"]}\nseen = set()\ndef dfs(node):\n    if node in\
      \ seen: return\n    seen.add(node)\n    for nxt in graph[node]: dfs(nxt)\ndfs(\"A\")\nprint(len(seen))"
    hint: 재귀가 매우 깊은 그래프에서는 재귀 깊이 제한을 고려하세요. 이웃 순서에 따라 방문 순서가 달라집니다. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 재귀가 매우 깊은 그래프에서는 재귀 깊이 제한을 고려하세요. 이웃 순서에 따라 방문 순서가 달라집니다. 같은 사이클을 차단하며
      깊이 탐색 동작을 구현하고 실행 결과 '3'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 방문 표시를 빼고 A→B→C→A 사이클에서 무한 재귀함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-81-dfs-output
    ...
    answerIndex: 0
    explanation:
      먼저 A를 표시하고 B로 갑니다. 이어서 B를 표시하고 C로 가며, C의 다음 A는 이미 방문해 돌아오므로 세 정점이 모여 출력 `3`이 됩니다. `실행 전에 반드시 오류가
      난다`는 틀린 선택지인데 재귀와 집합 연산이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기 때문입니다. 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-81-dfs-model
    question: "'깊이 우선 탐색과 순환' 개념을 이해하는 데 맞는 설명은?"
    choices:
    ...
    answerIndex: 1
    explanation:
      DFS는 이웃을 재귀적으로 끝까지 따라간다는 뜻이고, 이미 방문한 정점은 즉시 반환한다는 뜻은 순환에서 같은 작업을 반복하지 않는다는 뜻입니다. `방문 표시를 빼고 A→B→C→A
      사이클에서 무한 재귀함`은 반대 사례인데, 표시가 없으면 끝없이 돌기 때문입니다.
  - id: quiz-day-81-dfs-pitfall
    ...
    choices:
      ...
      - 방문 표시를 빼고 A→B→C→A 사이클에서 무한 재귀함
    answerIndex: 2
    explanation: 먼저 확인할 실수는 `방문 표시를 빼고 A→B→C→A 사이클에서 무한 재귀함`입니다. 잘못된 코드는 없는 원소를 빼려 해 KeyError로 멈춥니다. 기록 줄을 먼저 확인하세요.
  - id: quiz-day-81-dfs-transfer
    ...
    answerIndex: 0
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `사이클을 차단하며 깊이 탐색`이라는 의미와 입력·출력 계약입니다. C의 방문 배열과 Rust의 집합 기록으로 같은 동작을 구현하고,
      실행 결과 `3`으로 대조하면 옮김이 맞는지 확인할 수 있습니다.
playgroundSource:
  "graph = {\"A\": [\"B\"], \"B\": [\"C\"], \"C\": [\"A\"]}\nseen = set()\ndef dfs(node):\n    if\
  \ node in seen: return\n    seen.add(node)\n    for nxt in graph[node]: dfs(nxt)\ndfs(\"A\")\nprint(len(seen))"
---
...
예제의 핵심 표현은 `seen.add(node)`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본과 수정본 모두 `3`을 출력한다. 순환 구조라 시작점이 달라도 방문 집합은 같은 3개라 결과가 같습니다. 시작점만 달라지고 모이는 집합은 그대로이므로 중간 상태가 달라졌다고 쓰지 않는다.
...
두 줄을 나란히 놓으면 정상 줄 `seen.add(node)`이 `DFS는 이웃을 재귀적으로 끝까지 따라가고 이미 방문한 정점은 즉시 반환합니다.` 설명과 맞고, 잘못된 줄은 `방문 표시를 빼고 A→B→C→A 사이클에서 무한 재귀함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `A 방문`부터 `3`까지 처음 어긋난 곳을 찾습니다.
...
예측(`3` 맞히기) → 빈칸(`seen.add(node)` 채우기) → 변경(`'dfs("A")' → 'dfs("B")'`) → 오류 수정(`방문 표시를 빼고 A→B→C→A 사이클에서 무한 재귀함` 찾기) → 독립 구현(`'깊이 우선 탐색과 순환' 개념을 보여주는 작은 프로그램` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `3` 및 `seen.add(node)` 설명과 대조하세요.
