...
데이터와 관련 계산을 한 곳에 두면 사용자가 필드의 의미를 추적하기 쉽습니다. Day 52 "`Rust 구조체와 메서드`"에서는 Rust 코드가 `60`을(를) 만드는 과정을 따라가며, 데이터에 연결된 읽기 메서드 동작이 왜 필요한지 확인합니다. 데이터에 연결된 읽기 메서드을(를) 빠뜨리면 `읽기 전용 메서드에 self를 써 소유권을 불필요하게 이동함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 51](/learn/day-51-python-class)에서는 "Python 클래스와 인스턴스"을(를) 배웠습니다. "Python 클래스와 인스턴스"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`Rust 구조체와 메서드`"에서 새로 달라지는 조건을 찾아보세요. Day 52의 답은 `60`이며, 핵심 표현은 `&self`입니다.

## 머릿속 그림

impl의 &self는 소유권을 가져가지 않고 구조체 값을 읽는 메서드 수신자입니다. Day 52에서는 아래 흐름 순서대로 상태가 바뀌며, `&self`이(가) 결과를 가릅니다.

```text
s.minutes=30  →  &s 빌림  →  60 반환
```

위 흐름에서 `&self`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `60`입니다.
...
아래 Rust 코드는 Day 52 "`Rust 구조체와 메서드`"의 독립 예제입니다. 전체 3줄에서 `&self`이(가) 핵심이며, 실행 결과는 `60`입니다.
...
위 단계에서 `&self`이(가) 빠지면 `60`이(가) 나오지 않습니다. `처음 등장하는 숫자를 2에서 3로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `60`이(가) 됩니다. `s.minutes=30` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `60`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `&self`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 52의 `Rust 구조체와 메서드`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 읽기 전용 메서드에 self를 써 소유권을 불필요하게 이동함.** 정상 코드에서는 `60`이(가) 출력됩니다.
...
핵심 동작 **데이터에 연결된 읽기 메서드**은(는) 세 언어에서 같은 입력과 출력(`60`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `struct를 받는 함수와 const 포인터` 규칙으로 "`Rust 구조체와 메서드`"의 `60`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `self를 받는 인스턴스 메서드` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `impl Session와 &self` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`Rust 구조체와 메서드`"이(가) 필요한 상황을 `데이터에 연결된 읽기 메서드` 동작으로 설명해 보세요.
- 예제에서 `&self`이(가) 실행되기 직전의 상태와 직후의 출력 `60`을(를) 말해 보세요.
- "`읽기 전용 메서드에 self를 써 소유권을 불필요하게 이동함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `데이터에 연결된 읽기 메서드` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

impl의 &self는 소유권을 가져가지 않고 구조체 값을 읽는 메서드 수신자입니다. Day 52 "`Rust 구조체와 메서드`"의 예제는 `&self`을(를) 실행해 `60`을(를) 출력합니다. "`읽기 전용 메서드에 self를 써 소유권을 불필요하게 이동함`" 여부를 확인하고 Day 52을(를) 완료하세요.
