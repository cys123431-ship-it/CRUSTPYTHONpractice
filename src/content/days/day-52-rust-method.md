...
두 줄을 나란히 놓으면 정상 줄 `impl Session { fn doubled(&self) -> u32 { self.minutes * 2 } }`이 `impl의 &self는 소유권을 가져가지 않고 구조체 값을 읽는 메서드 수신자입니다.` 설명과 맞고, 잘못된 줄은 `읽기 전용 메서드에 self를 써 소유권을 불필요하게 이동함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `s.minutes=30`부터 `60`까지 처음 어긋난 곳을 찾습니다.
