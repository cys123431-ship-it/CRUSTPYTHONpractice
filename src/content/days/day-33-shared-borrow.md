...
exercises:
  - id: ex-day-33-shared-borrow-predict
    ...
    answer: 4 Rust
    hint: "&String은 소유권을 가져가지 않는 공유 참조이고 호출이 끝나도 topic의 소유자는 main입니다. 설명을 떠올리고 `topic 소유` 단계부터 순서대로 적어 보세요."
    explanation:
      topic 소유  →  &topic 빌림  →  길이 4  →  topic 여전히 유효 순서로 실행됩니다. `&topic` 부분이 `topic 여전히 유효` 단계를 확정해 최종
      출력 '4 Rust'가 됩니다. 이 흐름을 떠올리면 `소유권 이동 없이 읽기 전달` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
  ...
  - id: ex-day-33-shared-borrow-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: "'공유 빌림과 참조' 개념의 핵심 표현을 스스로 적는다."
    ...
    hint: 힌트 문장을 완전하게 읽으면 `topic이 Rust를 소유합니다` 단계에 필요한 표현이 `&topic`입니다.
    explanation:
      빈칸에 들어갈 표현은 '&topic'입니다. `println!("{} {topic}", length(&topic));` 줄을 완성해야 `소유권 이동 없이 읽기 전달` 동작이
      이어져 실행 결과 '4 Rust'가 됩니다. 힌트의 첫 단계 `topic이 Rust를 소유합니다`이 바로 이 줄입니다. 이어서 length가 주소를 읽기 전용으로 빌립니다 길이 4와 원본 Rust를
      출력합니다 순서로 진행됩니다.
    commonMistakes:
      - 참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-33-shared-borrow-guided
    ...
    prompt: "예시를 직접 타이핑한 뒤 다음을 바꾸세요: 첫 문자열을 'Rust'에서 'Rust!'로. 출력이 바뀌는지 먼저 예측하고, 그대로라면 그 이유도 설명하세요."
    starter:
      "fn length(text: &String) -> usize { text.len() }\nfn main() {\n    let topic = String::from(\"Rust\"\
      );\n    println!(\"{} {topic}\", length(&topic));\n}"
    answer:
    ...
    hint: "다음을 바꾼 뒤 원본 실행 추적과 처음 달라지는 지점을 찾아보세요: 첫 문자열을 'Rust'에서 'Rust!'로."
    explanation:
      바꾼 뒤 출력은 '5 Rust!'입니다. 원본 출력 '4 Rust'에서 달라졌습니다. 바뀐 줄은 `let topic = String::from("Rust");`에서 `let
      topic = String::from("Rust!");`로 바뀌었습니다. 바뀐 프로그램은 `topic 소유` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져
      최종 `5 Rust!`가 됩니다. 원본 추적 `topic 소유  →  &topic 빌림  →  길이 4  →  topic 여전히 유효`와 바뀐 줄 이후를 순서대로 비교하면 처음 달라지는 곳이 보입니다.
    commonMistakes:
      - 참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-33-shared-borrow-debug
    ...
    hint:
      "&String은 소유권을 가져가지 않는 공유 참조이고 호출이 끝나도 topic의 소유자는 main입니다. 설명과 어긋나는 줄을 찾으세요. `참조를 값 이동과 똑같이 생각해 원본을 쓰지
      못한다고 판단함` 상황이 단서가 됩니다."
    explanation:
      틀린 줄은 `println!("{} {topic}", length(topic.clone()));`입니다. 여기서는 `topic.clone()`을 써서 `&topic` 동작이
      깨집니다. 이대로 실행하면 `참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함` 문제가 생겨 원본 추적 `topic 소유  →  &topic 빌림  →  길이 4  →  topic 여전히
      유효`대로 '4 Rust'가 나오지 않습니다. 고친 줄 `println!("{} {topic}", length(&topic));`에서는 `&topic`가 `소유권 이동 없이 읽기 전달` 동작을
      지켜 '4 Rust'까지 도달합니다.
    commonMistakes:
      - 참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-33-shared-borrow-independent
    ...
    hint:
      "&String은 소유권을 가져가지 않는 공유 참조이고 호출이 끝나도 topic의 소유자는 main입니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께
      적으세요."
    explanation:
      한 가지 예시 해법은 위 코드입니다. `&topic` 부분이 `소유권 이동 없이 읽기 전달` 동작을 지켜 실행 결과 '4 Rust'가 됩니다. 같은 개념을 다른 입력으로 바꿔도
      `&topic`부터 `topic 여전히 유효`까지 추적할 수 있으면 정답입니다. `참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - 참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
...
quiz:
  - id: quiz-day-33-shared-borrow-output
    ...
    answerIndex: 0
    explanation:
      topic 소유  →  &topic 빌림  →  길이 4  →  topic 여전히 유효 순서로 실행되어 출력은 '4 Rust'입니다. `&topic` 부분이 마지막 단계를 확정하므로
  ...
  - id: quiz-day-33-shared-borrow-model
    ...
    answerIndex: 1
    explanation:
      "&String은 소유권을 가져가지 않는 공유 참조이고 호출이 끝나도 topic의 소유자는 main입니다. 이 설명이 맞는 이유는 `소유권 이동 없이 읽기 전달` 동작을 지키는
      조건과 같기 때문입니다. `참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다."
  - id: quiz-day-33-shared-borrow-pitfall
    ...
    answerIndex: 2
    explanation:
      참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함. 이 실수가 나오면 원본 추적 `topic 소유 → &topic 빌림 → 길이 4 → topic 여전히 유효`대로
      '4 Rust'가 나오지 않으므로 먼저 확인해야 합니다.
  - id: quiz-day-33-shared-borrow-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 0
    explanation: 문법은 달라도 `소유권 이동 없이 읽기 전달` 목적과 입력·출력은 유지합니다. 실행 결과 '4 Rust'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
원본 출력은 `4 Rust`이고, 바뀐 코드의 실행 결과는 `5 Rust!`입니다. 바뀐 줄은 `let topic = String::from("Rust");`에서 `let topic = String::from("Rust!");`로 바뀌었습니다. 바뀐 프로그램은 `topic 소유` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `5 Rust!`가 됩니다. 원본 추적 `topic 소유  →  &topic 빌림  →  길이 4  →  topic 여전히 유효`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
