...
exercises:
  ...
  - id: ex-day-33-shared-borrow-fill
    ...
    explanation: 빈칸에 들어갈 표현은 '&topic'입니다. topic이 Rust를 소유합니다 length가 주소를 읽기 전용으로 빌립니다 길이 4와 원본 Rust를 출력합니다
    commonMistakes:
      - 참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-33-shared-borrow-guided
    ...
    explanation:
      바꾼 뒤 출력은 '5 Rust!'입니다. 원본 출력 '4 Rust'에서 달라졌습니다. 다른 줄(let topic = String::from("Rust"); → let topic
      = String::from("Rust!");)에서 시작한 차이가 최종 출력에 반영되었습니다.
    commonMistakes:
      - 참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
...
quiz:
  - id: quiz-day-33-shared-borrow-output
    ...
    choices:
      ...
      - 아무것도 출력하지 않는다
    answerIndex: 0
    explanation: topic 소유  →  &topic 빌림  →  길이 4  →  topic 여전히 유효 순서로 실행되어 출력은 '4 Rust'입니다.
...
원본과 달라졌습니다. 원본 출력은 `4 Rust`입니다. 다른 줄(`let topic = String::from("Rust");` → `let topic = String::from("Rust!");`)에서 시작된 차이가 이후 흐름을 타고 최종 출력에 반영되었습니다. 원본 추적(`topic 소유` → …)과 바뀐 줄부터 대조해 보세요.
