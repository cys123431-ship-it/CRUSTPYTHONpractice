...
exercises:
  - id: ex-day-33-shared-borrow-predict
    ...
    answer: 4 Rust
    hint: topic 소유에서 시작해 topic 여전히 유효까지 순서대로 적어 보세요. &String은 소유권을 가져가지 않는 공유 참조이고 호출이 끝나도 topic의 소유자는 main입니다.
    explanation:
      1. topic이 Rust를 소유합니다. 2. length가 주소를 읽기 전용으로 빌립니다. 3. 길이 4와 원본 Rust를 출력합니다. `topic 소유 → &topic 빌림
      → 길이 4 → topic 여전히 유효` 흐름으로 실제 출력 `4 Rust`가 됩니다. 핵심 `&topic`은 소유권 없이 읽기를 전달하는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-33-shared-borrow-fill
    ...
    hint: 필요한 표현은 소유권 이동 없이 읽기 전달 동작을 잇는 &topic입니다.
    explanation:
      빈칸에 들어갈 표현은 '&topic'입니다. 이 표현이 없어도 이번 화면 출력은 같을 수 있으나 숨은 계약을 어깁니다. 1. topic이 Rust를 소유합니다. 2. length가
      주소를 읽기 전용으로 빌립니다. 3. 길이 4와 원본 Rust를 출력합니다. `topic 소유 → &topic 빌림 → 길이 4 → topic 여전히 유효` 흐름으로 실제 출력 `4 Rust`가
      됩니다. 핵심 `&topic`은 소유권 없이 읽기를 전달하는 자리에 쓰입니다.
    commonMistakes:
      - 참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-33-shared-borrow-guided
    ...
    hint: 첫 문자열을 'Rust'에서 'Rust!'로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '4 Rust'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '5 Rust!'입니다. 원본은 `4 Rust`, 수정본은 `5 Rust!`이다. 첫 변경 줄 `let topic = String::from("Rust");`에서
      `let topic = String::from("Rust!");`로 값을 바꾸면, 길이 계산과 뒤의 표시가 함께 바뀌어 두 자리가 달라진다.
    commonMistakes:
      - 참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-33-shared-borrow-debug
    ...
    hint:
      참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함 상황에서 어긋나는 줄을 &String은 소유권을 가져가지 않는 공유 참조이고 호출이 끝나도 topic의 소유자는 main입니다.
      설명과 대조해 보세요.
    explanation:
      틀린 줄은 `println!("{} {topic}", length(topic.clone()));`입니다. 복제본을 만들어 빌려주므로 이번 입력에서는 화면 출력이 같습니다. 오류
      분류는 같은 출력이지만 숨은 계약 위반입니다. 빌림으로 충분한 곳에 복제를 쓰면 비용이 들고, 참조를 값 이동과 똑같이 여기는 오해를 굳힙니다. 고친 줄의 `&topic`에서는 `소유권 이동 없이
      읽기 전달` 동작이 지켜집니다. 정상 코드는 소유→공유 빌림→길이 계산→원본 표시의 순서로 `4 Rust`를 출력합니다.
    commonMistakes:
      - 참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
  ...
  - id: ex-day-33-shared-borrow-independent
    ...
    hint: 바꾸려면 소유자가 변경 가능해야 하고 `&mut T`로 배타적으로 빌려야 합니다. 읽기와 쓰기 빌림을 구별하세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 바꾸려면 소유자가 변경 가능해야 하고 `&mut T`로 배타적으로 빌려야 합니다. 읽기와 쓰기 빌림을 구별하세요. 같은 소유권
      이동 없이 읽기 전달 동작을 구현하고 실행 결과 '4 Rust'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: rust
...
quiz:
  - id: quiz-day-33-shared-borrow-output
    ...
    answerIndex: 0
    explanation:
      먼저 topic이 Rust를 소유합니다. 이어서 length가 &topic을 읽기 전용으로 빌려 길이 4를 구하고, 뒤에 원본 Rust를 표시하므로 출력 `4 Rust`가 됩니다.
      `실행 전에 반드시 오류가 난다`는 틀린 선택지인데 빌림과 호출이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 println!이 실제로 호출되기 때문입니다. 다른 선택지는
      이 추적과 맞지 않습니다.
  - id: quiz-day-33-shared-borrow-model
    question: "'공유 빌림과 참조' 개념을 이해하는 데 맞는 설명은?"
    choices:
    ...
    answerIndex: 1
    explanation:
      "`&String`은 소유권을 가져가지 않는 공유 참조라는 뜻이고, 호출이 끝나도 topic의 소유자는 main이라는 뜻은 빌림 뒤에도 원본을 쓸 수 있다는 뜻입니다. `참조를
      값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함`은 반대 사례인데, 공유 빌림은 원본 사용을 막지 않기 때문입니다."
  - id: quiz-day-33-shared-borrow-pitfall
    ...
    answerIndex: 2
    explanation:
      먼저 확인할 실수는 `참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함`입니다. 잘못된 코드는 이번 입력에서는 화면 출력이 같습니다. 같다고 해서 맞는 것이 아니라,
      불필요한 복제라는 숨은 계약 위반입니다.
  - id: quiz-day-33-shared-borrow-transfer
    ...
    answerIndex: 0
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `소유권 이동 없이 읽기 전달`이라는 의미와 입력·출력 계약입니다. C의 const 포인터 읽기 전달과 Python의 읽기 전용 전달로
      같은 동작을 구현하고, 실행 결과 `4 Rust`로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
예제의 핵심 표현은 `&topic`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본 실행 결과는 `4 Rust`입니다. 아래 코드에서 원본과 달라지는 첫 줄을 표시하고, 그 줄의 변경 전후 값을 적어 보세요.
...
원본은 `4 Rust`, 수정본은 `5 Rust!`이다. 첫 변경 줄 `let topic = String::from("Rust");`에서 `let topic = String::from("Rust!");`로 값을 바꾸면, 길이 계산과 뒤의 표시가 함께 바뀌어 두 자리가 달라진다.
...
두 줄을 나란히 놓으면 정상 줄 `println!("{} {topic}", length(&topic));`이 `&String은 소유권을 가져가지 않는 공유 참조이고 호출이 끝나도 topic의 소유자는 main입니다.` 설명과 맞고, 잘못된 줄은 `참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `topic 소유`부터 `4 Rust`까지 처음 어긋난 곳을 찾습니다.
...
예측(`4 Rust` 맞히기) → 빈칸(`&topic` 채우기) → 변경(`첫 문자열을 'Rust'에서 'Rust!'로`) → 오류 수정(`참조를 값 이동과 똑같이 생각해 원본을 쓰지 못한다고 판단함` 찾기) → 독립 구현(`'공유 빌림과 참조' 개념을 보여주는 작은 프로그램` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `4 Rust` 및 `&topic` 설명과 대조하세요.
