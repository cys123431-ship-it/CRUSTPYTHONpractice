...
데이터 모델과 테스트를 함께 점검해 세 언어에서 같은 상태를 만들었는지 검증합니다. Day 56 "`구조체·enum·테스트 회상`"에서는 Rust 코드가 `검증 완료`을(를) 만드는 과정을 따라가며, 구조화된 데이터에 회귀 테스트 추가 동작이 왜 필요한지 확인합니다. 구조화된 데이터에 회귀 테스트 추가을(를) 빠뜨리면 `검사하지 않은 예시 출력만 보고 구현이 맞다고 결론 냄` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 55](/learn/day-55-module-test)에서는 "Python 모듈화와 assert"을(를) 배웠습니다. "Python 모듈화와 assert"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`구조체·enum·테스트 회상`"에서 새로 달라지는 조건을 찾아보세요. Day 56의 답은 `검증 완료`이며, 핵심 표현은 `assert_eq!`입니다.

## 머릿속 그림

Session의 minutes는 u32이고 assert_eq!가 실제 값 30과 기대 30을 비교합니다. Day 56에서는 아래 흐름 순서대로 상태가 바뀌며, `assert_eq!`이(가) 결과를 가릅니다.

```text
s.minutes=30  →  기대값=30  →  통과
```

위 흐름에서 `assert_eq!`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `검증 완료`입니다.
...
아래 Rust 코드는 Day 56 "`구조체·enum·테스트 회상`"의 독립 예제입니다. 전체 6줄에서 `assert_eq!`이(가) 핵심이며, 실행 결과는 `검증 완료`입니다.
...
Rust는 `rustc --edition=2024` 또는 Cargo로 실행하면 `검증 완료`이(가) 나옵니다. 브라우저에서는 임의 컴파일 대신 하단 실습 칸의 답안 비교를 사용하세요.
...
위 단계에서 `assert_eq!`이(가) 빠지면 `검증 완료`이(가) 나오지 않습니다. `초기 minutes 값과 assert_eq!의 기대값을 함께 30에서 31로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `검증 완료`이(가) 됩니다. `s.minutes=30` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 코드에서 `초기 minutes 값과 assert_eq!의 기대값을 함께 30에서 31로` 바꾸면 아래와 같이 됩니다.
...
원본 출력은 `검증 완료`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `assert_eq!`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 56의 `구조체·enum·테스트 회상`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 검사하지 않은 예시 출력만 보고 구현이 맞다고 결론 냄.** 정상 코드에서는 `검증 완료`이(가) 출력됩니다.

- 정상 줄: `assert_eq!(s.minutes,30);`
- 잘못된 줄: `println!(s.minutes,30);`

두 줄을 나란히 놓고 `Session의 minutes는 u32이고 assert_eq!가 실제 값 30과 기대 30을 비교합니다.` 기준으로 어느 쪽이 맞는지 설명하세요. 결과가 예상과 다르면 `s.minutes=30`부터 `검증 완료`까지 처음 어긋난 곳을 찾습니다.

## 다른 언어로 옮기기

핵심 동작 **구조화된 데이터에 회귀 테스트 추가**은(는) 세 언어에서 같은 입력과 출력(`검증 완료`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `struct와 assert.h` 규칙으로 "`구조체·enum·테스트 회상`"의 `검증 완료`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `dataclass와 assert` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `struct와 assert_eq!` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.
...
- "`구조체·enum·테스트 회상`"이(가) 필요한 상황을 `구조화된 데이터에 회귀 테스트 추가` 동작으로 설명해 보세요.
- 예제에서 `assert_eq!`이(가) 실행되기 직전의 상태와 직후의 출력 `검증 완료`을(를) 말해 보세요.
- "`검사하지 않은 예시 출력만 보고 구현이 맞다고 결론 냄`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `구조화된 데이터에 회귀 테스트 추가` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

Session의 minutes는 u32이고 assert_eq!가 실제 값 30과 기대 30을 비교합니다. Day 56 "`구조체·enum·테스트 회상`"의 예제는 `assert_eq!`을(를) 실행해 `검증 완료`을(를) 출력합니다. "`검사하지 않은 예시 출력만 보고 구현이 맞다고 결론 냄`" 여부를 확인하고 Day 56을(를) 완료하세요.
