...
exercises:
  - id: ex-day-66-heap-predict
    ...
    answer: "10"
    hint: "[30,10,20]에서 시작해 pop 10까지 순서대로 적어 보세요. 최소 힙은 부모가 자식 이하라는 규칙을 유지하고 루트에서 최소값을 꺼냅니다."
    explanation:
      1. 세 우선순위를 힙으로 재배열합니다. 2. 루트가 최소 10입니다. 3. heappop이 10을 제거·반환합니다. `[30,10,20] → heapify 최소 루트 10
      → pop 10` 흐름으로 실제 출력 `10`이 됩니다. 핵심 `heapify(waiting)`은 리스트를 힙 규칙으로 다시 엮는 자리에 쓰입니다.
    commonMistakes:
  ...
  - id: ex-day-66-heap-fill
    ...
    answer: "import heapq
      ...
      print(heapq.heappop(waiting))"
    hint: 필요한 표현은 최소 우선순위 추출 동작을 잇는 heapify(waiting)입니다.
    explanation:
      빈칸에 들어갈 표현은 'heapify(waiting)'입니다. 이 표현이 없으면 실행 중 오류로 멈춥니다. 1. 세 우선순위를 힙으로 재배열합니다. 2. 루트가 최소 10입니다.
      3. heappop이 10을 제거·반환합니다. `[30,10,20] → heapify 최소 루트 10 → pop 10` 흐름으로 실제 출력 `10`이 됩니다. 핵심 `heapify(waiting)`은
      리스트를 힙 규칙으로 다시 엮는 자리에 쓰입니다.
    commonMistakes:
      - 힙 내부 배열 전체가 정렬돼 있다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-66-heap-guided
    ...
    answer: "import heapq
      ...
      print(heapq.heappop(waiting))"
    hint: 처음 등장하는 숫자를 30에서 31로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '10'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '10'입니다. 원본과 수정본 모두 `10`을 출력한다. 바꾼 31이 최솟값이 아니라 꺼내는 값은 그대로 10입니다. 최솟값이 달라지지 않았으므로 중간 상태가
      달라졌다고 쓰지 않는다.
    commonMistakes:
      - 힙 내부 배열 전체가 정렬돼 있다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-66-heap-debug
    ...
    answer: "import heapq
      ...
      print(heapq.heappop(waiting))"
    hint: 힙 내부 배열 전체가 정렬돼 있다고 생각함 상황에서 어긋나는 줄을 최소 힙은 부모가 자식 이하라는 규칙을 유지하고 루트에서 최소값을 꺼냅니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `heapq.waiting.sort()`입니다. `heapq` 모듈에 `waiting`이라는 속성이 없으므로 첫 실행에서 AttributeError라는 런타임 오류가
      나고 프로그램이 멈춥니다. 오류 분류는 런타임 오류입니다. 흐름이 깨지는 첫 순간은 존재하지 않는 모듈 속성을 읽는 줄 자체입니다. `sort`와 `heapify`는 결과 배열이 우연히 비슷해
      보여도 계약이 다릅니다. heap의 보장은 전체 정렬이 아니라 부모가 자식보다 우선되는 부분 순서이며 `heappop`과 함께 씁니다. 고친 줄 `heapq.heapify(waiting)`에서는
      `최소 우선순위 추출` 동작이 지켜집니다. 정상 코드는 힙 구성→최소 루트 확정→꺼내기의 순서로 `10`을 출력합니다.
    commonMistakes:
      - 힙 내부 배열 전체가 정렬돼 있다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-66-heap-independent
    ...
    answer: "import heapq
      ...
      print(heapq.heappop(waiting))"
    hint: 빈 힙에서 `heappop`하면 오류가 납니다. 동점 작업의 순서는 보조 순번으로 정하세요. 예상 출력과 경계 조건도 함께 적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 빈 힙에서 `heappop`하면 오류가 납니다. 동점 작업의 순서는 보조 순번으로 정하세요. 같은 최소 우선순위 추출 동작을
      구현하고 실행 결과 '10'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 힙 내부 배열 전체가 정렬돼 있다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-66-heap-output
    ...
    answerIndex: 0
    explanation:
      먼저 [30,10,20]이 힙 규칙으로 재배열됩니다. 이어서 루트 최소값 10이 확정되고, heappop이 꺼내 출력하므로 `10`이 됩니다. `실행 전에 반드시 오류가 난다`는
      틀린 선택지인데 힙 연산이 모두 정상이기 때문입니다. `아무것도 출력하지 않는다`도 틀린 선택지인데 print가 실제로 호출되기 때문입니다. 다른 선택지는 이 추적과 맞지 않습니다.
  - id: quiz-day-66-heap-model
    question: "'힙과 우선순위 큐' 개념을 이해하는 데 맞는 설명은?"
    choices:
    ...
    answerIndex: 1
    explanation:
      최소 힙은 부모가 자식 이하라는 규칙을 유지한다는 뜻이고, 루트에서 최소값을 꺼낸다는 뜻은 전체 정렬 없이 최소만 보장한다는 뜻입니다. `힙 내부 배열 전체가 정렬돼 있다고
      생각함`은 반대 사례인데, 정렬된 목록이 필요하면 하나씩 꺼내야 하기 때문입니다.
  - id: quiz-day-66-heap-pitfall
    ...
    answerIndex: 2
    explanation:
      먼저 확인할 실수는 `힙 내부 배열 전체가 정렬돼 있다고 생각함`입니다. 잘못된 코드는 존재하지 않는 속성에 접근해 AttributeError로 멈춥니다. 정렬과 힙 규칙을
      구별하세요.
  - id: quiz-day-66-heap-transfer
    ...
    answerIndex: 0
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `최소 우선순위 추출`이라는 의미와 입력·출력 계약입니다. 정렬 뒤 꺼내기와 힙 꺼내기로 같은 동작을 구현하고, 실행 결과 `10`으로
...
예제의 핵심 표현은 `heapify(waiting)`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본과 수정본 모두 `10`을 출력한다. 바꾼 31이 최솟값이 아니라 꺼내는 값은 그대로 10입니다. 최솟값이 달라지지 않았으므로 중간 상태가 달라졌다고 쓰지 않는다.
...
두 줄을 나란히 놓으면 정상 줄 `heapq.heapify(waiting)`이 `최소 힙은 부모가 자식 이하라는 규칙을 유지하고 루트에서 최소값을 꺼냅니다.` 설명과 맞고, 잘못된 줄은 `힙 내부 배열 전체가 정렬돼 있다고 생각함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `[30,10,20]`부터 `10`까지 처음 어긋난 곳을 찾습니다.
...
예측(`10` 맞히기) → 빈칸(`heapify(waiting)` 채우기) → 변경(`처음 등장하는 숫자를 30에서 31로`) → 오류 수정(`힙 내부 배열 전체가 정렬돼 있다고 생각함` 찾기) → 독립 구현(`'힙과 우선순위 큐' 개념을 보여주는 작은 프로그램` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `10` 및 `heapify(waiting)` 설명과 대조하세요.
