...
exercises:
  - id: ex-day-66-heap-predict
    ...
    answer: "10"
    hint: 최소 힙은 부모가 자식 이하라는 규칙을 유지하고 루트에서 최소값을 꺼냅니다. 설명을 떠올리고 `[30,10,20]` 단계부터 순서대로 적어 보세요.
    explanation:
      "[30,10,20]  →  heapify 최소 루트 10  →  pop 10 순서로 실행됩니다. `heapify(waiting)` 부분이 `pop 10` 단계를 확정해 최종
      출력 '10'가 됩니다. 이 흐름을 떠올리면 `최소 우선순위 추출` 동작이 왜 필요한지 알 수 있습니다."
    commonMistakes:
  ...
  - id: ex-day-66-heap-fill
    title: 핵심 표현 빈칸 채우기
    kind: fill
    objective: "'힙과 우선순위 큐' 개념의 핵심 표현을 스스로 적는다."
    ...
    answer: "import heapq
      ...
      print(heapq.heappop(waiting))"
    hint: 힌트 문장을 완전하게 읽으면 `세 우선순위를 힙으로 재배열합니다` 단계에 필요한 표현이 `heapify(waiting)`입니다.
    explanation:
      빈칸에 들어갈 표현은 'heapify(waiting)'입니다. `heapq.heapify(waiting)` 줄을 완성해야 `최소 우선순위 추출` 동작이 이어져 실행 결과 '10'가
      됩니다. 힌트의 첫 단계 `세 우선순위를 힙으로 재배열합니다`이 바로 이 줄입니다. 이어서 루트가 최소 10입니다 heappop이 10을 제거·반환합니다 순서로 진행됩니다.
    commonMistakes:
      - 힙 내부 배열 전체가 정렬돼 있다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-66-heap-guided
    ...
    explanation:
      바꾼 뒤 출력은 '10'입니다. 원본 출력 '10'와 같습니다. 바뀐 줄은 `waiting = [31, 10, 20]`입니다. 바뀐 프로그램도 `[30,10,20]` 단계에서
      시작해 바뀐 줄을 지나 최종 `10`로 끝납니다. 바꾼 31이 최솟값이 아니라 꺼내는 값은 그대로 10입니다. 출력은 같지만 바뀐 줄 부분의 중간 값은 달라졌다가 같은 최종 값으로 이어진다는 점을
  ...
  - id: ex-day-66-heap-debug
    ...
    answer: "import heapq
      ...
      print(heapq.heappop(waiting))"
    hint: 최소 힙은 부모가 자식 이하라는 규칙을 유지하고 루트에서 최소값을 꺼냅니다. 설명과 어긋나는 줄을 찾으세요. `힙 내부 배열 전체가 정렬돼 있다고 생각함` 상황이 단서가 됩니다.
    explanation:
      틀린 줄은 `heapq.waiting.sort()`입니다. 여기서는 `waiting.sort()`을 써서 `heapify(waiting)` 동작이 깨집니다. 이대로 실행하면
      `힙 내부 배열 전체가 정렬돼 있다고 생각함` 문제가 생겨 원본 추적 `[30,10,20]  →  heapify 최소 루트 10  →  pop 10`대로 '10'가 나오지 않습니다. 고친 줄 `heapq.heapify(waiting)`에서는
      `heapify(waiting)`가 `최소 우선순위 추출` 동작을 지켜 '10'까지 도달합니다.
    commonMistakes:
      - 힙 내부 배열 전체가 정렬돼 있다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-66-heap-independent
    ...
    answer: "import heapq
      ...
      print(heapq.heappop(waiting))"
    hint: 최소 힙은 부모가 자식 이하라는 규칙을 유지하고 루트에서 최소값을 꺼냅니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `heapify(waiting)` 부분이 `최소 우선순위 추출` 동작을 지켜 실행 결과 '10'가 됩니다. 같은 개념을 다른 입력으로 바꿔도
      `heapify(waiting)`부터 `pop 10`까지 추적할 수 있으면 정답입니다. `힙 내부 배열 전체가 정렬돼 있다고 생각함` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - 힙 내부 배열 전체가 정렬돼 있다고 생각함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-66-heap-output
    ...
    answerIndex: 0
    explanation:
      "[30,10,20]  →  heapify 최소 루트 10  →  pop 10 순서로 실행되어 출력은 '10'입니다. `heapify(waiting)` 부분이 마지막 단계를
  ...
  - id: quiz-day-66-heap-model
    ...
    answerIndex: 1
    explanation:
      최소 힙은 부모가 자식 이하라는 규칙을 유지하고 루트에서 최소값을 꺼냅니다. 이 설명이 맞는 이유는 `최소 우선순위 추출` 동작을 지키는 조건과 같기 때문입니다. `힙 내부
      배열 전체가 정렬돼 있다고 생각함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-66-heap-pitfall
    ...
    answerIndex: 2
    explanation:
      힙 내부 배열 전체가 정렬돼 있다고 생각함. 이 실수가 나오면 원본 추적 `[30,10,20] → heapify 최소 루트 10 → pop 10`대로 '10'가 나오지 않으므로
      먼저 확인해야 합니다.
  - id: quiz-day-66-heap-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 0
    explanation: 문법은 달라도 `최소 우선순위 추출` 목적과 입력·출력은 유지합니다. 실행 결과 '10'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
...
바꾼 뒤 출력도 `10`입니다. 바꾼 31이 최솟값이 아니라 꺼내는 값은 그대로 10입니다. 바뀐 프로그램도 `[30,10,20]` 단계에서 시작해 `waiting = [31, 10, 20]` 줄을 지나 최종 `10`로 끝납니다. 원본 추적 `[30,10,20]  →  heapify 최소 루트 10  →  pop 10`와 바뀐 줄 이후를 순서대로 놓으면 출력은 같아도 중간 값이 어디서 달라졌다가 합쳐지는지 확인할 수 있습니다.
