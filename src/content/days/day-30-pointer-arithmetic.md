...
exercises:
  - id: ex-day-30-pointer-arithmetic-predict
    ...
    starter:
      ...
      %d\\n\", *(p + 1));\n    return 0;\n}"
    answer: "20"
    hint: p=&values[0]에서 시작해 20 출력까지 순서대로 적어 보세요. int 포인터 p+1은 다음 int 원소의 주소입니다. 역참조는 그 주소에 저장된 값을 읽습니다.
    explanation:
      1. p가 values[0]을 가리킵니다. 2. p+1은 두 번째 원소를 가리킵니다. 3. 역참조해 20을 출력합니다. `p=&values[0] → *p=10 → p+1=&values[1]
      → *(p+1)=20 → 20 출력` 흐름으로 실제 출력 `20`이 됩니다. 핵심 `p + 1`은 다음 원소의 주소로 이동하는 자리에 쓰입니다.
    commonMistakes:
      - 배열 끝을 지난 주소를 역참조함
  ...
  - id: ex-day-30-pointer-arithmetic-fill
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int values[] = {10, 20, 30};\n    int *p = values;\n    printf(\"\
      %d\\n\", *(p + 1));\n    return 0;\n}"
    hint: 필요한 표현은 배열 내부에서 다음 원소를 참조함 동작을 잇는 p + 1입니다.
    explanation:
      빈칸에 들어갈 표현은 'p + 1'입니다. 이 표현이 없으면 정의되지 않은 동작이 되어 어떤 결과도 보장할 수 없습니다. 1. p가 values[0]을 가리킵니다. 2. p+1은
      두 번째 원소를 가리킵니다. 3. 역참조해 20을 출력합니다. `p=&values[0] → *p=10 → p+1=&values[1] → *(p+1)=20 → 20 출력` 흐름으로 실제 출력 `20`이
      됩니다. 핵심 `p + 1`은 다음 원소의 주소로 이동하는 자리에 쓰입니다.
    commonMistakes:
      - 배열 끝을 지난 주소를 역참조함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-30-pointer-arithmetic-guided
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int values[] = {11, 20, 30};\n    int *p = values;\n    printf(\"\
      %d\\n\", *(p + 1));\n    return 0;\n}"
    hint: 처음 등장하는 숫자를 10에서 11로 바꾼 뒤 실행 결과를 먼저 적어 보세요. 원본 출력 '20'와 비교해 달라지는 첫 중간 값을 찾으면 됩니다.
    explanation:
      바꾼 뒤 출력은 '20'입니다. 원본과 수정본 모두 `20`을 출력한다. 바꾼 것은 0번 원소, 읽는 것은 1번 원소라 결과가 같습니다. 값이 실제로 달라진 것은 건드리지 않은
      칸뿐이므로 중간 상태가 달라졌다고 쓰지 않는다.
    commonMistakes:
      - 배열 끝을 지난 주소를 역참조함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-30-pointer-arithmetic-debug
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int values[] = {10, 20, 30};\n    int *p = values;\n    printf(\"\
      %d\\n\", *(p + 1));\n    return 0;\n}"
    hint: 배열 끝을 지난 주소를 역참조함 상황에서 어긋나는 줄을 int 포인터 p+1은 다음 int 원소의 주소입니다. 역참조는 그 주소에 저장된 값을 읽습니다. 설명과 대조해 보세요.
    explanation:
      틀린 줄은 `printf("%d\n", *(p + 3));`입니다. `p + 3`은 길이 3 배열의 범위 밖 주소를 가리키므로, 역참조하는 순간 C의 정의되지 않은 동작이 됩니다.
      특정 오답 숫자를 약속하지 않습니다. 우연히 어떤 값이 보일 수도 있고 비정상 종료될 수도 있으며, 어떤 결과도 보장할 수 없습니다. 오류 분류는 정의되지 않은 동작입니다. 고친 줄 `printf("%d\n",
      *(p + 1));`에서는 같은 배열 안에서 다음 원소를 참조하는 동작이 지켜집니다. 포인터 산술은 바이트 1개가 아니라 `int` 원소 단위로 이동합니다. 정상 코드는 주소 보관→다음 원소 이동→역참조
      출력의 순서로 `20`을 출력합니다.
    commonMistakes:
      - 배열 끝을 지난 주소를 역참조함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
  ...
  - id: ex-day-30-pointer-arithmetic-independent
    ...
    answer:
      ...
      %d\\n\", *(p + 1));\n    return 0;\n}"
    hint:
      길이가 2이면 `p + 2`는 끝 바로 뒤라 주소 계산은 가능해도 역참조해서 읽으면 안 됩니다. 주소 숫자를 외우지 말고 어느 원소를 가리키는가를 추적하세요. 예상 출력과 경계 조건도 함께
      적어 보세요.
    explanation:
      입력·처리·출력·경계 조건을 스스로 설계하세요. 길이가 2이면 `p + 2`는 끝 바로 뒤라 주소 계산은 가능해도 역참조해서 읽으면 안 됩니다. 주소 숫자를 외우지 말고 어느
      원소를 가리키는가를 추적하세요. 같은 배열 내부에서 다음 원소를 참조함 동작을 구현하고 실행 결과 '20'와 대조할 수 있으면, 예시 답안과 달라도 정답입니다.
    commonMistakes:
      - 배열 끝을 지난 주소를 역참조함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-30-pointer-arithmetic-output
    ...
    answerIndex: 0
    explanation:
      먼저 p가 values[0]의 주소를 보관합니다. 이어서 p+1이 두 번째 원소의 주소가 되고, 역참조해 20을 읽어 출력하므로 `20`이 됩니다. `실행 전에 반드시 오류가
  ...
  - id: quiz-day-30-pointer-arithmetic-model
    ...
    answerIndex: 1
    explanation:
      int 포인터 p+1은 다음 int 원소의 주소라는 뜻이고, 역참조는 그 주소에 저장된 값을 읽는다는 뜻은 주소 계산과 값 읽기가 분리된다는 뜻입니다. `배열 끝을 지난 주소를
      역참조함`은 반대 사례인데, 유효 범위를 벗어나면 정의되지 않은 동작이 되기 때문입니다.
  - id: quiz-day-30-pointer-arithmetic-pitfall
    ...
    choices:
      ...
      - 배열 끝을 지난 주소를 역참조함
    answerIndex: 2
    explanation: 먼저 확인할 실수는 `배열 끝을 지난 주소를 역참조함`입니다. 범위 밖 역참조는 정의되지 않은 동작이므로, 특정 값이 나온다고 약속하지 마세요. 같은 배열 안에서만 이동·역참조하세요.
  - id: quiz-day-30-pointer-arithmetic-transfer
    ...
    answerIndex: 0
    explanation:
      세 언어로 옮길 때 보존해야 하는 것은 `배열 내부에서 다음 원소를 참조함`이라는 의미와 입력·출력 계약입니다. Python의 인덱스와 Rust의 범위 검사 참조로 같은 동작을
...
예제의 핵심 표현은 `p + 1`입니다. 아래 줄 번호가 붙은 코드에서 이 표현이 있는 줄 번호를 말하고, 그 줄이 읽는 값과 바꾸는 값을 적어 보세요.
...
원본과 수정본 모두 `20`을 출력한다. 바꾼 것은 0번 원소, 읽는 것은 1번 원소라 결과가 같습니다. 값이 실제로 달라진 것은 건드리지 않은 칸뿐이므로 중간 상태가 달라졌다고 쓰지 않는다.
...
두 줄을 나란히 놓으면 정상 줄 `printf("%d\n", *(p + 1));`이 `int 포인터 p+1은 다음 int 원소의 주소입니다. 역참조는 그 주소에 저장된 값을 읽습니다.` 설명과 맞고, 잘못된 줄은 `배열 끝을 지난 주소를 역참조함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `p=&values[0]`부터 `20`까지 처음 어긋난 곳을 찾습니다.
