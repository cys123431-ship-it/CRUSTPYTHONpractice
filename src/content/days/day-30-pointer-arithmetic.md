...
exercises:
  - id: ex-day-30-pointer-arithmetic-predict
    ...
    starter:
      ...
      %d\\n\", *(p + 1));\n    return 0;\n}"
    answer: "20"
    hint: int 포인터 p+1은 다음 int 원소의 주소입니다. 역참조는 그 주소에 저장된 값을 읽습니다. 설명을 떠올리고 `p=&values[0]` 단계부터 순서대로 적어 보세요.
    explanation:
      p=&values[0]  →  *p=10  →  p+1=&values[1]  →  *(p+1)=20  →  20 출력 순서로 실행됩니다. `p + 1` 부분이 `20 출력`
      단계를 확정해 최종 출력 '20'가 됩니다. 이 흐름을 떠올리면 `배열 내부에서 다음 원소를 참조함` 동작이 왜 필요한지 알 수 있습니다.
    commonMistakes:
      - 배열 끝을 지난 주소를 역참조함
  ...
  - id: ex-day-30-pointer-arithmetic-fill
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int values[] = {10, 20, 30};\n    int *p = values;\n    printf(\"\
      %d\\n\", *(p + 1));\n    return 0;\n}"
    hint: 힌트 문장을 완전하게 읽으면 `p가 values[0]을 가리킵니다` 단계에 필요한 표현이 `p + 1`입니다.
    explanation:
      빈칸에 들어갈 표현은 'p + 1'입니다. `printf("%d\n", *(p + 1));` 줄을 완성해야 `배열 내부에서 다음 원소를 참조함` 동작이 이어져 실행 결과 '20'가
      됩니다. 힌트의 첫 단계 `p가 values[0]을 가리킵니다`이 바로 이 줄입니다. 이어서 p+1은 두 번째 원소를 가리킵니다 역참조해 20을 출력합니다 순서로 진행됩니다.
    commonMistakes:
      - 배열 끝을 지난 주소를 역참조함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-30-pointer-arithmetic-guided
    ...
    explanation:
      바꾼 뒤 출력은 '20'입니다. 원본 출력 '20'와 같습니다. 바뀐 줄은 `int values[] = {11, 20, 30};`입니다. 바뀐 프로그램도 `p=&values[0]`
      단계에서 시작해 바뀐 줄을 지나 최종 `20`로 끝납니다. 바꾼 것은 0번 원소, 읽는 것은 1번 원소라 결과가 같습니다. 출력은 같지만 바뀐 줄 부분의 중간 값은 달라졌다가 같은 최종 값으로
  ...
  - id: ex-day-30-pointer-arithmetic-debug
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int values[] = {10, 20, 30};\n    int *p = values;\n    printf(\"\
      %d\\n\", *(p + 1));\n    return 0;\n}"
    hint: int 포인터 p+1은 다음 int 원소의 주소입니다. 역참조는 그 주소에 저장된 값을 읽습니다. 설명과 어긋나는 줄을 찾으세요. `배열 끝을 지난 주소를 역참조함` 상황이 단서가 됩니다.
    explanation:
      틀린 줄은 `printf("%d\n", *(p + 3));`입니다. 여기서는 `p + 3`을 써서 `p + 1` 동작이 깨집니다. 이대로 실행하면 `배열 끝을 지난 주소를 역참조함`
      문제가 생겨 원본 추적 `p=&values[0]  →  *p=10  →  p+1=&values[1]  →  *(p+1)=20  →  20 출력`대로 '20'가 나오지 않습니다. 고친 줄 `printf("%d\n",
      *(p + 1));`에서는 `p + 1`가 `배열 내부에서 다음 원소를 참조함` 동작을 지켜 '20'까지 도달합니다.
    commonMistakes:
      - 배열 끝을 지난 주소를 역참조함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
  ...
  - id: ex-day-30-pointer-arithmetic-independent
    ...
    answer:
      "#include <stdio.h>\nint main(void) {\n    int values[] = {10, 20, 30};\n    int *p = values;\n    printf(\"\
      %d\\n\", *(p + 1));\n    return 0;\n}"
    hint: int 포인터 p+1은 다음 int 원소의 주소입니다. 역참조는 그 주소에 저장된 값을 읽습니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `p + 1` 부분이 `배열 내부에서 다음 원소를 참조함` 동작을 지켜 실행 결과 '20'가 됩니다. 같은 개념을 다른 입력으로 바꿔도
      `p + 1`부터 `20 출력`까지 추적할 수 있으면 정답입니다. `배열 끝을 지난 주소를 역참조함` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - 배열 끝을 지난 주소를 역참조함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: c
...
quiz:
  - id: quiz-day-30-pointer-arithmetic-output
    ...
    answerIndex: 0
    explanation:
      p=&values[0]  →  *p=10  →  p+1=&values[1]  →  *(p+1)=20  →  20 출력 순서로 실행되어 출력은 '20'입니다. `p + 1` 부분이
  ...
  - id: quiz-day-30-pointer-arithmetic-model
    ...
    answerIndex: 1
    explanation:
      int 포인터 p+1은 다음 int 원소의 주소입니다. 역참조는 그 주소에 저장된 값을 읽습니다. 이 설명이 맞는 이유는 `배열 내부에서 다음 원소를 참조함` 동작을 지키는
      조건과 같기 때문입니다. `배열 끝을 지난 주소를 역참조함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-30-pointer-arithmetic-pitfall
    ...
    answerIndex: 2
    explanation:
      배열 끝을 지난 주소를 역참조함. 이 실수가 나오면 원본 추적 `p=&values[0] → *p=10 → p+1=&values[1] → *(p+1)=20 → 20 출력`대로
      '20'가 나오지 않으므로 먼저 확인해야 합니다.
  - id: quiz-day-30-pointer-arithmetic-transfer
    ...
    choices:
      ...
      - 타입과 오류 처리를 모두 생략한다
    answerIndex: 0
    explanation: 문법은 달라도 `배열 내부에서 다음 원소를 참조함` 목적과 입력·출력은 유지합니다. 실행 결과 '20'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
---
...
바꾼 뒤 출력도 `20`입니다. 바꾼 것은 0번 원소, 읽는 것은 1번 원소라 결과가 같습니다. 바뀐 프로그램도 `p=&values[0]` 단계에서 시작해 `int values[] = {11, 20, 30};` 줄을 지나 최종 `20`로 끝납니다. 원본 추적 `p=&values[0]  →  *p=10  →  p+1=&values[1]  →  *(p+1)=20  →  20 출력`와 바뀐 줄 이후를 순서대로 놓으면 출력은 같아도 중간 값이 어디서 달라졌다가 합쳐지는지 확인할 수 있습니다.
