...
배열의 연속 원소에 주소로 접근할 때 이동 단위가 바이트가 아니라 원소 크기임을 알아야 합니다. Day 30 "`C 포인터 이동과 배열`"에서는 C 코드가 `20`을(를) 만드는 과정을 따라가며, 배열 내부에서 다음 원소를 참조함 동작이 왜 필요한지 확인합니다. 배열 내부에서 다음 원소를 참조함을(를) 빠뜨리면 `배열 끝을 지난 주소를 역참조함` 같은 문제가 생깁니다.

## 시작 전에 확인할 것

바로 앞선 [Day 29](/learn/day-29-references-borrowing)에서는 "포인터 · 참조 · 빌림 — 같은 값을 바라보는 세 방식"을(를) 배웠습니다. "포인터 · 참조 · 빌림 — 같은 값을 바라보는 세 방식"의 핵심 결과를 한 문장으로 말해 보고, 이번 "`C 포인터 이동과 배열`"에서 새로 달라지는 조건을 찾아보세요. Day 30의 답은 `20`이며, 핵심 표현은 `p + 1`입니다.

## 머릿속 그림

int 포인터 p+1은 다음 int 원소의 주소입니다. 역참조는 그 주소에 저장된 값을 읽습니다. Day 30에서는 아래 흐름 순서대로 상태가 바뀌며, `p + 1`이(가) 결과를 가릅니다.

```text
p=&values[0]  →  *p=10  →  p+1=&values[1]  →  *(p+1)=20  →  20 출력
```

위 흐름에서 `p + 1`이(가) 실행되는 지점을 찾으면 읽는 값과 바뀌는 값이 나뉘어 보입니다. 마지막 출력은 `20`입니다.
...
아래 C 코드는 Day 30 "`C 포인터 이동과 배열`"의 독립 예제입니다. 전체 7줄에서 `p + 1`이(가) 핵심이며, 실행 결과는 `20`입니다.
...
C17은 `gcc -std=c17 -Wall -Wextra`로 컴파일해 실행하면 `20`이(가) 나옵니다. 브라우저에서는 임의 컴파일 대신 하단 실습 칸의 답안 비교를 사용하세요.
...
위 단계에서 `p + 1`이(가) 빠지면 `20`이(가) 나오지 않습니다. `처음 등장하는 숫자를 10에서 11로` 실험에서 어느 줄부터 달라지는지 직접 확인하세요.
...
위 순서대로 실행하면 최종 출력 `20`이(가) 됩니다. `p=&values[0]` 단계와 마지막 단계를 비교하면 입력과 출력의 관계가 보입니다.
...
원본 출력은 `20`입니다. 바꾼 코드를 실행하기 전에 출력이 어떻게 달라질지 먼저 적어 보세요. 출력이 같았다면 `p + 1`이(가) 결과에 영향을 주지 않은 이유를, 달라졌다면 처음 달라진 중간 값을 설명하세요. Day 30의 `C 포인터 이동과 배열`에서 바뀐 줄부터 다시 추적하세요.

## 자주 틀리는 지점

**확인할 실수: 배열 끝을 지난 주소를 역참조함.** 정상 코드에서는 `20`이(가) 출력됩니다.
...
핵심 동작 **배열 내부에서 다음 원소를 참조함**은(는) 세 언어에서 같은 입력과 출력(`20`)으로 유지됩니다. 선언과 오류 처리는 언어마다 다릅니다.
...
C17에서는 `*(p+1)은 values[1]` 규칙으로 "`C 포인터 이동과 배열`"의 `20`을(를) 확인하고, 범위를 벗어난 접근은 직접 막아야 합니다. Python에서는 `values[1]로 경계 검사` 규칙을 따르고, 실패나 빈 입력은 예외로 드러내 조용히 넘기지 마세요. Rust에서는 `slice.get(1)은 Option<&T>` 규칙을 따르고, 빌림과 범위 검사를 컴파일 때 확인하세요.

## 실습 순서

예측(`20` 맞히기) → 빈칸(`p + 1` 채우기) → 변경(`처음 등장하는 숫자를 10에서 11로`) → 오류 수정(`배열 끝을 지난 주소를 역참조함` 찾기) → 독립 구현(`C 포인터 이동과 배열을 보여 주는` 만들기) 순으로 진행하세요. 각 단계의 답은 본문의 `20` 및 `p + 1` 설명과 대조하세요.

## 스스로 설명하기

- "`C 포인터 이동과 배열`"이(가) 필요한 상황을 `배열 내부에서 다음 원소를 참조함` 동작으로 설명해 보세요.
- 예제에서 `p + 1`이(가) 실행되기 직전의 상태와 직후의 출력 `20`을(를) 말해 보세요.
- "`배열 끝을 지난 주소를 역참조함`" 상황에서 어떤 입력과 출력이 어긋나는지 말해 보세요.
- 같은 `배열 내부에서 다음 원소를 참조함` 동작을 나머지 두 언어의 어떤 표현으로 옮길지 말해 보세요.

## 핵심 요약과 복습

int 포인터 p+1은 다음 int 원소의 주소입니다. 역참조는 그 주소에 저장된 값을 읽습니다. Day 30 "`C 포인터 이동과 배열`"의 예제는 `p + 1`을(를) 실행해 `20`을(를) 출력합니다. "`배열 끝을 지난 주소를 역참조함`" 여부를 확인하고 Day 30을(를) 완료하세요.
