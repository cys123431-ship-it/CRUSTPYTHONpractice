...
exercises:
  - id: ex-day-59-queue-predict
    ...
    answer: A
    hint: 큐는 뒤에 넣고 앞에서 꺼냅니다. deque의 popleft는 오래된 항목을 O(1)에 꺼냅니다. 설명을 떠올리고 `[A,B]` 단계부터 순서대로 적어 보세요.
    explanation:
      "[A,B]  →  [A,B,C]  →  A 처리 순서로 실행됩니다. `popleft()` 부분이 `A 처리` 단계를 확정해 최종 출력 'A'가 됩니다. 이 흐름을 떠올리면
      `먼저 도착한 요청부터 처리` 동작이 왜 필요한지 알 수 있습니다."
    commonMistakes:
  ...
  - id: ex-day-59-queue-fill
    ...
    answer: 'from collections import deque
      ...
      print(waiting.popleft())'
    hint: 힌트 문장을 완전하게 읽으면 `A,B가 기다립니다` 단계에 필요한 표현이 `popleft()`입니다.
    explanation:
      빈칸에 들어갈 표현은 'popleft()'입니다. `print(waiting.popleft())` 줄을 완성해야 `먼저 도착한 요청부터 처리` 동작이 이어져 실행 결과 'A'가
      됩니다. 힌트의 첫 단계 `A,B가 기다립니다`이 바로 이 줄입니다. 이어서 C가 뒤에 합류합니다 앞에 있는 A를 제거해 출력합니다 순서로 진행됩니다.
    commonMistakes:
      - pop()으로 마지막 접수 C를 먼저 처리함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-59-queue-guided
    ...
    explanation:
      바꾼 뒤 출력은 'A!'입니다. 원본 출력 'A'에서 달라졌습니다. 바뀐 줄은 `waiting = deque(["A", "B"])`에서 `waiting = deque(["A!",
      "B"])`로 바뀌었습니다. 바뀐 프로그램은 `[A,B]` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 그 차이가 이후 단계로 이어져 최종 `A!`가 됩니다. 원본 추적 `[A,B]  →  [A,B,C]  →  A
      처리`와 바뀐 줄 이후를 순서대로 비교하면 처음 달라지는 곳이 보입니다.
    commonMistakes:
      - pop()으로 마지막 접수 C를 먼저 처리함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-59-queue-debug
    ...
    answer: 'from collections import deque
      ...
      print(waiting.popleft())'
    hint:
      큐는 뒤에 넣고 앞에서 꺼냅니다. deque의 popleft는 오래된 항목을 O(1)에 꺼냅니다. 설명과 어긋나는 줄을 찾으세요. `pop()으로 마지막 접수 C를 먼저 처리함` 상황이
      단서가 됩니다.
    explanation:
      틀린 줄은 `print(waiting.pop())`입니다. 여기서는 `pop()`을 써서 `popleft()` 동작이 깨집니다. 이대로 실행하면 `pop()으로 마지막 접수
      C를 먼저 처리함` 문제가 생겨 원본 추적 `[A,B]  →  [A,B,C]  →  A 처리`대로 'A'가 나오지 않습니다. 고친 줄 `print(waiting.popleft())`에서는 `popleft()`가
      `먼저 도착한 요청부터 처리` 동작을 지켜 'A'까지 도달합니다.
    commonMistakes:
      - pop()으로 마지막 접수 C를 먼저 처리함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
  ...
  - id: ex-day-59-queue-independent
    ...
    answer: 'from collections import deque
      ...
      print(waiting.popleft())'
    hint: 큐는 뒤에 넣고 앞에서 꺼냅니다. deque의 popleft는 오래된 항목을 O(1)에 꺼냅니다. 흐름을 작은 입력으로 다시 만들어 보세요. 예상 출력과 빈 입력 같은 경계를 함께 적으세요.
    explanation:
      한 가지 예시 해법은 위 코드입니다. `popleft()` 부분이 `먼저 도착한 요청부터 처리` 동작을 지켜 실행 결과 'A'가 됩니다. 같은 개념을 다른 입력으로 바꿔도 `popleft()`부터
      `A 처리`까지 추적할 수 있으면 정답입니다. `pop()으로 마지막 접수 C를 먼저 처리함` 상황과 빈 입력 같은 경계도 함께 설명해 보세요.
    commonMistakes:
      - pop()으로 마지막 접수 C를 먼저 처리함
      - 실행 전에 출력과 중간 상태를 손으로 확인하지 않음
    language: python
...
quiz:
  - id: quiz-day-59-queue-output
    ...
    answerIndex: 2
    explanation:
      "[A,B]  →  [A,B,C]  →  A 처리 순서로 실행되어 출력은 'A'입니다. `popleft()` 부분이 마지막 단계를 확정하므로 다른 선택지는 이 추적과 맞지
      않습니다."
  - id: quiz-day-59-queue-model
    ...
    answerIndex: 0
    explanation:
      큐는 뒤에 넣고 앞에서 꺼냅니다. deque의 popleft는 오래된 항목을 O(1)에 꺼냅니다. 이 설명이 맞는 이유는 `먼저 도착한 요청부터 처리` 동작을 지키는 조건과
      같기 때문입니다. `pop()으로 마지막 접수 C를 먼저 처리함` 설명은 오히려 피해야 할 오류이므로 정답이 아닙니다.
  - id: quiz-day-59-queue-pitfall
    ...
    answerIndex: 1
    explanation: pop()으로 마지막 접수 C를 먼저 처리함. 이 실수가 나오면 원본 추적 `[A,B] → [A,B,C] → A 처리`대로 'A'가 나오지 않으므로 먼저 확인해야 합니다.
  - id: quiz-day-59-queue-transfer
    ...
    answerIndex: 2
    explanation: 문법은 달라도 `먼저 도착한 요청부터 처리` 목적과 입력·출력은 유지합니다. 실행 결과 'A'로 대조하면 옮김이 맞는지 확인할 수 있습니다.
...
원본 출력은 `A`이고, 바뀐 코드의 실행 결과는 `A!`입니다. 바뀐 줄은 `waiting = deque(["A", "B"])`에서 `waiting = deque(["A!", "B"])`로 바뀌었습니다. 바뀐 프로그램은 `[A,B]` 단계에서 시작해 바뀐 줄에서 다른 중간 값을 만들고, 이후 흐름을 따라 최종 `A!`가 됩니다. 원본 추적 `[A,B]  →  [A,B,C]  →  A 처리`에서 바뀐 줄 이후 단계와 하나씩 비교하면 처음 달라지는 곳이 보입니다.
