...
두 줄을 나란히 놓으면 정상 줄 `let copy = original.clone();`이 `clone은 내용을 새 메모리에 복제해 두 String이 각각 소유하게 합니다. 단순 대입은 소유권을 이동합니다.` 설명과 맞고, 잘못된 줄은 `String을 이동한 뒤 이전 변수를 다시 사용함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `original 소유`부터 `study study`까지 처음 어긋난 곳을 찾습니다.
