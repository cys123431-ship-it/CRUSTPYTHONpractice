...
두 줄을 나란히 놓으면 정상 줄 `source.read_to_string(&mut text).expect("읽기 실패");`이 `Cursor는 메모리 버퍼를 파일처럼 읽게 합니다. read_to_string은 text를 변경하고 Result를 돌려줍니다.` 설명과 맞고, 잘못된 줄은 `읽기 결과의 오류를 무시해 빈 데이터를 정상으로 취급함` 쪽으로 어긋납니다. 결과가 예상과 다르면 `source=30`부터 `30`까지 처음 어긋난 곳을 찾습니다.
